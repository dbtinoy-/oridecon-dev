...
@pytest.mark.skip(
    reason="This test requires Redis to be registered in container - use test_debug_routes_rate_limit_redis_sim.py for fake injection testing"
)
def test_debug_routes_redis_client_container_injection():
    app = Application(name="test-app")
    fake = FakeRedis()
    # Register fake redis in container under 'redis' key
