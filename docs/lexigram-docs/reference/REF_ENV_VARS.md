...
- **Missing descriptions**: 922 of 1764 (52%)
- **Complex defaults**: 33 — default values that could not be statically resolved
- **Packages with NO env vars**: lexigram-ai-relay, lexigram-all, lexigram-contracts
...
| `LEX_LEXIGRAM__LOGGING__REDACTION__ENABLED` | bool | True | — | `lexigram/src/lexigram/logging/config/redaction.py:LexigramConfig.logging.redaction.enabled` |
| `LEX_LEXIGRAM__LOGGING__REDACTION__FIELD_DENYLIST` | tuple[str, ...] | — | — | `lexigram/src/lexigram/logging/config/redaction.py:LexigramConfig.logging.redaction.field_denylist` |
...
| `LEX_LEXIGRAM__REDACTION__ENABLED` | bool | True | — | `lexigram/src/lexigram/logging/config/redaction.py:LoggingConfig.redaction.enabled` |
| `LEX_LEXIGRAM__REDACTION__FIELD_DENYLIST` | tuple[str, ...] | — | — | `lexigram/src/lexigram/logging/config/redaction.py:LoggingConfig.redaction.field_denylist` |
...
| `LEX_ADMIN__AUDIT__READ_AUDIT_ENABLED` | bool | False | Log read operations (off by default; compliance mode only). | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.audit.read_audit_enabled` |
| `LEX_ADMIN__AUTH__CSRF_TOKEN_LIFETIME` | int | 3600 | CSRF token expiry in seconds | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.auth.csrf_token_lifetime` |
| `LEX_ADMIN__AUTH__EMAIL_OTP__ENABLED` | bool | True | Enable email OTP factor | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.auth.email_otp.enabled` |
| `LEX_ADMIN__AUTH__EMAIL_OTP__RESEND_COOLDOWN_SECONDS` | int | 60 | Minimum seconds between email OTP sends | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.auth.email_otp.resend_cooldown_seconds` |
| `LEX_ADMIN__AUTH__EMAIL_OTP__TTL_MINUTES` | int | 10 | Code validity window in minutes | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.auth.email_otp.ttl_minutes` |
| `LEX_ADMIN__AUTH__EMAIL_VERIFICATION__ENABLED` | bool | True | Enable email verification flow | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.auth.email_verification.enabled` |
| `LEX_ADMIN__AUTH__EMAIL_VERIFICATION__ENFORCEMENT` | bool | True | Block login until the email is verified | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.auth.email_verification.enforcement` |
| `LEX_ADMIN__AUTH__EMAIL_VERIFICATION__TOKEN_TTL_HOURS` | int | 24 | Verify link validity in hours | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.auth.email_verification.token_ttl_hours` |
| `LEX_ADMIN__AUTH__ENABLED` | bool | True | Enable authentication | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.auth.enabled` |
| `LEX_ADMIN__AUTH__ENV` | Literal['development', 'staging', 'production'] | 'development' | Deployment environment for cookie security defaults | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.auth.env` |
| `LEX_ADMIN__AUTH__IDLE_TIMEOUT` | int | 3600 | Session idle timeout in seconds | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.auth.idle_timeout` |
| `LEX_ADMIN__AUTH__LOGIN_URL` | str | '/admin/login' | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.auth.login_url` |
| `LEX_ADMIN__AUTH__LOGOUT_URL` | str | '/admin/logout' | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.auth.logout_url` |
| `LEX_ADMIN__AUTH__MFA__ENABLED` | bool | True | Enable TOTP 2FA | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.auth.mfa.enabled` |
| `LEX_ADMIN__AUTH__MFA__FACTOR` | str | 'totp' | Second factor used at login: 'totp' (authenticator app) or 'email' (one-time ... | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.auth.mfa.factor` |
| `LEX_ADMIN__AUTH__MFA__ISSUER` | str | 'Lexigram Admin' | TOTP issuer label shown in authenticator apps | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.auth.mfa.issuer` |
| `LEX_ADMIN__AUTH__MFA__SKEW` | int | 1 | Allowed clock skew in 30 second steps | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.auth.mfa.skew` |
| `LEX_ADMIN__AUTH__OAUTH_ENABLED` | bool | False | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.auth.oauth_enabled` |
| `LEX_ADMIN__AUTH__OAUTH_PROVIDERS` | list[str] | — | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.auth.oauth_providers` |
| `LEX_ADMIN__AUTH__PASSWORD_POLICY__MAX_LENGTH` | int | 128 | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.auth.password_policy.max_length` |
| `LEX_ADMIN__AUTH__PASSWORD_POLICY__MIN_LENGTH` | int | 12 | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.auth.password_policy.min_length` |
| `LEX_ADMIN__AUTH__PASSWORD_POLICY__REJECT_COMMON_PASSWORDS` | bool | True | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.auth.password_policy.reject_common_passwords` |
| `LEX_ADMIN__AUTH__PASSWORD_POLICY__REJECT_CONTAINING_EMAIL` | bool | True | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.auth.password_policy.reject_containing_email` |
| `LEX_ADMIN__AUTH__PASSWORD_POLICY__REQUIRE_DIGIT` | bool | True | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.auth.password_policy.require_digit` |
| `LEX_ADMIN__AUTH__PASSWORD_POLICY__REQUIRE_LOWERCASE` | bool | True | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.auth.password_policy.require_lowercase` |
| `LEX_ADMIN__AUTH__PASSWORD_POLICY__REQUIRE_SPECIAL` | bool | True | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.auth.password_policy.require_special` |
| `LEX_ADMIN__AUTH__PASSWORD_POLICY__REQUIRE_UPPERCASE` | bool | True | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.auth.password_policy.require_uppercase` |
| `LEX_ADMIN__AUTH__PERMISSION_CACHE_TTL` | int | 300 | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.auth.permission_cache_ttl` |
| `LEX_ADMIN__AUTH__PRINCIPAL_SOURCE` | Literal['internal', 'app'] | 'internal' | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.auth.principal_source` |
| `LEX_ADMIN__AUTH__REGISTRATION__ALLOWED_EMAIL_DOMAINS` | list[str] | — | Restrict registration to these email domains (empty = any) | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.auth.registration.allowed_email_domains` |
| `LEX_ADMIN__AUTH__REGISTRATION__DEFAULT_ROLE` | str | 'admin' | Role granted to new accounts | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.auth.registration.default_role` |
| `LEX_ADMIN__AUTH__REGISTRATION__ENABLED` | bool | False | Allow self-service registration | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.auth.registration.enabled` |
| `LEX_ADMIN__AUTH__ROLES` | dict[str, Any] | — | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.auth.roles` |
| `LEX_ADMIN__AUTH__SECURITY__IP_RATE_LIMIT_ENABLED` | bool | True | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.auth.security.ip_rate_limit_enabled` |
| `LEX_ADMIN__AUTH__SECURITY__IP_RATE_LIMIT_PER_15_MINUTES` | int | 30 | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.auth.security.ip_rate_limit_per_15_minutes` |
| `LEX_ADMIN__AUTH__SECURITY__IP_RATE_LIMIT_PER_HOUR` | int | 60 | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.auth.security.ip_rate_limit_per_hour` |
| `LEX_ADMIN__AUTH__SECURITY__IP_RATE_LIMIT_PER_MINUTE` | int | 10 | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.auth.security.ip_rate_limit_per_minute` |
| `LEX_ADMIN__AUTH__SECURITY__LOCKOUT_THRESHOLDS` | list[tuple[int, int]] | — | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.auth.security.lockout_thresholds` |
| `LEX_ADMIN__AUTH__SECURITY__PERMANENT_LOCKOUT_THRESHOLD` | int | 50 | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.auth.security.permanent_lockout_threshold` |
| `LEX_ADMIN__AUTH__SECURITY__SETUP_TOKEN` | str \| None | None | Optional ADMIN_SETUP_TOKEN — when set, must be provided during first-run setup. | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.auth.security.setup_token` |
| `LEX_ADMIN__AUTH__SECURITY__SETUP_TOKEN_OPTIN_UNSAFE` | bool | False | Explicit escape hatch: boot without a setup token. Only for local/ephemeral e... | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.auth.security.setup_token_optin_unsafe` |
| `LEX_ADMIN__AUTH__SESSION_LIFETIME` | int | 86400 | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.auth.session_lifetime` |
| `LEX_ADMIN__AUTH__SESSION_SECRET` | SecretStr | SecretStr('change-me-in-production') | Session secret for signing | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.auth.session_secret` |
| `LEX_ADMIN__AUTH__USERS` | list[Any] | — | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.auth.users` |
| `LEX_ADMIN__CLUSTERS__EXTRA` | list[ClusterSpec] | — | Extra clusters beyond the built-in infrastructure cluster | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.clusters.extra` |
| `LEX_ADMIN__COMMANDS` | list[dict[str, Any]] | — | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.commands` |
| `LEX_ADMIN__CONTRIBUTORS` | dict[str, ContributorConfig] | — | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.contributors` |
| `LEX_ADMIN__CONTRIBUTOR_COLLISION_MODE` | Literal['warn', 'error'] | 'warn' | How to handle name collisions when multiple contributors register widgets, pa... | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.contributor_collision_mode` |
| `LEX_ADMIN__DASHBOARD_LAYOUT__LAYOUT` | Literal['grid', 'masonry'] | 'grid' | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.dashboard_layout.layout` |
| `LEX_ADMIN__DASHBOARD_LAYOUT__MAX_WIDGETS` | int | 20 | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.dashboard_layout.max_widgets` |
| `LEX_ADMIN__DASHBOARD_LAYOUT__WIDGET_REFRESH_DEFAULT` | int | 30 | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.dashboard_layout.widget_refresh_default` |
| `LEX_ADMIN__DATA__QUERY_TIMEOUT_SECONDS` | int | 5 | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.data.query_timeout_seconds` |
...
| `LEX_ADMIN__EXTENSIONS` | dict[str, Any] | — | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.extensions` |
| `LEX_ADMIN__FEATURES__ACTIVITY_FEED` | bool | False | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.features.activity_feed` |
| `LEX_ADMIN__FEATURES__API_DOCS` | bool | True | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.features.api_docs` |
| `LEX_ADMIN__FEATURES__AUDIT_LOGGING` | bool | True | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.features.audit_logging` |
| `LEX_ADMIN__FEATURES__AUTOSAVE` | bool | False | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.features.autosave` |
| `LEX_ADMIN__FEATURES__COMMAND_PALETTE` | bool | True | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.features.command_palette` |
| `LEX_ADMIN__FEATURES__KEYBOARD_SHORTCUTS` | bool | True | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.features.keyboard_shortcuts` |
| `LEX_ADMIN__FEATURES__NOTIFICATIONS` | bool | True | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.features.notifications` |
| `LEX_ADMIN__FEATURES__OPTIMISTIC_UPDATES` | bool | True | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.features.optimistic_updates` |
| `LEX_ADMIN__FEATURES__SEARCH` | bool | True | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.features.search` |
| `LEX_ADMIN__FEATURES__THEME_TOGGLE` | bool | True | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.features.theme_toggle` |
| `LEX_ADMIN__FEATURES__UNDO_REDO` | bool | True | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.features.undo_redo` |
| `LEX_ADMIN__FEATURES__WEBHOOKS` | bool | False | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.features.webhooks` |
| `LEX_ADMIN__FORM_DEFAULTS` | FormDefaults | — | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.form_defaults` |
| `LEX_ADMIN__FRAMEWORK_PAGES__ENABLED` | bool | True | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.framework_pages.enabled` |
| `LEX_ADMIN__FRAMEWORK_PAGES__REQUIRE_PERMISSION` | str | 'admin:framework:access' | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.framework_pages.require_permission` |
| `LEX_ADMIN__HTMX_PREFIX` | str | '/admin/htmx' | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.htmx_prefix` |
| `LEX_ADMIN__INTEGRATIONS__CACHE__DEFAULT_TTL_SECONDS` | int | 60 | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.integrations.cache.default_ttl_seconds` |
| `LEX_ADMIN__INTEGRATIONS__CACHE__ENABLED` | bool | True | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.integrations.cache.enabled` |
| `LEX_ADMIN__INTEGRATIONS__CACHE__KEY_PREFIX` | str | 'admin' | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.integrations.cache.key_prefix` |
| `LEX_ADMIN__INTEGRATIONS__ENABLED` | bool | True | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.integrations.enabled` |
| `LEX_ADMIN__INTEGRATIONS__FEATURES__ENABLED` | bool | True | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.integrations.features.enabled` |
| `LEX_ADMIN__INTEGRATIONS__MONITOR__ENABLED` | bool | True | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.integrations.monitor.enabled` |
| `LEX_ADMIN__INTEGRATIONS__RESILIENCE__CIRCUIT_FAILURE_THRESHOLD` | int | 5 | — | `...-admin/src/lexigram/admin/config.py:AdminConfig.integrations.resilience.circuit_failure_threshold` |
| `LEX_ADMIN__INTEGRATIONS__RESILIENCE__ENABLED` | bool | True | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.integrations.resilience.enabled` |
| `LEX_ADMIN__INTEGRATIONS__RESILIENCE__RETRY_MAX_ATTEMPTS` | int | 3 | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.integrations.resilience.retry_max_attempts` |
| `LEX_ADMIN__INTEGRATIONS__SEARCH__ENABLED` | bool | True | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.integrations.search.enabled` |
| `LEX_ADMIN__INTEGRATIONS__SEARCH__FALLBACK_TO_LIKE` | bool | True | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.integrations.search.fallback_to_like` |
| `LEX_ADMIN__INTEGRATIONS__STORAGE__ENABLED` | bool | True | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.integrations.storage.enabled` |
| `LEX_ADMIN__INTEGRATIONS__STORAGE__PRESIGNED_URL_EXPIRY` | int | 3600 | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.integrations.storage.presigned_url_expiry` |
| `LEX_ADMIN__INTEGRATIONS__TASKS__BULK_THRESHOLD` | int | 25 | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.integrations.tasks.bulk_threshold` |
| `LEX_ADMIN__INTEGRATIONS__TASKS__ENABLED` | bool | True | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.integrations.tasks.enabled` |
| `LEX_ADMIN__NAME` | str | "admin" | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.name` |
| `LEX_ADMIN__NAVIGATION_GROUPS` | dict[str, AdminNavigationGroup] | — | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.navigation_groups` |
| `LEX_ADMIN__OBSERVABILITY__HIGH_CARDINALITY_LABELS_ENABLED` | bool | False | — | `...gram-admin/src/lexigram/admin/config.py:AdminConfig.observability.high_cardinality_labels_enabled` |
| `LEX_ADMIN__OBSERVABILITY__METRICS_ENABLED` | bool | True | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.observability.metrics_enabled` |
| `LEX_ADMIN__PREFIX` | str | '/admin' | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.prefix` |
| `LEX_ADMIN__RATE_LIMIT__BULK_PER_MINUTE` | int | 5 | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.rate_limit.bulk_per_minute` |
| `LEX_ADMIN__RATE_LIMIT__BURST_SIZE` | int | 10 | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.rate_limit.burst_size` |
| `LEX_ADMIN__RATE_LIMIT__CREATE_PER_MINUTE` | int | 30 | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.rate_limit.create_per_minute` |
| `LEX_ADMIN__RATE_LIMIT__DELETE_PER_MINUTE` | int | 20 | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.rate_limit.delete_per_minute` |
| `LEX_ADMIN__RATE_LIMIT__ENABLED` | bool | True | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.rate_limit.enabled` |
| `LEX_ADMIN__RATE_LIMIT__REQUESTS_PER_HOUR` | int | 1000 | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.rate_limit.requests_per_hour` |
| `LEX_ADMIN__RATE_LIMIT__REQUESTS_PER_MINUTE` | int | 60 | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.rate_limit.requests_per_minute` |
| `LEX_ADMIN__RATE_LIMIT__UPDATE_PER_MINUTE` | int | 60 | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.rate_limit.update_per_minute` |
| `LEX_ADMIN__RBAC__SUPER_ADMIN_ROLE` | str | 'superadmin' | already special-cased by settings/widgets/impersonation. | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.rbac.super_admin_role` |
| `LEX_ADMIN__REQUIRE_AUTH` | bool | True | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.require_auth` |
| `LEX_ADMIN__RESOURCES` | dict[str, ResourceYAMLConfig] | — | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.resources` |
| `LEX_ADMIN__RESOURCE_DEFAULTS` | ResourceDefaults | — | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.resource_defaults` |
...
| `LEX_ADMIN__STRICT_RESOURCE_RESOLUTION` | bool | True | When True (production default), resource/controller resolution failures durin... | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.strict_resource_resolution` |
| `LEX_ADMIN__TABLE_DEFAULTS` | TableDefaults | — | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.table_defaults` |
| `LEX_ADMIN__TEMPLATES_DIR` | str \| None | None | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.templates_dir` |
| `LEX_ADMIN__TENANCY__COOKIE_NAME` | str | 'admin_tenant' | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.tenancy.cookie_name` |
| `LEX_ADMIN__TENANCY__DEFAULT_TENANT_ID` | str | '' | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.tenancy.default_tenant_id` |
| `LEX_ADMIN__TENANCY__ENABLED` | bool | False | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.tenancy.enabled` |
| `LEX_ADMIN__TENANCY__HEADER_NAME` | str | 'x-tenant-id' | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.tenancy.header_name` |
| `LEX_ADMIN__TENANCY__ROUTE_PREFIX_TEMPLATE` | str | '' | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.tenancy.route_prefix_template` |
| `LEX_ADMIN__TENANCY__TENANT_FIELD` | str | 'tenant_id' | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.tenancy.tenant_field` |
| `LEX_ADMIN__TITLE` | str | 'Lexigram Admin' | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.title` |
| `LEX_ADMIN__UI__CONTENT_MAX_WIDTH` | int \| None | None | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.ui.content_max_width` |
| `LEX_ADMIN__UI__FAVICON_URL` | str \| None | None | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.ui.favicon_url` |
| `LEX_ADMIN__UI__LOGO_URL` | str \| None | None | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.ui.logo_url` |
| `LEX_ADMIN__UI__PRIMARY_COLOR` | str | '#6B7280' | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.ui.primary_color` |
| `LEX_ADMIN__UI__SIDEBAR_COLLAPSED_WIDTH` | int | 64 | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.ui.sidebar_collapsed_width` |
| `LEX_ADMIN__UI__SIDEBAR_WIDTH` | int | 256 | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.ui.sidebar_width` |
| `LEX_ADMIN__UI__THEME` | Literal['light', 'dark', 'system'] | 'system' | — | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.ui.theme` |
...
| Env Var | Type | Default | Description | Source |
|---------|------|---------|-------------|--------|
| `LEX_AI_EVALUATION__DEFAULT_SEED` | int \| None | None | Default seed for reproducible experiment runs | `lexigram-ai-evaluation/src/lexigram/ai/evaluation/config.py:EvaluationConfig.default_seed` |
...
| `LEX_AI_EVALUATION__EXPERIMENT_DIR` | str \| None | None | Base directory for experiment tracking and checkpoint artifacts | `lexigram-ai-evaluation/src/lexigram/ai/evaluation/config.py:EvaluationConfig.experiment_dir` |
...
| `LEX_AI_GOVERNANCE__FAIL_OPEN_ON_PERSISTENCE_ERROR` | bool | False | Allow requests when the persistence backend is unavailable. When False (defau... | `...i-governance/src/lexigram/ai/governance/config.py:GovernanceConfig.fail_open_on_persistence_error` |
| `LEX_AI_GOVERNANCE__MAX_REQUEST_COST` | float \| None | None | Maximum cost in dollars for a single request. Requests with an estimated cost... | `lexigram-ai-governance/src/lexigram/ai/governance/config.py:GovernanceConfig.max_request_cost` |
...
| `LEX_AI_GOVERNANCE__RESOURCE_UNITS` | list | — | Resource units this governance instance tracks. Per-tenant limits are configu... | `lexigram-ai-governance/src/lexigram/ai/governance/config.py:GovernanceConfig.resource_units` |
| `LEX_AI_GOVERNANCE__RESTRICTED_MODELS` | list[str] | — | List of restricted models | `lexigram-ai-governance/src/lexigram/ai/governance/config.py:GovernanceConfig.restricted_models` |
...
| `LEX_AI_GOVERNANCE__SOFT_LIMIT_PCT` | float \| None | None | Fraction of monthly_budget at which to emit a soft-limit warning (e.g. 0.8 = ... | `lexigram-ai-governance/src/lexigram/ai/governance/config.py:GovernanceConfig.soft_limit_pct` |
...
| `LEX_AI_GUARD__PII_ENTITIES` | list[str] | (complex) | — | `lexigram-ai-guard/src/lexigram/ai/guard/config.py:GuardConfig.pii_entities` |
...
| `LEX_AI_GUARD__RESTRICTED_TOPICS` | list[str] | (complex) | — | `lexigram-ai-guard/src/lexigram/ai/guard/config.py:GuardConfig.restricted_topics` |
...
| `LEX_AI_LLM__COOLDOWN_SECONDS` | int | 300 | Cooldown applied to a cascade entry after an HTTP 429. Env var: LEX_AI_LLM__Q... | `lexigram-ai-llm/src/lexigram/ai/llm/routing/config.py:QuotaConfig.cooldown_seconds` |
...
| `LEX_AI_LLM__PIN_POLICY` | ModelPinPolicy | ModelPinPolicy.LATEST | Policy for enforcing the model revision pin. | `lexigram-ai-llm/src/lexigram/ai/llm/config.py:ClientConfig.pin_policy` |
| `LEX_AI_LLM__PRICING__CACHE_TTL` | int | 86400 | Pricing cache TTL in seconds (default: 24 hours). | `lexigram-ai-llm/src/lexigram/ai/llm/config.py:ClientConfig.pricing.cache_ttl` |
| `LEX_AI_LLM__PRICING__ENABLED` | bool | True | Register pricing manager and cost estimator. | `lexigram-ai-llm/src/lexigram/ai/llm/config.py:ClientConfig.pricing.enabled` |
| `LEX_AI_LLM__PRICING__ENABLE_FUZZY_MATCH` | bool | True | Allow substring matching of model names to prices. | `lexigram-ai-llm/src/lexigram/ai/llm/config.py:ClientConfig.pricing.enable_fuzzy_match` |
| `LEX_AI_LLM__PRICING__SOURCES` | list[PricingSourceConfig] | — | Pricing sources in priority order. | `lexigram-ai-llm/src/lexigram/ai/llm/config.py:ClientConfig.pricing.sources` |
...
| `LEX_AI_LLM__QUOTA__COOLDOWN_SECONDS` | int | 300 | Cooldown applied to a cascade entry after an HTTP 429. Env var: LEX_AI_LLM__Q... | `lexigram-ai-llm/src/lexigram/ai/llm/routing/config.py:LLMConfig.quota.cooldown_seconds` |
...
| `LEX_AI_OBSERVABILITY__TRACE_MAX_ATTRIBUTE_LENGTH` | int | 0 | Cap on string attribute values written to trace spans, in characters. 0 disab... | `...ervability/src/lexigram/ai/observability/config.py:ObservabilityConfig.trace_max_attribute_length` |
| `LEX_AI_OBSERVABILITY__TRACE_REDACTION_ENABLED` | bool | False | Redact secret-shaped keys (e.g. token, password, api_key) from trace span att... | `...observability/src/lexigram/ai/observability/config.py:ObservabilityConfig.trace_redaction_enabled` |
...
| `LEX_AI_RAG__TENANCY__ENABLED` | bool | False | Enable tenant-aware collection resolution in RAG pipeline | `lexigram-ai-rag/src/lexigram/ai/rag/config.py:RAGConfig.tenancy.enabled` |
...
| Env Var | Type | Default | Description | Source |
|---------|------|---------|-------------|--------|
| `LEX_AI_RELAY_GATEWAY__AUTO_DISABLE_ON_FAILURES` | bool | False | — | `...relay-gateway/src/lexigram/ai/relay/gateway/config.py:RelayGatewayConfig.auto_disable_on_failures` |
| `LEX_AI_RELAY_GATEWAY__AUTO_TEST_CHANNELS` | bool | False | — | `...am-ai-relay-gateway/src/lexigram/ai/relay/gateway/config.py:RelayGatewayConfig.auto_test_channels` |
| `LEX_AI_RELAY_GATEWAY__AUTO_TEST_INTERVAL_SECONDS` | int | 600 | — | `...lay-gateway/src/lexigram/ai/relay/gateway/config.py:RelayGatewayConfig.auto_test_interval_seconds` |
| `LEX_AI_RELAY_GATEWAY__CHANNELS` | tuple[RelayChannel, ...] | () | — | `lexigram-ai-relay-gateway/src/lexigram/ai/relay/gateway/config.py:RelayGatewayConfig.channels` |
| `LEX_AI_RELAY_GATEWAY__FAILOVER_FAILURE_THRESHOLD` | int | 3 | — | `...lay-gateway/src/lexigram/ai/relay/gateway/config.py:RelayGatewayConfig.failover_failure_threshold` |
| `LEX_AI_RELAY_GATEWAY__JOB_TTL_SECONDS` | int | 3600 | — | `lexigram-ai-relay-gateway/src/lexigram/ai/relay/gateway/config.py:RelayGatewayConfig.job_ttl_seconds` |
| `LEX_AI_RELAY_GATEWAY__LOAD_BALANCING` | Literal['deterministic', 'weighted'] | "deterministic" | — | `lexigram-ai-relay-gateway/src/lexigram/ai/relay/gateway/config.py:RelayGatewayConfig.load_balancing` |
| `LEX_AI_RELAY_GATEWAY__MAX_UPSTREAM_RETRIES` | int | 0 | — | `...-ai-relay-gateway/src/lexigram/ai/relay/gateway/config.py:RelayGatewayConfig.max_upstream_retries` |
| `LEX_AI_RELAY_GATEWAY__PROVIDER_OPTIONS` | Mapping[str, Mapping[str, JsonValue]] | (complex) | — | `...gram-ai-relay-gateway/src/lexigram/ai/relay/gateway/config.py:RelayGatewayConfig.provider_options` |
| `LEX_AI_RELAY_GATEWAY__RATE_LIMITS` | Mapping[str, Mapping[str, int]] | (complex) | — | `lexigram-ai-relay-gateway/src/lexigram/ai/relay/gateway/config.py:RelayGatewayConfig.rate_limits` |
| `LEX_AI_RELAY_GATEWAY__REQUIRE_AUTH` | bool | True | — | `lexigram-ai-relay-gateway/src/lexigram/ai/relay/gateway/config.py:RelayGatewayConfig.require_auth` |
...
| `LEX_AUTH__ARGON2_MEMORY_COST` | int | 65536 | Argon2id memory cost in KiB (OWASP floor is 19456) | `lexigram-auth/src/lexigram/auth/config.py:PasswordConfig.argon2_memory_cost` |
| `LEX_AUTH__ARGON2_PARALLELISM` | int | 4 | Argon2id parallelism | `lexigram-auth/src/lexigram/auth/config.py:PasswordConfig.argon2_parallelism` |
| `LEX_AUTH__ARGON2_TIME_COST` | int | 3 | Argon2id time cost | `lexigram-auth/src/lexigram/auth/config.py:PasswordConfig.argon2_time_cost` |
...
| `LEX_AUTH__BCRYPT_ROUNDS` | int | 12 | bcrypt cost factor for new hashes (minimum 12 in production) | `lexigram-auth/src/lexigram/auth/config.py:PasswordConfig.bcrypt_rounds` |
...
| `LEX_AUTH__PASSWORD__ARGON2_MEMORY_COST` | int | 65536 | Argon2id memory cost in KiB (OWASP floor is 19456) | `lexigram-auth/src/lexigram/auth/config.py:AuthConfig.password.argon2_memory_cost` |
| `LEX_AUTH__PASSWORD__ARGON2_PARALLELISM` | int | 4 | Argon2id parallelism | `lexigram-auth/src/lexigram/auth/config.py:AuthConfig.password.argon2_parallelism` |
| `LEX_AUTH__PASSWORD__ARGON2_TIME_COST` | int | 3 | Argon2id time cost | `lexigram-auth/src/lexigram/auth/config.py:AuthConfig.password.argon2_time_cost` |
...
| `LEX_AUTH__PASSWORD__BCRYPT_ROUNDS` | int | 12 | bcrypt cost factor for new hashes (minimum 12 in production) | `lexigram-auth/src/lexigram/auth/config.py:AuthConfig.password.bcrypt_rounds` |
...
| `LEX_AUTH__RELAY_VERIFICATION` | bool | False | Enable binding ``RelayAuthVerifierProtocol`` for the relay gateway's inbound ... | `lexigram-auth/src/lexigram/auth/config.py:AuthConfig.relay_verification` |
...
| `LEX_AUTH__SECRET_KEY` | SecretStr | — | Secret key for signing tokens | `lexigram-auth/src/lexigram/auth/config.py:JWTConfig.secret_key` |
...
| `LEX_AUTH__TOKEN__SECRET_KEY` | SecretStr | — | Secret key for signing tokens | `lexigram-auth/src/lexigram/auth/config.py:AuthConfig.token.secret_key` |
...
| `LEX_GRAPH__TENANCY__ENABLED` | bool | False | Enable tenant-aware graph resolution | `lexigram-graph/src/lexigram/graph/config.py:GraphConfig.tenancy.enabled` |
| `LEX_GRAPH__TENANCY__STRATEGY` | str | 'node_property' | Which tenancy strategy to use. One of ``"node_property"`` or ``"graph_per_ten... | `lexigram-graph/src/lexigram/graph/config.py:GraphConfig.tenancy.strategy` |
| `LEX_GRAPH__TENANCY__TEMPLATE` | str | '{logical}_t_{tenant}' | Collection name template for ``GRAPH_PER_TENANT`` strategy. Supports ``{logic... | `lexigram-graph/src/lexigram/graph/config.py:GraphConfig.tenancy.template` |

### `lexigram-graphql` (85 vars)

| Env Var | Type | Default | Description | Source |
|---------|------|---------|-------------|--------|
| `LEX_GRAPHQL__ALIAS_LIMIT__ENABLED` | bool | True | — | `lexigram-graphql/src/lexigram/graphql/config.py:GraphQLConfig.alias_limit.enabled` |
| `LEX_GRAPHQL__ALIAS_LIMIT__MAX_ALIASES` | int | const.DEFAULT_MAX_ALIASES | — | `lexigram-graphql/src/lexigram/graphql/config.py:GraphQLConfig.alias_limit.max_aliases` |
...
| `LEX_GRAPHQL__MAX_ALIASES` | int | const.DEFAULT_MAX_ALIASES | — | `lexigram-graphql/src/lexigram/graphql/config.py:AliasLimitConfig.max_aliases` |
...
| `LEX_HTTP__ENFORCE_URL_SAFETY` | bool | True | — | `lexigram-http/src/lexigram/http/config.py:HTTPClientConfig.enforce_url_safety` |
...
| Env Var | Type | Default | Description | Source |
|---------|------|---------|-------------|--------|
| `LEX_MONITOR__ALERT_CHANNELS` | list[str] | — | Alert channel names for SLO violation dispatch | `lexigram-monitor/src/lexigram/monitor/config.py:SLOConfig.alert_channels` |
...
| `LEX_MONITOR__DSN` | str \| None | — | Sentry DSN; error tracking is a no-op when unset | `lexigram-monitor/src/lexigram/monitor/config.py:ErrorTrackingConfig.dsn` |
...
| `LEX_MONITOR__ERROR_TRACKING__DSN` | str \| None | — | Sentry DSN; error tracking is a no-op when unset | `lexigram-monitor/src/lexigram/monitor/config.py:MonitorConfig.error_tracking.dsn` |
| `LEX_MONITOR__ERROR_TRACKING__ENVIRONMENT` | str \| None | — | Environment tag for captured events | `lexigram-monitor/src/lexigram/monitor/config.py:MonitorConfig.error_tracking.environment` |
| `LEX_MONITOR__ERROR_TRACKING__SEND_DEFAULT_PII` | bool | — | Send default PII fields to the error tracker | `lexigram-monitor/src/lexigram/monitor/config.py:MonitorConfig.error_tracking.send_default_pii` |
| `LEX_MONITOR__ERROR_TRACKING__TRACES_SAMPLE_RATE` | float | — | Traces sample rate (0.0 to 1.0) | `lexigram-monitor/src/lexigram/monitor/config.py:MonitorConfig.error_tracking.traces_sample_rate` |
| `LEX_MONITOR__EVALUATION_INTERVAL` | float | — | SLO evaluation interval in seconds | `lexigram-monitor/src/lexigram/monitor/config.py:SLOConfig.evaluation_interval` |
...
| `LEX_MONITOR__SEND_DEFAULT_PII` | bool | — | Send default PII fields to the error tracker | `lexigram-monitor/src/lexigram/monitor/config.py:ErrorTrackingConfig.send_default_pii` |
...
| `LEX_MONITOR__SLO__ALERT_CHANNELS` | list[str] | — | Alert channel names for SLO violation dispatch | `lexigram-monitor/src/lexigram/monitor/config.py:MonitorConfig.slo.alert_channels` |
| `LEX_MONITOR__SLO__ENABLED` | bool | — | Enable periodic SLO evaluation worker | `lexigram-monitor/src/lexigram/monitor/config.py:MonitorConfig.slo.enabled` |
| `LEX_MONITOR__SLO__EVALUATION_INTERVAL` | float | — | SLO evaluation interval in seconds | `lexigram-monitor/src/lexigram/monitor/config.py:MonitorConfig.slo.evaluation_interval` |
| `LEX_MONITOR__SLO__SUPPRESSION_WINDOW_SECONDS` | int | — | Alert suppression window in seconds | `lexigram-monitor/src/lexigram/monitor/config.py:MonitorConfig.slo.suppression_window_seconds` |
...
| `LEX_MONITOR__SUPPRESSION_WINDOW_SECONDS` | int | — | Alert suppression window in seconds | `lexigram-monitor/src/lexigram/monitor/config.py:SLOConfig.suppression_window_seconds` |
...
| `LEX_MONITOR__TRACES_SAMPLE_RATE` | float | — | Traces sample rate (0.0 to 1.0) | `lexigram-monitor/src/lexigram/monitor/config.py:ErrorTrackingConfig.traces_sample_rate` |
...
| Env Var | Type | Default | Description | Source |
|---------|------|---------|-------------|--------|
| `LEX_MULTIMEDIA__BEAT` | BeatAnalysisConfig | — | — | `lexigram-multimedia/src/lexigram/multimedia/config.py:MultimediaConfig.beat` |
| `LEX_MULTIMEDIA__CACHE_RESULTS` | bool | False | — | `lexigram-multimedia/src/lexigram/multimedia/config.py:MultimediaConfig.cache_results` |
| `LEX_MULTIMEDIA__IMAGE` | ImageConfig | — | — | `lexigram-multimedia/src/lexigram/multimedia/config.py:MultimediaConfig.image` |
| `LEX_MULTIMEDIA__INTERPOLATE` | InterpolationConfig | — | — | `lexigram-multimedia/src/lexigram/multimedia/config.py:MultimediaConfig.interpolate` |
| `LEX_MULTIMEDIA__MUSIC` | MusicConfig | — | — | `lexigram-multimedia/src/lexigram/multimedia/config.py:MultimediaConfig.music` |
| `LEX_MULTIMEDIA__STORAGE_PATH_PREFIX` | str | "multimedia/" | — | `lexigram-multimedia/src/lexigram/multimedia/config.py:MultimediaConfig.storage_path_prefix` |
| `LEX_MULTIMEDIA__TTS` | TTSConfig | — | — | `lexigram-multimedia/src/lexigram/multimedia/config.py:MultimediaConfig.tts` |
| `LEX_MULTIMEDIA__UPSCALE` | UpscaleConfig | — | — | `lexigram-multimedia/src/lexigram/multimedia/config.py:MultimediaConfig.upscale` |
| `LEX_MULTIMEDIA__VIDEO` | VideoConfig | — | — | `lexigram-multimedia/src/lexigram/multimedia/config.py:MultimediaConfig.video` |

### `lexigram-multimedia-beat` (6 vars)

| Env Var | Type | Default | Description | Source |
|---------|------|---------|-------------|--------|
| `LEX_MULTIMEDIA_BEAT__BACKEND` | Literal['librosa', 'madmom'] | "librosa" | — | `lexigram-multimedia-beat/src/lexigram/multimedia/beat/config.py:BeatAnalysisConfig.backend` |
| `LEX_MULTIMEDIA_BEAT__LIBROSA_SAMPLE_RATE` | int | 22050 | — | `...ram-multimedia-beat/src/lexigram/multimedia/beat/config.py:BeatAnalysisConfig.librosa_sample_rate` |
| `LEX_MULTIMEDIA_BEAT__MADMOM_BASE_URL` | str | "http://localhost:5600" | — | `lexigram-multimedia-beat/src/lexigram/multimedia/beat/config.py:BeatAnalysisConfig.madmom_base_url` |
| `LEX_MULTIMEDIA_BEAT__MAX_ANALYZE_SAMPLES` | int | 60000000 | — | `...ram-multimedia-beat/src/lexigram/multimedia/beat/config.py:BeatAnalysisConfig.max_analyze_samples` |
| `LEX_MULTIMEDIA_BEAT__MAX_ASSET_BYTES` | int | 25 * 1024 * 1024 | — | `lexigram-multimedia-beat/src/lexigram/multimedia/beat/config.py:BeatAnalysisConfig.max_asset_bytes` |
| `LEX_MULTIMEDIA_BEAT__TIMEOUT` | float | 30.0 | — | `lexigram-multimedia-beat/src/lexigram/multimedia/beat/config.py:BeatAnalysisConfig.timeout` |

### `lexigram-multimedia-image` (13 vars)

| Env Var | Type | Default | Description | Source |
|---------|------|---------|-------------|--------|
| `LEX_MULTIMEDIA_IMAGE__BACKEND` | Literal['local-http', 'stability', 'openai', 'comfyui'] | "local-http" | — | `lexigram-multimedia-image/src/lexigram/multimedia/image/config.py:ImageConfig.backend` |
| `LEX_MULTIMEDIA_IMAGE__COMFYUI_BASE_URL` | str | "http://localhost:8188" | — | `lexigram-multimedia-image/src/lexigram/multimedia/image/config.py:ImageConfig.comfyui_base_url` |
| `LEX_MULTIMEDIA_IMAGE__COMFYUI_CFG_SCALE` | float | 7.0 | — | `lexigram-multimedia-image/src/lexigram/multimedia/image/config.py:ImageConfig.comfyui_cfg_scale` |
| `LEX_MULTIMEDIA_IMAGE__COMFYUI_CHECKPOINT` | str | "sd_xl_base_1.0.safetensors" | — | `lexigram-multimedia-image/src/lexigram/multimedia/image/config.py:ImageConfig.comfyui_checkpoint` |
| `LEX_MULTIMEDIA_IMAGE__COMFYUI_POLL_INTERVAL` | float | 1.0 | — | `lexigram-multimedia-image/src/lexigram/multimedia/image/config.py:ImageConfig.comfyui_poll_interval` |
| `LEX_MULTIMEDIA_IMAGE__COMFYUI_STEPS` | int | 20 | — | `lexigram-multimedia-image/src/lexigram/multimedia/image/config.py:ImageConfig.comfyui_steps` |
| `LEX_MULTIMEDIA_IMAGE__COMFYUI_WORKFLOW_PATH` | str \| None | None | — | `lexigram-multimedia-image/src/lexigram/multimedia/image/config.py:ImageConfig.comfyui_workflow_path` |
| `LEX_MULTIMEDIA_IMAGE__LOCAL_HTTP_BASE_URL` | str | "http://localhost:5005" | — | `lexigram-multimedia-image/src/lexigram/multimedia/image/config.py:ImageConfig.local_http_base_url` |
| `LEX_MULTIMEDIA_IMAGE__OPENAI_API_KEY_SECRET_NAME` | str | "openai_api_key" | — | `...m-multimedia-image/src/lexigram/multimedia/image/config.py:ImageConfig.openai_api_key_secret_name` |
| `LEX_MULTIMEDIA_IMAGE__OPENAI_BASE_URL` | str | "https://api.openai.com" | — | `lexigram-multimedia-image/src/lexigram/multimedia/image/config.py:ImageConfig.openai_base_url` |
| `LEX_MULTIMEDIA_IMAGE__OPENAI_MODEL` | str | "dall-e-3" | — | `lexigram-multimedia-image/src/lexigram/multimedia/image/config.py:ImageConfig.openai_model` |
| `LEX_MULTIMEDIA_IMAGE__STABILITY_API_KEY_SECRET_NAME` | str | "stability_api_key" | — | `...ultimedia-image/src/lexigram/multimedia/image/config.py:ImageConfig.stability_api_key_secret_name` |
| `LEX_MULTIMEDIA_IMAGE__TIMEOUT` | float | 60.0 | — | `lexigram-multimedia-image/src/lexigram/multimedia/image/config.py:ImageConfig.timeout` |

### `lexigram-multimedia-interpolate` (3 vars)

| Env Var | Type | Default | Description | Source |
|---------|------|---------|-------------|--------|
| `LEX_MULTIMEDIA_INTERPOLATE__BACKEND` | Literal['rife'] | "rife" | — | `...-multimedia-interpolate/src/lexigram/multimedia/interpolate/config.py:InterpolationConfig.backend` |
| `LEX_MULTIMEDIA_INTERPOLATE__RIFE_BASE_URL` | str | "http://localhost:5500" | — | `...media-interpolate/src/lexigram/multimedia/interpolate/config.py:InterpolationConfig.rife_base_url` |
| `LEX_MULTIMEDIA_INTERPOLATE__TIMEOUT` | float | 15.0 | — | `...-multimedia-interpolate/src/lexigram/multimedia/interpolate/config.py:InterpolationConfig.timeout` |

### `lexigram-multimedia-music` (5 vars)

| Env Var | Type | Default | Description | Source |
|---------|------|---------|-------------|--------|
| `LEX_MULTIMEDIA_MUSIC__ACE_STEP_BASE_URL` | str | "http://localhost:5300" | — | `lexigram-multimedia-music/src/lexigram/multimedia/music/config.py:MusicConfig.ace_step_base_url` |
| `LEX_MULTIMEDIA_MUSIC__BACKEND` | Literal['local-http', 'stability-audio', 'ace-step', 'stable-audio-open'] | "local-http" | — | `lexigram-multimedia-music/src/lexigram/multimedia/music/config.py:MusicConfig.backend` |
| `LEX_MULTIMEDIA_MUSIC__LOCAL_HTTP_BASE_URL` | str | "http://localhost:5003" | — | `lexigram-multimedia-music/src/lexigram/multimedia/music/config.py:MusicConfig.local_http_base_url` |
| `LEX_MULTIMEDIA_MUSIC__STABLE_AUDIO_OPEN_BASE_URL` | str | "http://localhost:5301" | — | `...m-multimedia-music/src/lexigram/multimedia/music/config.py:MusicConfig.stable_audio_open_base_url` |
| `LEX_MULTIMEDIA_MUSIC__TIMEOUT` | float | 60.0 | — | `lexigram-multimedia-music/src/lexigram/multimedia/music/config.py:MusicConfig.timeout` |

### `lexigram-multimedia-tts` (18 vars)

| Env Var | Type | Default | Description | Source |
|---------|------|---------|-------------|--------|
| `LEX_MULTIMEDIA_TTS__BACKEND` | Literal['local-http', 'elevenlabs', 'openai', 'chatterbox', 'kokoro', 'f5-tts', 'piper'] | "local-http" | — | `lexigram-multimedia-tts/src/lexigram/multimedia/tts/config.py:TTSConfig.backend` |
| `LEX_MULTIMEDIA_TTS__CHATTERBOX_BASE_URL` | str | "http://localhost:5100" | — | `lexigram-multimedia-tts/src/lexigram/multimedia/tts/config.py:TTSConfig.chatterbox_base_url` |
| `LEX_MULTIMEDIA_TTS__CHATTERBOX_CFG_WEIGHT` | float | 0.5 | — | `lexigram-multimedia-tts/src/lexigram/multimedia/tts/config.py:TTSConfig.chatterbox_cfg_weight` |
| `LEX_MULTIMEDIA_TTS__CHATTERBOX_EXAGGERATION` | float | 0.5 | — | `lexigram-multimedia-tts/src/lexigram/multimedia/tts/config.py:TTSConfig.chatterbox_exaggeration` |
| `LEX_MULTIMEDIA_TTS__CHATTERBOX_TEMPERATURE` | float | 0.85 | — | `lexigram-multimedia-tts/src/lexigram/multimedia/tts/config.py:TTSConfig.chatterbox_temperature` |
| `LEX_MULTIMEDIA_TTS__ELEVENLABS_API_KEY_SECRET_NAME` | str | "elevenlabs_api_key" | — | `...ram-multimedia-tts/src/lexigram/multimedia/tts/config.py:TTSConfig.elevenlabs_api_key_secret_name` |
| `LEX_MULTIMEDIA_TTS__ELEVENLABS_VOICE_ID` | str \| None | None | — | `lexigram-multimedia-tts/src/lexigram/multimedia/tts/config.py:TTSConfig.elevenlabs_voice_id` |
| `LEX_MULTIMEDIA_TTS__F5_TTS_BASE_URL` | str | "http://localhost:5102" | — | `lexigram-multimedia-tts/src/lexigram/multimedia/tts/config.py:TTSConfig.f5_tts_base_url` |
| `LEX_MULTIMEDIA_TTS__KOKORO_BASE_URL` | str | "http://localhost:5101" | — | `lexigram-multimedia-tts/src/lexigram/multimedia/tts/config.py:TTSConfig.kokoro_base_url` |
| `LEX_MULTIMEDIA_TTS__KOKORO_DEFAULT_VOICE` | str | "af_heart" | — | `lexigram-multimedia-tts/src/lexigram/multimedia/tts/config.py:TTSConfig.kokoro_default_voice` |
| `LEX_MULTIMEDIA_TTS__LOCAL_HTTP_BASE_URL` | str | "http://localhost:5002" | — | `lexigram-multimedia-tts/src/lexigram/multimedia/tts/config.py:TTSConfig.local_http_base_url` |
| `LEX_MULTIMEDIA_TTS__OPENAI_API_KEY_SECRET_NAME` | str | "openai_api_key" | — | `lexigram-multimedia-tts/src/lexigram/multimedia/tts/config.py:TTSConfig.openai_api_key_secret_name` |
| `LEX_MULTIMEDIA_TTS__OPENAI_BASE_URL` | str | "https://api.openai.com" | — | `lexigram-multimedia-tts/src/lexigram/multimedia/tts/config.py:TTSConfig.openai_base_url` |
| `LEX_MULTIMEDIA_TTS__OPENAI_MODEL` | str | "tts-1" | — | `lexigram-multimedia-tts/src/lexigram/multimedia/tts/config.py:TTSConfig.openai_model` |
| `LEX_MULTIMEDIA_TTS__OPENAI_VOICE` | str | "alloy" | — | `lexigram-multimedia-tts/src/lexigram/multimedia/tts/config.py:TTSConfig.openai_voice` |
| `LEX_MULTIMEDIA_TTS__PIPER_BASE_URL` | str | "http://localhost:5103" | — | `lexigram-multimedia-tts/src/lexigram/multimedia/tts/config.py:TTSConfig.piper_base_url` |
| `LEX_MULTIMEDIA_TTS__PIPER_DEFAULT_VOICE` | str | "en_US-lessac-medium" | — | `lexigram-multimedia-tts/src/lexigram/multimedia/tts/config.py:TTSConfig.piper_default_voice` |
| `LEX_MULTIMEDIA_TTS__TIMEOUT` | float | 60.0 | — | `lexigram-multimedia-tts/src/lexigram/multimedia/tts/config.py:TTSConfig.timeout` |

### `lexigram-multimedia-upscale` (4 vars)

| Env Var | Type | Default | Description | Source |
|---------|------|---------|-------------|--------|
| `LEX_MULTIMEDIA_UPSCALE__BACKEND` | Literal['real-esrgan', 'hat'] | "real-esrgan" | — | `lexigram-multimedia-upscale/src/lexigram/multimedia/upscale/config.py:UpscaleConfig.backend` |
| `LEX_MULTIMEDIA_UPSCALE__HAT_BASE_URL` | str | "http://localhost:5401" | — | `lexigram-multimedia-upscale/src/lexigram/multimedia/upscale/config.py:UpscaleConfig.hat_base_url` |
| `LEX_MULTIMEDIA_UPSCALE__REAL_ESRGAN_BASE_URL` | str | "http://localhost:5400" | — | `...m-multimedia-upscale/src/lexigram/multimedia/upscale/config.py:UpscaleConfig.real_esrgan_base_url` |
| `LEX_MULTIMEDIA_UPSCALE__TIMEOUT` | float | 30.0 | — | `lexigram-multimedia-upscale/src/lexigram/multimedia/upscale/config.py:UpscaleConfig.timeout` |

### `lexigram-multimedia-video` (25 vars)

| Env Var | Type | Default | Description | Source |
|---------|------|---------|-------------|--------|
| `LEX_MULTIMEDIA_VIDEO__BACKEND` | Literal['local-http', 'runway', 'openai', 'wan22', 'cogvideox', 'svd', 'comfyui'] | "local-http" | — | `lexigram-multimedia-video/src/lexigram/multimedia/video/config.py:VideoConfig.backend` |
| `LEX_MULTIMEDIA_VIDEO__COGVIDEOX_BASE_URL` | str | "http://localhost:5201" | — | `lexigram-multimedia-video/src/lexigram/multimedia/video/config.py:VideoConfig.cogvideox_base_url` |
| `LEX_MULTIMEDIA_VIDEO__COMFYUI_BASE_URL` | str | "http://localhost:8188" | — | `lexigram-multimedia-video/src/lexigram/multimedia/video/config.py:VideoConfig.comfyui_base_url` |
| `LEX_MULTIMEDIA_VIDEO__COMFYUI_CHECKPOINT` | str | "svd_xt_1_1.safetensors" | — | `lexigram-multimedia-video/src/lexigram/multimedia/video/config.py:VideoConfig.comfyui_checkpoint` |
| `LEX_MULTIMEDIA_VIDEO__COMFYUI_FPS` | int | 6 | — | `lexigram-multimedia-video/src/lexigram/multimedia/video/config.py:VideoConfig.comfyui_fps` |
| `LEX_MULTIMEDIA_VIDEO__COMFYUI_MOTION_BUCKET_ID` | int | 127 | — | `...ram-multimedia-video/src/lexigram/multimedia/video/config.py:VideoConfig.comfyui_motion_bucket_id` |
| `LEX_MULTIMEDIA_VIDEO__COMFYUI_POLL_INTERVAL` | float | 1.0 | — | `lexigram-multimedia-video/src/lexigram/multimedia/video/config.py:VideoConfig.comfyui_poll_interval` |
| `LEX_MULTIMEDIA_VIDEO__COMFYUI_WORKFLOW_PATH` | str \| None | None | — | `lexigram-multimedia-video/src/lexigram/multimedia/video/config.py:VideoConfig.comfyui_workflow_path` |
| `LEX_MULTIMEDIA_VIDEO__FFMPEG_BINARY` | str | "ffmpeg" | — | `...gram-multimedia-video/src/lexigram/multimedia/video/config.py:VideoProcessingConfig.ffmpeg_binary` |
| `LEX_MULTIMEDIA_VIDEO__LOCAL_HTTP_BASE_URL` | str | "http://localhost:5004" | — | `lexigram-multimedia-video/src/lexigram/multimedia/video/config.py:VideoConfig.local_http_base_url` |
| `LEX_MULTIMEDIA_VIDEO__MAX_ASSET_BYTES` | int | 25 * 1024 * 1024 | — | `...am-multimedia-video/src/lexigram/multimedia/video/config.py:VideoProcessingConfig.max_asset_bytes` |
| `LEX_MULTIMEDIA_VIDEO__MAX_CONCURRENT_JOBS` | int | 2 | — | `...ultimedia-video/src/lexigram/multimedia/video/config.py:VideoProcessingConfig.max_concurrent_jobs` |
| `LEX_MULTIMEDIA_VIDEO__OPENAI_API_KEY_SECRET_NAME` | str | "openai_api_key" | — | `...m-multimedia-video/src/lexigram/multimedia/video/config.py:VideoConfig.openai_api_key_secret_name` |
| `LEX_MULTIMEDIA_VIDEO__OPENAI_BASE_URL` | str | "https://api.openai.com" | — | `lexigram-multimedia-video/src/lexigram/multimedia/video/config.py:VideoConfig.openai_base_url` |
| `LEX_MULTIMEDIA_VIDEO__OPENAI_MODEL` | str | "sora-2" | — | `lexigram-multimedia-video/src/lexigram/multimedia/video/config.py:VideoConfig.openai_model` |
| `LEX_MULTIMEDIA_VIDEO__PROCESSING__FFMPEG_BINARY` | str | "ffmpeg" | — | `...ram-multimedia-video/src/lexigram/multimedia/video/config.py:VideoConfig.processing.ffmpeg_binary` |
| `LEX_MULTIMEDIA_VIDEO__PROCESSING__MAX_ASSET_BYTES` | int | 25 * 1024 * 1024 | — | `...m-multimedia-video/src/lexigram/multimedia/video/config.py:VideoConfig.processing.max_asset_bytes` |
| `LEX_MULTIMEDIA_VIDEO__PROCESSING__MAX_CONCURRENT_JOBS` | int | 2 | — | `...ltimedia-video/src/lexigram/multimedia/video/config.py:VideoConfig.processing.max_concurrent_jobs` |
| `LEX_MULTIMEDIA_VIDEO__PROCESSING__TEMP_DIR` | str \| None | None | — | `lexigram-multimedia-video/src/lexigram/multimedia/video/config.py:VideoConfig.processing.temp_dir` |
| `LEX_MULTIMEDIA_VIDEO__PROCESSING__TIMEOUT` | float | 300.0 | — | `lexigram-multimedia-video/src/lexigram/multimedia/video/config.py:VideoConfig.processing.timeout` |
| `LEX_MULTIMEDIA_VIDEO__RUNWAY_API_KEY_SECRET_NAME` | str | "runway_api_key" | — | `...m-multimedia-video/src/lexigram/multimedia/video/config.py:VideoConfig.runway_api_key_secret_name` |
| `LEX_MULTIMEDIA_VIDEO__SVD_BASE_URL` | str | "http://localhost:5202" | — | `lexigram-multimedia-video/src/lexigram/multimedia/video/config.py:VideoConfig.svd_base_url` |
| `LEX_MULTIMEDIA_VIDEO__TEMP_DIR` | str \| None | None | — | `lexigram-multimedia-video/src/lexigram/multimedia/video/config.py:VideoProcessingConfig.temp_dir` |
| `LEX_MULTIMEDIA_VIDEO__TIMEOUT` | float | 300.0 | — | `lexigram-multimedia-video/src/lexigram/multimedia/video/config.py:VideoProcessingConfig.timeout` |
| `LEX_MULTIMEDIA_VIDEO__WAN22_BASE_URL` | str | "http://localhost:5200" | — | `lexigram-multimedia-video/src/lexigram/multimedia/video/config.py:VideoConfig.wan22_base_url` |
...
| `LEX_NOTIFICATION__CONSOLE_FALLBACK` | bool | True | When no backends are configured, bind a ConsoleMailer as the default MailerPr... | `lexigram-notification/src/lexigram/notification/config.py:MailerConfig.console_fallback` |
...
| Env Var | Type | Default | Description | Source |
|---------|------|---------|-------------|--------|
| `LEX_SECRETS__AUDIT_ACTOR_ID` | str | — | Actor identifier for audit log entries | `lexigram-secrets/src/lexigram/secrets/config.py:SecretsConfig.audit_actor_id` |
| `LEX_SECRETS__BACKEND_OPTIONS` | dict | — | Keyword arguments for backend constructor | `lexigram-secrets/src/lexigram/secrets/config.py:SecretsConfig.backend_options` |
| `LEX_SECRETS__BACKEND_TYPE` | str | — | Backend store type (memory, vault, ...) | `lexigram-secrets/src/lexigram/secrets/config.py:SecretsConfig.backend_type` |
| `LEX_SECRETS__ENABLED` | bool | — | Whether secrets subsystem is enabled | `lexigram-secrets/src/lexigram/secrets/config.py:SecretsConfig.enabled` |
| `LEX_SECRETS__MAX_AGE_SECONDS` | float | — | Seconds before automatic rotation | `lexigram-secrets/src/lexigram/secrets/config.py:SecretsConfig.max_age_seconds` |
| `LEX_SECRETS__NAME` | str | — | Configuration name | `lexigram-secrets/src/lexigram/secrets/config.py:SecretsConfig.name` |
| `LEX_SECRETS__TENANT_ID` | str \| None | — | Optional tenant namespace | `lexigram-secrets/src/lexigram/secrets/config.py:SecretsConfig.tenant_id` |
| `LEX_SECRETS__WARNING_BEFORE_SECONDS` | float | — | Seconds before expiry to warn | `lexigram-secrets/src/lexigram/secrets/config.py:SecretsConfig.warning_before_seconds` |
...
| `LEX_TENANCY__RESOLUTION__STRICT_MEMBERSHIP` | bool | True | — | `lexigram-tenancy/src/lexigram/tenancy/config.py:TenancyConfig.resolution.strict_membership` |
...
| `LEX_TENANCY__RESOLUTION__TRUSTED_RESOLVERS` | list[str] | (complex) | — | `lexigram-tenancy/src/lexigram/tenancy/config.py:TenancyConfig.resolution.trusted_resolvers` |
...
| `LEX_VECTOR__TENANCY__ENABLED` | bool | False | Enable tenant-aware collection name resolution | `lexigram-vector/src/lexigram/vector/config.py:VectorConfig.tenancy.enabled` |
| `LEX_VECTOR__TENANCY__RESOLVER_KIND` | str | 'templated' | Which ``TenantCollectionResolver`` to use. One of ``"templated"`` or ``"pinec... | `lexigram-vector/src/lexigram/vector/config.py:VectorConfig.tenancy.resolver_kind` |
...
| Env Var | Type | Default | Description | Source |
|---------|------|---------|-------------|--------|
| `LEX_WEB__ALLOWED_HOSTS` | list[str] | — | Hostnames permitted to reach the application. Empty by default; must be confi... | `lexigram-web/src/lexigram/web/security/config.py:SecurityConfig.allowed_hosts` |
...
| `LEX_WEB__API_DOCS__ENABLED` | bool | True | Enable API documentation endpoints (/docs, /redoc) and auto-configure CSP for... | `lexigram-web/src/lexigram/web/config.py:WebConfig.api_docs.enabled` |
...
| `LEX_WEB__CORS__DEBUG_PERMISSIVE` | bool | False | When True and debug mode is active, allow any origin via wildcard (explicit o... | `lexigram-web/src/lexigram/web/security/config.py:SecurityConfig.cors.debug_permissive` |
...
| `LEX_WEB__CSRF__EXCLUDED_PATHS` | list[str] | — | URL path prefixes exempt from CSRF validation for cookie-less requests; cooki... | `lexigram-web/src/lexigram/web/security/config.py:SecurityConfig.csrf.excluded_paths` |
| `LEX_WEB__CSRF__EXCLUDE_AUTH_SCHEMES` | list[str] | — | Authorization header schemes that bypass CSRF validation (explicit opt-in). | `lexigram-web/src/lexigram/web/security/config.py:SecurityConfig.csrf.exclude_auth_schemes` |
| `LEX_WEB__CSRF__EXCLUDE_CONTENT_TYPES` | list[str] | — | Content-Type values that bypass CSRF validation (explicit opt-in — JSON reque... | `lexigram-web/src/lexigram/web/security/config.py:SecurityConfig.csrf.exclude_content_types` |
...
| `LEX_WEB__CSRF__SECRET_KEY` | str \| None | None | HMAC secret used to sign and verify CSRF tokens (populated via LEX_WEB__SECUR... | `lexigram-web/src/lexigram/web/security/config.py:SecurityConfig.csrf.secret_key` |
...
| `LEX_WEB__DEBUG_PERMISSIVE` | bool | False | When True and debug mode is active, allow any origin via wildcard (explicit o... | `lexigram-web/src/lexigram/web/security/config.py:CORSConfig.debug_permissive` |
...
| `LEX_WEB__ENABLED` | bool | True | Enable API documentation endpoints (/docs, /redoc) and auto-configure CSP for... | `lexigram-web/src/lexigram/web/config.py:APIDocsConfig.enabled` |
...
| `LEX_WEB__EXCLUDED_PATHS` | list[str] | — | URL path prefixes exempt from CSRF validation for cookie-less requests; cooki... | `lexigram-web/src/lexigram/web/security/config.py:CSRFConfig.excluded_paths` |
| `LEX_WEB__EXCLUDE_AUTH_SCHEMES` | list[str] | — | Authorization header schemes that bypass CSRF validation (explicit opt-in). | `lexigram-web/src/lexigram/web/security/config.py:CSRFConfig.exclude_auth_schemes` |
| `LEX_WEB__EXCLUDE_CONTENT_TYPES` | list[str] | — | Content-Type values that bypass CSRF validation (explicit opt-in — JSON reque... | `lexigram-web/src/lexigram/web/security/config.py:CSRFConfig.exclude_content_types` |
...
| `LEX_WEB__PATH` | str | '' | Path to guard ('/**' suffix matches the prefix) | `lexigram-web/src/lexigram/web/config.py:RoleGuardRuleConfig.path` |
...
| `LEX_WEB__RATE_LIMIT__ENABLED` | bool | True | Enable rate limiting. When true, RateLimitMiddleware enforces the matched per... | `lexigram-web/src/lexigram/web/config.py:WebConfig.rate_limit.enabled` |
| `LEX_WEB__RATE_LIMIT__RULES` | dict[str, RateLimitRuleConfig] | — | Per-path rate limit rules; longest-prefix match wins | `lexigram-web/src/lexigram/web/config.py:WebConfig.rate_limit.rules` |
...
| `LEX_WEB__ROLES` | list[str] | — | Role identifiers allowed to pass | `lexigram-web/src/lexigram/web/config.py:RoleGuardRuleConfig.roles` |
| `LEX_WEB__ROLE_GUARD__RULES` | list[RoleGuardRuleConfig] | — | Role guard rules in declaration order | `lexigram-web/src/lexigram/web/config.py:WebConfig.role_guard.rules` |
| `LEX_WEB__RULES` | dict[str, RateLimitRuleConfig] | — | Per-path rate limit rules; longest-prefix match wins | `lexigram-web/src/lexigram/web/config.py:RateLimitConfig.rules` |
| `LEX_WEB__SECRET_KEY` | str \| None | None | HMAC secret used to sign and verify CSRF tokens (populated via LEX_WEB__SECUR... | `lexigram-web/src/lexigram/web/security/config.py:CSRFConfig.secret_key` |
| `LEX_WEB__SECURITY__ALLOWED_HOSTS` | list[str] | — | Hostnames permitted to reach the application. Empty by default; must be confi... | `lexigram-web/src/lexigram/web/security/config.py:WebConfig.security.allowed_hosts` |
...
| `LEX_WEB__SECURITY__CORS__DEBUG_PERMISSIVE` | bool | False | When True and debug mode is active, allow any origin via wildcard (explicit o... | `lexigram-web/src/lexigram/web/security/config.py:WebConfig.security.cors.debug_permissive` |
...
| `LEX_WEB__SECURITY__CSRF__EXCLUDED_PATHS` | list[str] | — | URL path prefixes exempt from CSRF validation for cookie-less requests; cooki... | `lexigram-web/src/lexigram/web/security/config.py:WebConfig.security.csrf.excluded_paths` |
| `LEX_WEB__SECURITY__CSRF__EXCLUDE_AUTH_SCHEMES` | list[str] | — | Authorization header schemes that bypass CSRF validation (explicit opt-in). | `lexigram-web/src/lexigram/web/security/config.py:WebConfig.security.csrf.exclude_auth_schemes` |
| `LEX_WEB__SECURITY__CSRF__EXCLUDE_CONTENT_TYPES` | list[str] | — | Content-Type values that bypass CSRF validation (explicit opt-in — JSON reque... | `lexigram-web/src/lexigram/web/security/config.py:WebConfig.security.csrf.exclude_content_types` |
...
| `LEX_WEB__SECURITY__CSRF__SECRET_KEY` | str \| None | None | HMAC secret used to sign and verify CSRF tokens (populated via LEX_WEB__SECUR... | `lexigram-web/src/lexigram/web/security/config.py:WebConfig.security.csrf.secret_key` |
...
| Env Var | Type | Default | Description | Source |
|---------|------|---------|-------------|--------|
| `LEX_WEBHOOK__ALLOW_PRIVATE_URLS` | bool | — | Allow registering and delivering webhooks to private, loopback, or link-local... | `lexigram-webhook/src/lexigram/webhook/config.py:WebhookConfig.allow_private_urls` |
...
| `LEX_WORKFLOW__DEFAULT_TTL_SECONDS` | int | 86400 | Default TTL for cache-backed checkpoint stores. | `lexigram-workflow/src/lexigram/workflow/config.py:ContentCheckpointConfig.default_ttl_seconds` |
...
| `LEX_WORKFLOW__INLINE_THRESHOLD_BYTES` | int | 1048576 | Max inline bytes before blob offload. | `lexigram-workflow/src/lexigram/workflow/config.py:ContentCheckpointConfig.inline_threshold_bytes` |
