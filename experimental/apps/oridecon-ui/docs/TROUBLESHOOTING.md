...
**Cause:** `oridecon-ui` not installed or the add command can't find it.

**Solution:**

```bash
# Verify installation
uv run python -c "import oridecon.ui; print(oridecon.ui.__file__)"
...
## Problem: `oridecon-ui add` copies files but they don't work
...
```bash
oridecon-ui add form --output src/components/ui --force
```
...
- Enable debug mode: `ORI_UI__DEBUG_COMPONENTS=true` — renders `data-component` markers
