...
Learn how to use `oridecon-ui` effectively.
...
```python
from oridecon.ui import Component, el, render_to_string
...
html = render_to_string(Greeting(name="Oridecon"))
# <h1 class="text-foreground text-2xl font-bold">Hello, Oridecon!</h1>
```
...
```python
# Atom
from oridecon.ui import Button, Label, Input

# Molecule
from oridecon.ui import InputGroup, Card
...
```python
from oridecon.ui import Button
...
```python
from oridecon.ui import Zones, Zone, SwapMode
...
The `oridecon-ui add` command copies component source files into your project, similar to `npx shadcn add`:

```bash
# List available components
oridecon-ui add --help

# Add a button component
oridecon-ui add button

# Add a form with custom output directory
oridecon-ui add form -o my_app/components/ui
```
...
```python
from oridecon.ui import ServerToastChannel
...
```python
from oridecon.ui import validation_error, not_found_error, server_error, htmx_error_response
...
```python
from oridecon.ui import AriaAttrs
...
```python
from oridecon.ui import SkipLink
...
```python
from oridecon.ui import announce
