...
Internal design of the `oridecon-ui` package.
...
```bash
oridecon-ui add button     # Copy button component into src/components/ui/
oridecon-ui add card -o my_components/ui  # Custom output directory
```
...
- Resolves the installed `oridecon-ui` package path
...
**HTMX zones** are registered in `oridecon.ui.core.zones.Zones` and target server-rendered partials. **Alpine** is used for local interactivity that never needs a server round-trip. Both can coexist on the same page — HTMX handles data flows, Alpine handles UI choreography.
...
`oridecon.ui.core.zones.Zones` defines canonical HTMX swap targets:
...
The `UI_CSP_REQUIREMENTS` dict in `constants.py` provides the Content Security Policy directives needed for oridecon-ui's external dependencies:
