...
Complete reference of every configuration dataclass in `oridecon-ui`.
...
Controls rendering defaults. Read from the `[ui]` section of `application.yaml` (or env vars prefixed with `ORI_UI__`).
...
All fields are overridable via `ORI_UI__<FIELD>`:

```bash
ORI_UI__THEME=dark ORI_UI__ENABLE_SSE=true
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `site_name` | `str` | `"Oridecon Admin"` | Site name for title suffix |
...
```python
from oridecon.ui.constants import UI_CSP_REQUIREMENTS
```

Add these to your application's CSP headers when using oridecon-ui with external CDN assets (HTMX, Alpine.js, Lucide icons, Tailwind CDN).
...
flowchart LR
    subgraph Sources[Config Sources]
        ...
        ENV[Environment variables<br/>ORI_UI__*]
...
export ORI_UI__ENABLE_SSE=true
```
