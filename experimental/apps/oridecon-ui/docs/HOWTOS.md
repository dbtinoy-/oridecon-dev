...
```mermaid
flowchart LR
    A[Create file<br/>oridecon/ui/atoms/your_atom.py] --> B[Define class<br/>extending Component]
    B --> C[Add lazy import<br/>to __init__.py]
    C --> D[Add symbol to<br/>CANONICAL_API_SYMBOLS]
    D --> E[Write unit tests<br/>in test_atoms.py]
```

1. Create a file in `oridecon/ui/atoms/your_atom.py`
...
3. Add the lazy import to `oridecon/ui/__init__.py`
...
```python
from oridecon.ui import AbstractInput, el
...
```python
from oridecon.ui.layouts import LayoutBase
...
oridecon-ui add card

# Add a modal to a custom directory with force overwrite
oridecon-ui add modal --output src/components/ui --force
```
...
```python
from oridecon.ui import Zone, Zones, SwapMode
...
## How to Configure CSP for oridecon-ui
