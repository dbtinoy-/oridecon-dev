...
``oridecon.search.filterset.BlockQueryTranslator`` (which lowers it to
...
below are display-only.  ``oridecon-ui`` is a leaf package and cannot
import oridecon-search, so this list mirrors the translator's
        ...
        controls.extend(
            [
                ...
                el(
                    ...
                    type="button",
                    **{"x-on:click": f"removeNode({var}.id)"},
                    title="Remove rule",
        ...
        body: list[Any] = [
            ...
            el(
                ...
                el(
                    ...
                    "Group",
                    type="button",
                    **{"x-on:click": f"addGroup({var}.id, 'AND')"},
                    class_="inline-flex items-center px-2 py-1 border border-dashed border-border rounded text-xs text-foreground bg-card hover:bg-accent",
                ),
                class_="flex gap-2",
            ),
