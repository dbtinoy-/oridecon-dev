...
from oridecon.ui import Component, Zones, el
...
class FilterDrawer(Component):
    ...
    def render(self) -> Any:
        ...
        return el(
            ...
            panel,
            **{
                "x-data": "{ filterDrawerOpen: $persist(false).as('oridecon_filter_drawer_open') }",
                "class": "relative",
            },
        )
    ...
    def _render_panel(self) -> Any:
        ...
        apply_attrs: dict[str, Any] = {
            ...
            "@click": (
                ...
                f"  var key = 'oridecon_filter_{res_prefix}_' + (el.name || el.id || ''); "
                f"  if (key !== 'oridecon_filter_{res_prefix}_') {{ localStorage.setItem(key, el.value); }} "
                "});"
            ),
        }
