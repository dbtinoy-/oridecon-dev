"""``oridecon-ui add`` — copy components into the user's project."""
...
from oridecon.ui.cli.registry import COMPONENT_REGISTRY, ComponentEntry
...
def add(
    ...
    if ui_pkg is None:
        typer.echo(
            "Cannot locate oridecon-ui source. Install it in editable mode:\n"
            "  uv pip install -e path/to/oridecon-ui",
            err=True,
        )
        raise typer.Exit(1)
...
def _find_ui_package() -> Path | None:
    """Locate the oridecon-ui source directory (package root containing oridecon/)."""
    import oridecon.ui  # noqa: F811

    pkg_path = Path(oridecon.ui.__file__).resolve().parent
    # pkg_path is the ui module directory
    if pkg_path.name == "ui" and pkg_path.parent.name == "oridecon":
        # source layout: .../src/oridecon/ui/ or .../oridecon/ui/
        parent = pkg_path.parent.parent  # .../src/ or .../
        if (parent / "oridecon" / "ui").exists():
            return parent
        # Check if src/ prefix is present
        if parent.name == "src" and (parent.parent / "oridecon" / "ui").exists():
