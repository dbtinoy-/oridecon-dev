...
from oridecon.cli.lib.config_gen import (
    _collect_comments,
    _dict_to_yaml_lines,
    _format_yaml_value,
    _get_default_value,
...
class TestImportConfigClass:
    def test_import_success(self) -> None:
        with patch("oridecon.cli.lib.config_gen.importlib.import_module") as mock_import:
            mock_mod = MagicMock()
            mock_mod.DatabaseConfig = "DatabaseConfigCls"
            mock_import.return_value = mock_mod
            result = _import_config_class("oridecon.sql.config:DatabaseConfig")
            assert result == "DatabaseConfigCls"

    def test_import_failure(self) -> None:
        with patch("oridecon.cli.lib.config_gen.importlib.import_module", side_effect=ImportError):
...
class TestModelToYamlDict:
    def test_converts_fields(self) -> None:
        ...
        with patch("oridecon.cli.lib.config_gen._get_default_value", side_effect=get_default):
...
class TestGeneratePackageConfig:
    @patch("oridecon.cli.lib.config_gen._import_config_class")
    @patch("oridecon.cli.lib.config_gen._model_to_yaml_dict", return_value={"key": "val"})
    @patch("oridecon.cli.lib.config_gen._collect_comments", return_value={})
    def test_generates_config(self, mock_comments, mock_model, mock_import) -> None:
        ...
        result = generate_package_config("database", "oridecon.sql.config:DatabaseConfig")
        assert "database" in result
        assert "key" in result

    @patch("oridecon.cli.lib.config_gen._import_config_class", side_effect=ImportError("no module"))
...
class TestRunConfigGen:
    def test_all_packages(self) -> None:
        ...
        with patch("pathlib.Path.mkdir"):
            with patch("oridecon.cli.lib.config_gen.generate_package_config", return_value="key: val\n"):
    ...
    def test_specific_package(self) -> None:
        ...
        with patch("pathlib.Path.mkdir"):
            with patch("oridecon.cli.lib.config_gen.generate_package_config", return_value="database:\n  key: val\n"):
                with patch("pathlib.Path.write_text"):
                    run_config_gen(mock_args)
