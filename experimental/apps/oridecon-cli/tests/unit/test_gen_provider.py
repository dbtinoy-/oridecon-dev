"""Tests for `oridecon gen provider` command."""
...
from oridecon.cli.assembly.assembler import _detect_src_dir
...
class TestGenProvider:
    ...
    def test_gen_provider_correct_imports(
        ...
        """Generated provider must use correct oridecon.di import path."""
        self.runner.invoke(
            cli_app_with_core_generators,
            ["gen", "provider", "billing", "--output-dir", str(tmp_path)],
        )
        content = (tmp_path / "billing_provider.py").read_text()
        assert "from oridecon.di import" in content
