...
from oridecon.cli.registry.health import (
    CheckStatus,
    CrossExtensionImportCheck,
    InstalledOrideconPackagesCheck,
    ProviderPackagesCheck,
...
class TestInstalledOrideconPackagesCheck:
    """Tests for InstalledOrideconPackagesCheck."""
    ...
    def test_pass_when_packages_found(self) -> None:
        """Returns PASS with a count+list when oridecon packages are installed."""
        fake_dists = [
            self._make_dist("oridecon", "0.1.0"),
            self._make_dist("oridecon-web", "1.0.0"),
            self._make_dist("unrelated-package", "99.0"),
        ]
        with patch(
            "oridecon.cli.registry.health_checks.deps.distributions",
            return_value=fake_dists,
        ):
            result = InstalledOrideconPackagesCheck().check()

        assert result.status is CheckStatus.PASS
        assert "2 package(s)" in result.message
        assert "oridecon==0.1.0" in result.message
        assert "oridecon-web==1.0.0" in result.message
    ...
    def test_warning_when_no_packages(self) -> None:
        """Returns WARNING when no oridecon-* packages are installed."""
        with patch(
            "oridecon.cli.registry.health_checks.deps.distributions",
        ...
        assert "No oridecon" in result.message
    ...
    def test_get_name_and_category(self) -> None:
        ...
        chk = InstalledOrideconPackagesCheck()
...
class TestProviderPackagesCheck:
    ...
    def test_skip_when_no_oridecon_yaml(self, tmp_path: Path) -> None:
    ...
    def test_pass_when_all_packages_installed(self, tmp_path: Path) -> None:
        ...
        try:
            with patch(
                "oridecon.cli.registry.health_checks.providers.distribution",
    ...
    def test_fail_when_package_missing(self, tmp_path: Path) -> None:
        ...
        try:
            with patch(
                "oridecon.cli.registry.health_checks.providers.distribution",
                side_effect=PackageNotFoundError("oridecon-cache"),
            ):
                result = ProviderPackagesCheck().check()
        finally:
            os.chdir(orig_cwd)
    ...
    def test_security_section_is_not_treated_as_deleted_provider(
        ...
        """Security config should not require the deleted oridecon-security package."""
        (tmp_path / "application.yaml").write_text(
            "security:\n  headers:\n    enabled: true\n"
        )
...
class TestCrossExtensionImportCheck:
    ...
    def test_pass_when_no_violations(self, tmp_path: Path) -> None:
        ...
        src = tmp_path / "src" / "oridecon" / "web"
        src.mkdir(parents=True)
        (src / "handler.py").write_text(
            "from oridecon.contracts.web import ControllerProtocol\n"
        )
    ...
    def test_fail_when_violation_detected(self, tmp_path: Path) -> None:
        """Returns FAIL when a oridecon.web file imports from oridecon.sql directly."""
        src = tmp_path / "src" / "oridecon" / "web"
        src.mkdir(parents=True)
        (src / "repo.py").write_text(
            textwrap.dedent("""\
                from oridecon.sql.session import AsyncSession
        ...
        assert "oridecon.web" in result.details["violations"][0]
        assert "oridecon.sql" in result.details["violations"][0]
    ...
    def test_multiple_violations_counted_correctly(self, tmp_path: Path) -> None:
        ...
        for name in ("a.py", "b.py"):
            web_dir = tmp_path / "src" / "oridecon" / "web"
            web_dir.mkdir(parents=True, exist_ok=True)
            (web_dir / name).write_text("from oridecon.sql.models import Base\n")
