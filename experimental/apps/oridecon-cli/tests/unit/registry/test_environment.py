...
from oridecon.cli.registry.environment import (
...
class TestStagingEnvironment:
    ...
    def test_validate_missing_db_url(self) -> None:
        with patch("oridecon.cli.registry.environment.os.environ.get", return_value=""):
            env = StagingEnvironment()
            valid, msg = env.validate()
    ...
    def test_validate_success(self) -> None:
        with patch("oridecon.cli.registry.environment.os.environ.get", return_value="postgres://localhost/db"):
            env = StagingEnvironment()
            valid, msg = env.validate()
            assert valid is True
...
class TestProductionEnvironment:
    ...
    def test_validate_localhost_db(self) -> None:
        with patch("oridecon.cli.registry.environment.os.environ.get", side_effect=lambda k, d=None: {
...
class TestEnvironmentRegistry:
    ...
    def test_set_current(self) -> None:
        registry = EnvironmentRegistry.with_defaults()
        with patch("oridecon.cli.registry.environment.os.environ") as mock_env:
            registry.set_current("development")
            mock_env.__setitem__.assert_called_with("ORI_ENV", "development")
    ...
    def test_get_current(self) -> None:
        registry = EnvironmentRegistry.with_defaults()
        with patch("oridecon.cli.registry.environment.os.environ.get", return_value="test"):
            assert registry.get_current() == "test"

    def test_get_current_default(self) -> None:
        registry = EnvironmentRegistry.with_defaults()
        with patch.dict("oridecon.cli.registry.environment.os.environ", {}, clear=True):
            assert registry.get_current() == "development"

    def test_get_current_env(self) -> None:
        registry = EnvironmentRegistry.with_defaults()
        with patch("oridecon.cli.registry.environment.os.environ.get", return_value="development"):
...
class TestEnvironmentManager:
    def test_switch_success(self) -> None:
        with patch("oridecon.cli.registry.environment.os.environ"):
    ...
    def test_validate_current_no_env(self) -> None:
        with patch("oridecon.cli.registry.environment.os.environ.get", return_value="unknown"):
            manager = EnvironmentManager()
            valid, msg = manager.validate_current()
            assert valid is False
