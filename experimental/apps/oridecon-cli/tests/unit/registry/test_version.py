...
from oridecon.cli.registry.version import (
    GitVersionSource,
...
class TestPyPackageVersionSource:
    ...
    def test_get_version_found(self, mock_version: MagicMock) -> None:
        source = PyPackageVersionSource("oridecon")
...
class TestGitVersionSource:
    @patch("oridecon.cli.registry.version.subprocess.run")
    ...
    @patch("oridecon.cli.registry.version.subprocess.run", side_effect=RuntimeError)
...
class TestVersionRegistry:
    ...
    def test_with_defaults_populates_all_sources(self) -> None:
        ...
        assert registry.get("oridecon") is not None
        assert registry.get("python") is not None

class TestGetVersion:
    @patch("oridecon.cli.registry.version.PyPackageVersionSource.get_version", return_value="1.0")
    ...
    @patch("oridecon.cli.registry.version.PyPackageVersionSource.get_version", return_value="3.0")
