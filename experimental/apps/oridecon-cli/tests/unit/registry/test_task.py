...
from oridecon.cli.registry.task import (
    MypyRunner,
    PytestRunner,
    RuffRunner,
    TaskResult,
...
class TestPytestRunner:
    def test_is_available_true(self) -> None:
        with patch("oridecon.cli.registry.task.shutil.which", return_value="/usr/bin/pytest"):
            runner = PytestRunner()
            assert runner.is_available() is True

    def test_is_available_false(self) -> None:
        with patch("oridecon.cli.registry.task.shutil.which", return_value=None):
            runner = PytestRunner()
            assert runner.is_available() is False

    def test_run_success(self) -> None:
        with patch("oridecon.cli.registry.task.subprocess.run") as mock_run:
    ...
    def test_run_subprocess_error(self) -> None:
        with patch("oridecon.cli.registry.task.subprocess.run", side_effect=OSError("no pytest")):
            runner = PytestRunner()
            result = runner.run()
            assert result.success is False
...
class TestRuffRunner:
    def test_is_available_true(self) -> None:
        with patch("oridecon.cli.registry.task.shutil.which", return_value="/usr/bin/ruff"):
            runner = RuffRunner()
            assert runner.is_available() is True
    ...
    def test_run_subprocess_error(self) -> None:
        with patch("oridecon.cli.registry.task.subprocess.run", side_effect=OSError):
            runner = RuffRunner()
            result = runner.run()
            assert result.success is False
...
class TestMypyRunner:
    def test_is_available_true(self) -> None:
        with patch("oridecon.cli.registry.task.shutil.which", return_value="/usr/bin/mypy"):
            runner = MypyRunner()
            assert runner.is_available() is True
...
class TestTaskRunnerRegistry:
    ...
    def test_get_available(self) -> None:
        ...
        with patch("oridecon.cli.registry.task.shutil.which", side_effect=which_side_effect):
