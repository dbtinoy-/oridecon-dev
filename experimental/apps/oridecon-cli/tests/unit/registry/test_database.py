...
from oridecon.cli.registry.database import (
    DatabaseBackend,
    DatabaseConnection,
    DatabaseRegistry,
    MySQLBackend,
...
class TestSQLiteBackend:
    ...
    def test_get_client_binary_found(self) -> None:
        with patch("oridecon.cli.registry.database.shutil.which", return_value="/usr/bin/sqlite3"):
            b = SQLiteBackend()
            assert b.get_client_binary() == "/usr/bin/sqlite3"

    def test_get_client_binary_not_found(self) -> None:
        with patch("oridecon.cli.registry.database.shutil.which", return_value=None):
            b = SQLiteBackend()
...
class TestPostgreSQLBackend:
    ...
    def test_build_backup_command(self) -> None:
        with patch("oridecon.cli.registry.database.shutil.which", return_value="/usr/bin/pg_dump"):
            b = PostgreSQLBackend()
...
class TestMySQLBackend:
    ...
    def test_password_never_in_argv(self) -> None:
        ...
        with patch.object(MySQLBackend, "get_client_binary", return_value="/usr/bin/mysql"):
            with patch("oridecon.cli.registry.database.shutil.which", return_value="/usr/bin/mysqldump"):
                b = MySQLBackend()
...
class TestDatabaseConnection:
    ...
    def test_open_shell(self) -> None:
        ...
        with patch.object(conn.backend, "build_shell_command", return_value=["sqlite3", "dev.db"]):
            with patch("oridecon.cli.registry.database.subprocess.run"):
                conn.open_shell()

    def test_open_shell_os_error(self) -> None:
        conn = DatabaseConnection(url="sqlite:///./dev.db")
        with patch.object(conn.backend, "build_shell_command", return_value=["sqlite3", "dev.db"]):
            with patch("oridecon.cli.registry.database.subprocess.run", side_effect=OSError):
                with pytest.raises(RuntimeError):
                    conn.open_shell()
