...
from oridecon.cli.registry.template import (
    FullStackTemplate,
    GraphQLTemplate,
    MinimalTemplate,
    ProjectBuilder,
...
class TestTemplateConfig:
    ...
    def test_custom(self) -> None:
        cfg = TemplateConfig(name="api", packages=["oridecon-web"], env_vars={"DB": "sqlite"})
        assert "oridecon-web" in cfg.packages
...
class TestMinimalTemplate:
    ...
    def test_packages(self) -> None:
        assert "oridecon" in MinimalTemplate.packages

    def test_get_config(self) -> None:
        t = MinimalTemplate()
        cfg = t.get_config()
        assert cfg.name == "minimal"
        assert "oridecon" in cfg.packages

    def test_get_dependencies(self) -> None:
        t = MinimalTemplate()
        assert "oridecon" in t.get_dependencies()
...
class TestWebAPITemplate:
    ...
    def test_packages(self) -> None:
        ...
        assert "oridecon" in pkgs
        assert "oridecon-web" in pkgs
...
class TestGraphQLTemplate:
    ...
    def test_packages(self) -> None:
        assert "oridecon-graphql" in GraphQLTemplate.packages
...
class TestWorkerTemplate:
    ...
    def test_packages(self) -> None:
        assert "oridecon-tasks" in WorkerTemplate.packages
        assert "oridecon-queue" in WorkerTemplate.packages
...
class TestFullStackTemplate:
    ...
    def test_packages(self) -> None:
        ...
        assert "oridecon-web" in pkgs
        assert "oridecon-auth" in pkgs
        assert "oridecon-admin" in pkgs
