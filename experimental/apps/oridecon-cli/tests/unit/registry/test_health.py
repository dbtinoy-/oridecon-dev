...
class TestPythonVersionCheck:
    def test_pass_on_current_python(self) -> None:
        ...
        mock_vi.micro = 0
        with patch(
            "oridecon.cli.registry.health_checks.python_env.sys.version_info", mock_vi
        ):
...
class TestPackageManagerCheck:
    def test_uv_found(self) -> None:
        with patch(
            "oridecon.cli.registry.health_checks.tools.shutil.which",
...
class TestConfigFileCheck:
    def test_file_not_found(self) -> None:
        with patch(
            "oridecon.cli.registry.health_checks.project.Path.exists",
            return_value=False,
        ):
    ...
    def test_valid_yaml(self, tmp_path: Path) -> None:
        ...
        with patch(
            "oridecon.cli.registry.health_checks.project.Path", return_value=config
        ):
            result = ConfigFileCheck().check()
            assert result.status is CheckStatus.PASS
...
class TestDependenciesCheck:
    ...
    def test_uv_lock_exists(self) -> None:
        with patch(
            ...
            with patch(
                "oridecon.cli.registry.health_checks.project.Path.glob",
                return_value=[],
            ):
                result = DependenciesCheck().check()
                assert result.status is CheckStatus.WARNING
...
class TestGitCheck:
    def test_git_not_installed(self) -> None:
        with patch(
            "oridecon.cli.registry.health_checks.vcs_docker.shutil.which",
            return_value=None,
        ):
    ...
    def test_git_repo(self) -> None:
        with patch(
            ...
            with patch(
                "oridecon.cli.registry.health_checks.vcs_docker.Path.exists",
                return_value=True,
            ):
                with patch(
                    "oridecon.cli.registry.health_checks.vcs_docker.subprocess.run"
                ) as mock_run:
