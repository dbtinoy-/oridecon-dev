...
from oridecon.cli.commands.events import _make_evolution
...
class TestMakeEvolution:
    ...
    def test_make_evolution_success(self, mock_import: MagicMock) -> None:
        ...
        mock_import.side_effect = lambda name: {
            "oridecon.events.schema.evolution": type("evo", (), {"SchemaEvolution": mock_evo_cls}),
            "oridecon.events.schema.registry": type("reg", (), {"SchemaRegistry": mock_reg_cls}),
            "oridecon.events.schema.store": type("store", (), {"InMemorySchemaStore": mock_store_cls}),
        }[name]
...
class TestEventsCommand:
    ...
    @patch("oridecon.cli.commands.events._make_evolution")
    def test_list_event_types(self, mock_evo: MagicMock) -> None:
        from oridecon.cli.commands.events import app as events_app
