...
from oridecon.cli.commands.config import _mask_secrets, _dict_diff
...
class TestConfigCommand:
    ...
    @patch("oridecon.cli.commands.config.asyncio.run")
    def test_show_no_config(self, mock_run: MagicMock) -> None:
        from oridecon.cli.commands.config import app as config_app
    ...
    @patch("oridecon.cli.commands.config.Path.exists", return_value=False)
    @patch("oridecon.cli.commands.config.save_config_yaml_async", new_callable=MagicMock)
    def test_init_creates_file(
        self, mock_save: MagicMock, mock_exists: MagicMock
    ) -> None:
        from oridecon.cli.commands.config import app as config_app
        mock_save.return_value = None
        with patch("oridecon.cli.commands.config.asyncio.run") as mock_run:
    ...
    @patch("oridecon.cli.commands.config.Path.exists", side_effect=[True, False])
