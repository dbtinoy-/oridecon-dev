...
from oridecon.cli.commands.run import _is_factory, app
...
class TestRunCommand:
    ...
    @patch("oridecon.cli.commands.run.discover_entry_point", return_value="/tmp/fake_app.py")
    @patch("oridecon.cli.commands.run.detect_factory_attr", return_value="create_app")
    @patch("oridecon.cli.commands.run.path_to_module", return_value="fake_app")
    @patch("oridecon.cli.commands.run._server_registry")
    @patch("oridecon.cli.commands.run.ServerManager")
    ...
    @patch("oridecon.cli.commands.run.Path.exists", return_value=False)
    ...
    def test_run_invalid_server(self) -> None:
        with (
            patch("oridecon.cli.commands.run._server_registry") as mock_registry,
            patch("pathlib.Path.exists", return_value=True),
        ):
    ...
    def test_run_with_target_and_profile(self) -> None:
        with patch("oridecon.cli.commands.run._server_registry") as mock_factory:
            mock_registry = MagicMock()
    ...
    @patch("oridecon.cli.commands.run.discover_entry_point", return_value="/tmp/test_app.py")
    @patch("oridecon.cli.commands.run.detect_factory_attr", return_value=None)
    @patch("oridecon.cli.commands.run.Path.exists", return_value=True)
