...
from oridecon.cli.commands.system import _resolve_callable
...
class TestSystemCommand:
    ...
    @patch("oridecon.cli.commands.system.Path.exists", return_value=False)
    def test_info(self, mock_exists: MagicMock) -> None:
        from oridecon.cli.commands.system import app as system_app
        result = self.runner.invoke(system_app, ["info"])
        assert result.exit_code == 0
        assert "Python Version" in result.stdout

    @patch("oridecon.cli.commands.system.Path.exists", return_value=True)
    ...
    @patch("oridecon.cli.commands.system.run_health_checks", return_value={})
    @patch("oridecon.cli.contributors.runtime.ContributorRuntime")
    def test_doctor(self, mock_runtime_cls: MagicMock, mock_checks: MagicMock) -> None:
        ...
        from oridecon.cli.commands.system import app as system_app
        with patch("oridecon.cli.commands.system.Path.exists", return_value=False):
            result = self.runner.invoke(system_app, ["doctor"])
        assert result.exit_code == 0

    @patch("oridecon.cli.commands.system.console")
    @patch("code.interact")
    ...
    def test_health_no_project(self, mock_exists: MagicMock) -> None:
        ...
        with (
            patch("oridecon.cli.commands.system.run_health_checks", return_value={}),
            patch(
                "oridecon.cli.contributors.runtime.ContributorRuntime",
            ) as mock_runtime_cls,
        ):
            mock_runtime = MagicMock()
