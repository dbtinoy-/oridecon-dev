...
from oridecon.cli.contributors.runtime import ENTRY_POINT_GROUP

_NON_SCOPE_WITH_ENTRY_POINT = {
    "admin",  # oridecon-admin — experimental tier, documented as non-scope
}
...
def test_cli_runtime_does_not_import_extension_packages_directly() -> None:
    ...
    cli_src = Path(__file__).parent.parent.parent / "src" / "oridecon" / "cli"
    forbidden_prefixes = {
        "oridecon.web",
        "oridecon.sql",
        "oridecon.auth",
...
def test_deploy_not_in_command_registry() -> None:
    from oridecon.cli.runtime.main import _BUILTIN_COMMANDS
