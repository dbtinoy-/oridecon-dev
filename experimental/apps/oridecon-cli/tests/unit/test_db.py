...
from oridecon.cli.commands.db import app
...
class TestDbCommand:
    ...
    @patch(
        "oridecon.cli.commands.db_bootstrap.get_migration_manager",
    ...
    @patch(
        "oridecon.cli.commands.db_bootstrap._bootstrap_migration_runner",
        new_callable=AsyncMock,
    )
...
async def test_bootstrap_db_provider_resolves_usable_provider(
    ...
    from oridecon.cli.commands.db_bootstrap import _bootstrap_db_provider
...
def test_db_setup_reports_nothing_to_do_when_no_contributions(monkeypatch):
    ...
    from oridecon.cli.contributors import runtime as runtime_module
