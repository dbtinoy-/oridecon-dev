"""Tests for `oridecon run` entry-point auto-detection."""
...
from oridecon.cli.lib.entry_point import detect_factory_attr as _detect_factory_attr
...
class TestDetectFactoryAttr:
    ...
    def test_detects_app_from_import(self, tmp_path: Path) -> None:
        """Gear-1 style: `from oridecon.web import app, get`."""
        f = tmp_path / "app.py"
        f.write_text(textwrap.dedent("""\
            from oridecon.web import app, get
    ...
    def test_prefers_create_app_over_app(self, tmp_path: Path) -> None:
        f = tmp_path / "app.py"
        f.write_text("from oridecon.web import app\ndef create_app(): ...")
        assert _detect_factory_attr(str(f)) == "create_app"
