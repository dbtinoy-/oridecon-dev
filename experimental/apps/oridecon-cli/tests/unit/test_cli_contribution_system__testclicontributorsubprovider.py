...
from oridecon.cli.assembly import CommandAssembler
from oridecon.cli.commands.contrib import app as plugin_app
...
from oridecon.cli.di.sub_providers.contributor import CliContributorSubProvider
...
class TestCliProviderWiring:
    def test_core_contributor_entry_point_declared(self) -> None:
        """The oridecon.cli.contributors entry-point group must be declared in pyproject.toml."""
        import pathlib
        ...
        assert "core" in group, (
            "Expected 'core' key in [project.entry-points.\"oridecon.cli.contributors\"]"
        )
        assert group["core"] == "oridecon.cli.contributors.core:CoreCliContributor", (
...
class TestCliContributorSubProvider:
    ...
    async def test_boot_entry_point_registers_contributors_in_both_registries(
        ...
        with patch(
            "oridecon.cli.di.sub_providers.contributor.populate_cli_registries"
        ) as mock_populate:
...
class TestPluginListCommand:
    """Tests for the ``oridecon plugin list`` entry-point discovery command."""
    ...
    def test_plugin_list_discovers_core_contributor(self) -> None:
        ...
        runner = CliRunner()
        with patch(
            "oridecon.cli.contributors.runtime.entry_points",
...
class TestCommandAssemblerPackageLayout:
    def test_definition_command_preserves_src_suffix_for_package_layout(
        ...
        from oridecon.cli.generators.base import GenerationResult
...
class TestCliContributorDiscovery:
    def test_load_cli_contributors_skips_bad_entry_point_load(self, capsys) -> None:
        ...
        from oridecon.cli.contributors.discovery import load_cli_contributors
    ...
    def test_gen_module_reload_tolerates_broken_contributor(self, capsys) -> None:
        ...
        from oridecon.cli.commands import gen as gen_module
...
class TestCacheCliContributorDiscovery:
    ...
    def test_cache_contributor_generator_path_points_to_package_local(self) -> None:
        """CacheCliContributor's generator_path must point to oridecon.cache, not oridecon.cli."""
        from oridecon.cache.cli.contributor import CacheCliContributor
        ...
        assert path.startswith("oridecon.cache."), (
            f"generator_path must be in oridecon.cache package, got: {path}"
        )
        assert "oridecon.cli" not in path
