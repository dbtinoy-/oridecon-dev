...
as a top-level script. Moving it into `oridecon.testing` makes it
...
def _packages_root() -> Path:
    ...
    We locate this file under `packages/oridecon/src/oridecon/testing/run_tests.py`;
    ascend to find the repository packages directory and return its parent.
    """
    p = Path(__file__).resolve()
    # Expected layout: .../packages/oridecon/src/oridecon/testing/run_tests.py
...
def run_package_tests(package_name: str, packages_root: Path | None = None) -> bool:
    ...
    for test_dir in test_dirs:
        try:
            ...
            # core 'oridecon' package src is available for packages that import it.
            env = None
            try:
                env = os.environ.copy()
                parts = []
                ...
                core_src = packages_root / "packages" / "oridecon" / "src"
                if core_src.exists():
