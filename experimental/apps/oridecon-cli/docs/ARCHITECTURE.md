...
Internal design of the `oridecon-cli` package.

---

## Role in the System

`oridecon-cli` is the framework's command-line interface layer — a single
`oridecon` binary that provides project scaffolding, code generation,
...
flowchart BT
    subgraph UserLayer[Developer Tooling]
        CLI[oridecon-cli<br/>oridecon command]
    end
    subgraph Framework[Oridecon Framework]
        Core[oridecon · DI · Config · Providers]
        Contracts[oridecon-contracts<br/>Protocols · Types · Exceptions]
    end
...
**Dependency direction:** `oridecon-cli` imports from `oridecon` and
`oridecon-contracts`. Extension packages register CLI surfaces via the
`oridecon.cli.contributors` entry point — they contribute generators,
commands, health checks, and hooks without `oridecon-cli` importing them
directly.
...
| `add` | `commands/add.py` | Add a provider (`oridecon-sql`, `oridecon-auth`, etc.) |
...
`CommandAssembler` wires them as Typer subcommands under `oridecon gen`,
and dynamic dispatch resolves a `GeneratorAdapter` at invocation time.

```mermaid
sequenceDiagram
    actor User
    participant Shell as oridecon gen provider
    ...
    User->>Gen: oridecon gen provider UserService --fields name:str,email:string
...
```
oridecon.codegen.base.GeneratorBase       ← framework base
...
`oridecon-cli` integrates with the framework's DI container via the
...
sequenceDiagram
    ...
    EP->>EP: load entry points (oridecon.cli.contributors)
...
|----------|--------|---------|
| 1 | `--config` / `-c` CLI flag | `oridecon dev --config /etc/app.yaml` |
| 2 | `ORI_CONFIG` env var | `ORI_CONFIG=/etc/app.yaml oridecon dev` |
...
`ORI_CLI__*` environment variables (`ORI_CLI__VERBOSE=true`).
...
register under the `oridecon.cli.contributors` entry-point group and
...
Loads all `oridecon.cli.contributors` entry points with:
...
2. **Scope priority** — packages in `_SCOPE_DISTS` (the canonical
   `oridecon-*` set) take priority over third-party packages.
...
├── generators/
│   ├── base.py              # Re-exports GeneratorBase from oridecon.codegen
...
| `provider.py` | `Provider`, `ProviderRegistry`, `ProviderInstaller` | `oridecon add` provider management |
...
```mermaid
flowchart LR
    subgraph CLI[oridecon-cli]
        CE[CliError<br/>ORI_ERR_CLI_001]
        CNF[ConfigNotFoundError<br/>ORI_ERR_CLI_002]
        PNI[ProviderNotInstalledError<br/>ORI_ERR_CLI_003]
    end
...
|------|-----------|-------------|
| `ORI_ERR_CLI_001` | `CliError` | Base CLI exception with `causes` and `suggestions` lists |
| `ORI_ERR_CLI_002` | `ConfigNotFoundError` | `application.yaml` not found |
| `ORI_ERR_CLI_003` | `ProviderNotInstalledError` | Required provider package missing |

All extend `OrideconError` from `oridecon-contracts`.
...
# oridecon/cli/di/sub_providers/contributor.py
...
`oridecon-*` packages) and **third-party** contributors. This is
enforced via `_SCOPE_DISTS` in `contributors/runtime.py`:

```python
_SCOPE_DISTS = frozenset({
    "oridecon", "oridecon-contracts", "oridecon-web", "oridecon-sql",
    "oridecon-cache", "oridecon-auth", "oridecon-events", "oridecon-ai",
