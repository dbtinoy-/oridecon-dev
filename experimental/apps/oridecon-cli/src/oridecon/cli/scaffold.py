"""Canonical Oridecon project scaffold.

Single source of truth for project bootstrapping:

- ``oridecon new project --template <name>`` (``commands/new.py``)
- ``ProjectBuilder.create_project`` (``registry/template.py``)
- scaffold alignment tests (``tests/unit/test_scaffold_alignment.py``)

Every generated file targets the **real** oridecon APIs (``Application``,
``OrideconConfig``, ``WebModule``, ``Controller``) and the **canonical
generator map** in :mod:`oridecon.cli.layout`, so a scaffolded project and
its generators stay aligned:

    oridecon new project myapp --template web-api
    cd myapp && pip install -e .
    oridecon gen controller users  # src/myapp/controllers/users_controller.py
    oridecon dev                   # WebModule auto-discovers it
...
``oridecon new module billing`` adds ``src/<app>/modules/billing/``, and
``oridecon gen controller orders --module billing`` writes into it. Nothing
...
The generated ``application.yaml`` is validated by :class:`OrideconConfig`
(extra sections such as ``web`` / ``sql`` are consumed through the
``oridecon.config`` entry-point registry), and ``create_app()`` is a
standard composition root: bootable via ``oridecon dev`` / ``oridecon run``
...
from oridecon.cli.layout import (
    COMPONENTS,
    ROOT_DIRS,
)
from oridecon.cli.layout import (
...
TEMPLATES: dict[str, ProjectTemplateSpec] = {
    "minimal": _spec(
        "minimal",
        "Bare minimum Oridecon application",
        ("oridecon",),
    ),
    "api": _spec(
        "api",
        "REST API with the web framework",
        ("oridecon", "oridecon-web", "uvicorn"),
        frozenset({"web"}),
    ),
    "web-api": _spec(
        "web-api",
        "REST API with web + SQL persistence",
        ("oridecon", "oridecon-web", "oridecon-sql", "uvicorn"),
        frozenset({"web", "sql"}),
        (
            (
                "Controllers are auto-discovered from src/<app>/controllers "
                "and from every module; run `oridecon gen controller users` "
    ...
    "graphql": _spec(
        ...
        ("oridecon", "oridecon-web", "oridecon-sql", "oridecon-graphql", "uvicorn"),
        frozenset({"web", "sql", "graphql"}),
        (
            (
                "Add serializers via `oridecon gen graphql product`; the schema "
    ...
    "worker": _spec(
        ...
        ("oridecon", "oridecon-queue", "oridecon-tasks", "oridecon-sql"),
        frozenset({"queue", "tasks", "sql"}),
        (
            (
                "Define jobs with `oridecon gen task nightly_report`, then start "
    ...
    "full": _spec(
        ...
        (
            (
                "Auth ships with dev secret keys; replace ORI_AUTH__SECRET_KEY "
                "and ORI_AUTH__TOKEN__SECRET_KEY before deploying."
            ),
            (
...
def _application_yaml(project_name: str, spec: ProjectTemplateSpec) -> str:
    ...
    lines = [
        "# Oridecon configuration — validated by OrideconConfig plus the",
        "# `oridecon.config` entry-point models (web/sql sections are typed).",
        ...
        "  entry_point_group: oridecon.modules",
        "",
        "health:",
    ...
    if "auth" in features:
        lines += [
            ...
            "  # Override with ORI_AUTH__SECRET_KEY / ORI_AUTH__TOKEN__SECRET_KEY.",
...
def _module_imports(features: frozenset[str]) -> list[str]:
    ...
    names: list[str] = []
    if "web" in features:
        names.append("from oridecon.web import WebModule")
    if "sql" in features:
        names.append("from oridecon.sql import DatabaseModule")
    if "graphql" in features:
        names.append("from oridecon.graphql import GraphQLModule")
    if "auth" in features:
        names.append("from oridecon.auth import AuthModule")
    if "admin" in features:
        names.append("from oridecon.admin import AdminModule")
        names.append(
            "from oridecon.admin.config import AdminAuthConfig, AdminConfig, "
            "AdminSecurityConfig"
        )
    if "cache" in features:
        names.append("from oridecon.cache import CacheModule")
    if "features" in features:
        names.append("from oridecon.features import FeatureFlagsModule")
    if "tasks" in features:
        names.append("from oridecon.tasks import TasksModule")
    if "queue" in features:
        names.append("from oridecon.queue import QueueModule")
    if "monitor" in features:
        names.append("from oridecon.monitor import MonitorModule")
    names.sort()
    return names
...
def _feature_calls(
    ...
    *,
    ...
    if "web" in features and web_discover is not None:
        calls.append(
            (
                "# Web server - controllers written by `oridecon gen controller` are",
...
def _infrastructure_py(package_name: str, spec: ProjectTemplateSpec) -> str:
    ...
    imports.append("from oridecon.di.module import DynamicModule")
    imports.sort()
...
``oridecon new module <name>`` appends bounded contexts here; the composition
root registers every entry in ``MODULES``.  ``oridecon gen <component> <name>
...
def _module_boundary(
    ...
    """A modular feature-module boundary (``oridecon new module`` output)."""
    return f'''"""{module} module - bounded context.

``oridecon gen <component> <name> --module {module}`` writes module-local
components (controllers, models, services, repositories, ...) into this
package; ``oridecon gen`` without ``--module`` writes cross-cutting
...
def _module_scaffold_files(
    ...
    """Bare files for ``oridecon new module <name>``."""
...
def _app_py(
    ...
    ``MODULES`` -- empty until ``oridecon new module`` appends to it -- and
    ...
    :func:`oridecon.builder.gen.modular.composition.emit_app`, and the two
    ...
    if "web" in features:
        imports.append("from oridecon.web import WebModule")
    if "graphql" in features:
        imports.append("from oridecon.graphql import GraphQLModule")
    local_imports = [
...
`oridecon gen controller <name>` writes additional controllers into this
...
`oridecon gen controller <name> --module {module}` writes additional
...
def _readme(
    ...
    features = spec.features
    run = "oridecon dev" if "web" in features else "pytest"
    gen_example = (
        "oridecon gen controller users"
        if "web" in features
        else "oridecon gen task nightly_report"
    )
    ...
    sections += [
        ...
        "```",
        "",
        "List all generators with `oridecon gen list`.",
        "",
        "## Project layout",
        "",
        "- `application.yaml` — typed configuration (see `oridecon config show`)",
...
def _env_example(project_name: str) -> str:
    return f"""# Oridecon environment overrides (ORI_<SECTION>__<FIELD> syntax).
# Copy to .env and uncomment what you need.

# ORI_APP_NAME={project_name}
# ORI_DEBUG=false
# ORI_ENV=development

# Web server (web.server)
# ORI_WEB__SERVER__HOST=127.0.0.1
# ORI_WEB__SERVER__PORT=8000

# Database URL (sql.backend) — consumed by oridecon-sql
# ORI_SQL__BACKEND__URL=sqlite:///./dev.db

# Auth secret — set a real value before deploying with auth enabled
# ORI_AUTH__SECRET_KEY=change-me-in-production
"""
...
def render_project(
    ...
    *,
    ...
    """Render a canonical Oridecon project into *target_dir*.
...
def render_module(module_name: str, target_dir: Path) -> list[Path]:
    ...
    ``oridecon new module <name>`` writes
    ...
    """
    from oridecon.cli.layout import read_project_layout
