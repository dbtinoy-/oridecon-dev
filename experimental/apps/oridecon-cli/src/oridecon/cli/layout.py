...
- ``oridecon new project`` scaffolding (which component packages exist)
- ``oridecon gen`` (where a generator writes inside the current project)
...
# The canonical map.  Keys mirror ``oridecon gen`` defaults exactly.
...
@dataclass(frozen=True)
class ProjectLayout:
    """Project layout metadata read from ``[tool.oridecon]``.
    ...
    def module_target(self) -> str:
        """Return the default ``[tool.oridecon] module`` target.
...
def read_project_layout(cwd: Path | None = None) -> ProjectLayout:
    """Read ``[tool.oridecon]`` from ``pyproject.toml`` in *cwd*.
    ...
    if not isinstance(tool, dict):
        return ProjectLayout()
    oridecon_tool = tool.get("oridecon", {})
    if not isinstance(oridecon_tool, dict):
        return ProjectLayout()

    module_target = oridecon_tool.get("module", "")
    app_package = "app"
    ...
    return ProjectLayout(app_package=app_package, declared=bool(oridecon_tool))
