"""Canonical oridecon-* extension package scaffold.

``oridecon new package my-feature`` renders this layout with the real
framework APIs:

- ``pyproject.toml`` — hatchling build, pytest-asyncio config
- ``src/oridecon/<package>/`` — package source with ``py.typed``
- ``di/provider.py`` — ``Provider`` subclass using the real
  ``oridecon.di.provider`` base and ``oridecon.di`` container protocols
...
def _provider_module(package_name: str, class_name: str) -> str:
    return f'''"""{class_name} provider for the Oridecon DI container."""
from __future__ import annotations

from oridecon.contracts.core.di import BootContainerProtocol
from oridecon.di import ContainerRegistrarProtocol, Provider, ProviderPriority
...
def _init_module(package_name: str, class_name: str, description: str) -> str:
    return f'''"""oridecon-{package_name} — {description}."""
from __future__ import annotations

from oridecon.{package_name}.di.provider import {class_name}Provider
...
packages = ["src/oridecon/{package_name}"]
...
def _readme(package_name: str, description: str) -> str:
    return f"""# oridecon-{package_name}
...
```python
from oridecon.{package_name} import {package_name}Provider
...
def _files(package_name: str, class_name: str, description: str) -> dict[str, str]:
    return {
        ...
        f"src/oridecon/{package_name}/__init__.py": _init_module(
            package_name, class_name, description
        ),
        f"src/oridecon/{package_name}/py.typed": "",
        f"src/oridecon/{package_name}/di/__init__.py": '"""DI providers."""\n',
        f"src/oridecon/{package_name}/di/provider.py": _provider_module(
...
def render_package(
    ...
    *,
    ...
    """Render a canonical oridecon-* extension package into *target_dir*.

    Args:
        package_name: Normalized package slug (underscores, no ``oridecon-``).
    ...
    description = description or f"Oridecon extension package: oridecon-{package_name}"
