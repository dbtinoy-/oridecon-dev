...
2. ORI_CONFIG environment variable
...
from oridecon.cli.exceptions import CliError, ConfigNotFoundError
from oridecon.config.lib.merge import interpolate_string
...
def find_config(
    *,
    ...
    """Locate the oridecon config file.
    ...
    env_path = os.environ.get("ORI_CONFIG")
    if env_path:
        path = Path(env_path)
        if path.is_file():
            return path
