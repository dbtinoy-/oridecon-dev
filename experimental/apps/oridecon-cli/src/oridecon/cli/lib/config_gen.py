...
Usage::
    ...
    python -m oridecon.cli config-gen --all --output-dir config/
    python -m oridecon.cli config-gen --package database --output-dir config/
"""
...
# This defines the canonical set of Oridecon subpackage configs.
PACKAGE_CONFIGS: dict[str, str] = {
    "database": "oridecon.sql.config:DatabaseConfig",
    ...
    "tasks": "oridecon.tasks.config:TaskConfig",
    ...
    "messaging": "oridecon.messaging.config:MessagingConfig",
    "feature_flags": "oridecon.features.config:FeatureFlagsConfig",
    "graphql": "oridecon.graphql.config:GraphQLConfig",
}


def _import_config_class(import_path: str) -> type[Any]:
    """Import a config class from a dotted path like 'oridecon.sql.config:DatabaseConfig'.

    Raises ImportError if oridecon-domain is not installed.
    """
...
def _is_pydantic_model(annotation: Any) -> bool:
    ...
    try:
        ...
        domain_mod = importlib.import_module("oridecon.domain")
...
def generate_package_config(
    ...
    Args:
        ...
        import_path: Import path like 'oridecon.sql.config:DatabaseConfig'.
    ...
    try:
        ...
        if "oridecon.domain" in str(e) or "DomainModel" in str(e):
            error_msg = (
                "oridecon-domain is required for config generation — "
                "install it with: uv add oridecon-domain"
            )
