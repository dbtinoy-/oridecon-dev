"""Commands for managing Oridecon configuration."""
...
from oridecon.cli.commands.config_utils import (
    _dict_diff as _dict_diff,
)
from oridecon.cli.commands.config_utils import (
    _mask_secrets as _mask_secrets,
)
from oridecon.cli.lib import find_config, load_config_yaml_async, save_config_yaml_async
from oridecon.cli.output import OutputManager
from oridecon.cli.registry.config import (
    has_errors,
    validate_config,
)
    ...
    async def _run() -> None:
        try:
            ...
            out.error(
                ...
                hint="Run `oridecon init` to create one.",
            )
...
def init(
    ...
    """Initialize a new Oridecon configuration file with default settings."""
    out = OutputManager()
    path = Path(output)
    ...
    async def _run() -> None:
        try:
            await save_config_yaml_async(
                ...
                default_config,
                header=(
                    "# Oridecon Framework Configuration\n"
                    "# Documented at https://docs.oridecon.dev/config\n\n"
                ),
            )

            out.success(f"Created configuration file: {output}")
            out.info("Run 'oridecon config validate' to check your configuration.")
...
def schema(
    ...
    async def _run() -> None:
        try:
            ...
            schema = OrideconConfig.model_json_schema()
...
def doctor(
    ...
    async def _run() -> None:
        ...
        if not config_path.exists():
            ...
            out.info("Run 'oridecon init' to create one")
            raise typer.Exit(1)
