...
async def _bootstrap_migration_runner() -> tuple[Any, Any, Any]:
    ...
    Prefers the DI-managed runner when oridecon-sql is available so that
    ...
    try:
        ...
        sql_provider_mod = importlib.import_module("oridecon.sql.di.provider")
        ...
        # DatabaseConfig; the bare CLI container has no OrideconConfig.
        ...
        # oridecon-sql not installed — fall back to legacy direct factory.
        try:
            db_cli = importlib.import_module("oridecon.sql.cli")
            manager = db_cli.create_cli_migration_manager(db_url)
            return manager, None, None
        except ImportError as e:
            out = OutputManager()
            out.error(
                f"oridecon-sql is required for database commands — install it with: uv add oridecon-sql ({e})"
            )
...
async def _bootstrap_db_provider() -> tuple[Any, Any]:
    ...
    try:
        ...
        sql_config_mod = importlib.import_module("oridecon.sql.config")
        ...
        # in __init__), so the OrideconConfig lookup — unavailable in this
...
async def get_migration_manager() -> Any:
    ...
    try:
        ...
        out = OutputManager()
        out.error(f"Failed to import oridecon-sql: {e}")
        raise typer.Exit(1) from None
