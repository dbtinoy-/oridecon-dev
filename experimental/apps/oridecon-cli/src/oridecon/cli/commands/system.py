...
from oridecon.cli.lib.console import console
...
@app.command()
def info() -> None:
    """Display information about the Oridecon environment."""
    console.print("[bold cyan]Oridecon Environment Info[/bold cyan]")
    import platform
    import sys
    ...
    for pkg in ["oridecon-common", "oridecon-auth", "oridecon-sql", "oridecon-web"]:
        console.print(f"- {pkg}")
...
def health(
    ...
    """Check the health of the current Oridecon project."""
    from oridecon.cli.contributors.runtime import ContributorRuntime  # noqa: PLC0415

    console.print("[info]Checking Oridecon project health...[/info]")
...
@app.command()
def shell() -> None:
    """Open an interactive Oridecon shell."""
    console.print("[info]Opening Oridecon interactive shell...[/info]")
    try:
        import IPython  # type: ignore[import-not-found]

        IPython.embed(header="Oridecon Shell - DI Container available as 'container'")
    except ImportError:
        import code
...
def doctor(
    ...
    if fix and total_fail > 0:
        ...
        if not Path("application.yaml").exists():
            console.print("[info]Run 'oridecon init' to create configuration[/info]")
