...
Provides ``oridecon contrib list`` and ``oridecon contrib inspect`` which
discover installed CLI contributors registered under the
``oridecon.cli.contributors`` entry-point group and report their full
...
def plugin_list(
    ...
    ``oridecon.cli.contributors`` group, loads each contributor, and prints
...
def plugin_inspect(
    ...
    if commands:
        ...
        for c in commands:
            out.print(f"  [cyan]oridecon {c.name}[/cyan]  {c.help}")
