"""oridecon new — scaffold projects and extension packages."""
...
from oridecon.cli.scaffold import (
    render_module,
    render_project,
    resolve_template,
    template_names,
...
def main(
    ...
    """Create a new Oridecon project, fully aligned with the framework."""
    out = OutputManager()
...
def module(
    ...
    out.print(
        f"\n[bold]Next steps:[/bold]\n"
        f"  oridecon gen controller users --module {module_name}\n"
        f"  oridecon gen service billing --module {module_name}\n"
        f"  oridecon dev",
    )
...
def package(
    name: str = typer.Argument(
        ..., help="Package name (e.g. 'my-feature' → oridecon-my-feature)"
    ),
    ...
    Creates oridecon-<name>/ with pyproject.toml, src/ layout, py.typed,
    di/provider.py, and a unit test scaffold.
    """
    out = OutputManager()

    # Normalise: strip leading "oridecon-" if user typed it
    slug = _valid_name(name.removeprefix("oridecon-"), "Package")
    package_name = slug.replace("-", "_")
    full_name = f"oridecon-{slug}"
    class_name = _to_class_name(slug)
    pkg_description = description or f"Oridecon extension package: {full_name}"
    ...
    from oridecon.cli.scaffold_package import render_package
