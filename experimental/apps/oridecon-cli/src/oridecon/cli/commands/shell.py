...
This module provides an interactive shell with the Oridecon application context pre-loaded.
"""
...
def _banner() -> str:
    return """
[bold green]Oridecon Interactive Shell[/bold green]
...
def _run_plain_repl(use_ipython: bool = False) -> None:
    ...
    code.interact(banner="Oridecon Python Shell", exitmsg="Goodbye!")
...
def _run_repl_with_app(use_ipython: bool = False) -> None:
    ...
    out.print()
    out.print(
        "[dim]Alternative: Use 'oridecon shell --no-app' for plain Python REPL[/dim]"
    )
    out.print()
