...
Provides ``oridecon events migrate`` and ``oridecon events status`` to
inspect and apply event-schema migrations managed by
:class:`~oridecon.events.schema.evolution.SchemaEvolution`.
"""
...
from oridecon.cli.runtime import handle_errors
...
def _make_evolution() -> Any:
    ...
    When ``oridecon-events`` is installed and discoverable, this returns a
    ...
    try:
        ...
        _evo_mod = _importlib.import_module("oridecon.events.schema.evolution")
        _reg_mod = _importlib.import_module("oridecon.events.schema.registry")
        _store_mod = _importlib.import_module("oridecon.events.schema.store")
        ...
        out.error(
            "oridecon-events is not installed.  Run `uv add oridecon-events` to add it."
        )
...
def migrate(
    ...
    Example::
        ...
        oridecon events migrate UserCreated --from 1 --to 3
        oridecon events migrate UserCreated --from 1 --to -1 --dry-run
    """
...
def status(
    ...
    Example::
        ...
        oridecon events status
        oridecon events status UserCreated
    """
...
def list_event_types() -> None:
    ...
    out.print(
        "Start the app and use [cyan]oridecon inspect events[/cyan] "
        "to list live event handlers."
    )
    out.print(
        "\nFor offline inspection, run [cyan]oridecon gen list[/cyan] "
