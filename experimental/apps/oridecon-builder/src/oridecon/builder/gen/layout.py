...
:func:`oridecon.cli.layout.resolve_output_dir` — the framework's own
canonical generator→path map, shared with ``oridecon new project``,
``oridecon gen`` and the alignment gate (``dev/checks/generator_output.py``).
...
from oridecon.builder.graph.palette import KIND_COMPONENT, SHARED_KINDS
...
BUILDER_COMPONENTS: dict[str, BuilderComponent] = {
    "di": BuilderComponent(
        ...
        why=(
            ...
            "is the framework's own convention, unanimously: every oridecon "
            "subsystem with a DI provider keeps it at `<subsystem>/di/"
            "provider.py`, 30 of them, no exceptions. (`oridecon-sql` also "
    ...
    "emails": BuilderComponent(
        ...
        why=(
            ...
            "(`oridecon-notification`; no package ships an `emails/`), so "
    ...
    "uploads": BuilderComponent(
        ...
        why=(
            ...
            "by another name -- though `oridecon-web` ships its own "
...
class WriterLayout:
    ...
    def pkg(self, name: str, *parts: str, module: str | None = None) -> str:
        ...
        Mirrors :func:`oridecon.cli.layout.resolve_output_dir` semantics for
