...
from oridecon.builder.gen.emitters.webhook_emitter import emit_webhook_controllers
from oridecon.builder.gen.layout import (
    DEFAULT_LAYOUT,
    WriterLayout,
)
from oridecon.builder.gen.modular.composition import asgi_target
    ...
    *,
    ...
    files: dict[str, str] = {
        "pyproject.toml": _pyproject(
            ...
            + (("oridecon-http>=0.1.0",) if api_clients else ())
            + (("oridecon-storage>=0.1.0",) if storage_drivers else ())
            + (("oridecon-tasks>=0.1.0",) if task_entries else ())
            + (("oridecon-cache>=0.1.0",) if caches else ())
            + (("oridecon-graphql>=0.1.0",) if graphql else ())
            + (("oridecon-monitor>=0.1.0",) if health_checks else ())
            + (("oridecon-events>=0.1.0",) if (events or sagas) else ())
            + (("oridecon-features>=0.1.0",) if flags else ())
            + (("oridecon-auth>=0.1.0",) if (auths or api_key_groups) else ())
            + (
                ("oridecon-notification>=0.1.0",)
                if email_templates
                else ()
            ),
...
def _pyproject(
    ...
    *,
    ...
    tasks_source = (
        f'oridecon-tasks = {{ path = "{rel}/packages/oridecon-tasks", editable = true }}\n'
        if with_tasks and not pypi_sources
        else ""
    )
    cache_source = (
        f'oridecon-cache = {{ path = "{rel}/packages/oridecon-cache", editable = true }}\n'
        if with_cache and not pypi_sources
        else ""
    )
    graphql_source = (
        f'oridecon-graphql = {{ path = "{rel}/packages/oridecon-graphql", editable = true }}\n'
        if with_graphql and not pypi_sources
        else ""
    )
    monitor_source = (
        f'oridecon-monitor = {{ path = "{rel}/packages/oridecon-monitor", editable = true }}\n'
        if with_monitor and not pypi_sources
        else ""
    )
    events_source = (
        f'oridecon-events = {{ path = "{rel}/packages/oridecon-events", editable = true }}\n'
        if with_events and not pypi_sources
        else ""
    )
    features_source = (
        f'oridecon-features = {{ path = "{rel}/packages/oridecon-features", editable = true }}\n'
        if with_features and not pypi_sources
        else ""
    )
    auth_source = (
        f'oridecon-auth = {{ path = "{rel}/packages/oridecon-auth", editable = true }}\n'
        if with_auth and not pypi_sources
        else ""
    )
    notification_source = (
        f'oridecon-notification = {{ path = "{rel}/packages/oridecon-notification", editable = true }}\n'
        if with_email and not pypi_sources
        else ""
    )
    http_source = (
        f'oridecon-http = {{ path = "{rel}/packages/oridecon-http", editable = true }}\n'
        if with_http and not pypi_sources
        else ""
    )
    storage_source = (
        f'oridecon-storage = {{ path = "{rel}/packages/oridecon-storage", editable = true }}\n'
...
oridecon = {{ path = "{rel}/core/oridecon", editable = true }}
oridecon-contracts = {{ path = "{rel}/core/oridecon-contracts", editable = true }}
oridecon-web = {{ path = "{rel}/packages/oridecon-web", editable = true }}
oridecon-sql = {{ path = "{rel}/packages/oridecon-sql", editable = true }}
...
name = "{app_name}"
version = "0.1.0"
description = "Generated by oridecon-builder"
requires-python = ">=3.11"
dependencies = [
    "alembic>=1.13",
    "httpx>=0.27.0",
    "oridecon>=0.1.4",
    "oridecon-contracts>=0.1.4",
    "oridecon-web[uvicorn]>=0.1.2",
    "oridecon-sql>=0.1.4",
    "pytest>=8.0.0",
...
Generated by **oridecon-builder** — do not edit generated files by hand;
...
def _profile_yaml(profile: str) -> str:
    ...
    ``ORI_PROFILE`` matches, so anything omitted here falls through. That
...
# ORI_PROFILE={profile}. Set only what differs -- omitted keys fall through
# to the base file, so this stays small as the base file grows.
#
# Secrets belong in the environment (ORI_*), not here: this file is
...
def _init(app_name: str) -> str:
    return f"""{GENERATED_HEADER}
\"\"\"
{app_name} — generated by oridecon-builder.
\"\"\"
from __future__ import annotations
...
def _webhook_controller_entries(
    ...
    mods = mods or DEFAULT_LAYOUT.module_names()
    from oridecon.contracts.cli.generators import pascal_case as _pascal
...
class TasksRegistrationProvider(Provider):
    ...
    Boots after :class:`oridecon.tasks.di.provider.TaskProvider` (the
...
def _channels_provider(
    ...
    (see WS-2 in docs/ORIDECON_FRAMEWORK_BUGS.md) — and (b) appends a
...
from oridecon.contracts.core.provider import ProviderPriority
from oridecon.di.provider import Provider
from oridecon.web.routing.route_handlers import _wrap_websocket_handler
...
class ChannelsRegistrationProvider(Provider):
    ...
    Boots after :class:`oridecon.web.di.provider.WebProvider` so the
...
from oridecon.contracts.core.provider import ProviderPriority
from oridecon.di.provider import Provider
from oridecon.web.filters.pipeline import add_global_filter
...
# ``[tool.oridecon] module`` names ``{mods['app']}.app:app`` -- the composition
...
async def serve() -> None:
    ...
    from oridecon.web.server.runner import run_server_async
