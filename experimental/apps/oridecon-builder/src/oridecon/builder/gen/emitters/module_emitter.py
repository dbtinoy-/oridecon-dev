...
node must produce exactly what ``oridecon new module <slug>`` produces,
because the day the two disagree is the day a canvas-built project stops
being a normal Oridecon project. A test comparing our templates against the
...
from oridecon.builder.graph.models import ModuleConfig
from oridecon.cli.scaffold import (  # noqa: PLC2701 -- see module docstring
...
def emit_module_package(
    ...
    *,
    ...
    if config.protocols:
        ...
        # `oridecon new module` always writes protocols.py, so removing it is
...
def emit_modules_registry(slugs: tuple[str, ...], *, app_package: str) -> str:
    ...
    ``oridecon new module`` converge on the same bytes instead of racing to
    append.
    """
...
``oridecon gen <component> <name> --module {slug}`` writes module-local
...
def _provider(config: ModuleConfig) -> str:
    ...
    priority_import = (
        ...
        if priority != "DOMAIN"
        else "from oridecon.di.provider import Provider"
    )
