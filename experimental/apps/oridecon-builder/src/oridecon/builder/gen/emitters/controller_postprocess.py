...
(``oridecon.web.cli.generators.controller:ControllerGenerator``) renders a
...
1. it imports the repository as ``from repositories.<model>_repository import ...``
   ...
   see ``docs/ORIDECON_FRAMEWORK_BUGS.md`` (LEX-1);
...
def reconcile_controller(
    ...
    *,
    ...
    if exc_import not in text:
        # The framework template renders `from oridecon.web.exceptions import
        ...
        single_line = re.search(
            r"^from oridecon\.web\.exceptions import [^\n]+\n", text, re.MULTILINE
        )
