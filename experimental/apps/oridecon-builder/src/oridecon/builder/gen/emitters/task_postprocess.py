...
The ``oridecon-tasks`` task template (``task.py.jinja2``) renders code that is
not lint-clean out of the box (see docs/ORIDECON_FRAMEWORK_BUGS.md, TASK-3):
...
def reconcile_task(text: str) -> TaskReconcileResult:
    ...
    text = re.sub(r"^from oridecon\.di import injectable\n", "", text, flags=re.MULTILINE)
    text = re.sub(r"^from uuid import uuid4\n", "", text, flags=re.MULTILINE)
    # Filter the oridecon.tasks import line to the decorator(s) still used by
    ...
    def _filter_tasks_import(match: re.Match[str]) -> str:
        ...
        return "from oridecon.tasks import " + ", ".join(kept)

    text = re.sub(
        r"^from oridecon\.tasks import (.+)$",
...
def _merge_tasks_imports(text: str) -> str:
    """Merge a separated `from oridecon.tasks import <decorator>` into the
    main `from oridecon.tasks import JobProtocol, TaskProvider` line.
    """
    ...
    deco = re.search(
        r"^from oridecon\.tasks import (scheduled|task)\s*\n",
        text,
        flags=re.MULTILINE,
    )
    ...
    if not deco or (main is not None and main.start() == deco.start()):
        return text
    if main is None:
        # After trimming, a lone ``from oridecon.tasks import scheduled``
    ...
    names.sort()
    merged = f"from oridecon.tasks import {', '.join(names)}\n"
    ...
    text = re.sub(
        r"\n*from oridecon\.tasks import (?:" + decorator + r")\s*\n",
        "\n",
        text,
        count=1,
    )
