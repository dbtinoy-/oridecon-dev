...
The ``oridecon-web`` webhook template (``webhook.py.jinja2``) produces a
...
* the ``from oridecon...`` import block is merged into the third-party block.
  The framework repo groups ``oridecon`` as first-party, but a generated app
...
Also see docs/ORIDECON_FRAMEWORK_BUGS.md WEBHOOK-1: the template's ASGI shim
...
# ``from oridecon.xxx import ...`` immediately followed by a blank line and
# another ``from ...`` import — i.e. the framework's first-party grouping.
_ORIDECON_FIRST_PARTY_BLOCK = re.compile(
    r"(?m)(^from oridecon\.[^\n]*\n)\n+(?=from \w)"
)


def _merge_oridecon_imports(text: str) -> str:
    """Merge ``from oridecon...`` into the following third-party block.

    The framework template emits ``oridecon`` imports in their own
    blank-line-separated block (first-party convention). A generated app,
    however, depends on ``oridecon`` as a third-party framework, and this
    module is linted with ``ruff --isolated`` (no first-party config), which
    sorts ``oridecon`` alphabetically alongside pydantic/starlette — so the
    separating blank line is dropped.
    """
    return _ORIDECON_FIRST_PARTY_BLOCK.sub(r"\1", text)
...
def reconcile_webhook(text: str) -> WebhookReconcileResult:
    ...
    # Merge the first-party-styled oridecon import block into third-party.
    text = _merge_oridecon_imports(text)
