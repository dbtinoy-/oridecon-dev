...
Generates oridecon-flavored code snippets without writing to disk.
"""

from __future__ import annotations

from oridecon.builder.gen.emitters.apikey_emitter import (
    emit_api_key_repository,
    emit_api_keys_auth_module,
    emit_api_keys_migration,
    merged_api_key_scopes,
)
from oridecon.builder.gen.emitters.audit_emitter import (
    emit_audit_migration,
    emit_audit_repository,
)
from oridecon.builder.gen.emitters.audit_postprocess import (
    ControllerAuditHooks,
    apply_audit,
)
from oridecon.builder.gen.emitters.auth_policy_emitter import emit_auth_policy_module
from oridecon.builder.gen.emitters.context import (
    pascal_entity,
    snake_case,
    table_name,
)
from oridecon.builder.gen.emitters.contract_postprocess import (
    ContractBinding,
    ControllerContract,
    apply_contract,
    emit_contract_module,
)
from oridecon.builder.gen.emitters.dataloader_emitter import emit_dataloader_module
from oridecon.builder.gen.emitters.email_emitter import (
    emit_email_module,
    emit_emails_init,
    emit_mailer_helper,
)
from oridecon.builder.gen.emitters.file_upload_emitter import (
    emit_upload_controller,
    emit_upload_storage,
)
from oridecon.builder.gen.emitters.flag_postprocess import (
    ControllerFlagGates,
    apply_flag_gates,
)
from oridecon.builder.gen.emitters.guard_postprocess import (
    ControllerGuards,
    apply_guards,
    emit_rate_limit_middleware,
    emit_rate_limit_module,
)
from oridecon.builder.gen.emitters.http_client_emitter import emit_api_client_module
from oridecon.builder.gen.emitters.interceptor_emitter import emit_interceptor_module
from oridecon.builder.gen.emitters.saga_emitter import emit_saga_module
from oridecon.builder.gen.emitters.scaffold import emit_scaffold_files
from oridecon.builder.gen.emitters.search_emitter import (
    effective_engine,
    emit_search_controller,
    emit_search_migration,
    emit_search_repository,
)
from oridecon.builder.gen.emitters.storage_driver_emitter import (
    emit_storage_driver_module,
)
from oridecon.builder.gen.emitters.validator_emitter import (
    emit_validator_module,
)
from oridecon.builder.gen.layout import DEFAULT_LAYOUT, WriterLayout
from oridecon.builder.gen.modular.placement import Placement
from oridecon.builder.gen.node_generators import dest_for
from oridecon.builder.graph.models import (
    ApiClientConfig,
    ApiKeyGroupConfig,
    AppSettingsConfig,
    AuditLogConfig,
...
from oridecon.builder.graph.modules import drop_muted
from oridecon.builder.graph.palette import KIND_MODULE
...
def emit_code_preview(document: GraphDocument) -> list[dict[str, str]]:
    ...
    # output of the oridecon-features `feature_flag` generator).
    ...
    # Guard scaffolds (mirrors the writer's staged oridecon-auth output).
    ...
    for entity_name in sorted(validator_rules_by_entity):
        ...
        files.append({
            ...
            "language": "python",
            "content": "# generated by oridecon-builder - do not edit\n",
        })
    ...
    for _slug, upload_group in place.group(list(upload_entities.values())):
        files.append({
            ...
            "language": "python",
            "content": (
                "# Generated by oridecon-builder. Do not edit; regenerate"
                " instead.\n"
            ),
        })
...
def _flag_preview(flag: FeatureFlagConfig) -> str:
    ...
    return (
        "# generated by oridecon-builder - do not edit\n"
...
def _controller_preview(
    ...
    *,
    ...
    imports = [
        ...
        "from oridecon.web import Controller, post, get, put, delete",
        "from oridecon.web.exceptions import BadRequestError, NotFoundError",
    ...
    if with_flags:
        imports.append("from oridecon.features import FlagManager")
...
def _role_guard_preview(role: RoleConfig) -> str:
    ...
    return (
        ...
        "from __future__ import annotations\n"
        "\n"
        "from oridecon.auth.web.guards import RoleGuard\n"
        "\n"
        "\n"
