...
1. :func:`reconcile_guard` — the oridecon-auth ``guard`` generator imports
...
2. :func:`apply_guards` — applies ``@require_auth()`` /
   ``@require_roles(...)`` (from ``oridecon.auth.web``) to the controller
...
from oridecon.builder.graph.models import RateLimitConfig
...
_CONTROLLER_IMPORT_LINE = re.compile(
    r"^from oridecon\.web import Controller.*$", re.MULTILINE
)
...
def reconcile_guard(text: str) -> str:
    ...
    for name in ("PermissionGuard", "RoleGuard"):
        if text.count(name) == 1:
            text = re.sub(
                rf"^from oridecon\.auth\.web\.guards import ([^,\n]*, )?{name}"
                rf"(, )?([^,\n]*)$",
...
def _rejoin_import(head: str | None, tail: str | None) -> str:
    ...
    if not parts:
        return ""
    return f"from oridecon.auth.web.guards import {', '.join(parts)}"
...
def _ensure_guard_import(text: str, *, need_roles: bool) -> str:
    """Add the ``oridecon.auth.web`` guard imports once, after the
    controller base import line."""
    names = ["require_roles"] if need_roles else ["require_auth"]
    if all(re.search(rf"^from oridecon\.auth\.web import .*\b{n}\b", text, re.MULTILINE) for n in names):
        return text
    import_line = (
        "from oridecon.auth.web import require_auth, require_roles\n"
        if need_roles
        else "from oridecon.auth.web import require_auth\n"
    )
...
def emit_rate_limit_middleware(
    ...
    imported from the middleware package's ``*_rate_limit.py`` so definition and
    ...
    # Policy constants live beside this middleware (shared in every
    # structure), so enforcement never imports across a module boundary.
    policy_dir = layout.dest("src/middleware", generator="middleware")
    policy_pkg = WriterLayout.module_path(policy_dir)
    imports: list[str] = []
    entries: list[str] = []
    for config, _paths in policies:
        alias = config.name.upper()
        module = f"{policy_pkg}.{config.name}_rate_limit"
    ...
    return (
        ...
        "\n"
        f"Enforces the policy constants from ``{policy_dir}/*_rate_limit.py``\n"
...
def emit_rate_limit_module(
    ...
    *,
    ...
    return (
        ...
        f"{layout.dest('src/middleware', generator='middleware')}"
        "/rate_limit.py enforces them for\n"
        "the wired paths.\n"
