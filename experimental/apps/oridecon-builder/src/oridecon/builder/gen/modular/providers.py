...
every oridecon subsystem keeps its DI provider and a generated app should
...
from oridecon.builder.graph.models import EntityConfig, GraphDocument
from oridecon.builder.graph.modules import derive_modules
from oridecon.cli.scaffold import (  # noqa: PLC2701 -- see _framework_imports
    _module_imports,
)
...
def framework_modules(document: GraphDocument) -> tuple[str, ...]:
    ...
    return tuple(
        sorted(
            ...
            if card.on
            and card.oridecon.endswith("Module")
            and card.oridecon not in _COMPOSITION_ROOT_MODULES
        )
    )
...
def _framework_imports(modules: tuple[str, ...]) -> list[str]:
    ...
    ``oridecon.sql`` and ``MonitorModule`` in ``oridecon.monitor``. This was
    ...
    for name in modules:
        ...
        if line is None:
            ...
            line = f"from oridecon.{root} import {name}"
        lines.append(line)
