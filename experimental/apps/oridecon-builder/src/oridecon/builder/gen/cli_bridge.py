...
Resolves generator definitions through ``oridecon.cli.contributors``
entry points (the exact discovery the oridecon-cli ``CommandAssembler``
...
from oridecon.builder.exceptions import GenerationError
...
def load_generator(verb: str, *, output_dir: Path) -> Any:
    ...
    Mirrors oridecon-cli's ``GeneratorAdapter``: the generator is
