...
Each drawable node kind that produces files through a framework ``oridecon``
CLI generator (discovered via :mod:`oridecon.builder.gen.cli_bridge`) is
described by a :class:`VerbSpec`. The spec captures everything the
:class:`~oridecon.builder.gen.writer.ProjectWriter` needs to stage, generate
...
from oridecon.builder.gen.emitters.channel_postprocess import reconcile_channel
from oridecon.builder.gen.emitters.contract_postprocess import (
    ControllerContract,
    apply_contract,
)
from oridecon.builder.gen.emitters.controller_postprocess import reconcile_controller
from oridecon.builder.gen.emitters.error_postprocess import reconcile_error
from oridecon.builder.gen.emitters.event_postprocess import reconcile_event
from oridecon.builder.gen.emitters.filter_postprocess import (
...
from oridecon.builder.gen.emitters.graphql_postprocess import reconcile_graphql
from oridecon.builder.gen.emitters.guard_postprocess import (
    ControllerGuards,
    apply_guards,
    reconcile_guard,
)
from oridecon.builder.gen.emitters.handler_postprocess import (
    reconcile_event_handler,
)
from oridecon.builder.gen.emitters.health_postprocess import reconcile_health
from oridecon.builder.gen.emitters.model_postprocess import reconcile_model
from oridecon.builder.gen.emitters.projection_postprocess import (
    reconcile_projection,
)
from oridecon.builder.gen.emitters.seeder_postprocess import reconcile_seeder
from oridecon.builder.gen.emitters.task_postprocess import reconcile_task
from oridecon.builder.gen.emitters.webhook_postprocess import reconcile_webhook
from oridecon.builder.gen.layout import (
    DEFAULT_LAYOUT,
    WriterLayout,
    component_directory,
)
...
from oridecon.contracts.cli.generators import pascal_case
...
class VerbSpec:
    ...
    Attributes:
        ...
        default_output_dir: The contributor-declared default output
            ...
            destination: :class:`oridecon.builder.gen.layout.WriterLayout`
            maps it onto the active project structure. Values are validated
            against the canonical map by
            ``oridecon.cli.layout.validate_definition``.
...
VERB_SPECS: dict[str, VerbSpec] = {
    ...
    for spec in (
        ...
        # oridecon-features `feature_flag` generator: one <Name>Flag
        # definition module per flag node (canonical key + default rollout).
        VerbSpec("feature_flag", "src/features"),
        # oridecon-auth guard scaffolds: one <Name>AuthGuard definition per
