...
from oridecon.builder.exceptions import GraphValidationError
...
from oridecon.builder.graph.modules import drop_muted, module_import_graph
from oridecon.builder.graph.palette import (
    ALLOWED_EDGES,
    AUTH_PROVIDERS,
    DB_PRESETS,
    ENTITY_OPS,
...
from oridecon.builder.types import Diagnostic, DiagnosticSeverity
...
#: the value of ``ORI_PROFILE``. Restricting it to this shape is what stops
        ...
        out.append(
            _diag(
                ...
                hint=(
                    ...
                    "and as the value of ORI_PROFILE."
                ),
            )
        )
    return out
        ...
        if isinstance(node.config, ModuleConfig) and node.config.name not in members:
            diagnostics.append(
                Diagnostic(
                    ...
                    hint=(
                        "This is legal -- it matches `oridecon new module` -- "
