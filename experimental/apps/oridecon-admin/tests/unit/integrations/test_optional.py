...
from oridecon.admin.integrations._optional import is_installed, require_or_noop
...
class TestIsInstalled:
    def test_for_present_module(self) -> None:
        assert is_installed("oridecon.contracts") is True

    def test_for_missing_module(self) -> None:
        assert is_installed("oridecon.does_not_exist") is False


class TestRequireOrNoop:
    def test_returns_none_when_installed(self) -> None:
        result = require_or_noop("oridecon.contracts", DummyNoop)
        assert result is None

    def test_returns_noop_when_not_installed(self, capsys: Any) -> None:
        result = require_or_noop("oridecon.does_not_exist", DummyNoop)
