...
from oridecon.admin.settings.panel import (
    BrandingSpec,
    CacheSpec,
    ConfigRegistry,
    SecuritySpec,
...
from oridecon.admin.settings.panel.registry import MemoryStore
...
class TestSpecs:
    ...
    def test_semantic_overrides_retain_model_constraints(self) -> None:
        from oridecon.admin.settings.panel import I18nSpec, NotificationsSpec
...
class TestPackageSourceGrouping:
    def test_get_package_sources_returns_distinct_sorted_sources(self) -> None:
        from oridecon.admin.settings.panel.nodes import ConfigSpec, StringNode
    ...
    def test_default_package_source_is_built_in(self) -> None:
        from oridecon.admin.settings.panel.nodes import ConfigSpec
...
class TestStoreTenantIdParameter:
    ...
    async def test_env_store_accepts_and_ignores_tenant_id_kwarg(
        ...
        from oridecon.admin.settings.panel.registry import EnvStore
...
class TestSpecScope:
    ...
    def test_tenant_customizable_specs_are_scoped_to_tenant(self) -> None:
        from oridecon.admin.settings.panel import BrandingSpec, FeaturesSpec, I18nSpec
