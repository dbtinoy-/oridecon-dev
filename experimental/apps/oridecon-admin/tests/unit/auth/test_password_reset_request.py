...
from oridecon.admin.auth.errors import RateLimitExceededError
...
from oridecon.admin.auth.types import (
...
class TestIssueInvite:
    """R45 (doc 41): set-your-password invite via the reset token store."""

    _USER = {
        "user_id": "u-9",
        "name": "Invitee",
        "email": "admin@example.com",
        "hashed_password": "fake-hash:old",
    }

    def _invite_notification(self, ok: bool = True) -> MagicMock:
        notification = MagicMock()
        result = (
            Ok(object())
            if ok
        ...
        notification.notify_user_invited = AsyncMock(return_value=result)
        notification.notify_password_reset = AsyncMock()
        return notification

    @pytest.mark.asyncio
    async def test_creates_seven_day_token_and_uses_invite_template(self) -> None:
        notification = self._invite_notification()
        service, _users, token_store, _audit, _auth = _make_service(
            users=[self._USER], notification=notification
        )
        ...
        assert result.is_ok()
        token = token_store.created[-1]
        lifetime = (token.expires_at - datetime.now(UTC)).total_seconds()
        assert 6.9 * 86400 < lifetime <= 7 * 86400  # 7 days, not the reset hour
        kwargs = notification.notify_user_invited.await_args.kwargs
        assert "/admin/password-reset/" in kwargs["invite_url"]
        assert kwargs["expires_in"] == "7 days"
        notification.notify_password_reset.assert_not_awaited()  # invite template
        metadata = _audit.log_event.await_args.kwargs["metadata"]
        assert metadata["invite"] is True

    @pytest.mark.asyncio
    async def test_unknown_email_is_a_quiet_noop(self) -> None:
        notification = self._invite_notification()
        service, _users, token_store, _audit, _auth = _make_service(
            users=[self._USER], notification=notification
        )
        ...
        notification.notify_user_invited.assert_not_awaited()

    @pytest.mark.asyncio
    async def test_notify_failure_returns_err(self) -> None:
        notification = self._invite_notification(ok=False)
        service, _users, _tokens, _audit, _auth = _make_service(
            users=[self._USER], notification=notification
        )
