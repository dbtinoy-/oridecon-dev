"""Pool health controller honors a configured super-admin role name."""
...
class TestPoolHealthSuperadminConfigurable:
    @pytest.fixture
    ...
    def _controller(
        ...
        kwargs = {"renderer": renderer, "pool_manager": MagicMock()}
        if role is not None:
            kwargs["rbac_config"] = AdminRbacConfig(super_admin_role=role)
        return PoolHealthController(**kwargs)
