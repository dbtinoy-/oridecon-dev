...
from oridecon.admin.dashboard.naming_policy import NameCollisionError, NamingPolicy
from oridecon.admin.resources.namespace import apply_namespace
...
class _ContributorCustomHandler:
    ...
    def get_routes(self):
        return [
            AdminRouteSpec(
                path="/custom", method="GET", handler=lambda: None, name="custom"
            )
        ]
...
class TestResourceCollector:
    ...
    def test_collect_multiple_contributors(self) -> None:
        ...
        result = collector.collect(
            [_SimpleContributor(), _ContributorWithoutResources()]
        )
        assert len(result) == 1
