...
The page is a thin consumer of the passive ``oridecon-plugins`` primitives:
``discover_plugins()`` lists metadata-only descriptors, and
``load_disabled``/``update_disabled`` read and mutate the boot-file mirror
(``.oridecon/plugins.json`` by default) under an exclusive file lock.
...
if TYPE_CHECKING:
    ...
    from oridecon.contracts.plugins import PluginDescriptor
...
def _load_toolbox() -> tuple[Any, Any] | None:
    """Import the oridecon-plugins toolbox, returning None when absent.

    Returns:
        A ``(discovery, state)`` module pair, or ``None`` when the
        ``oridecon-plugins`` package is not installed.
    """
    try:
        from oridecon.plugins import discovery, state
...
class PluginsController(AdminController):
    ...
    async def toggle(self, request: Request) -> Response:
        ...
        from oridecon.plugins.exceptions import PluginStateError
