"""Authentication and authorization guards for oridecon-admin.

Provides middleware and guard utilities for protecting routes.
Integrates with oridecon-auth session management.
...
from oridecon.admin.auth.permissions import get_user_permissions
from oridecon.admin.auth.route_guards import (
...
from oridecon.contracts.web import RequestProtocol, ResponseProtocol
    ...
    async def _get_authenticated_user(self, request: RequestProtocol) -> Any | None:
        ...
        if self.config.allow_bearer_tokens:
            ...
            if auth_header.startswith("Bearer "):
                token = auth_header[7:]
                try:
                    # oridecon-auth uses authenticate_user or verify_token but AuthGuard usually checks tokens too
