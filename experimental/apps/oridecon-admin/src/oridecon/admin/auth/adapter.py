"""Adapter to align oridecon-admin's YAML/config-driven admin users and roles
with the canonical `oridecon-auth` provider and role manager.

This keeps `AdminProvider` UX but delegates core authn/authz to `oridecon-auth`.
"""
...
if TYPE_CHECKING:
    from oridecon.contracts.core.di import ContainerProtocol, ContainerResolverProtocol

try:
    from oridecon.admin.auth.session_manager import AdminSessionManager
    from oridecon.admin.auth.store import (
...
class AdminAuthAdapter:
    """Sync admin config (users + roles) into ``oridecon-auth`` components.
    ...
    async def sync(self, container: ContainerProtocol) -> None:
        ...
        try:
            ...
            admin_store = await container.resolve(
                "oridecon.admin.auth.AbstractAdminUserStore",
            )
        except UnresolvableDependencyError:
            admin_store = None
...
class AdminAuthServiceAdapter:
    """Bridges admin auth operations to ``oridecon-auth``'s ``AuthProviderProtocol``.
...
class _NoOpAuthProvider:
    """Fallback when oridecon-auth is not installed."""
