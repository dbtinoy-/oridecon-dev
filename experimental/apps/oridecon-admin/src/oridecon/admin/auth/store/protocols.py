...
class AdminUserStoreProtocol(Protocol):
    ...
    user store managed by oridecon-auth).

    Implementations:
        - :class:`~oridecon.admin.auth.store.direct_sql.DirectSQLAdminUserStore`
          — production SQL backend (``admin_users`` table)
        - :class:`~oridecon.admin.auth.store.memory.MemoryAdminUserStore`
    ...
    async def get_admin_count(self) -> int:
        ...
        Used by :class:`~oridecon.admin.middleware.setup.SetupMiddleware` to
    ...
    async def get_by_id(self, user_id: str) -> Any | None:
        ...
        Called by :class:`~oridecon.admin.middleware.auth.AdminAuthMiddleware`
