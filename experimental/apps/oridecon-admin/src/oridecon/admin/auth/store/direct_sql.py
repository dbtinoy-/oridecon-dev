...
(:mod:`oridecon.admin.auth.store.user_sql`,
:mod:`oridecon.admin.auth.store.user_rows`); this store owns schema
...
from oridecon.admin.auth.errors import SetupAlreadyCompletedError
from oridecon.admin.auth.store.user_rows import (
    CreatedUser,
    first_row,
    row_to_user,
)
from oridecon.admin.auth.store.user_sql import (
...
from oridecon.admin.sql_dialect import is_postgres
...
class DirectSQLAdminUserStore:
    ...
    async def create_user(
        ...
        **kwargs,  # Accept and ignore extra parameters like 'profile'
        ...
        try:
            ...
            err_str = str(e).lower()
            from oridecon.contracts.exceptions import DuplicateKeyError
    ...
    async def claim_first_admin(
        ...
        if not inserted:
            # Some drivers (e.g. aiosqlite through the query pipeline) report
            # row_count=0 even for successful INSERT ... SELECT statements.
            # The admin id is a fresh UUID generated by *this* call, so its
            # presence in the table proves this call inserted the row —
            # a concurrent submission would have inserted a different id.
            check = await self.db_provider.execute_query(
                "SELECT id, name, email FROM admin_users WHERE id = ?",
                [admin_id],
            )
            check_row = first_row(check)
            if check_row:
                return Ok(
                    CreatedUser(
                        str(check_row.get("id")),
                        str(check_row.get("name") or ""),
                        str(check_row.get("email") or ""),
                    )
                )
