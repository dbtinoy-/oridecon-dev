...
This is the *only* module in ``oridecon-admin`` that issues raw SQL against
the ``admin_sessions`` table.  The session-management service layer depends
on the ``SessionRepositoryProtocol`` protocol from ``oridecon-contracts`` and never
...
from oridecon.contracts.data import DatabaseProviderProtocol, Table
...
class AdminSessionSqlRepository:
    ...
    satisfies ``DatabaseProviderProtocol`` from ``oridecon-contracts``; the
