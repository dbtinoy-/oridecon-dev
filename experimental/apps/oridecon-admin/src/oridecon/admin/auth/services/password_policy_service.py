...
All rule evaluation is delegated to the single oridecon-auth
...
from oridecon.admin.auth.protocols import AdminPasswordPolicyServiceProtocol
...
from oridecon.contracts.auth import PasswordPolicyProtocol
...
class AdminPasswordPolicyService:
    ...
    from oridecon-auth.  Keeps the admin-only email-containment rule and
    translates the auth engine's failure report into per-rule
    ``AdminPasswordViolation`` entries.

    Args:
        policy: oridecon-auth policy implementation carrying the
    ...
    def validate(
        ...
        Delegates the shared rule evaluation to oridecon-auth's policy
