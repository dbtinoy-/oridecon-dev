"""Delegating auth adapter — wraps oridecon-auth services for admin use.
...
delegates the heavy lifting to ``oridecon.auth``'s ``AuthenticationService``
and ``SessionCookieBackend``.

Admin-specific concerns (rate limiting, lockout, audit logging) that live
outside oridecon-auth are handled through injected protocol implementations
...
from oridecon.admin.auth.errors import (
    AccountLockedError,
    AdminAuthError,
...
from oridecon.contracts import AuthenticatedUserProtocol
...
class DelegatingAuthAdapter:
    """Wraps oridecon-auth services for admin authentication and session mgmt.
    ...
    lifecycle are delegated to oridecon-auth.
    ...
    async def authenticate_admin(
        ...
        """Authenticate an admin user via oridecon-auth.
        ...
        # Step 3 — Credential verification via oridecon-auth
