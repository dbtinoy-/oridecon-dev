...
oridecon-auth's RFC 6238 primitives (``authn.mfa``, period = code TTL);
storage is a sha256 digest verified by atomic consume.  Depends only on
``AdminEmailOtpStoreProtocol`` from ``oridecon.admin.auth.protocols``.
"""
...
from oridecon.admin.config import AdminEmailOtpConfig
...
if TYPE_CHECKING:
    from oridecon.admin.auth.errors import AdminAuthError
