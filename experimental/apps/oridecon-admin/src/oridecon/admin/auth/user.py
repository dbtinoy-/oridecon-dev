...
the :class:`oridecon.contracts.auth.AuthenticatedUserProtocol` protocol without
importing from ``oridecon-auth``.

Cross-extension communication uses contracts only; the concrete ``User``
from ``oridecon-auth`` must not be imported here.
"""
...
class AdminUserRecord:
    ...
    ``oridecon.contracts.auth``.  Instances are constructed locally within
    ``oridecon-admin`` (e.g. from ``AdminUserEntity.to_user()`` or from
