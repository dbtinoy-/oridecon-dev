...
from oridecon.admin.dashboard.page_filters import widget_fetch_url
from oridecon.admin.dashboard.widget_defs import IWidget
...
def _render_live_widget_script(admin_prefix: str = "/admin") -> str:
    ...
    return (
        ...
        "es.onerror=function(){"
        "if(es.readyState===2&&window.__orideconLiveWidgets===es){"
        "window.__orideconLiveWidgets=null;"
        "}"
        "};"
        # Close on unload so the stream does not hold a connection slot.
        "window.addEventListener('pagehide',function(){"
        "if(window.__orideconLiveWidgets===es){window.__orideconLiveWidgets=null;}"
        "es.close();"
        "});"
        "})();"
        "</script>"
...
def _render_widget_state_script() -> str:
    ...
    return (
        "<script>"
        "(function(){"
        "if(window.__orideconWidgetState)return;"
        "window.__orideconWidgetState=true;"
        "function body(elt){"
