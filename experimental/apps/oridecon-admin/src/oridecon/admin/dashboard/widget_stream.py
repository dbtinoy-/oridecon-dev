...
oridecon-web), narrowing the caller-requested `resources` filter to
...
from oridecon.admin.realtime import SubjectAdminEventHub
from oridecon.contracts.web.sse import ReactiveSseBridgeProtocol
from oridecon.reactive import Stream
...
def build_widget_event_stream_handler(
    ...
    Args:
        ...
        sse_bridge: Optional :class:`ReactiveSseBridgeProtocol` bridging the
            ...
            container by the mount pipeline; provided by oridecon-web).
    ...
    def _require_bridge() -> ReactiveSseBridgeProtocol:
        if sse_bridge is None:
            raise RuntimeError(
                ...
                "from the container (oridecon-web) before building the "
