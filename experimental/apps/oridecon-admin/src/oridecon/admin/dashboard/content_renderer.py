...
``WidgetController`` is the only ``oridecon-ui`` caller for widget fragments —
...
if TYPE_CHECKING:
    from oridecon.ui.atoms.badge import BadgeVariant
...
def render_chart_fragment(
    ...
    :class:`~oridecon.contracts.admin.widget_content.ChartContent` from raw
...
def render_stat_fragment(stats: list[Stat]) -> str:
    ...
    :class:`~oridecon.contracts.admin.widget_content.StatContent` and
