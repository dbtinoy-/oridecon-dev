...
All operations are async-only for consistency with the oridecon ecosystem.
"""
...
from oridecon.config import ConfigLoader as BaseConfigLoader
...
class AdminConfigLoader:
    """Layered configuration loader for oridecon-admin.

    Configuration priority (highest to lowest):
    1. Runtime overrides (hot-reloadable)
    2. Environment variables (ORI_ADMIN__*)
    ...
    Environment variable mapping:
        admin.auth.session_lifetime -> ORI_ADMIN__AUTH__SESSION_LIFETIME
    """

    ENV_PREFIX = "ORI_ADMIN__"
    YAML_SECTION = "admin"
    ...
    def _load_env(self) -> dict[str, Any]:
        ...
        Maps ORI_ADMIN__* environment variables to nested config:
        - ORI_ADMIN__DEBUG=true -> {"debug": True}
        - ORI_ADMIN__AUTH__SESSION_LIFETIME=3600 -> {"auth": {"session_lifetime": 3600}}
        """
        result: dict[str, Any] = {}
        ...
        for key, value in os.environ.items():
            ...
            if "__" in config_key:
                # Canonical format: ORI_ADMIN__AUTH__SESSION_LIFETIME
                resolved_path = [
                    segment.lower() for segment in config_key.split("__") if segment
                ]
            else:
                # Backward-compat fallback: ORI_ADMIN_AUTH_SESSION_LIFETIME
