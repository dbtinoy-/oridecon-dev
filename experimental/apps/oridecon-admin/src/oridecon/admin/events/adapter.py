...
``oridecon-events`` implementation) for pub/sub event dispatch.  When no
...
class AdminEventBusAdapter:
    ...
    def stream(self) -> Any:
        ...
        Returns:
            An ``EventStream[Any]`` from ``oridecon.reactive`` fed by the
            bus (or the fallback dispatcher) through a shared Subject.
            Live events only; use ``oridecon.events.reactive.from_bus``
