"""Feature flags service for oridecon-admin.

Provides :class:`AdminFeatureFlagService` — a thin admin-specific wrapper
around :class:`~oridecon.contracts.feature_flags.protocols.FlagManagerProtocol`
...
from oridecon.contracts.exceptions import ConfigurationError, InfrastructureError
    ...
    def __init__(
        ...
        Args:
            manager: Feature flag manager whose provider is a LocalProvider.
                Requires ``oridecon-features`` to be installed.
...
async def get_feature_flag_service(
    ...
    from oridecon.admin.lib.di import get_admin_resolver
