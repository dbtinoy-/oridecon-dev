...
from oridecon.admin.services.filter_types import FilterDefinition, FilterPreset
    ...
    def __init__(
        ...
        Args:
            ...
            translator: ``FilterSetTranslator`` used to convert the active
                filter dict into a :class:`~oridecon.search.engine.SearchQuery`.
                Defaults to a bare :class:`FilterSetTranslator` instance when
                ``None`` and ``oridecon-search`` is installed (it is stateless,
        ...
        * all other values → ``FilterOperator.EQ``
        ...
        Args:
            ...
            search_query: Optional free-text query forwarded to
                :attr:`~oridecon.search.filterset.FilterSet.search_query`.
