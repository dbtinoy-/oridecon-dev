"""WebSocket support for oridecon-admin real-time updates.
...
from oridecon.admin.realtime.messages import WSMessage, WSMessageType
...
# Placeholder types for when oridecon-web is not available
# These are replaced via container registration when oridecon-web is present


class WebSocket:
    """Base WebSocket protocol.

    This is a local implementation that does not depend on oridecon-web.
    When oridecon-web is available, its WebSocket is registered in the container
...
class AdminWebSocketManager:
    ...
    def __init__(self, tracker: Any | None = None) -> None:
        ...
        """
        from oridecon.admin.realtime.connection_tracker import ConnectionTracker
...
class AdminWebSocketHandler(WebSocketHandler if HAS_WEBSOCKET else object):  # type: ignore[misc]
    ...
    async def on_message(self, websocket: Any, message: dict[str, Any]) -> None:
        ...
        try:
            ...
            from oridecon.admin.realtime.ws_handler_registry import (
