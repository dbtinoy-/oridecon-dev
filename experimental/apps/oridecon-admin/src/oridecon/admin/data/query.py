"""Unified query specification for oridecon-admin.
...
--------------------------------
``oridecon.ui.organisms.query_builder.QueryBuilder`` is **not** a superseded
...
* ``QueryBuilder`` is a *frontend* editor. It renders a constraint tree and
  ...
  ``oridecon.search.filterset.BlockQueryTranslator`` lowers to a
...
from oridecon.admin.data.query_filters import (
    FilterCondition,
    FilterOperator,
    condition_to_repo_key,
)
...
class QuerySpec:
    ...
    def to_repository_filters(self) -> dict[str, Any] | None:
        ...
        for condition in self.where:
            result[condition_to_repo_key(condition)] = condition.value
