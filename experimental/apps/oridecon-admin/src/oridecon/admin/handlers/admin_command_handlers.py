"""Command handlers for oridecon-admin.

Handlers process commands and emit corresponding events.
They integrate with oridecon-events CommandBusProtocol.
"""
...
from oridecon.admin.exceptions import AdminError, NotFoundError
from oridecon.contracts.events import CommandHandlerProtocol as BaseCommandHandler
...
if TYPE_CHECKING:
    from oridecon.admin.cqrs.commands import (
        BulkDeleteResources,
        CreateResource,
        DeleteResource,
        ExportResources,
...
class ExportCommandHandler(BaseCommandHandler):
    ...
    def __init__(
        ...
        if export_dir:
            ...
            temp_dir = Path(tempfile.gettempdir()) / "oridecon_exports"
