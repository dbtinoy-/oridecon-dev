...
def _user_has_permission(user: Any, permission: str) -> bool:
    ...
    :class:`~oridecon.admin.auth.user.AdminUserRecord`) and plain
...
def prepare_navigation(
    ...
    """
    from oridecon.admin.navigation.types import SidebarNavItem
    from oridecon.admin.ui.organisms.sidebar import SidebarSection
    ...
    for item in nav_items:
        ...
        # (see oridecon.admin.auth.permission_scheme) so a user granted
        ...
        if (
            ...
            if resource:
                from oridecon.admin.auth.permission_scheme import (
