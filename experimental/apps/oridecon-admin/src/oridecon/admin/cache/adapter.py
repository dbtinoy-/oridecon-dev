"""Adapter that delegates admin caching to ``oridecon-cache``.

When ``oridecon-cache`` is installed, ``AdminCacheServiceAdapter`` wraps
``oridecon.cache.service.core.CacheService`` for rich caching features
...
class AdminCacheServiceAdapter:
    """Bridge between admin's cache needs and ``oridecon-cache``'s ``CacheService``.
