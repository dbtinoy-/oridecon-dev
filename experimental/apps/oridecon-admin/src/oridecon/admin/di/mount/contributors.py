...
class AdminMountContributorsMixin:
    ...
    async def _mount_contributors(self, resolver: Any, ctx: MountContext) -> None:
            ...
            for name, resource in list(ctx.resources.items()):
                ...
                if search_spec and search_spec.index_name:
                    try:
                        ...
                        from oridecon.admin.integrations.search_query import (
                            SearchQueryDataSourceWrapper,
                        )
                        from oridecon.admin.integrations.search_sync import (
        ...
        try:
            from oridecon.admin.dashboard.resource_inventory import ResourceInventory
    ...
    async def _mount_integration(self, container: Any, ctx: MountContext) -> None:
        ...
        from oridecon.admin.dashboard.naming_policy import NamingPolicy
        from oridecon.admin.dashboard.route_integrator import RouteIntegrator
    ...
    async def _mount_sse_widgets(self, container: Any, ctx: MountContext) -> None:
        ...
        if router is None:
            return
        try:
            from oridecon.admin.dashboard.widget_stream import (
    ...
    async def _mount_export_center(self, container: Any, ctx: MountContext) -> None:
        ...
        * ``GET  /exports/{job_id}/download`` — artifact download (R28),
        ...
        if router is None:
            return
        try:
            from oridecon.admin.services.export.download import (
                build_export_download_handler,
            )
            from oridecon.admin.services.export.pages import ExportCenter
            ...
            renderer: Any = None
            try:
                from oridecon.admin.engine.renderer import (
    ...
    async def _mount_app_state(self, app: Any, ctx: MountContext) -> None:
        ...
        if registry is not None and ctx.contributors:
            from oridecon.admin.navigation.assembler import (
                NavigationAssembler,
