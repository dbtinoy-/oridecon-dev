...
:class:`oridecon.admin.di.sub_providers.auth.AdminAuthSubProvider` at
...
from oridecon.admin.services.notifications import AdminNotificationService
...
async def _resolve_admin_authorizer(container: Any) -> Any:
    """Return the oridecon-auth AuthorizationService, constructing on demand."""
...
def register_core_services(
    ...
    from oridecon.admin.auth.guard_chain import AdminGuardChain
    from oridecon.admin.auth.guards import GuardConfig
    from oridecon.admin.auth.session_manager import AdminSessionManager
    ...
    from oridecon.admin.middleware.input_sanitizer import AdminInputSanitizer
    from oridecon.admin.middleware.security_headers import AdminSecurityHeaders
    from oridecon.admin.rbac.service import PermissionService
    from oridecon.contracts.auth.repositories import SessionRepositoryProtocol
    ...
    if getattr(config.auth, "principal_source", "internal") == "app":
        from oridecon.admin.auth.store.app_principal import (
    ...
    # The one authorization engine: oridecon-auth's AuthorizationService.
    ...
    from oridecon.admin.middleware.authorization import (
        DefaultRequestAuthorizer,
        RequestAuthorizerProtocol,
    )
    from oridecon.contracts.auth import AuthorizerProtocol
...
def register_new_auth_services(
    ...
    from oridecon.admin.auth.services.audit_log_service import AdminAuditLogService
    from oridecon.admin.auth.services.auth_service import AdminAuthService
    from oridecon.admin.auth.services.csrf_service import AdminCsrfService
    from oridecon.admin.auth.services.login_attempt_service import (
        AdminLoginAttemptService,
    )
    from oridecon.admin.auth.services.password_policy_service import (
        AdminPasswordPolicyService,
    )
    from oridecon.admin.auth.services.password_reset_service import (
        AdminPasswordResetService,
    )
    from oridecon.admin.auth.services.session_service import AdminSessionService
    from oridecon.admin.auth.store.audit_log_sql import AdminAuditLogSqlStore
    from oridecon.admin.auth.store.lockout_sql import AdminAccountLockoutSqlStore
    from oridecon.admin.auth.store.login_attempt_sql import (
        AdminLoginAttemptSqlStore,
    )
    from oridecon.admin.auth.store.password_reset_token_sql import (
    ...
    # Rule evaluation delegates to oridecon-auth's PasswordPolicy;
    # the admin service keeps only the email-containment rule + the
    # admin violation/message contract.
    from oridecon.auth import PasswordPolicy
    ...
    from oridecon.admin.auth.services.session_user_cache import SessionUserCache
    ...
    # the hasher falls back to oridecon-auth's PasswordHasher at runtime,
    ...
    from oridecon.admin.auth.services.mfa_service import AdminMfaService
    from oridecon.admin.auth.store.mfa_sql import AdminMfaSqlStore
    from oridecon.admin.config import AdminMfaConfig
    from oridecon.security.encryption import EncryptionService
    ...
    from oridecon.admin.auth.services.email_otp_service import AdminEmailOtpService
    from oridecon.admin.auth.services.email_verification_service import (
        AdminEmailVerificationService,
    )
    from oridecon.admin.auth.store.email_otp_sql import AdminEmailOtpSqlStore
    from oridecon.admin.auth.store.email_verification_sql import (
        AdminEmailVerificationSqlStore,
    )
    from oridecon.admin.config import (
        AdminEmailOtpConfig,
    ...
    if getattr(config, "debug", False) is True and not container.has(MailerProtocol):
        from oridecon.admin.services.notifications.console_mailer import (
            AdminConsoleMailer,
        )
    ...
    from oridecon.admin.rbac.protocols import (
        AdminRoleServiceProtocol,
        AdminRoleStoreProtocol,
    )
    from oridecon.admin.rbac.role_service import AdminRoleService
    from oridecon.admin.rbac.roles_sql import AdminRoleSqlStore
