...
Follows the AuthBundleProvider pattern from oridecon-auth.

Mount-time phases (controllers, contributors, router) live in the
``oridecon.admin.di.mount`` mixins; fail-loud boot resolution guards live in
:mod:`oridecon.admin.di.boot_guards`, built-in singleton registrations in
:mod:`oridecon.admin.di.registrations`, and health aggregation in
:mod:`oridecon.admin.di.health`. This module keeps the provider lifecycle
...
from oridecon.admin.config import AdminRbacConfig
from oridecon.admin.di.boot_guards import AdminBootGuardsMixin
from oridecon.admin.di.health import aggregate_sub_provider_health
from oridecon.admin.di.mount import (
    AdminMountContributorsMixin,
    AdminMountControllersMixin,
    AdminMountCoreMixin,
    MountContext,
)
from oridecon.admin.di.registrations import register_builtin_singletons
...
class AdminProvider(
    ...
    async def register(self, container: ContainerRegistrarProtocol) -> None:
        ...
        """
        from oridecon.admin.di.sub_providers.auth import AdminAuthSubProvider
        from oridecon.admin.di.sub_providers.contributor import (
            AdminContributorSubProvider,
        )
        from oridecon.admin.di.sub_providers.core import AdminCoreSubProvider
        from oridecon.admin.di.sub_providers.dashboard import AdminDashboardSubProvider
        from oridecon.admin.di.sub_providers.export import AdminExportSubProvider
        from oridecon.admin.di.sub_providers.integrations import (
            AdminIntegrationsSubProvider,
        )
        from oridecon.admin.di.sub_providers.realtime import AdminRealtimeSubProvider
        from oridecon.admin.di.sub_providers.resource import AdminResourceSubProvider
        from oridecon.admin.di.sub_providers.tenancy import AdminTenancySubProvider
    ...
    async def _mount_middleware(self, admin_resolver: Any, ctx: MountContext) -> None:
        ...
        try:
            ...
            from oridecon.admin.middleware.setup import SetupMiddleware
        ...
        from oridecon.admin.middleware.csrf import AdminCsrfMiddleware
        ...
        if self._config.require_auth:
            try:
                from oridecon.admin.middleware.auth_guard import (
        ...
        from oridecon.admin.middleware.auth import AdminAuthMiddleware
        ...
        try:
            from oridecon.admin.auth.services.session_user_cache import (
                SessionUserCache,
            )
        ...
        try:
            from oridecon.admin.middleware.error import AdminErrorMiddleware
    ...
    async def _get_csrf_service(self, admin_resolver: Any) -> Any:
        ...
        from oridecon.admin.auth.protocols import AdminCsrfServiceProtocol
