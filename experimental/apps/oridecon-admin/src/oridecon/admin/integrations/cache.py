...
class CacheIntegration:
    ...
    Gracefully no-ops when ``oridecon-cache`` is not installed or when the
    ...
    def register(self, container: ContainerRegistrarProtocol) -> None:
        from oridecon.admin.config import CacheIntegrationConfig
        ...
        if not cfg.enabled:
            ...
            return
        if not is_installed("oridecon.cache"):
            self._cache = _NoOpCache()
            return
        self._enabled = True
