...
class TasksIntegration:
    """Adapter that dispatches bulk actions via oridecon-tasks.

    Gracefully no-ops when ``oridecon-tasks`` is not installed or the
    ...
    def register(self, container: ContainerRegistrarProtocol) -> None:
        from oridecon.admin.config import TasksIntegrationConfig
        ...
        if not cfg.enabled:
            ...
            return
        if not is_installed("oridecon.tasks"):
            self._tasks = _NoOpTasks()
            return
        self._enabled = True
