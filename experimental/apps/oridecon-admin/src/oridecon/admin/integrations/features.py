...
class FeaturesIntegration:
    ...
    Gracefully no-ops (all flags pass) when ``oridecon-features`` is not
    ...
    def register(self, container: ContainerRegistrarProtocol) -> None:
        from oridecon.admin.config import FeaturesIntegrationConfig
        ...
        if not cfg.enabled:
            ...
            return
        if not is_installed("oridecon.features"):
            self._flags = _NoOpFeatures()
            return
        self._enabled = True
