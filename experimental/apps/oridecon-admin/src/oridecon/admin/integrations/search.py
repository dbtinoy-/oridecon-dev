...
class SearchIntegration:
    ...
    Gracefully no-ops when ``oridecon-search`` is not installed or the
    ...
    def register(self, container: ContainerRegistrarProtocol) -> None:
        from oridecon.admin.config import SearchIntegrationConfig
        ...
        if not cfg.enabled:
            ...
            return
        if not is_installed("oridecon.search"):
            self._search = _NoOpSearch()
            return
        self._enabled = True
