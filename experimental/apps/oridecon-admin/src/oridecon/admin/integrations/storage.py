...
from oridecon.contracts.infra.storage import StorageUnsupportedOperationError
...
class StorageIntegration:
    """Adapter that delegates file storage to oridecon-storage.

    Gracefully no-ops when ``oridecon-storage`` is not installed or the
    ...
    def register(self, container: ContainerRegistrarProtocol) -> None:
        from oridecon.admin.config import StorageIntegrationConfig
        ...
        if not cfg.enabled:
            ...
            return
        if not is_installed("oridecon.storage"):
            self._store = _NoOpStorage()
            return
        self._enabled = True
