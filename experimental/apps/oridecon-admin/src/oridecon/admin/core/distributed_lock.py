"""Distributed lock support for oridecon-admin.
...
from oridecon.contracts.exceptions import LockError as CoreLockError
...
class AdminLockManager:
    ...
    Implementations are provided by ``oridecon-cache`` (Redis) or
    ``oridecon-sql`` (advisory locks / lock table).
