"""Cache integration for oridecon-admin.
...
``@invalidate_cache``) live in ``oridecon.cache.service.decorators``.
Request-scoped caching lives in ``oridecon.cache.service.request_cache``.
"""
