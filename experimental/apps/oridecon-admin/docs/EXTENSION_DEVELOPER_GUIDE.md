...
capabilities to `oridecon-admin` — resources, pages, widgets, settings panels,
...
- Python 3.11+
- `oridecon-admin>=0.2.0`
- `oridecon-contracts>=0.2.0`
...
dependencies = ["oridecon-admin>=0.2.0", "oridecon-contracts>=0.2.0"]
...
from oridecon.admin.schema import TextField, EnumField, DateField
from oridecon.admin.rbac.schema import ResourcePermissions
...
|---|---|---|
| `cacheable` | `bool` | Cache `list()` results; auto-invalidate on create/update/delete (requires `oridecon-cache`) |
| `searchable` | `bool` | Index resources on create/update/delete; search via `?q=` (requires `oridecon-search`) |
...
from oridecon.admin.pages.types import PageResponse, NavigationEntry
...
```python
from oridecon.contracts.admin.types import ManagementPageDefinition, PageCategory
...
```python
from oridecon.contracts.admin.types import SettingsPanelDefinition
...
from oridecon.contracts.admin.types import DashboardWidgetDefinition
...
def get_routes(self):
    from oridecon.contracts.admin.types import AdminRouteSpec
    return [
        AdminRouteSpec(
...
```python
from oridecon.admin.actions.standard import ExportAction, ExportBulkAction

class ArchiveWidgetsAction(ExportBulkAction):
    name = "archive_old"
    label = "Archive Selected"
    # Optional: run via background tasks
    task_runner = "tasks"  # requires oridecon-tasks
...
def get_actions(self):
    from oridecon.contracts.admin.types import AdminActionDefinition
    return [
...
```python
from oridecon.contracts.admin.types import NavigationContribution
...
- Verify the entry point group: `oridecon.admin.contributors`
- Run: `python -c "from importlib.metadata import entry_points; print(list(entry_points(group='oridecon.admin.contributors')))"`
...
- Ensure the optional package is installed: `uv pip list | grep oridecon-cache`
