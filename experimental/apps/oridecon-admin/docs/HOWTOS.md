...
Task-oriented recipes for `oridecon-admin`.

## Register a resource

```python
from oridecon.admin.resources.base import Resource
from oridecon.admin.schema import TextField, EmailField
...
from oridecon.admin.actions.types import ActionColor
...
```python
from oridecon.contracts.admin.types import DashboardWidgetDefinition, WidgetCategory, WidgetSize
...
Register via `[project.entry-points."oridecon.admin.contributors"]` in `pyproject.toml`.
...
- Contributors are discovered via `oridecon.admin.contributors` entry point group.
