...
specific to admin; for the framework-wide Oridecon rules see the root
...
- **Field-level validation** returns `Result[Ok, FieldError]`. Examples:
  ...
  `src/oridecon/admin/validation/rules.py`.
- **Domain operations on aggregates and services** raise typed exceptions
  ...
  `src/oridecon/admin/exceptions.py:29-73`.
- **Infrastructure failures** (database connection, cache, queue) raise
  exceptions from the source package (`oridecon-sql`, `oridecon-cache`).
...
Always `get_logger(__name__)` from `oridecon.logging`. Never `print()`,
...
`src/oridecon/admin/services/background_jobs.py:137-138`.
...
- UI symbols come from `from oridecon.ui import X` only. Never deep-path
  into `oridecon.ui.atoms.*`, `oridecon.ui.molecules.*`, etc. The
