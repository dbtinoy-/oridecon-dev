...
Guide to the data source system in `oridecon-admin`.
...
The system lives in `src/oridecon/admin/data/` and consists of:
...
`oridecon.admin.data.data_source`. It defines the contract that every
...
`oridecon.admin.data.query`. It is the canonical type for passing query
...
class MyResource(Resource):
    ...
    async def get_data_source(cls) -> IDataSource[MyModel]:
        from oridecon.admin.lib.di import resolve_admin_service
...
```python
from oridecon.admin.data.data_source import DataSourceBase, QueryResult
from oridecon.admin.data.query import QuerySpec
