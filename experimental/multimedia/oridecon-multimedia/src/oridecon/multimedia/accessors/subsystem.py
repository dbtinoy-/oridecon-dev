"""Sync accessor wrapping one sub-provider's backend + task manager + storage normalizer.

Sibling accessors — ``VideoAccessor`` (video generation + processing) and
``ComposeAccessor`` (timeline rendering) — live alongside in this package.
"""
