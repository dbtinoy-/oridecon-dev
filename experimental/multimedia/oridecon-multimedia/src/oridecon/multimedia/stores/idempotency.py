...
Used only when no IdempotencyStoreProtocol (e.g. from oridecon-resilience)
is bound in the container. oridecon-multimedia does not take a hard
dependency on oridecon-resilience just for this — that isn't one of the
...
oridecon-resilience's InMemoryIdempotencyStore — IdempotencyManager.check_duplicate()
already handles both shapes defensively.

TTL semantics mirror oridecon-resilience's in-memory store: entries carry a
