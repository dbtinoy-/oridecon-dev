...
Task-oriented recipes for `oridecon-multimedia`. Each uses real classes from the source.
...
from oridecon.contracts.multimedia import BurnSubtitles, SubtitleCue, Trim, VideoRequest
...
```python
from oridecon.multimedia import MultimediaModule
from oridecon.contracts.multimedia import ImageRequest
...
```python
from oridecon.multimedia import MultimediaModule, Timeline
from oridecon.contracts.multimedia import MediaAsset, EncodeSpec, TransitionSpec
...
```python
from oridecon.contracts.multimedia import BeatAnalysisRequest
...
- Submit-based handlers are only wired when `oridecon-tasks` is bound; otherwise `submit()`
