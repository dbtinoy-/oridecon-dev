...
Learn how to use `oridecon-multimedia` to generate and compose media across every subsystem.

---

## Overview

`oridecon-multimedia` is the **orchestration layer** for the Oridecon multimedia subsystem.
One module wires **seven independent generation packages** into a single DI graph:

| Subsystem | Package | Protocol | Default backend |
|-----------|---------|----------|-----------------|
| Text-to-speech | `oridecon-multimedia-tts` | `TTSProvider` | `local-http` |
| Music | `oridecon-multimedia-music` | `MusicProvider` | `local-http` |
| Video | `oridecon-multimedia-video` | `VideoProvider` + `VideoProcessor` | `local-http` |
| Image | `oridecon-multimedia-image` | `ImageProvider` | `local-http` |
| Upscale | `oridecon-multimedia-upscale` | `UpscaleProvider` | `real-esrgan` |
| Interpolate | `oridecon-multimedia-interpolate` | `InterpolationProvider` | `rife` |
| Beat analysis | `oridecon-multimedia-beat` | `BeatAnalysisProvider` | `librosa` |
...
```python
from oridecon.contracts.multimedia import TTSRequest
...
to `oridecon-tasks`, deduplicated by `idempotency_key`, and the result asset is normalized
...
```python
from oridecon.contracts.multimedia import Trim
...
```python
from oridecon.contracts.multimedia import MediaAsset, SubtitleCue, TransitionSpec
...
- **`oridecon-storage`** — optional. If a `BlobStoreProtocol` is bound, submission
  uploads bytes-carrying assets before they cross the task queue, and finished results
  are normalized to URIs.
- **`oridecon-tasks`** — optional. Without it only the synchronous `generate()` path
  ...
  and `timeline_render`.
- **`oridecon-cache`** — optional. When a `CacheBackendProtocol` is bound **and**
  `cache_results: true`, accessor `generate()` results are cached by canonical-JSON +
  sha256 key (`multimedia:{task_name}:{digest}`).
- **`oridecon-events`** — optional. When an `EventBusProtocol` is bound, each successful
  accessor `generate()` publishes a `MultimediaGenerationEvent` (`media_type`, `provider`,
  `size_bytes`, `duration_seconds`) mirroring the `oridecon-ai-llm` observability pattern.
- **`oridecon-resilience`** — optional. `RetryPolicyProtocol` and `CircuitBreakerProtocol`
  are wired into the `madmom` beat backend and other resilient clients.
- **Contracts** — all types and protocols come from `oridecon.contracts.multimedia`;
...
- ❌ Don't omit the `oridecon-storage` provider and then expect `submit()` results to be
