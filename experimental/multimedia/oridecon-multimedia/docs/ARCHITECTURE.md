...
Internal design of `oridecon-multimedia` — the orchestration umbrella for the multimedia subsystem.

---

## Role in the System

`oridecon-multimedia` is the **composition layer** that turns seven independently
...
flowchart TB
    ...
    subgraph Umbrella[oridecon-multimedia]
        MM[MultimediaModule]
    ...
    subgraph Sibling[oridecon-multimedia-*]
        TTSpkg[oridecon-multimedia-tts]
        MUSpkg[oridecon-multimedia-music]
        VIDpkg[oridecon-multimedia-video]
        IMGpkg[oridecon-multimedia-image]
        UPpkg[oridecon-multimedia-upscale]
        INTPkg[oridecon-multimedia-interpolate]
        BEATpkg[oridecon-multimedia-beat]
    end

    subgraph Optional[Optional Integrations]
        Store[BlobStoreProtocol<br/>oridecon-storage]
        Cache[CacheBackendProtocol<br/>oridecon-cache]
        Bus[EventBusProtocol<br/>oridecon-events]
        Tasks[TaskQueueProtocol + TaskProvider<br/>oridecon-tasks]
        Res[RetryPolicy/CircuitBreaker<br/>oridecon-resilience]
    end
...
**Import rule:** the umbrella imports only from `oridecon`, `oridecon-contracts`, and the
sibling packages it declares as dependencies. It imports `oridecon-tasks` and
`oridecon-storage` types inside methods (lazy) so optional integration is graceful.
...
1. **register()** — binds `MultimediaConfig`, instantiates each sub-provider from its typed
   ...
   discovers extra subsystems via the `oridecon.multimedia.subsystems` entry-point group
...
when `oridecon-tasks` is present: `tts_generation`, `music_generation`, `video_generation`,
...
All protocols and value types come from `oridecon.contracts.multimedia`
...
**Exceptions:** `MultimediaError` (base, `ORI_ERR_MM_001`) and leaves `TTSError`,
`MusicGenerationError`, `VideoGenerationError`, `ImageGenerationError`, `UpscaleError`,
`BeatAnalysisError`, `ProviderNotInstalledError` (`ORI_ERR_MM_006`).
...
`oridecon-storage`) may not have bound `BlobStoreProtocol` yet during this provider's
`register()`.
...
- **Namespace-package sharing.** `multimedia/__init__.py` calls
  `pkgutil.extend_path()` so sibling wheels co-exist under `oridecon.multimedia.*`.
...
- **`JobHandle` decouples callers from `oridecon-tasks`.** It wraps
...
| Point | Mechanism |
|-------|-----------|
| Add a subsystem | Ship a package exposing a `oridecon.multimedia.subsystems` entry point pointing at a provider class (auto-registered in `boot`) |
| Add a module stub | Expose a `oridecon.multimedia.modules` entry point; `MultimediaModule.stub()` loads it automatically |
