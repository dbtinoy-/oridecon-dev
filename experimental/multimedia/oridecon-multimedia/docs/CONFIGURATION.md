...
Configuration options for `oridecon-multimedia` — the umbrella config plus per-subsystem trees.
...
`MultimediaConfig` (extends `oridecon.config.BaseConfig`) declares
...
|-------|------|---------|-------------|
| `tts` | `TTSConfig` | `TTSConfig()` | TTS subsystem config (oridecon-multimedia-tts) |
| `music` | `MusicConfig` | `MusicConfig()` | Music generation config (oridecon-multimedia-music) |
| `video` | `VideoConfig` | `VideoConfig()` | Video generation + processing config (oridecon-multimedia-video) |
| `image` | `ImageConfig` | `ImageConfig()` | Image generation config (oridecon-multimedia-image) |
| `upscale` | `UpscaleConfig` | `UpscaleConfig()` | Upscale config (oridecon-multimedia-upscale) |
| `interpolate` | `InterpolationConfig` | `InterpolationConfig()` | Frame interpolation config (oridecon-multimedia-interpolate) |
| `beat` | `BeatAnalysisConfig` | `BeatAnalysisConfig()` | Beat analysis config (oridecon-multimedia-beat) |
...
`ORI_MULTIMEDIA__`. Nested subsystem fields prefix with the subsystem name.

| Variable | Description |
|---------|-------------|
| `ORI_MULTIMEDIA__STORAGE_PATH_PREFIX` | Blob storage prefix for assets (`multimedia/`) |
| `ORI_MULTIMEDIA__CACHE_RESULTS` | Enable result caching (`true`/`false`) |
| `ORI_MULTIMEDIA__TTS__BACKEND` | TTS backend name |
| `ORI_MULTIMEDIA__TTS__ELEVENLABS_VOICE_ID` | ElevenLabs voice id |
| `ORI_MULTIMEDIA__MUSIC__BACKEND` | Music backend name |
| `ORI_MULTIMEDIA__VIDEO__BACKEND` | Video backend name |
| `ORI_MULTIMEDIA__IMAGE__BACKEND` | Image backend name |
| `ORI_MULTIMEDIA__UPSCALE__BACKEND` | Upscale backend name |
| `ORI_MULTIMEDIA__INTERPOLATE__BACKEND` | Interpolation backend name |
| `ORI_MULTIMEDIA__BEAT__BACKEND` | Beat backend name (`librosa`/`madmom`) |
| `ORI_MULTIMEDIA__BEAT__MADMOM_BASE_URL` | Madmom server URL |

```bash
ORI_MULTIMEDIA__TTS__BACKEND=elevenlabs \
ORI_MULTIMEDIA__TTS__ELEVENLABS_VOICE_ID=21m00Tcm4TlvDq8ikWAM \
ORI_MULTIMEDIA__VIDEO__BACKEND=local-http \
ORI_MULTIMEDIA__CACHE_RESULTS=true \
  python -m my_app
```

> A subsystem installed standalone uses its own prefix instead —
> e.g. `ORI_MULTIMEDIA_BEAT__BACKEND` (see the beat package's Configuration doc).
...
`oridecon.multimedia.modules` entry-point group.
...
- Prefer environment variables for secrets and per-environment switches
  (`ORI_MULTIMEDIA__TTS__BACKEND=elevenlabs`), YAML for structure.
