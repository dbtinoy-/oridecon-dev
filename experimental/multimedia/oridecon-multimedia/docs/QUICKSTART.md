...
`oridecon-multimedia` pulls in `oridecon`, `oridecon-contracts`, and all seven sibling
extension packages (`oridecon-multimedia-tts`, `-music`, `-video`, `-image`, `-upscale`,
`-interpolate`, `-beat`) plus `oridecon-tasks` and `oridecon-storage`.
