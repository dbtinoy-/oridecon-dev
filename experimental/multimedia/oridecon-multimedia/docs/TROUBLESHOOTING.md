...
Common issues when using `oridecon-multimedia` and how to fix them.
...
**Cause:** `submit()` requires `oridecon-tasks`: a `TaskProvider` *and* a
...
`multimedia_no_storage_bound`. Without `oridecon-storage`, bytes assets are never
normalized to storage, in `generate()` or in submitted task handlers.

**Solution:** Configure `oridecon-storage` before the multimedia provider boots:
...
```bash
uv add "oridecon-multimedia-beat[librosa]"
```
...
```bash
oridecon-tts-chatterbox-serve   # or oridecon-tts-kokoro-serve / oridecon-tts-f5-tts-serve
oridecon-video-wan22-serve      # or oridecon-video-cogvideox-serve / oridecon-video-svd-serve
...
- Inspect `provider._sub_providers` keys: exactly seven for the core
  ...
  discovered via `oridecon.multimedia.subsystems`.
...
- Open an issue at https://github.com/dbtinoy-/oridecon/issues