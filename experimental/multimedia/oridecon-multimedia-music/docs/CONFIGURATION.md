# Configuration: oridecon-multimedia-music

Configuration options for the music generation subsystem. Everything is driven by one dataclass — `MusicConfig` in `src/oridecon/multimedia/music/config.py`.
...
2. **Environment variables** — prefix `ORI_MULTIMEDIA_MUSIC__`, e.g. `ORI_MULTIMEDIA_MUSIC__BACKEND=ace-step`.
...
|----------|-------------|
| `ORI_MULTIMEDIA_MUSIC__BACKEND` | Backend selector: `local-http`, `stability-audio`, `ace-step`, `stable-audio-open` |
| `ORI_MULTIMEDIA_MUSIC__LOCAL_HTTP_BASE_URL` | URL of the local-http server |
| `ORI_MULTIMEDIA_MUSIC__ACE_STEP_BASE_URL` | URL of the ACE-Step reference server |
| `ORI_MULTIMEDIA_MUSIC__STABLE_AUDIO_OPEN_BASE_URL` | URL of the Stable Audio Open reference server |
| `ORI_MULTIMEDIA_MUSIC__TIMEOUT` | Request timeout in seconds |

```bash
ORI_MULTIMEDIA_MUSIC__BACKEND=ace-step \
ORI_MULTIMEDIA_MUSIC__ACE_STEP_BASE_URL=http://10.0.0.5:5300 \
ORI_MULTIMEDIA_MUSIC__TIMEOUT=180 \
  python -m your_app
```
...
# ORI_MULTIMEDIA_MUSIC__BACKEND=stable-audio-open
# ORI_MULTIMEDIA_MUSIC__STABLE_AUDIO_OPEN_BASE_URL=http://models.internal:5301
```
