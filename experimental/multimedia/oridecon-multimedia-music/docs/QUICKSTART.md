# Quickstart: oridecon-multimedia-music

Generate music and sound effects from a text prompt, wired through the Oridecon DI container. The default backend talks to a local HTTP reference server, so it works with **zero API keys**.
...
```bash
uv add "oridecon-multimedia-music[ace-step-server]"          # local ACE-Step full-song server (torch)
uv add "oridecon-multimedia-music[stable-audio-open-server]" # local Stable Audio Open FX server (torch)
```

> `oridecon-multimedia-music` depends on `oridecon` and `oridecon-contracts` (installed automatically). `aiohttp` is a hard dependency used by every backend.
...
```bash
oridecon-music-stable-audio-open-serve   # serves /generate and /health on :5301
```
