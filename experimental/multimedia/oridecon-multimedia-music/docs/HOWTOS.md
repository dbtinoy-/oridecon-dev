# How-To Guides: oridecon-multimedia-music

Task-oriented recipes for common music generation operations. All identifiers come from the package source — `src/oridecon/multimedia/music/`.
...
```bash
pip install "oridecon-multimedia-music[ace-step-server]"
oridecon-music-ace-step-serve          # loads AceStepPipeline once, serves :5300
```
...
```bash
pip install "oridecon-multimedia-music[stable-audio-open-server]"
oridecon-music-stable-audio-open-serve    # serves :5301
```
...
`MusicGenerationTask` is already bound to the same backend instance the provider registered. `run()` returns a JSON-serializable dict — safe for `oridecon-tasks`' result store.
...
Unhappy path: when `backend.generate()` returns `Err`, `run()` **raises** `result.unwrap_err()` — the task is marked failed, which oridecon-tasks can retry.
...
from oridecon.contracts.core.result import Ok
from oridecon.contracts.multimedia.types import MediaAsset
