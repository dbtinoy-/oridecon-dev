# Architecture: oridecon-multimedia-music

Internal design of the music generation subsystem — which components exist, how they wire together, and how to extend it. Verified against `src/oridecon/multimedia/music/`.

---

## Role in the System

`oridecon-multimedia-music` is a **generation subsystem** in the `oridecon-multimedia-*` family: it converts text prompts to audio bytes. It is a thin, provider-pattern layer over the `MusicProvider` contract — it owns no model weights itself. Two reference `aiohttp` servers ship alongside so self-hosted models (ACE-Step, Stable Audio Open) can be deployed out-of-process.
...
flowchart BT
    ...
    subgraph Core[oridecon core]
        Di[Container · Provider lifecycle]
        Health[Health subsystem]
        Res[oridecon-resilience<br/>RetryPolicyProtocol · CircuitBreakerProtocol]
    end
    subgraph Music[oridecon-multimedia-music]
    ...
    subgraph Contracts[oridecon-contracts]
        ...
        Umbrella[oridecon-multimedia umbrella<br/>auto-discovery entry points]
    end
...
**Import rule:** the package imports only `oridecon`, `oridecon-contracts`, and `aiohttp`. Cross-subsystem access happens through contracts and the container — never direct imports (monorepo §1).
...
| `MusicGenerationTask` | `tasks.py` | `oridecon-tasks`-compatible handler; `run(params) -> dict` for JSON-safe job results |
| `ace_step_server.py` | `servers/` | Reference server entry point on `:5300` (console script `oridecon-music-ace-step-serve`) |
| `stable_audio_open_server.py` | `servers/` | Reference server entry point on `:5301` (console script `oridecon-music-stable-audio-open-serve`) |
...
```
oridecon.multimedia.subsystems: music → oridecon.multimedia.music.di.provider:AudioMusicProvider
oridecon.multimedia.modules:     music → oridecon.multimedia.music.module:AudioMusicModule
```
...
|----------|----------|----------------|
| `MusicProvider` | `oridecon.contracts.multimedia.protocols` | `LocalHttpMusicProvider`, `AceStepMusicProvider`, `StableAudioOpenMusicProvider` |
| `MusicRequest` | `oridecon.contracts.multimedia.types` | — (frozen dataclass: `prompt`, `duration_seconds=30.0`, `format="mp3"`, `extra`) |
| `MediaAsset` | `oridecon.contracts.multimedia.types` | — (frozen dataclass: `mime_type`, `provider`, `bytes_data`/`uri`, `metadata`; `has_bytes`/`has_uri` properties) |
| `MultimediaError` / `MusicGenerationError` / `ProviderNotInstalledError` | `oridecon.contracts.multimedia.exceptions` | — (codes `ORI_ERR_MM_001` / `_003` / `_006`) |
...
- **JSON-safe job results** — `MusicGenerationTask.run()` returns a dict (`provider`, `mime_type`, `bytes_data`, `uri`, `metadata`), never a `MediaAsset`, because `oridecon-tasks` JSON-serializes `JobResult`. Umbrella storage persistence happens before this dict is built.
...
| Task pipeline | Resolve `MusicGenerationTask` from the container and submit via `oridecon-tasks`; `params` map 1:1 onto `MusicRequest` fields |
| Umbrella integration | Already wired — entry points `oridecon.multimedia.subsystems` / `oridecon.multimedia.modules` are discovered by `oridecon-multimedia` |
