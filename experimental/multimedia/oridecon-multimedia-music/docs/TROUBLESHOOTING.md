# Troubleshooting: oridecon-multimedia-music

Common issues with music generation and how to fix them. Error text comes from the package source; identifiers cross-reference `src/oridecon/multimedia/music/`.
...
Prefer the shipped reference servers (`oridecon-music-ace-step-serve`, `oridecon-music-stable-audio-open-serve`); they implement the matching contract on `:5300` / `:5301`.
...
- Check the [oridecon-multimedia-music](https://github.com/dbtinoy-/oridecon) repository and open an issue with the full error text, `MusicConfig.backend`, and your server's `/health` response.