...
def test_chatterbox_server_caps_body_at_1_mib() -> None:
    from oridecon.multimedia.tts.servers.chatterbox_server import MAX_BODY_BYTES

    assert MAX_BODY_BYTES == 1 * 1024 * 1024


def test_kokoro_server_caps_body_at_1_mib() -> None:
    from oridecon.multimedia.tts.servers.kokoro_server import MAX_BODY_BYTES

    assert MAX_BODY_BYTES == 1 * 1024 * 1024


def test_piper_server_caps_body_at_1_mib() -> None:
    from oridecon.multimedia.tts.servers.piper_server import MAX_BODY_BYTES

    assert MAX_BODY_BYTES == 1 * 1024 * 1024


def test_f5_tts_server_caps_body_at_64_mib() -> None:
    from oridecon.multimedia.tts.servers.f5_tts_server import MAX_BODY_BYTES
...
async def test_f5_reference_audio_fetch_capped(aiohttp_client: Any) -> None:
    from oridecon.multimedia.tts.servers.f5_tts_server import _resolve_reference_audio
...
async def test_f5_generate_maps_bad_reference_audio_to_400(
    ...
    from oridecon.multimedia.tts.servers.f5_tts_server import handle_generate
