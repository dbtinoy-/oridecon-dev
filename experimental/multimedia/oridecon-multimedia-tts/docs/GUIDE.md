...
`oridecon-multimedia-tts` synthesizes speech from text. Like its music sibling, it is **backend-agnostic**: your code depends on the `TTSProvider` contract, and `TTSConfig.backend` decides which of the **seven** backends answers — from a zero-config local HTTP server to hosted ElevenLabs/OpenAI APIs to four in-process local model servers.

The umbrella `oridecon-multimedia` auto-discovers it via entry points; it also works standalone through `AudioTTSModule`.
...
- **`TTSProvider`** — structural protocol (from `oridecon-contracts`): `async generate(request: TTSRequest) -> Result[MediaAsset, MultimediaError]`.
...
- **`TTSError`** — the package error family (leaf of `MultimediaError`, code `ORI_ERR_MM_002`). Returned in `Err(...)` for domain/transport failures.
...
- **Reference servers** — `aiohttp` servers that load a model once and serve `/generate` + `/health` (`oridecon-tts-*-serve` scripts).
...
Resolve `TTSGenerationTask` from the container and submit via `oridecon-tasks`. `run()` returns a JSON-serializable dict (never raw bytes) — safe for the JSON result store:
...
- **`oridecon` core** — `Application.boot()`, provider lifecycle (`register` → `boot`), container singletons for `TTSConfig`, `TTSProvider`, `TTSGenerationTask`.
- **`oridecon-contracts`** — `TTSProvider` protocol, `TTSRequest` / `MediaAsset` types, and the error family (`TTSError`, `MultimediaError`, `ProviderNotInstalledError`).
...
- **`oridecon-tasks`** — `TTSGenerationTask` is a compatible handler for the async job path.
- **`oridecon-multimedia` umbrella** — entry points `oridecon.multimedia.subsystems: tts` and `oridecon.multimedia.modules: tts` enable auto-discovery.
