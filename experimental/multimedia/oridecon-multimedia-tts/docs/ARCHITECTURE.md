# Architecture: oridecon-multimedia-tts

Internal design of the TTS subsystem — components, wiring, lifecycle, and extension points. Verified against `src/oridecon/multimedia/tts/`.

---

## Role in the System

`oridecon-multimedia-tts` is a **generation subsystem** in the `oridecon-multimedia-*` family: it converts text to speech audio. It is a provider-pattern layer over the `TTSProvider` contract — no model weights live in the package. Four reference `aiohttp` servers ship alongside so self-hosted models (Chatterbox, Kokoro, F5-TTS, Piper) deploy out-of-process, plus `local-http` for any conforming third-party server, plus hosted API backends (ElevenLabs, OpenAI).
...
flowchart BT
    ...
    subgraph TTS[oridecon-multimedia-tts]
...
**Import rule:** the package imports only `oridecon`, `oridecon-contracts`, and `aiohttp`. Cross-subsystem access happens through contracts and the container.
...
| `TTSGenerationTask` | `tasks.py` | `oridecon-tasks`-compatible handler; `run(params) -> dict` (JSON-safe) |
...
| Reference servers ×4 | `servers/` | `chatterbox_server.py` (:5100), `kokoro_server.py` (:5101), `f5_tts_server.py` (:5102), `piper_server.py` (:5103) — console scripts `oridecon-tts-*-serve` |
...
- **`elevenlabs`** — guarded import; `ImportError` → `ProviderNotInstalledError` ("install oridecon-multimedia-tts[elevenlabs]"). Then `resolve_credential(secret_store, elevenlabs_api_key_secret_name)`; missing `elevenlabs_voice_id` → `ProviderNotInstalledError` (fail fast).
...
```
oridecon.multimedia.subsystems: tts → oridecon.multimedia.tts.di.provider:AudioTTSProvider
oridecon.multimedia.modules:     tts → oridecon.multimedia.tts.module:AudioTTSModule
```
...
|----------|----------|----------------|
| `TTSProvider` | `oridecon.contracts.multimedia.protocols` | All seven backend classes |
| `TTSRequest` | `oridecon.contracts.multimedia.types` | — (frozen dataclass: `text`, `voice`, `format="mp3"`, `reference_audio_uri`, `emotion`, `extra`) |
| `MediaAsset` | `oridecon.contracts.multimedia.types` | — (frozen dataclass with `has_bytes`/`has_uri`) |
| `MultimediaError` / `TTSError` / `ProviderNotInstalledError` | `oridecon.contracts.multimedia.exceptions` | — (codes `ORI_ERR_MM_001` / `_002` / `_006`) |
| `TTSAuthenticationError` | `oridecon.multimedia.tts.exceptions` (this package) | leaf exception, code `ORI_ERR_MM_TTS_002` — 401 from hosted APIs |
...
- **JSON-safe job results** — `TTSGenerationTask.run()` returns a plain dict (never a `MediaAsset`) because oridecon-tasks JSON-serializes `JobResult`; umbrella storage persists bytes before this dict is built.
...
| Task pipeline | Resolve `TTSGenerationTask` and submit through `oridecon-tasks`; `params` map onto `TTSRequest` fields |
| Umbrella integration | Already wired — `oridecon.multimedia.subsystems` / `oridecon.multimedia.modules` entry points |
