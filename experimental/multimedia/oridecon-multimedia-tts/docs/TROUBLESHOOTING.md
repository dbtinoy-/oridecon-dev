# Troubleshooting: oridecon-multimedia-tts

Common issues with text-to-speech and how to fix them. Error text comes from the package source; identifiers cross-reference `src/oridecon/multimedia/tts/`.
...
```bash
uv add "oridecon-multimedia-tts[elevenlabs]"
# or openai:  uv add "oridecon-multimedia-tts[openai]"
```
...
```python
from oridecon.contracts.multimedia.types import TTSRequest
...
```bash
oridecon-tts-chatterbox-serve   # :5100
oridecon-tts-kokoro-serve       # :5101
oridecon-tts-f5-tts-serve       # :5102
oridecon-tts-piper-serve        # :5103
...
- Check the [oridecon-multimedia-tts](https://github.com/dbtinoy-/oridecon) repository and open an issue with the backend name, full error text, and your server's `/health` response.