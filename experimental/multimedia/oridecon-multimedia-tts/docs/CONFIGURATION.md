# Configuration: oridecon-multimedia-tts

Configuration options for the TTS subsystem. Everything is driven by one dataclass — `TTSConfig` in `src/oridecon/multimedia/tts/config.py`.
...
2. **Environment variables** — prefix `ORI_MULTIMEDIA__TTS__`, e.g. `ORI_MULTIMEDIA__TTS__BACKEND=openai`.
...
|----------|-------------|
| `ORI_MULTIMEDIA__TTS__BACKEND` | Backend selector: `local-http`, `elevenlabs`, `openai`, `chatterbox`, `kokoro`, `f5-tts`, `piper` |
| `ORI_MULTIMEDIA__TTS__LOCAL_HTTP_BASE_URL` | URL of the self-hosted local-http server |
| `ORI_MULTIMEDIA__TTS__ELEVENLABS_VOICE_ID` | ElevenLabs voice ID |
| `ORI_MULTIMEDIA__TTS__ELEVENLABS_API_KEY_SECRET_NAME` | Secrets name for the ElevenLabs key |
| `ORI_MULTIMEDIA__TTS__OPENAI_API_KEY_SECRET_NAME` | Secrets name for the OpenAI key |
| `ORI_MULTIMEDIA__TTS__OPENAI_VOICE` | Default OpenAI voice |
| `ORI_MULTIMEDIA__TTS__OPENAI_MODEL` | OpenAI TTS model |
| `ORI_MULTIMEDIA__TTS__OPENAI_BASE_URL` | OpenAI-compatible base URL |
| `ORI_MULTIMEDIA__TTS__CHATTERBOX_BASE_URL` | Chatterbox server URL |
| `ORI_MULTIMEDIA__TTS__CHATTERBOX_EXAGGERATION` | Chatterbox exaggeration |
| `ORI_MULTIMEDIA__TTS__CHATTERBOX_CFG_WEIGHT` | Chatterbox CFG weight |
| `ORI_MULTIMEDIA__TTS__CHATTERBOX_TEMPERATURE` | Chatterbox temperature |
| `ORI_MULTIMEDIA__TTS__KOKORO_BASE_URL` | Kokoro server URL |
| `ORI_MULTIMEDIA__TTS__KOKORO_DEFAULT_VOICE` | Default Kokoro voice |
| `ORI_MULTIMEDIA__TTS__F5_TTS_BASE_URL` | F5-TTS server URL |
| `ORI_MULTIMEDIA__TTS__PIPER_BASE_URL` | Piper server URL |
| `ORI_MULTIMEDIA__TTS__PIPER_DEFAULT_VOICE` | Default Piper voice |
| `ORI_MULTIMEDIA__TTS__TIMEOUT` | Request timeout in seconds |

```bash
ORI_MULTIMEDIA__TTS__BACKEND=openai \
ORI_MULTIMEDIA__TTS__OPENAI_VOICE=echo \
ORI_MULTIMEDIA__TTS__OPENAI_MODEL=tts-1-hd \
  python -m your_app
```
...
# ORI_MULTIMEDIA__TTS__BACKEND=kokoro
# ORI_MULTIMEDIA__TTS__KOKORO_BASE_URL=http://tts.internal:5101
```
