# How-To Guides: oridecon-multimedia-tts

Task-oriented recipes for common TTS operations. All identifiers come from the source — `src/oridecon/multimedia/tts/`.
...
```bash
uv add "oridecon-multimedia-tts[chatterbox-server]"
oridecon-tts-chatterbox-serve          # serves :5100
```
...
```bash
uv add "oridecon-multimedia-tts[f5-tts-server]"
oridecon-tts-f5-tts-serve             # serves :5102
```
...
```bash
uv add "oridecon-multimedia-tts[piper-server]"
oridecon-tts-piper-serve              # serves :5103
```
...
from oridecon.contracts.multimedia.types import MediaAsset, TTSRequest
