"""Piper (ONNX, CPU-only) local TTS reference-server client.

Lightest-weight of the four local engines — sub-second cold start, no GPU.
...
from oridecon.multimedia.tts.exceptions import TTSError
...
class PiperTTSProvider:
    """Talks to a piper_server.py reference server via TTSProvider."""
    ...
    async def generate(self, request: TTSRequest) -> Result[MediaAsset, TTSError]:
        ...
        try:
            ...
            return Err(TTSError(f"Piper request failed: {exc}", cause=exc))

        if status != 200:
            return Err(TTSError(f"Piper server returned {status}: {body!r}"))

        return Ok(MediaAsset(mime_type="audio/wav", provider="piper", bytes_data=body))
