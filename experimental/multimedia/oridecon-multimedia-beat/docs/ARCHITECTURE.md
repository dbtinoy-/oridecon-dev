...
Internal design of `oridecon-multimedia-beat` — audio tempo/beat detection in Oridecon.

---

## Role in the System

`oridecon-multimedia-beat` is the **sync-only analysis** member of the multimedia family.
...
flowchart LR
    ...
    subgraph Pkg[oridecon-multimedia-beat]
        ...
        SRV[Reference Server<br/>oridecon-beat-madmom-serve]
    end
    subgraph Ext[External]
        ResNet[RetryPolicyProtocol / CircuitBreakerProtocol<br/>oridecon-resilience - optional]
    end
...
**Import rule:** the package depends only on `oridecon` and `oridecon-contracts`
...
- **`madmom_server.py`** — the reference server: aiohttp app exposing `/analyze` and
  `/health`, driven by the `oridecon-beat-madmom-serve` console script.
...
`oridecon.multimedia.subsystems` entry point (`beat`) and its module via
`oridecon.multimedia.modules` (`beat`).
...
| `BeatAnalysisError` | `contracts/multimedia/exceptions.py` | domain base (`ORI_ERR_MM_008`) |
...
(`BeatAnalysisError` subclass, `ORI_ERR_MM_BEAT_003`).
...
- **Result over exceptions.** `analyze()` returns `Result[BeatAnalysisResult, MultimediaError]`;
  ...
  with `oridecon.contracts.multimedia` where `MultimediaError` is a recoverable domain error.
...
- **Optional resilience.** `resolve_optional()` means the backend gracefully skips
  retry/breaker when `oridecon-resilience` is absent instead of importing it eagerly.
...
| Umbrella integration | Installed alongside `oridecon-multimedia`, the umbrella wires `config.beat` and exposes `MultimediaProvider.beat` |