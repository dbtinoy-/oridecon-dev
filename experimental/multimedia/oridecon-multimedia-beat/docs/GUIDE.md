...
Learn how to use `oridecon-multimedia-beat` for tempo and beat detection.

---

## Overview

`oridecon-multimedia-beat` analyzes an audio `MediaAsset` and returns a
...
The package also ships the reference server itself: `oridecon-beat-madmom-serve` runs a
...
- **`BeatAnalysisProvider`** — the protocol from `oridecon.contracts.multimedia`:
...
When installed alongside `oridecon-multimedia`, the beat subsystem is wired by the
...
- **`oridecon-multimedia` (umbrella)** — the umbrella's `MultimediaProvider.register()`
  constructs `BeatAnalysisGenerationProvider(config=self._config.beat)` and its `.beat`
  accessor delegates to the bound backend. The beat subsystem's `oridecon.multimedia.subsystems`
  entry point (`beat → BeatAnalysisGenerationProvider`) is what registers it.
- **`oridecon-resilience`** — optional. When `RetryPolicyProtocol` and/or
  `CircuitBreakerProtocol` are bound, `MadmomBeatAnalysisProvider` wraps its HTTP call in
  them (retry → circuit-breaker → raw post).
- **`oridecon-contracts`** — the protocol and value types live in
  `oridecon.contracts.multimedia`; `BeatAnalysisError` (`ORI_ERR_MM_008`) is the base
...
- **Contracts → your services** — a downstream composer service injects
  ...
  schedule `oridecon-multimedia-video` operations.
