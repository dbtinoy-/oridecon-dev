...
Configuration options for `oridecon-multimedia-beat`.
...
standalone, or from `multimedia.beat:` when nested under the `oridecon-multimedia`
umbrella. Environment variable prefix: `ORI_MULTIMEDIA_BEAT__`.

`BeatAnalysisConfig` extends `oridecon.config.BaseConfig` and declares
...
|---------|-------------|
| `ORI_MULTIMEDIA_BEAT__BACKEND` | Beat backend: `librosa` or `madmom` |
| `ORI_MULTIMEDIA_BEAT__LIBROSA_SAMPLE_RATE` | Sample rate for the librosa backend |
| `ORI_MULTIMEDIA_BEAT__MADMOM_BASE_URL` | Madmom reference-server URL |
| `ORI_MULTIMEDIA_BEAT__TIMEOUT` | Request timeout in seconds |

```bash
ORI_MULTIMEDIA_BEAT__BACKEND=madmom \
ORI_MULTIMEDIA_BEAT__MADMOM_BASE_URL=http://10.0.0.5:5600 \
ORI_MULTIMEDIA_BEAT__TIMEOUT=60.0 \
  python -m my_app
```

> Under the umbrella, the nested key uses `multimedia:`:
> `ORI_MULTIMEDIA__BEAT__BACKEND=madmom` — see the umbrella's Configuration doc.
