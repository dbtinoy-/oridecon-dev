...
def test_image_config_from_dict_and_redaction() -> None:
    cfg = ImageConfig.from_dict(
        {"backend": "openai", "stability_api_key_secret_name": "ori_stability"}
    )
...
def test_stability_backend_configured_explicitly() -> None:
    config = ImageConfig(
        backend="stability",
        stability_api_key_secret_name="ori_my_stability_key",
    )
    assert config.backend == "stability"
    assert config.stability_api_key_secret_name == "ori_my_stability_key"
