...
multimedia_image:
  ...
  comfyui_workflow_path: "/etc/oridecon/workflows/custom.json"
```
...
`ImageGenerationTask.run(params)` is the `oridecon-tasks` entry point — submit
plain-dict params and receive a JSON-serializable result dict:

```python
from oridecon.multimedia.image import ImageGenerationTask
...
`JobResult`. Under the `oridecon-multimedia` umbrella, the wrapper persists the
asset's bytes into `oridecon-storage` before this dict is constructed, so
...
from oridecon.multimedia.image import ImageGenerationProvider
