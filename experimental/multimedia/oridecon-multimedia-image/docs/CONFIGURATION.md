...
All configuration options for `oridecon-multimedia-image`.
...
2. Profile overlay `application.{ORI_PROFILE}.yaml` (if set)
3. `ORI_*` environment variables

`ImageGenerationProvider` passes the section directly to the container:
`config_key = "multimedia_image"`. Under the `oridecon-multimedia` umbrella the
...
Prefix: `ORI_MULTIMEDIA__IMAGE__` (umbrella-layout YAML) or
`ORI_MULTIMEDIA_IMAGE__` (flat `multimedia_image` section). Field names map
directly, `__` separates nesting:

| Variable | Description |
|---------|------------|
| `ORI_MULTIMEDIA__IMAGE__BACKEND` | Backend selection |
| `ORI_MULTIMEDIA__IMAGE__LOCAL_HTTP_BASE_URL` | Local server URL |
| `ORI_MULTIMEDIA__IMAGE__OPENAI_API_KEY_SECRET_NAME` | OpenAI key secret name |
| `ORI_MULTIMEDIA__IMAGE__OPENAI_MODEL` | OpenAI model |
| `ORI_MULTIMEDIA__IMAGE__OPENAI_BASE_URL` | OpenAI-compatible base URL |
| `ORI_MULTIMEDIA__IMAGE__STABILITY_API_KEY_SECRET_NAME` | Stability key secret name |
| `ORI_MULTIMEDIA__IMAGE__COMFYUI_BASE_URL` | ComfyUI URL |
| `ORI_MULTIMEDIA__IMAGE__COMFYUI_CHECKPOINT` | ComfyUI checkpoint |
| `ORI_MULTIMEDIA__IMAGE__COMFYUI_WORKFLOW_PATH` | Custom workflow JSON path |
| `ORI_MULTIMEDIA__IMAGE__COMFYUI_STEPS` | Sampling steps |
| `ORI_MULTIMEDIA__IMAGE__COMFYUI_CFG_SCALE` | CFG scale |
| `ORI_MULTIMEDIA__IMAGE__COMFYUI_POLL_INTERVAL` | Poll interval seconds |
| `ORI_MULTIMEDIA__IMAGE__TIMEOUT` | Request timeout seconds |
...
export ORI_MULTIMEDIA__IMAGE__TIMEOUT=120
```
...
```python
from oridecon.multimedia.image.providers import OpenAIImageProvider
