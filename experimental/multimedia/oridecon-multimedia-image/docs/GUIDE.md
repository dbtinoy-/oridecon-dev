...
Understand and use `oridecon-multimedia-image` effectively.

---

## Overview

`oridecon-multimedia-image` generates still images from a text prompt inside the
Oridecon framework. It is a backend-selection layer: one `ImageConfig.backend`
...
job handler, the umbrella `oridecon-multimedia` module — talks to
...
- **`ImageProvider`** — the structural contract
  (`oridecon.contracts.multimedia.protocols`). One method:
...
- **`ImageGenerationProvider`** — the DI provider. Selects the backend from
  ...
  singletons.
- **`ImageGenerationTask`** — the `oridecon-tasks` bridge. Its `run(params)`
...
In tests or examples, keep the module real but pinned to the offline backend:
...
- **`oridecon-multimedia` umbrella** — `MultimediaModule` auto-discovers this
  package through the `oridecon.multimedia.subsystems` / `oridecon.multimedia.modules`
  entry points, and wraps the task handler to persist bytes into
  `oridecon-storage` before the job result dict is built. In the umbrella your
  YAML nests the config under `multimedia: image:`.
- **`oridecon-tasks`** — `ImageGenerationTask.run()` is the submit-path entry
...
- **Secrets backend** — keys are resolved by name via
  `AsyncSecretStoreProtocol` (`oridecon.di.provider_utils.resolve_credential`).
