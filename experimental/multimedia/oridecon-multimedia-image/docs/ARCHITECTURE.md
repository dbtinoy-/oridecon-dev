...
Internal design of `oridecon-multimedia-image` and how it fits the Oridecon multimedia subsystem.

---

## Role in the System

`oridecon-multimedia-image` is one of seven generation packages under the
`oridecon-multimedia` umbrella. It owns **still-image generation**: it turns an
`ImageRequest` into a `MediaAsset` through one of four interchangeable HTTP
backends. It depends only on `oridecon` and `oridecon-contracts` — the
`ImageProvider` protocol is the entire public boundary.

```
oridecon-contracts  ←  oridecon  ←  oridecon-multimedia-image → (entry points) oridecon-multimedia
...
| `ImageGenerationTask` | `tasks.py` | `oridecon-tasks` job handler (dict in, dict out) |
...
|--------|-----------|-------|
| `ImageGenerationProvider` | `ImageConfig`, `ImageProvider`, `ImageGenerationTask` as singletons | Provider `name = "image"`; entry point `oridecon.multimedia.subsystems` → `image` |
| `ImageModule` | Re-exported via `oridecon.multimedia.modules` entry point `image` | Exports `[ImageProvider, ImageGenerationTask]` |

Unknown/unimplemented backends raise `ProviderNotInstalledError`
(`ORI_ERR_MM_006` from contracts) at registration — an eager, actionable
...
|----------|----------|----------------|
| `ImageProvider` | `oridecon.contracts.multimedia.protocols` (`generate -> Result[MediaAsset, MultimediaError]`) | All four backends (structural) |
| `AsyncSecretStoreProtocol` | `oridecon.contracts.security.stores` | Resolved optionally; feeds `resolve_credential` |
| `RetryPolicyProtocol` | `oridecon.contracts.infra.resilience.protocols` | Optional; wraps every HTTP call |
| `CircuitBreakerProtocol` | `oridecon.contracts.infra.resilience.protocols` | Optional; wraps every HTTP call |
| `ImageRequest` / `MediaAsset` | `oridecon.contracts.multimedia.types` | Frozen dataclasses; request in, asset out |

Error values (not exceptions) are returned in the `Err` slot:
`ImageGenerationError` base (`ORI_ERR_MM_005`) with leaves
`ImageTimeoutError` (`ORI_ERR_MM_IMAGE_001`) and
`ImageGenerationAuthenticationError` (`ORI_ERR_MM_IMAGE_002`); container-level
failures use `ProviderNotInstalledError` (`ORI_ERR_MM_006`). All inherit
`MultimediaError` → `DomainError` → `OrideconError`.
...
- **`ImageGenerationTask` returns dicts** — `oridecon-tasks` result stores are
...
| Umbrella orchestration | The `oridecon.multimedia.subsystems` / `oridecon.multimedia.modules` entry points let `oridecon-multimedia` pick up this package automatically |