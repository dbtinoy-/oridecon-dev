...
Internal design of `oridecon-multimedia-video` and how it fits into the Oridecon multimedia subsystem.
...
flowchart BT
    ...
    subgraph Pkg[oridecon-multimedia-video]
    ...
    subgraph Servers[Reference Servers]
        WS[oridecon-video-wan22-serve · :5200]
        CS[oridecon-video-cogvideox-serve · :5201]
        SS[oridecon-video-svd-serve · :5202]
    end
...
| `VideoProcessor` | `contracts/multimedia/protocols.py` | implemented by `FFmpegVideoProcessor`; consumed by `oridecon-multimedia-upscale`'s `VideoUpscaleService` |
...
Package leaf exceptions (`exceptions.py`): `VideoTimeoutError` (`ORI_ERR_MM_VIDEO_001`), `VideoGenerationAuthenticationError` (`ORI_ERR_MM_VIDEO_002`), `VideoProcessingError` (`ORI_ERR_MM_VIDEO_003`) — all extend `VideoGenerationError`.
...
| Custom reference server | Any HTTP server speaking `POST /generate` (+ `GET /health`); add a console script (`oridecon-video-*-serve`) |
