...
Learn how to use `oridecon-multimedia-video` effectively.

---

## Overview

`oridecon-multimedia-video` gives Oridecon applications two distinct capabilities behind two protocol boundaries:
...
- **`Error` conventions** — expected failures return `Err(VideoGenerationError)` (base from contracts, `ORI_ERR_MM_004`). Package leaves: `VideoTimeoutError` (`ORI_ERR_MM_VIDEO_001`), `VideoGenerationAuthenticationError` (`ORI_ERR_MM_VIDEO_002`), `VideoProcessingError` (`ORI_ERR_MM_VIDEO_003`) — all in `oridecon.multimedia.video.exceptions`.
- **Tasks** — `VideoGenerationTask` / `VideoProcessingTask` adapt flat param dicts to `VideoRequest` / `VideoOperation` for the `oridecon-tasks` job path.
...
```python
from oridecon.contracts.multimedia import VideoMode, VideoRequest
...
```python
from oridecon.contracts.multimedia import MediaAsset, OverlayText, Trim
import asyncio
...
```python
from oridecon.contracts.multimedia import Concat, MediaAsset, OverlayText, Trim, TransitionSpec
...
### Pattern: frame pipeline (e.g. video upscaling with oridecon-multimedia-upscale)

```python
from oridecon.contracts.multimedia import UpscaleRequest
...
- **`oridecon` core** — provider/module lifecycle, container `singleton` bindings, `Application.boot()`.
- **`oridecon-contracts`** — `VideoProvider`, `VideoProcessor` protocols; all types under `oridecon.contracts.multimedia` (advertised convenience import path `oridecon.contracts.multimedia`).
...
- **`oridecon-multimedia-upscale`** — consumes this package's `VideoProcessor` (via contract only) through `VideoUpscaleService`.
- **`oridecon-tasks`** — task handlers for the async job path; umbrella persists result bytes to oridecon-storage.
- Discovered by the umbrella via entry points `oridecon.multimedia.subsystems` (`video`) and `oridecon.multimedia.modules` (`video`).
