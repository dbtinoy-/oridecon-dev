...
uv add "oridecon-multimedia-video[wan22-server]"          # Wan2.2 server
uv add "oridecon-multimedia-video[cogvideox-server]"      # CogVideoX server
uv add "oridecon-multimedia-video[svd-server]"            # SVD server
# Optional extras for hosted APIs
uv add "oridecon-multimedia-video[runway]"                # Runway API
uv add "oridecon-multimedia-video[openai]"                # OpenAI video gateway
```

`oridecon-multimedia-video` depends on `oridecon`, `oridecon-contracts`, and `aiohttp`. The `VideoProcessor` (editing) additionally requires the **`ffmpeg` binary on `PATH`** — without it the processor simply is not registered.
