"""RIFE local frame-interpolation reference-server client."""
...
class RifeInterpolationProvider:
    ...
    async def _post(self, payload: dict[str, object]) -> tuple[int, bytes, str]:
        async with (
            ...
            session.post(f"{self._base_url}/interpolate", json=payload) as resp,
        ):
            if resp.status != 200:
    ...
    async def interpolate(
        ...
        payload: dict[str, object] = {
            "frame_a_bytes": base64.b64encode(request.frame_a.bytes_data or b"").decode(
                "ascii"
            ),
            "frame_b_bytes": base64.b64encode(request.frame_b.bytes_data or b"").decode(
                "ascii"
            ),
        }
        try:
            ...
            return Err(MultimediaError(f"RIFE request failed: {exc}", cause=exc))

        if status != 200:
            return Err(MultimediaError(f"RIFE server returned {status}: {body!r}"))

        return Ok(MediaAsset(mime_type=content_type, provider="rife", bytes_data=body))


__all__ = ["RifeInterpolationProvider"]
