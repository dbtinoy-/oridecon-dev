...
Frame-rate interpolation with `oridecon-multimedia-interpolate` — up and running in minutes.
...
```bash
uv add "oridecon-multimedia-interpolate[rife-server]"
```
...
2. During `Application.boot()` the provider's `register()` ran:
   ...
   - `InterpolationTask` was wired around the same backend for `oridecon-tasks`
     `submit()` jobs.
