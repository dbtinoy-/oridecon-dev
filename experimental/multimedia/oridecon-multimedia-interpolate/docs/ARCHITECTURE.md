...
Internal design of `oridecon-multimedia-interpolate` and how it fits the Oridecon multimedia subsystem.

---

## Role in the System

`oridecon-multimedia-interpolate` is the frame-interpolation member of the
`oridecon-multimedia` umbrella. It owns two jobs:
...
2. **Whole-video interpolation** — doubling or quadrupling a clip's frame rate
   ...
   `oridecon-multimedia-video`) **through contracts only**.

The package depends exclusively on `oridecon` and `oridecon-contracts` — the
only `oridecon-multimedia-video` connection is the `VideoProcessor` protocol,
...
```
oridecon-contracts  ←  oridecon  ←  oridecon-multimedia-interpolate → (entry points) oridecon-multimedia
                                          │
                                          ├── VideoProcessor (protocol, resolved) ── oridecon-multimedia-video
...
| `InterpolationTask` | `tasks.py` | `oridecon-tasks` job handler (dict in, dict out) |
...
|--------|-----------|-------|
| `InterpolationGenerationProvider` | `InterpolationConfig`, `InterpolationProvider`, `InterpolationTask`, `VideoInterpolationService` (conditional) | Provider `name = "interpolate"`; entry point `oridecon.multimedia.subsystems` → `interpolate` |
| `InterpolationModule` | Re-exported via `oridecon.multimedia.modules` entry point `interpolate` | Exports `[InterpolationProvider, InterpolationTask]` |
...
|----------|----------|---------|
| `InterpolationProvider` | `oridecon.contracts.multimedia.protocols` | `RifeInterpolationProvider` (structural impl); consumed by `VideoInterpolationService` |
| `VideoProcessor` | `oridecon.contracts.multimedia.protocols` | Optional dependency: `extract_frames`, `assemble_frames`; gates service registration |
| `RetryPolicyProtocol` / `CircuitBreakerProtocol` | `oridecon.contracts.infra.resilience.protocols` | Optional; wrap every `/interpolate` call |
| `InterpolationRequest` / `MediaAsset` | `oridecon.contracts.multimedia.types` | Frozen dataclasses; both in/out types |
| `MultimediaError` | `oridecon.contracts.multimedia.exceptions` | The `Err` type for `interpolate()` and `interpolate_video()` |

This package defines **no own exceptions** (`exceptions.py` is empty) — the
contracts' `MultimediaError` (code `ORI_ERR_MM_001`) covers all failures, and
`ProviderNotInstalledError` (`ORI_ERR_MM_006`) is raised by the provider for
unknown backends.
...
- **Contracts-only cross-package composition** — `VideoInterpolationService`
  ...
  constructor injection. There is no import of `oridecon-multimedia-video`
...
- **Dict-in/dict-out task handler** — `InterpolationTask` returns
  JSON-serializable shapes for `oridecon-tasks`' result store (bytes persist at
  the umbrella layer).
...
| Umbrella orchestration | `oridecon.multimedia.subsystems` / `oridecon.multimedia.modules` entry points (`interpolate`) for automatic discovery by `oridecon-multimedia` |