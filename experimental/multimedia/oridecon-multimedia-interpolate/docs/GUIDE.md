...
Understand and use `oridecon-multimedia-interpolate` effectively.

---

## Overview

`oridecon-multimedia-interpolate` synthesizes frames between two input frames
...
`oridecon-multimedia` umbrella: frame-pair work happens here, video-level work
...
- **`InterpolationProvider`** — the structural contract
  (`oridecon.contracts.multimedia.protocols`):
...
- **`InterpolationTask`** — the `oridecon-tasks` bridge. `run(params)` rebuilds
...
- **`rife_server.py`** — the packaged reference server: an `aiohttp` web app
  ...
  `oridecon-interpolate-rife-serve` console script.
...
`oridecon-multimedia-video`) is registered in the container — otherwise
...
- **`oridecon-multimedia` umbrella** — discovered via the
  `oridecon.multimedia.subsystems` / `oridecon.multimedia.modules` entry
  points; config nests under `multimedia: interpolate:`, and the umbrella
  wraps the task handler to persist result bytes into `oridecon-storage`.
- **`oridecon-multimedia-video`** — via the `VideoProcessor` protocol only,
  ...
  this package at all.
- **`oridecon-tasks`** — `InterpolationTask.run()` is the submit path; errors
