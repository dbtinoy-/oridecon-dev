...
Task-oriented recipes for `oridecon-multimedia-interpolate`.
...
```bash
pip install "oridecon-multimedia-interpolate[rife-server]"
oridecon-interpolate-rife-serve
```

The `oridecon-interpolate-rife-serve` console script (entry point →
...
via `oridecon-multimedia-video`. The provider auto-composes and registers
`VideoInterpolationService` during `register()`.

```python
from oridecon.contracts.multimedia import MediaAsset
from oridecon.multimedia.interpolate import VideoInterpolationService
...
`InterpolationTask.run(params)` is the `oridecon-tasks` submit-path handler.
Frames arrive as plain dicts and the result is a JSON-serializable dict —
never raw `MediaAsset` bytes.

```python
from oridecon.multimedia.interpolate import InterpolationTask
...
Under the `oridecon-multimedia` umbrella, the wrapper persists bytes into
`oridecon-storage` before the dict is built — expect `bytes_data: null` and a
...
from oridecon.multimedia.interpolate import InterpolationGenerationProvider
