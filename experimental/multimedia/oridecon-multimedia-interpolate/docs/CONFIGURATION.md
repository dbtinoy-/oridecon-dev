...
All configuration options for `oridecon-multimedia-interpolate`.
...
(`config_key = "multimedia_interpolate"`). Under the `oridecon-multimedia`
...
Prefix: `ORI_MULTIMEDIA__INTERPOLATE__` (umbrella-layout YAML) or
`ORI_MULTIMEDIA_INTERPOLATE__` (flat `multimedia_interpolate` section). Field
names map directly:

| Variable | Description |
|---------|------------|
| `ORI_MULTIMEDIA__INTERPOLATE__BACKEND` | Backend selection (`rife`) |
| `ORI_MULTIMEDIA__INTERPOLATE__RIFE_BASE_URL` | RIFE server URL |
| `ORI_MULTIMEDIA__INTERPOLATE__TIMEOUT` | Request timeout in seconds |

Example:

```bash
export ORI_MULTIMEDIA__INTERPOLATE__RIFE_BASE_URL=http://rife.internal:5500
export ORI_MULTIMEDIA__INTERPOLATE__TIMEOUT=45
```
...
has a `VideoProcessor` (from `oridecon-multimedia-video`), the provider
