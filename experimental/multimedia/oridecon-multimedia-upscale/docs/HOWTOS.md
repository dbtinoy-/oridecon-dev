...
Task-oriented recipes for `oridecon-multimedia-upscale`.
...
```python
from oridecon.contracts.multimedia import MediaAsset, UpscaleRequest
...
oridecon-upscale-hat-serve   # start the HAT reference server on :5401
```
...
Requires a `VideoProcessor` in the container — install `oridecon-multimedia-video` and register its module alongside:

```python
from oridecon.contracts.multimedia import MediaAsset
from oridecon.multimedia.upscale import VideoUpscaleService
...
```bash
oridecon-upscale-real-esrgan-serve   # binds :5400
oridecon-upscale-hat-serve           # binds :5401
```
