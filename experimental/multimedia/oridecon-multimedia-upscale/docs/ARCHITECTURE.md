...
Internal design of `oridecon-multimedia-upscale` and how it fits into the Oridecon multimedia subsystem.
...
flowchart BT
    ...
    subgraph Pkg[oridecon-multimedia-upscale]
    ...
    subgraph Servers[Reference Servers — separate processes]
        RE[oridecon-upscale-real-esrgan-serve · :5400]
        HAT[oridecon-upscale-hat-serve · :5401]
    end
    subgraph Video[oridecon-multimedia-video]
...
- **`servers/`** — reference server entry points (`oridecon-upscale-real-esrgan-serve`, `oridecon-upscale-hat-serve`).
...
- Callers depend on `UpscaleProvider` from `oridecon-contracts` — never on a backend class.
...
Resolved optionally at `register()`: `RetryPolicyProtocol`, `CircuitBreakerProtocol` (from `oridecon.di.provider_utils.resolve_optional`).

---

## Contracts

Used from `oridecon-contracts` (`oridecon.contracts.multimedia`):
...
| `VideoProcessor` | `contracts/multimedia/protocols.py` | Consumed by `VideoUpscaleService` (implemented by `oridecon-multimedia-video`) |
| `MediaAsset`, `UpscaleRequest` | `contracts/multimedia/types.py` | Request/response value objects |
| `UpscaleError` | `contracts/multimedia/exceptions.py` | Domain error (`ORI_ERR_MM_007`), re-exported by this package's `exceptions.py` |
...
| New reference server | Any HTTP server exposing `POST /upscale` + `GET /health`; add a console script in `[project.scripts]` (`oridecon-upscale-*-serve`) |
| Custom video pipeline | Provide any `VideoProcessor` implementation in the container (`oridecon-multimedia-video`'s `FFmpegVideoProcessor` is one) |
