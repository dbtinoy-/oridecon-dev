...
Common issues with `oridecon-multimedia-upscale` and how to fix them.
...
```bash
oridecon-upscale-real-esrgan-serve   # :5400 (backend real-esrgan)
oridecon-upscale-hat-serve           # :5401 (backend hat)
...
```bash
pip install torch timm oridecon-multimedia-upscale
# install the real HAT package your deployment vendored
oridecon-upscale-hat-serve
```
...
**Cause:** `VideoUpscaleService` only registers when a `VideoProcessor` is present in the container. `oridecon-multimedia-video` registers its `FFmpegVideoProcessor` only when `ffmpeg` is found on `PATH`.

**Fix:** Install `oridecon-multimedia-video`, register `VideoModule`, and confirm `ffmpeg` is installed:
...
- Open an issue at [oridecon](https://github.com/dbtinoy-/oridecon/issues) with the provider, backend, and the exact `UpscaleError` message.