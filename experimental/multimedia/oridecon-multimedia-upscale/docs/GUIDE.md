...
Learn how to use `oridecon-multimedia-upscale` effectively.

---

## Overview

`oridecon-multimedia-upscale` provides **single-image super-resolution** (2x or 4x) and **frame-level video upscaling** for Oridecon applications.
...
- Whole-video upscaling (`VideoUpscaleService`) is composed from the video package's `VideoProcessor` protocol — no direct dependency on `oridecon-multimedia-video`.
...
- **`UpscaleError`** — domain error from `oridecon-contracts` (`ORI_ERR_MM_007`). Recoverable failures are returned as `Err(...)`, not raised.
- **Backends** — `RealEsrganUpscaleProvider` (default, port `5400`) and `HatUpscaleProvider` (port `5401`). Selected by `UpscaleConfig.backend`.
- **`UpscaleTask`** — callable task handler for the async job path (`oridecon-tasks`). Accepts a flat `params` dict, returns a plain asset dict.
...
Install and register `oridecon-multimedia-video` alongside this package, then resolve `VideoUpscaleService`:
...
- **`oridecon` core** — module/provider lifecycle; container `singleton` bindings; `Application.boot()`.
- **`oridecon-contracts`** — `UpscaleProvider`, `VideoProcessor` protocols; `MediaAsset`, `UpscaleRequest` types; `UpscaleError`, `ProviderNotInstalledError` exceptions. Import path: `oridecon.contracts.multimedia`.
- **`oridecon-multimedia-video`** — provides the `VideoProcessor` (ffmpeg-backed) that activates `VideoUpscaleService`. Communication is contract-only: no direct package import.
- **`oridecon-tasks`** — `UpscaleTask` is the async job adapter; the multimedia umbrella persists result bytes to oridecon-storage.
- **`oridecon-resilience`** — optional `RetryPolicyProtocol` / `CircuitBreakerProtocol` injection.
- The multimedia umbrella discovers this subsystem via the `oridecon.multimedia.subsystems` entry point (`upscale`).
