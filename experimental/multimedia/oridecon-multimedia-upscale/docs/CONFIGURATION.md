...
Configuration options for `oridecon-multimedia-upscale`.

---

## Overview

Configuration lives on `UpscaleConfig` (a `oridecon.config.BaseConfig` subclass). As a subsystem of the `oridecon-multimedia` umbrella, it is nested under the `multimedia:` key in `application.yaml` with environment-variable prefix `ORI_MULTIMEDIA__UPSCALE__`. Alternatively, pass an `UpscaleConfig` directly to `UpscaleModule.configure(...)`.
...
|----------|-------------|
| `ORI_MULTIMEDIA__UPSCALE__BACKEND` | Override `backend` |
| `ORI_MULTIMEDIA__UPSCALE__REAL_ESRGAN_BASE_URL` | Override `real_esrgan_base_url` |
| `ORI_MULTIMEDIA__UPSCALE__HAT_BASE_URL` | Override `hat_base_url` |
| `ORI_MULTIMEDIA__UPSCALE__TIMEOUT` | Override `timeout` |

```bash
export ORI_PROFILE=production
export ORI_MULTIMEDIA__UPSCALE__BACKEND=hat
export ORI_MULTIMEDIA__UPSCALE__HAT_BASE_URL=http://10.0.0.5:5401
```
...
The umbrella `oridecon-multimedia` orchestrator may also import this subsystem by entry point `oridecon.multimedia.subsystems` (`upscale`) or `oridecon.multimedia.modules` (`upscale`) — no config needed in that case beyond the `multimedia: upscale:` YAML section.

### Optional cross-package wiring

`VideoUpscaleService` only registers when a `VideoProcessor` is resolvable in the container (provided by `oridecon-multimedia-video` when `ffmpeg` is on `PATH`). No upscale-side flag controls this — presence of the processor is the switch.
...
- ✅ Point `*_base_url` at machines that run `oridecon-upscale-*-serve`.
