...
class TestObservabilityConstants:
    ...
    def test_env_prefix(self) -> None:
        ...
        from oridecon.ai.observability.constants import ENV_PREFIX

        assert ENV_PREFIX == "ORI_AI_OBSERVABILITY__"

    def test_env_nested_delimiter(self) -> None:
        """Verify nested delimiter."""
        from oridecon.ai.observability.constants import ENV_NESTED_DELIMITER

        assert ENV_NESTED_DELIMITER == "__"

    def test_default_check_interval(self) -> None:
        """Verify default health check interval."""
        from oridecon.ai.observability.constants import DEFAULT_CHECK_INTERVAL_SECONDS

        assert DEFAULT_CHECK_INTERVAL_SECONDS == 30

    def test_default_check_timeout(self) -> None:
        """Verify default health check timeout."""
        from oridecon.ai.observability.constants import DEFAULT_CHECK_TIMEOUT_SECONDS
...
class TestMetricPrefixes:
    ...
    def test_llm_metric_prefix(self) -> None:
        ...
        from oridecon.ai.observability.constants import METRIC_PREFIX_LLM

        assert METRIC_PREFIX_LLM == "oridecon.ai.llm"

    def test_vector_metric_prefix(self) -> None:
        """Verify vector metric prefix."""
        from oridecon.ai.observability.constants import METRIC_PREFIX_VECTOR

        assert METRIC_PREFIX_VECTOR == "oridecon.ai.vector"

    def test_embedding_metric_prefix(self) -> None:
        """Verify embedding metric prefix."""
        from oridecon.ai.observability.constants import METRIC_PREFIX_EMBEDDING

        assert METRIC_PREFIX_EMBEDDING == "oridecon.ai.embedding"
...
class TestObservabilityConstantsExports:
    ...
    def test_constants_exported(self) -> None:
        ...
        from oridecon.ai.observability import constants
