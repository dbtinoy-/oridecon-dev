"""P2 hook surface import verification for oridecon-ai-observability."""
...
def test_observability_hooks_root_module_exists() -> None:
    ...
    assert (
        oridecon.ai.observability.AIObservabilityStartedHook
        is AIObservabilityStartedHook
    )
    assert oridecon.ai.observability.LLMCallTracedHook is LLMCallTracedHook
    assert oridecon.ai.observability.HealthCheckRunHook is HealthCheckRunHook
