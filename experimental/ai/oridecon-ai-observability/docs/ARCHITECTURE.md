...
Internal design of the `oridecon-ai-observability` package.

---

## Role in the System

`oridecon-ai-observability` provides distributed tracing, metrics collection, and health monitoring for AI operations (LLM calls, vector search, embeddings, RAG, document ingestion). It depends only on `oridecon` and `oridecon-contracts` — it discovers `LLMClientProtocol` and `VectorStoreProtocol` via the container and wraps them transparently.
...
flowchart BT
    ...
    subgraph OBS[oridecon-ai-observability]
...
`AITracer` wraps a `TracerProtocol` (from `oridecon-monitor`) and exposes domain-specific span methods. Each span carries standardised attributes for LLM provider, model, token counts, latency, and cost.
...
`AIMetrics` registers all instruments against `MetricsCollectorProtocol` (from `oridecon-monitor`). 24 instruments total, grouped by domain:
...
Tracing and metrics are exported through `oridecon-monitor`, which provides an `Exporter` abstraction chain. Backends are injected at the monitor layer — `oridecon-ai-observability` never couples to a specific exporter.

| Backend | Tracing | Metrics | Configuration |
|---------|---------|---------|---------------|
| Console | Yes | Yes | `ORI_MONITOR__TRACING_EXPORTERS__0__TYPE=console` |
| OpenTelemetry (OTLP) | Yes | Yes | `ORI_MONITOR__TRACING_EXPORTERS__0__TYPE=otlp` and `ORI_MONITOR__TRACING_EXPORTERS__0__ENDPOINT=...` |
| Datadog (via OTLP) | Yes | Yes | Datadog OTLP endpoint config |
| Prometheus | No | Yes | `ORI_MONITOR__OPENTELEMETRY__METRICS_EXPORTERS='[{"type":"prometheus"}]'` |
...
|----------|--------|-------------|------|
| `AITracerProtocol` | `oridecon.contracts.observability.ai` | `AITracer` | AI tracing API |
| `AIMetricsProtocol` | `oridecon.contracts.observability.ai` | `AIMetrics` | AI metrics API |
| `AIHealthMonitorProtocol` | `oridecon.contracts.observability.ai` | `AIHealthMonitor` | AI health check API |
| `ObservabilityProtocol` | `oridecon.contracts.observability.ai` | — (composite) | Combined observability |
| `TracerProtocol` | `oridecon.contracts.observability.tracing` | `AITracer` (injected) | Span creation & context |
| `SpanProtocol` | `oridecon.contracts.observability.tracing` | `AITracer` (returned) | Span attribute/event API |
| `MetricsCollectorProtocol` | `oridecon.contracts.observability.metrics` | `AIMetrics` (injected) | Instrument creation |
| `LLMClientProtocol` | `oridecon.contracts.ai` | `ObservableLLMClient` (wrapped) | Proxied LLM calls |
| `VectorStoreProtocol` | `oridecon.contracts.data.vector.protocols` | `ObservableVectorStore` (wrapped) | Proxied vector ops |
| `AIAuditStoreProtocol` | `oridecon.contracts.ai.governance` | `ObservableLLMClient` (optional) | Audit event emission |
| `CallbackHandlerProtocol` | `oridecon.contracts.ai.callbacks` | `AITracer`, `CallbackManagerImpl` | Observe callbacks |
...
```python
# oridecon/ai/observability/module.py
@module()
...
```
AIError (contracts)
└── ObservabilityError         # ORI_ERR_OBS_001 — base for this package
    ├── TracingError           # ORI_ERR_OBS_004 — span/trace operations
    ├── MetricsError           # ORI_ERR_OBS_003 — metric recording operations
    └── HealthCheckError       # ORI_ERR_OBS_002 — health check failures
```
...
|--------|-------|-------------|
| `ENV_PREFIX` | `ORI_AI_OBSERVABILITY__` | Env var prefix for config overrides |
| `METRIC_PREFIX_LLM` | `oridecon.ai.llm` | Metric namespace for LLM ops |
| `METRIC_PREFIX_VECTOR` | `oridecon.ai.vector` | Metric namespace for vector ops |
...
`ObservabilityConfig` is loaded from the `ai_observability:` key in `application.yaml` with `ORI_AI_OBSERVABILITY__*` env var overrides:
