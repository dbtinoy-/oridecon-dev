...
Wraps any :class:`~oridecon.contracts.ai.LLMClientProtocol` with tracing and metrics
...
if TYPE_CHECKING:
    ...
    from oridecon.contracts.ai.governance import AIAuditStoreProtocol
    from oridecon.contracts.core import HealthCheckResult
    from oridecon.contracts.core.logging import RedactorProtocol
...
class ObservableLLMClient:
    ...
    :class:`~oridecon.contracts.ai.LLMClientProtocol` protocol while every
    ``complete()`` and ``stream_chat()`` call is:

    - **Traced** via :class:`~oridecon.ai.observability.tracing.AITracer`
    - **Metered** via :class:`~oridecon.ai.observability.metrics.AIMetrics`
    ...
    Example:
        >>> from oridecon.ai.observability.observable_llm import ObservableLLMClient
    ...
    def stream_chat(self, messages: list[Any], **kwargs: Any) -> Any:
        ...
        from oridecon.contracts.infra import AsyncStream
    ...
    def _emit_audit(
        ...
        *,
        ...
        from oridecon.contracts.ai.governance import AIAuditEvent, AuditEventType
