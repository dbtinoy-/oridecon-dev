...
if TYPE_CHECKING:
    ...
    from oridecon.contracts.ai.providers import ModelInfo
    from oridecon.contracts.ai.types import ModelCapability
...
class ProviderRegistry(Registry[str, ProviderInfo]):
    ...
    def _default_entries(cls) -> dict[str, ProviderInfo]:
        ...
        The catalogue lives in :mod:`oridecon.ai.llm.registry.builtins`;
        each client module is imported lazily inside
        :func:`~oridecon.ai.llm.registry.builtins.builtin_provider_entries`
        so SDK imports stay deferred until a registry is populated.
        """
        from oridecon.ai.llm.registry.builtins import builtin_provider_entries
