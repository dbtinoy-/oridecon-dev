"""Cloudflare Workers AI client for the Oridecon LLM routing system.

Implements the :class:`~oridecon.contracts.ai.protocols.LLMClientProtocol` protocol
...
from oridecon.ai.llm.clients._message_utils import serialize_text_only
...
from oridecon.ai.llm.http.client import ResilientHTTPClient
...
class CloudflareWorkersClient(AbstractLLMClient):
    ...
    and normalises the response to a standard :class:`~oridecon.ai.llm.types.Completion`.
...
def _parse_cf_response_with_tools(data: dict[str, Any], model: str) -> Completion:
    ...
    Returns:
        Normalised :class:`~oridecon.ai.llm.types.Completion` with optional
