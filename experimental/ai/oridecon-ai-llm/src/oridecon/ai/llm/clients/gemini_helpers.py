...
Both :class:`~oridecon.ai.llm.clients.gemini.GeminiClient` and
:class:`~oridecon.ai.llm.clients.vertex_ai.VertexAIClient` target the same
...
from oridecon.ai.llm.clients._message_utils import serialize_content_for_gemini
...
from oridecon.serialization import loads as _loads
...
def parse_gemini_response(data: dict[str, Any], model: str) -> Completion:
    ...
    :attr:`~oridecon.ai.llm.types.Completion.thinking` with a
    :class:`~oridecon.contracts.ai.thinking.ThinkingResult` when the model
    ...
    Returns:
        Normalised :class:`~oridecon.ai.llm.types.Completion`.
...
def tool_to_gemini_function(tool: Any) -> dict[str, Any]:
    ...
    (following the Oridecon tool registration convention) as well as plain
...
def _role_str(msg: Any) -> str:
    """Convert a Oridecon role string to a Gemini role string.
