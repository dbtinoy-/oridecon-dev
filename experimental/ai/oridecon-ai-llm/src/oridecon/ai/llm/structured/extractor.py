...
from oridecon.ai.llm.exceptions import (
    ExtractionMaxRetriesError,
    ExtractionParseError,
    ExtractionValidationError,
    LLMError,
)
from oridecon.ai.llm.structured.parser import (
    build_json_schema,
    extract_json_block,
    validate_against_model,
)
from oridecon.contracts.ai.llm import ChatMessage, LLMClientProtocol, Role
from oridecon.logging import (
    get_logger,
)
from oridecon.result import Err, Ok, Result
from oridecon.serialization import JSONDecodeError, dumps_str, loads
...
class JSONExtractor:
    ...
    def extract(text: str, multiple: bool = False) -> Any:
        ...
        from oridecon.ai.llm.extraction._json_scanner import extract_json_objects
        ...
        from oridecon.ai.llm.structured.exceptions import ParseError
...
class StructuredExtractor:
    ...
    The extractor wraps a :class:`~oridecon.contracts.ai.protocols.LLMClientProtocol`
    ...
    are surfaced as :class:`~oridecon.result.Result` — the
...
def _get_content(completion: Any) -> str:
    ...
    Accepts:
    - Objects with a ``.content`` attribute (``oridecon-ai-llm`` Completion).
    - Plain strings.
...
class StructuredOutputExtractor:
    ...
    :func:`~oridecon.ai.llm.structured.parser._extract_first_json_block`;
    ...
    def _extract_json_block(self, text: str) -> str | None:
        ...
        """
        from oridecon.ai.llm.structured.parser import _extract_first_json_block
