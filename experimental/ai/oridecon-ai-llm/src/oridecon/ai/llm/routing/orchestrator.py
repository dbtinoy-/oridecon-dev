...
from oridecon.contracts.ai.models import ModelRequest, ModelResponse
from oridecon.contracts.ai.providers import ModelCapability, SelectionStrategy
...
class LLMOrchestrator:
    ...
    (a :class:`~oridecon.contracts.ai.providers.SelectionStrategy` value or
    ...
    def _select_model(self, request: ModelRequest) -> _ModelSelection | None:
        ...
        to :attr:`~oridecon.contracts.ai.providers.SelectionStrategy.CAPABILITY_MATCH`.
        ...
        3. ``COST_OPTIMAL`` / ``LATENCY_OPTIMAL`` / ``CAPABILITY_MATCH`` / ``PREFERRED``
           ...
           yet available in :class:`~oridecon.ai.llm.registry.core.ProviderRegistry`).
