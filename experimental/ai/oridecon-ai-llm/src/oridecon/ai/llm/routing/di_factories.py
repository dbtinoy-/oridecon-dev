...
:class:`~oridecon.contracts.ai.LLMClientProtocol` instances from a
:class:`~oridecon.ai.llm.routing.config.LLMConfig`.
"""
...
def _build_deepseek_client(p: ProviderConfig) -> LLMClientProtocol:
    from oridecon.ai.llm.clients.openai_compatible import OpenAICompatibleClient
...
def create_routing_clients(config: LLMConfig) -> dict[str, LLMClientProtocol]:
    """Build a provider-keyed dict of :class:`~oridecon.contracts.ai.LLMClientProtocol` instances.
    ...
    Returns:
        ...
        ``name:model``) → :class:`~oridecon.contracts.ai.LLMClientProtocol`.
