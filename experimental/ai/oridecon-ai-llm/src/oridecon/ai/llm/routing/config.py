...
variables following the ``ORI_AI_LLM__`` prefix convention.
"""
...
from oridecon.validation import ConfigDict, Field, SecretStr
...
class QuotaConfig(BaseConfig):
    ...
    cooldown_seconds: int = Field(
        ...
        description=(
            ...
            "Env var: ORI_AI_LLM__QUOTA__COOLDOWN_SECONDS."
        ),
    )
...
class ProviderConfig(BaseConfig):
    ...
    suppress_thinking: bool = Field(
        ...
        description=(
            ...
            "Env var: ``ORI_AI_LLM__PROVIDERS__{NAME}__SUPPRESS_THINKING=true``."
        ),
    )
...
class LLMConfig(BaseConfig):
    ...
    from ``ORI_AI_LLM__`` environment variables.
    ...
    Environment variables (prefix ``ORI_AI_LLM__``)::

        Global:

            ORI_AI_LLM__STRATEGY              sequential | parallel_race |
                                                   cost_optimized | latency_optimized
            ORI_AI_LLM__DEFAULTS__TEMPERATURE float  (default 0.2)
            ORI_AI_LLM__DEFAULTS__MAX_TOKENS  int    (default: provider default)
            ORI_AI_LLM__QUOTA__BACKEND        memory | database  (default memory)
            ORI_AI_LLM__LOG__BACKEND          memory | database  (default memory)
            ORI_AI_LLM__LOG__MAX_ENTRIES      int    (default 1000)

        Per-provider (pattern: ORI_AI_LLM__PROVIDERS__{NAME}__{FIELD}):
        ...
        Azure-specific extras (activated by AZURE__API_KEY + AZURE__BASE_URL):
            ...
            ORI_AI_LLM__PROVIDERS__AZURE__EXTRAS__AZURE_RESOURCE
            ORI_AI_LLM__PROVIDERS__AZURE__EXTRAS__AZURE_DEPLOYMENT
            ORI_AI_LLM__PROVIDERS__AZURE__EXTRAS__AZURE_API_VERSION

        Cloudflare-specific extras (activated by CLOUDFLARE__EXTRAS__CF_ACCOUNT_ID):

            ORI_AI_LLM__PROVIDERS__CLOUDFLARE__EXTRAS__CF_ACCOUNT_ID  <- activates
            ORI_AI_LLM__PROVIDERS__CLOUDFLARE__EXTRAS__CF_API_TOKEN
            ORI_AI_LLM__PROVIDERS__CLOUDFLARE__MODEL

        AWS Bedrock extras (activated by BEDROCK__EXTRAS__AWS_REGION):

            ORI_AI_LLM__PROVIDERS__BEDROCK__EXTRAS__AWS_REGION        <- activates
            ORI_AI_LLM__PROVIDERS__BEDROCK__MODEL
            ORI_AI_LLM__PROVIDERS__BEDROCK__EXTRAS__AWS_ACCESS_KEY_ID
            ORI_AI_LLM__PROVIDERS__BEDROCK__EXTRAS__AWS_SECRET_ACCESS_KEY

        Google Vertex AI extras (activated by VERTEX__EXTRAS__VERTEX_PROJECT):

            ORI_AI_LLM__PROVIDERS__VERTEX__EXTRAS__VERTEX_PROJECT      <- activates
            ORI_AI_LLM__PROVIDERS__VERTEX__EXTRAS__VERTEX_LOCATION
            ORI_AI_LLM__PROVIDERS__VERTEX__MODEL
            ORI_AI_LLM__PROVIDERS__VERTEX__EXTRAS__VERTEX_CREDENTIALS_FILE
    """
    ...
    def from_env(cls) -> LLMConfig:
        """Build a routing config from ``ORI_AI_LLM__`` environment variables.
