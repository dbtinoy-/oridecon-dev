"""LLM Routing Provider for Oridecon Framework dependency injection.
...
if TYPE_CHECKING:
    from oridecon.ai.llm.selection.core import ModelSelector
...
from oridecon.ai.llm.routing.di_factories import create_routing_clients
...
class LLMRoutingProvider(Provider):
    ...
    Builds the :class:`~oridecon.ai.llm.routing.LLMRouter` from a
    :class:`~oridecon.ai.llm.routing.LLMConfig`, chooses the appropriate
    quota backend and inference logger, and registers everything as singletons.

    Configuration is explicit-only: ``LLMConfig`` is built from ``ORI_AI_LLM__``
    environment variables via ``LLMConfig.from_env()`` and is not bound to a
    ``OrideconConfig`` section, so this provider declares no ``config_key``/
