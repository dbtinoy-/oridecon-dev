...
from oridecon.ai.llm.routing.config import LLMConfig, ProviderConfig
...
class TestSuppressThinkingEnvParsing:
    def test_openai_compatible_suppress_thinking_true(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("ORI_AI_LLM__PROVIDERS__OPENAI_COMPATIBLE__BASE_URL", "http://localhost:1234/v1")
        monkeypatch.setenv("ORI_AI_LLM__PROVIDERS__OPENAI_COMPATIBLE__MODEL", "gemma-4")
        monkeypatch.setenv("ORI_AI_LLM__PROVIDERS__OPENAI_COMPATIBLE__SUPPRESS_THINKING", "true")
    ...
    def test_openai_compatible_suppress_thinking_false_by_default(
        ...
        monkeypatch.setenv("ORI_AI_LLM__PROVIDERS__OPENAI_COMPATIBLE__BASE_URL", "http://localhost:1234/v1")
        monkeypatch.setenv("ORI_AI_LLM__PROVIDERS__OPENAI_COMPATIBLE__MODEL", "gemma-4")
        monkeypatch.delenv("ORI_AI_LLM__PROVIDERS__OPENAI_COMPATIBLE__SUPPRESS_THINKING", raising=False)
        config = LLMConfig.from_env()
        openai_compatible = next(p for p in config.providers if p.name == "openai_compatible")
    ...
    def test_ollama_suppress_thinking_true(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("ORI_AI_LLM__PROVIDERS__OLLAMA__BASE_URL", "http://localhost:11434")
        monkeypatch.setenv("ORI_AI_LLM__PROVIDERS__OLLAMA__SUPPRESS_THINKING", "true")
        config = LLMConfig.from_env()
    ...
    def test_key_auth_provider_suppress_thinking(
        ...
        monkeypatch.setenv("ORI_AI_LLM__PROVIDERS__OPENAI__API_KEY", "sk-test")
        monkeypatch.setenv("ORI_AI_LLM__PROVIDERS__OPENAI__SUPPRESS_THINKING", "true")
        config = LLMConfig.from_env()
...
from oridecon.ai.llm.routing.di_factories import _build_openai_compatible_client
from oridecon.contracts.ai.thinking import ThinkingConfig  # noqa: F401
...
class TestDiFactorySuppressThinking:
    def test_openai_compatible_suppress_true_creates_thinking_config(self) -> None:
        ...
        with patch(
            "oridecon.ai.llm.clients.openai_compatible.OpenAICompatibleClient.__init__",
            fake_init,
        ):
