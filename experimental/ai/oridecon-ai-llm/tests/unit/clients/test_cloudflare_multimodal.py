...
from oridecon.ai.llm.clients.cloudflare_workers import (
    _convert_messages_for_cloudflare,
)
from oridecon.ai.llm.types import ChatMessage
from oridecon.contracts.ai.llm import Role
...
class TestCloudflareMultimodalMessageSerialization:
    ...
    def test_string_content_passes_through_unchanged(self) -> None:
        ...
        with patch(
            "oridecon.ai.llm.clients.cloudflare_workers.serialize_text_only",
    ...
    def test_image_url_part_logged_and_skipped(self) -> None:
        ...
        with patch("oridecon.ai.llm.clients.cloudflare_workers.logger", mock_logger):
            result = _convert_messages_for_cloudflare(messages)
            assert len(result) == 1
            assert result[0]["role"] == "user"
