---
title: oridecon-ai-llm Providers
...
`oridecon-ai-llm` provides built-in clients for the following providers:
...
```python
from oridecon.ai.llm.types import StreamChunk
...
```python
from oridecon.ai.llm.types import ToolCall
...
`LLMRoutingProvider` (from `oridecon.ai.llm.di.routing_provider`) enables multi-provider dispatch via `ProviderRegistry`:
