---
title: oridecon-ai-llm Guide
description: Mental model, core concepts, and typical usage of `oridecon-ai-llm`.
---
...
| `oridecon-cache` | Optional | Response caching |
| `oridecon-resilience` | Optional | Retry and rate limiting |

## What problem does it solve?

`oridecon-ai-llm` provides a **unified, async LLM client interface** across multiple providers (OpenAI, Anthropic, Ollama, Groq, Cohere, Mistral, OpenRouter, Gemini, and more). It handles:
...
```python
from oridecon.ai.llm import ConversationManager, ConversationConfig
...
from oridecon.contracts.ai.llm import LLMError
...
```python
from oridecon.ai.llm import JSONExtractor
...
from oridecon.ai.llm.routing import LLMConfig
...
- **Set `api_key` via env var** — use `ORI_AI_LLM__API_KEY` instead of hardcoding.
- **Install only the provider extras you need** — `pip install oridecon-ai-llm[openai]` keeps dependencies minimal.
...
- [Architecture](/packages/oridecon-ai-llm/architecture/) — client classes, caching, model management
- [Configuration](/packages/oridecon-ai-llm/configuration/) — all config keys
- [How-tos](/packages/oridecon-ai-llm/howtos/) — streaming, conversation management, structured output
- [Troubleshooting](/packages/oridecon-ai-llm/troubleshooting/) — common errors and fixes
