...
Internal design of the `oridecon-ai-llm` package — the LLM client abstraction layer.
...
| `LLMCacheProtocol` | `oridecon-ai-llm/protocols.py` | `LLMCache`, `RedisLLMCache` |
...
| `RoutingStrategyProtocol` | `oridecon-ai-llm/routing/strategies/` | 4 strategies |
...
Domain bases in `oridecon-contracts`, leaf exceptions in `oridecon-ai-llm`. Infrastructure errors raise; domain errors return as `Result`.
