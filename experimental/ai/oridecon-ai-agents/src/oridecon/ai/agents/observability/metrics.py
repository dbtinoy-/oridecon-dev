...
Integrates with ``oridecon-monitor`` (if available) to provide:
...
from oridecon.ai.agents.constants import (
...
class AgentMetrics:
    ...
    Delegates to ``MetricsRecorderProtocol`` from ``oridecon-monitor`` if
