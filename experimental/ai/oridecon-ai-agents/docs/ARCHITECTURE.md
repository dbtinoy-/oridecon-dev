...
Internal design of the `oridecon-ai-agents` package.

---

## Role in the System

`oridecon-ai-agents` is the agent execution framework, orchestrating reasoning loops, tool execution, memory, governance, and observability.
...
**Import rule:** Agent code imports from `oridecon` and `oridecon-contracts` only. Cross-package communication goes through protocols resolved via the DI container.
...
**Boundary:** Tools are atomic, stateless, single-purpose. Skills (from `oridecon-ai-skills`) are composed multi-step workflows. Skills may invoke tools. Tools must NOT invoke skills.
...
|----------|-------------|---------|
| `AgentExecutorProtocol` | `oridecon.contracts.ai` | Agent execution entry point |
| `ToolRegistryProtocol` | `oridecon.contracts.ai` | Tool registration and lookup |
| `AgentProtocol` | `oridecon.contracts.ai.agents` | Agent identity and capabilities |
| `StrategyProtocol` | `oridecon.contracts.ai.agents` | Reasoning strategy interface |
| `ToolProtocol` | `oridecon.contracts.ai.agents` | Atomic tool interface |
| `AgentResponse` / `AgentEvent` | `oridecon.contracts.ai.agents` | Result and streaming types |
| `MemoryProtocol` | `oridecon.contracts.ai.agents` | Agent-attached memory |
| `AgentError` / `ToolError` / `StrategyError` | `oridecon.contracts.ai.agents` | Domain error bases |
| `LLMClientProtocol` | `oridecon.contracts.ai.llm` | LLM completion |
| `ChatMessage` / `Role` | `oridecon.contracts.ai.llm` | Conversation types |
| `AIGovernanceProtocol` | `oridecon.contracts.ai.governance` | Budget/rate limits |
| `GuardPipelineProtocol` | `oridecon.contracts.ai.guards` | Input/output guardrails |
| `WorkingMemoryProtocol` | `oridecon.contracts.ai.memory` | Context assembly |
| `SessionManagerProtocol` | `oridecon.contracts.ai.session` | Multi-turn sessions |
| `SkillExecutorProtocol` / `SkillRegistryProtocol` | `oridecon.contracts.ai.skills` | Composable skills |
| `EventBusProtocol` | `oridecon.contracts.events.protocols` | Domain events |
| `MetricsRecorderProtocol` | `oridecon.contracts.observability.metrics` | Prometheus metrics |
| `TracerProtocol` | `oridecon.contracts.observability.tracing` | OpenTelemetry tracing |
...
OrideconError (contracts)
    ...
    │   ├── AgentConfigurationError    — Invalid config       (ORI_ERR_AGT_004)
    │   ├── AgentExecutionError        — Unrecoverable exec   (ORI_ERR_AGT_005)
    │   └── BudgetExceededError        — Governance exceeded  (ORI_ERR_AGT_010)
    ├── ToolError (contracts)
    │   ├── ToolNotFoundError          — Not in registry      (ORI_ERR_AGT_006)
    │   ├── ToolExecutionError         — Runtime failure      (ORI_ERR_AGT_007)
    │   ├── ToolAccessDeniedError      — Visibility blocked   (ORI_ERR_AGT_008)
    │   └── ToolValidationError        — Bad arguments        (ORI_ERR_AGT_011)
    └── StrategyError (contracts)
        └── MaxIterationsExceededError — Iteration limit      (ORI_ERR_AGT_009)
```
...
| Strategy registration | `AgentStrategyRegistry` or `oridecon.agent.strategies` entry-point |
