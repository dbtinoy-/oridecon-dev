...
Protocol-neutral conversion engine for the Oridecon AI relay — OpenAI Chat, Responses, Anthropic, and Gemini

---

## Overview

Protocol-neutral conversion engine for the Oridecon AI relay. Converts between OpenAI Chat Completions, OpenAI Responses, Anthropic Messages, and Gemini generateContent wire formats through one canonical intermediate representation.

The engine is synchronous and side-effect free: it never performs HTTP, channel selection, billing, or model selection. Host capabilities (Claude default `max_tokens`, Gemini safety thresholds, media resolution, model suffixes) are supplied as typed callbacks through `RelayConversionContext` from `oridecon-contracts`.
...
```python
from oridecon.ai.relay import RelayConverterRegistry, convert_request_by_id
from oridecon.contracts.ai.relay import (
    OpenAIChatMessage,
    OpenAIChatRequest,
    RelayFormat,
)
from oridecon.contracts.ai.relay.context import RelayConversionContext
...
|------|-----------------|
| `src/oridecon/ai/relay/module.py` | `RelayModule.configure()` and `RelayModule.stub()` |
| `src/oridecon/ai/relay/engine.py` | `RelayConverterEngine` and `convert_*_via` / `convert_*_by_id` helpers |
| `src/oridecon/ai/relay/context.py` | Nil-safe `ConversionContext` over `RelayConversionContext` |
| `src/oridecon/ai/relay/mappers/` | `FormatMapper` base plus `openai_chat`, `openai_responses`, `claude`, `gemini` |
| `src/oridecon/ai/relay/stream/` | Per-format stream session scaffolds and state |
| `src/oridecon/ai/relay/registry.py` | `RelayConverterRegistry`, `Route`, `RouteSpec` |
| `src/oridecon/ai/relay/quality.py` | Conversion route-quality computation |
| `src/oridecon/ai/relay/media.py` | Data-URI decoding and URL media resolution |
| `src/oridecon/ai/relay/errors.py` | Error translation helpers |