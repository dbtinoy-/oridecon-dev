...
from oridecon.ai.relay.di.provider import RelayProvider
from oridecon.ai.relay.engine import RelayConverterEngine
from oridecon.ai.relay.registry import RelayConverterRegistry
from oridecon.contracts.ai.relay.protocols import (
    RelayConverterProtocol,
    RelayRegistryProtocol,
)
from oridecon.contracts.ai.relay.types import RelayFormat
...
def test_entry_points_resolve() -> None:
    ...
    provider_ep = EntryPoint(
        name="relay",
        value="oridecon.ai.relay.di.provider:RelayProvider",
        group="oridecon.providers",
    )
    ...
    subsystem_ep = EntryPoint(
        name="relay",
        value="oridecon.ai.relay.di.provider:RelayProvider",
        group="oridecon.ai.subsystems",
    )
    assert subsystem_ep.load() is RelayProvider

    module_ep = EntryPoint(
        name="relay",
        value="oridecon.ai.relay.module:RelayModule",
        group="oridecon.ai.modules",
    )
...
async def test_entry_point_discovery_produces_registered_provider(
    ...
    from oridecon.di.provider import Provider as ProviderBase
    ...
    def fake_entry_points(**kwargs: Any) -> Any:
        ...
        if group == "oridecon.ai.subsystems":
            return [entry_point]
        return []
    ...
    discovered = [
        ep.load()
        for ep in metadata.entry_points(group="oridecon.ai.subsystems")
    ]
