---
title: oridecon-ai-skills Guide
...
| `oridecon-ai-llm` | Optional | LLM-powered skills |
| `oridecon-cache` | Optional | Skill result caching |

## Problem

AI agents and applications need to expose composable, discoverable capabilities — call an LLM, query a database, search the web, read a file. Each capability needs parameter validation, permission checks, result caching, timeout enforcement, and retry logic. **oridecon-ai-skills** provides a unified framework for defining, registering, discovering, and executing these capabilities.
...
Every skill implements `SkillProtocol` from `oridecon.contracts.ai.skills`. It has three members:
...
```python
from oridecon.ai.skills import AbstractSkill
...
```python
from oridecon.ai.skills import SkillRegistry
...
```python
from oridecon.ai.skills import SkillExecutor, SkillRegistry
from oridecon.ai.skills import PermissionChecker
...
```python
from oridecon.ai.skills import SkillChain
...
```python
from oridecon.ai.skills import SkillResultCache
...
- [Architecture](/packages/oridecon-ai-skills/architecture/) — provider lifecycle, module mapping, extension points
- [Configuration](/packages/oridecon-ai-skills/configuration/) — full config reference
- [How-tos](/packages/oridecon-ai-skills/howtos/) — task-oriented recipes
- [Troubleshooting](/packages/oridecon-ai-skills/troubleshooting/) — common errors and fixes
