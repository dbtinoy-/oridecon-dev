...
Internal design of the `oridecon-ai-guard` package.
...
```mermaid
sequenceDiagram
    participant App as OrideconApplication
...
The package re-exports its protocols from `oridecon.contracts.ai.guards` for convenience at `oridecon.ai.guard.protocols`:
...
- `GuardConfig` — `BaseConfig` subclass with env-prefix `ORI_AI_GUARD__`
...
| Configuration override | `GuardConfig` fields + environment variables (`ORI_AI_GUARD__*`) |
