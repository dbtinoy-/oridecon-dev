...
class GovernancePersistenceError(GovernanceError):
    ...
    Raised by
    :class:`~oridecon.ai.governance.persistence.RedisGovernancePersistence`
    when the cache backend reports failure (an ``Err`` result).  The
    :class:`~oridecon.ai.governance.manager.AIGovernanceManager` catches this at
