...
- :class:`RedisGovernancePersistence` — distributed state backed by a
  :class:`~oridecon.contracts.cache.CacheBackendProtocol`, enabling multi-replica
  consistency for rate limiting and budget enforcement.
- :class:`DatabaseGovernancePersistence` — SQL-backed state via a
  :class:`~oridecon.contracts.data.DatabaseProviderProtocol` for deployments
  where Redis is unavailable.

The :class:`~oridecon.ai.governance.manager.AIGovernanceManager` accepts any
...
from oridecon.ai.governance.persistence.database import (
    DatabaseGovernancePersistence as DatabaseGovernancePersistence,
)
from oridecon.ai.governance.persistence.in_memory import (
    InMemoryGovernancePersistence as InMemoryGovernancePersistence,
)
from oridecon.ai.governance.persistence.redis_backend import (
