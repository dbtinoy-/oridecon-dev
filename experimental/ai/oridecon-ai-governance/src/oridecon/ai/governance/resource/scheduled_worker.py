"""Periodic worker base class for governance background loops.

Governance-side implementation of the shared scheduled-worker pattern.  The
scheduling loop runs against
:class:`~oridecon.contracts.infra.tasks.TaskManagerProtocol` (resolved from
the container) so that ``oridecon.ai.governance`` never imports
``oridecon.tasks`` directly.
"""
...
from oridecon.contracts.infra.tasks import OnErrorPolicy
from oridecon.logging import get_logger

if TYPE_CHECKING:
    from oridecon.contracts.infra.tasks import TaskManagerProtocol

logger = get_logger(__name__)


class GovernanceScheduledWorker(ABC):
    """Periodic worker base class for governance services.
    ...
    :class:`~oridecon.contracts.infra.tasks.TaskManagerProtocol`.

    Class-level defaults (override per subclass or via constructor)::

        class MyWorker(GovernanceScheduledWorker):
            interval_seconds = 60.0
            initial_delay_seconds = 0.0
            max_jitter_seconds = 0.0
            on_error_policy = OnErrorPolicy.LOG_AND_CONTINUE

    The concrete task manager (e.g. ``BackgroundTaskManager`` from
    ``oridecon.tasks``) is supplied by the application that wires both
    packages together — never imported here.
    """
...
__all__ = ["GovernanceScheduledWorker"]
