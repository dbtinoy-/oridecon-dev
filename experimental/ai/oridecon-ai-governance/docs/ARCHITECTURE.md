...
Internal design of the `oridecon-ai-governance` package.

---

## Role in the System

`oridecon-ai-governance` provides AI governance policies that gate which LLM requests are allowed, denied, or throttled based on budget, rate, model, content, and data-classification rules.
...
**Import direction:** Application code depends on `AIGovernanceProtocol` from `oridecon-contracts`. The governance package implements that contract. No other governance package imports from the application layer.
...
|----------------|--------|------|
| `AIGovernanceProtocol` | `oridecon.contracts.ai.governance` | Primary service contract for `check_request`, `check_budget`, `track_cost`, `reload_config` |
| `CostTrackingProtocol` | `oridecon.contracts.ai.governance` | Cost recording (`track_cost`) |
| `AIAuditStoreProtocol` | `oridecon.contracts.ai.governance` | Audit event persistence (`record`, `query`, `aggregate`) |
| `GovernanceError` | `oridecon.contracts.ai.governance` | Base domain exception |
| `BudgetExceededError` | `oridecon.contracts.ai.governance` | Monthly spend cap breached |
| `CacheBackendProtocol` | `oridecon.contracts.infra.cache` | Distributed counter/spend storage (optional) |
| `DatabaseProviderProtocol` | `oridecon.contracts.data` | SQL-backed persistence (optional) |
| `EventBusProtocol` | `oridecon.contracts.events` | Budget alert emission (optional) |
| `ContainerRegistrarProtocol` | `oridecon.contracts.core.di` | Provider registration |
| `ContainerResolverProtocol` | `oridecon.contracts.core.di` | Provider boot-time resolution |
