...
from oridecon.contracts.admin import PageContent
from oridecon.contracts.admin.types import NavigationContribution, WidgetParams
...
from oridecon.contracts.ai.relay import RelayUsage
...
class TestContributorDiscovery:
    def test_entry_point_registered(self) -> None:
        ...
        matches = [
            ep
            for ep in entry_points(group="oridecon.admin.contributors")
    ...
    def test_get_routes_returns_empty_default(self) -> None:
        """Routing is handled by oridecon-admin's WidgetController, not per-contributor."""
        contributor = GovernanceAdminContributor()
...
class TestAdminBoot:
    ...
    async def test_on_admin_boot_raises_if_an_action_handler_cannot_be_resolved(
        ...
        from oridecon.ai.governance.admin import contributor as contributor_module

        bad_actions = (
            replace(
                contributor_module._ACTIONS[0],
                handler="oridecon.ai.governance.admin.ledger_actions:does_not_exist",
            ),
        )
