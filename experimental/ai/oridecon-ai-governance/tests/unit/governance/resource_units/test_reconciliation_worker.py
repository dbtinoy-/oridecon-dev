...
the boundary rule: the module must never import ``oridecon.tasks``.
"""
...
from oridecon.ai.governance.persistence import InMemoryGovernancePersistence
from oridecon.ai.governance.resource.reconciliation import (
    GaugeReconciliationWorker,
)
from oridecon.ai.governance.resource.registry import ResourceUnitRegistry
from oridecon.ai.governance.resource.tracker import ResourceUnitTracker
...
def test_reconciliation_module_imports_no_oridecon_tasks() -> None:
    """Importing the worker must not pull ``oridecon.tasks`` into a fresh process.

    Guards the inviolable rule that ``oridecon.ai.governance`` never imports
    ``oridecon.tasks`` (spec 2026-08-18-architecture-ai-governance-tasks-import).
    """
    code = (
        "import importlib, sys\n"
        "importlib.import_module('oridecon.ai.governance.resource.reconciliation')\n"
        "assert 'oridecon.tasks' not in sys.modules, 'oridecon.tasks imported'\n"
    )
