"""P3 hook surface import verification for oridecon-ai-governance."""
...
def test_governance_hooks_root_module_exists() -> None:
    ...
    assert oridecon.ai.governance.GovernanceAuditRecordedHook is GovernanceAuditRecordedHook
    assert (
        oridecon.ai.governance.GovernancePolicyEvaluatedHook
        is GovernancePolicyEvaluatedHook
    )
    assert (
        oridecon.ai.governance.GovernancePersistenceWrittenHook
        is GovernancePersistenceWrittenHook
    )
    assert "GovernanceAuditRecordedHook" in oridecon.ai.governance.__all__
    assert "GovernancePolicyEvaluatedHook" in oridecon.ai.governance.__all__
    assert "GovernancePersistenceWrittenHook" in oridecon.ai.governance.__all__
