...
class RAGProvider(Provider):
    ...
    async def register(self, container: ContainerRegistrarProtocol) -> None:
        ...
        from oridecon.ai.rag.context_compression.strategy_registry import (
        ...
        if _llmlingua_available():
            from oridecon.ai.rag.context_compression.strategies.llmlingua2 import (
        ...
        from oridecon.ai.rag.hyde.strategy_registry import HyDEStrategyRegistry

        hyde_registry = HyDEStrategyRegistry.with_defaults()
        container.singleton(HyDEStrategyRegistry, instance=hyde_registry)

        from oridecon.ai.rag.reasoning.strategy_registry import (
        ...
        from oridecon.ai.rag.pipeline.stages.synthesis_registry import (
        ...
        from oridecon.ai.rag.chunking.strategy_registry import (
        ...
        if _flashrank_available():
            from oridecon.ai.rag.reranking.strategies.flashrank import (
    ...
    async def _register_knowledge_graph(
        ...
        from oridecon.ai.rag.knowledge_graph.core import KnowledgeGraph
    ...
    async def _discover_strategies(self, container: ContainerRegistrarProtocol) -> None:
        ...
        for group in (
            "oridecon.chunking.strategies",
            "oridecon.retrieval.strategies",
        ):
    ...
    async def _maybe_wrap_with_tenancy(
        ...
        from oridecon.ai.rag.tenancy.resolver import (
            TemplatedTenantCollectionResolver,
        )
        from oridecon.contracts.ai.rag import RAGPipelineProtocol
        from oridecon.primitives.context import Context
        ...
        async def _default_factory(config: RAGConfig) -> RAGPipelineProtocol:
            ...
            from oridecon.ai.rag.pipeline._stage_factory import (
    ...
    async def health_check(self, timeout: float = 5.0) -> HealthCheckResult:
        ...
        Returns:
            :class:`~oridecon.contracts.core.HealthCheckResult` with status
        ...
        try:
            from oridecon.contracts.ai import EmbeddingClientProtocol
        ...
        try:
            from oridecon.contracts.ai import DocumentVectorStoreProtocol
