...
from oridecon.ai.rag.exceptions import MissingCitationsError
from oridecon.ai.rag.pipeline._stage_factory import build_pipeline_stages
from oridecon.ai.rag.pipeline.base import PipelineStageProtocol
from oridecon.ai.rag.pipeline.builder_config import PipelineConfigWiring
from oridecon.ai.rag.pipeline.executor import PipelineExecutor
from oridecon.ai.rag.pipeline.types import ErrorStrategy
from oridecon.contracts.ai.exceptions import RAGError
from oridecon.contracts.ai.rag import RAGContext, RAGEvaluatorProtocol, RAGResponse
...
if TYPE_CHECKING:
    from oridecon.ai.rag.pipeline.types import PipelineContext
    from oridecon.contracts.ai.memory import WorkingMemoryProtocol

from oridecon.primitives.builder import AbstractBuilder
    ...
    def __init__(
        ...
        Args:
            ...
            evaluator: Optional evaluator implementing
                :class:`~oridecon.contracts.ai.rag.RAGEvaluatorProtocol` for
                automatic per-request quality evaluation.  Evaluation
                frequency is controlled by :attr:`~oridecon.ai.rag.config.PipelineConfig.auto_evaluate_every_n`.
...
class PipelineBuilder(PipelineConfigWiring, AbstractBuilder[RAGPipeline]):
    ...
    :class:`~oridecon.ai.rag.pipeline.builder_config.PipelineConfigWiring`;
