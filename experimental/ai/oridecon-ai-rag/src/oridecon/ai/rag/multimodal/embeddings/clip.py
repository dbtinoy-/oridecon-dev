...
from oridecon.ai.rag.deps import MissingOptionalDependencyError, ensure_packages
...
from oridecon.ai.rag.exceptions import CLIPEmbeddingError
from oridecon.ai.rag.multimodal.types import ImageDocument
        ...
        if not CLIP_AVAILABLE:
            try:
                _ = ensure_packages(
                    ...
                    hint="pip install 'oridecon-ai[llm]' or 'oridecon-ai[all]'",
                )
            except MissingOptionalDependencyError as e:
                msg = (
                    "CLIP embeddings require transformers, torch, and pillow. "
                    "Install with: pip install 'oridecon-ai[llm]' or 'oridecon-ai[all]'."
                )
            ...
            torch = sys.modules.get("oridecon.ai.rag.multimodal.embeddings.clip").torch  # type: ignore[union-attr]
