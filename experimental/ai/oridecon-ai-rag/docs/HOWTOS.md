...
```python
from oridecon.ai.rag.chunking import create_chunker
...
```python
from oridecon.ai.rag.exceptions import RetrievalError, SynthesisError
...
Create a provider that implements `RetrievalStrategyProtocol` and register it under the `oridecon.retrieval.strategies` entry-point group:

```python
# my_package/strategies.py
from oridecon.contracts.ai.rag import RetrievalStrategyProtocol
...
```toml
# pyproject.toml
[project.entry-points."oridecon.retrieval.strategies"]
