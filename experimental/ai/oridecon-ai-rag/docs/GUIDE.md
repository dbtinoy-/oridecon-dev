...
| `oridecon-ai-llm` | Optional | LLM-based synthesis |
| `oridecon-vector` | Optional | Vector store integration |

`oridecon-ai-rag` provides a modular, async RAG pipeline that retrieves relevant context from a knowledge base and synthesises it into grounded answers with citations.
...
```python
result = await pipeline.execute(RAGContext(query="What is Oridecon?"))
if result.is_ok():
...
from oridecon.ai.rag import RAGModule, RAGConfig, PipelineConfig
...
`oridecon-ai-rag` communicates with other packages through contracts in `oridecon-contracts`:
...
| `SynthesizerProtocol` | LLM-based answer generation | `oridecon-ai-llm` (optional) |
| `WorkingMemoryProtocol` | Conversation memory context | `oridecon-ai-memory` (optional) |
| `GraphStoreProtocol` | Knowledge graph enhancement | `oridecon-graph` (optional) |
