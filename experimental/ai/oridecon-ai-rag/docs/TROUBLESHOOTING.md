---
title: oridecon-ai-rag Troubleshooting
...
**Exception:** `oridecon.contracts.ai.rag.RetrievalError`
...
**Exception:** `oridecon.contracts.ai.rag.SynthesisError`
...
**Exception:** `oridecon.ai.rag.exceptions.MissingCitationsError`
...
**Exception:** `oridecon.ai.rag.exceptions.PreprocessingError`
...
- For PDFs, install `oridecon-ai-rag[pdf]`.
- For web pages, install `oridecon-ai-rag[web]`.
...
**Fix:**
```bash
uv add "oridecon-ai-rag[compression,reranking]"
```
...
**Fix:**
```python
from oridecon.ai.rag.config import QualityAssuranceConfig
...
```python
from oridecon.ai.rag.loaders.core import AbstractDocumentLoader
