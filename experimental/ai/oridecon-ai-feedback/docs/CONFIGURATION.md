---
title: oridecon-ai-feedback Configuration
...
|-----|------|---------|-------------|-------------|
| `enabled` | `bool` | `true` | `ORI_AI_FEEDBACK__ENABLED` | Master on/off switch for all feedback collection |
| `async_processing` | `bool` | `true` | `ORI_AI_FEEDBACK__ASYNC_PROCESSING` | Process feedback handlers asynchronously in the background |
| `store_raw_payloads` | `bool` | `false` | `ORI_AI_FEEDBACK__STORE_RAW_PAYLOADS` | Persist raw incoming feedback payloads for auditing |
...
export ORI_AI_FEEDBACK__ASYNC_PROCESSING=true
export ORI_AI_FEEDBACK__STORE_RAW_PAYLOADS=true
```
