...
Internal design of the `oridecon-ai-feedback` package.

---

## Role in the System

`oridecon-ai-feedback` is the **feedback collection layer** for AI-generated outputs — data types, storage backends, and services for collecting, processing, and querying user feedback on AI responses.

```mermaid
flowchart BT
    Contracts[oridecon-contracts<br/>FeedbackProtocol · FeedbackStoreProtocol<br/>FeedbackType · FeedbackItem · FeedbackSummary]
    Core[oridecon<br/>DI · Config · Result · Logging]
    subgraph Feedback[oridecon-ai-feedback]
...
Depends only on `oridecon` and `oridecon-contracts`. Storage backends wired dynamically in `boot()` via optional container resolution.
...
|----------|-------------|----------------|
| `FeedbackProtocol` | `oridecon.contracts.ai.feedback` | `FeedbackService` |
| `FeedbackStoreProtocol` | `oridecon.contracts.ai.feedback` | `DatabaseFeedbackStore`, `CachedFeedbackStore` |
| `FeedbackType` | `oridecon.contracts.ai.feedback` | `StrEnum` |
| `FeedbackItem` | `oridecon.contracts.ai.feedback` | Frozen dataclass |
| `FeedbackSummary` | `oridecon.contracts.ai.feedback` | Frozen dataclass |
| `DatabaseProviderProtocol` | `oridecon.contracts.data` | *consumed* |
| `CacheBackendProtocol` | `oridecon.contracts.infra.cache` | *consumed* |
| `DomainEvent` | `oridecon.contracts.domain.events` | `FeedbackSubmittedEvent` |
