...
Internal design of the `oridecon-ai-memory` package.

---

## Role in the System

`oridecon-ai-memory` provides the memory layer for AI agents. It sits between the LLM invocation layer and the persistence layer, managing what an agent remembers across turns and sessions.

```mermaid
flowchart BT
    LLM[LLM Invocation<br/>oridecon-ai-llm]
    Agents[Agent Execution<br/>oridecon-ai-agents]
    Memory[Memory Layer<br/>oridecon-ai-memory]
    Backends[Persistence Backends<br/>In-Memory · Redis · SQL · Vector Store]
    Events[Event Bus<br/>oridecon-events]
...
Procedural memory is not implemented as a separate store. Skills and tool definitions are registered via `oridecon-ai-agents` and `oridecon-ai-skills`. The memory layer reserves this concept as an extension point for future learned-procedure storage.

---

## Store Abstraction

All memory backends implement `MemoryStoreProtocol` from `oridecon-contracts`. This protocol defines the full CRUD interface for memory entries:
...
|----------|-------------|---------|
| `MemoryEntry` | `oridecon.contracts.ai.memory` | Core data unit with importance, embedding, metadata |
| `MemoryQuery` | `oridecon.contracts.ai.memory` | Query parameters with configurable weightings |
| `MemorySearchResult` | `oridecon.contracts.ai.memory` | Result with score and source attribution |
| `ConsolidationResult` | `oridecon.contracts.ai.memory` | Consolidation statistics |
| `MemoryStoreProtocol` | `oridecon.contracts.ai.memory` | Storage CRUD for memory entries |
| `EpisodicMemoryProtocol` | `oridecon.contracts.ai.memory` | Temporal event record/recall |
| `SemanticMemoryProtocol` | `oridecon.contracts.ai.memory` | Fact storage and query |
| `WorkingMemoryProtocol` | `oridecon.contracts.ai.memory` | Token-budgeted context assembly |
| `MemoryConsolidatorProtocol` | `oridecon.contracts.ai.memory` | Consolidation pipeline |
| `TokenCounterProtocol` | `oridecon.contracts.ai.llm` | Token counting for budget allocation |
| `CacheBackendProtocol` | `oridecon.contracts.infra.cache` | Cache-backed store (optional) |
| `DatabaseProviderProtocol` | `oridecon.contracts.data` | SQL-backed store (optional) |
| `DocumentVectorStoreProtocol` | `oridecon.contracts.ai.vector` | Vector-backed store (optional) |
| `HealthCheckResult` | `oridecon.contracts.core` | Health status for all services |
...
```
OrideconError → AIError → AIMemoryError (contracts)
...
`ConsolidationError` and `StorageError` live in `oridecon-contracts` (they appear in protocol method signatures).
...
Environment variable prefix: `ORI_AI_MEMORY__`
