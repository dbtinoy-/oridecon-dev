...
import pytest
from oridecon.validation import ValidationError  # noqa: F401 (kept for future use)

from oridecon.ai.memory.config import MemoryConfig, WorkingMemoryConfig


class TestMemoryConfig:
    def test_memory_config_model_config_has_env_prefix(self) -> None:
        mc = MemoryConfig.model_config
        assert mc.get("env_prefix") == "ORI_AI_MEMORY__"
