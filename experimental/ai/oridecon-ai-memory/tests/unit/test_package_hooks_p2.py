"""P2 hook surface import verification for oridecon-ai-memory."""
...
def test_memory_hooks_root_module_exists() -> None:
    ...
    assert oridecon.ai.memory.MemoryWrittenHook is MemoryWrittenHook
    assert oridecon.ai.memory.MemoryRetrievedHook is MemoryRetrievedHook
    assert oridecon.ai.memory.MemoryConsolidatedHook is MemoryConsolidatedHook
