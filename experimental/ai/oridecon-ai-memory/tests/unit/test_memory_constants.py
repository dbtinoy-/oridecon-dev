...
class TestConstantsExports:
    ...
    def test_exports_all_constants(self) -> None:
        from oridecon.ai.memory.constants import __all__
...
class TestEnvironmentConstants:
    ...
    def test_env_prefix(self) -> None:
        from oridecon.ai.memory.constants import ENV_PREFIX
        assert isinstance(ENV_PREFIX, str)
        assert ENV_PREFIX.startswith("ORI_")

    def test_nested_delimiter(self) -> None:
        from oridecon.ai.memory.constants import ENV_NESTED_DELIMITER
...
class TestWorkingMemoryDefaults:
    ...
    def test_default_system_prompt_tokens(self) -> None:
        from oridecon.ai.memory.constants import DEFAULT_SYSTEM_PROMPT_TOKENS
...
class TestSemanticMemoryDefaults:
    ...
    def test_default_min_confidence(self) -> None:
        from oridecon.ai.memory.constants import DEFAULT_MIN_CONFIDENCE
        assert 0.0 <= DEFAULT_MIN_CONFIDENCE <= 1.0

    def test_default_max_facts_per_entity(self) -> None:
        from oridecon.ai.memory.constants import DEFAULT_MAX_FACTS_PER_ENTITY
...
class TestConsolidationDefaults:
    ...
    def test_default_consolidation_interval_s(self) -> None:
        from oridecon.ai.memory.constants import DEFAULT_CONSOLIDATION_INTERVAL_S
    ...
    def test_default_consolidation_age_threshold_hours(self) -> None:
        from oridecon.ai.memory.constants import DEFAULT_CONSOLIDATION_AGE_THRESHOLD_HOURS
    ...
    def test_default_consolidation_importance_prune(self) -> None:
        from oridecon.ai.memory.constants import DEFAULT_CONSOLIDATION_IMPORTANCE_PRUNE
        assert 0.0 <= DEFAULT_CONSOLIDATION_IMPORTANCE_PRUNE <= 1.0

    def test_default_consolidation_batch_size(self) -> None:
        from oridecon.ai.memory.constants import DEFAULT_CONSOLIDATION_BATCH_SIZE
...
class TestRootConfigDefaults:
    ...
    def test_default_backend(self) -> None:
        from oridecon.ai.memory.constants import DEFAULT_BACKEND
        assert isinstance(DEFAULT_BACKEND, str)
        assert len(DEFAULT_BACKEND) > 0

    def test_default_ttl_seconds(self) -> None:
        from oridecon.ai.memory.constants import DEFAULT_TTL_SECONDS
...
class TestVersionConstant:
    ...
    def test_version_exists(self) -> None:
        from oridecon.ai.memory.constants import __version__
