"""P3 hook surface import verification for oridecon-ai-prompt."""
...
def test_prompt_hooks_root_module_exists() -> None:
    ...
    assert (
        oridecon.ai.prompt.PromptTemplateResolvedHook is PromptTemplateResolvedHook
    )
    assert oridecon.ai.prompt.PromptRenderedHook is PromptRenderedHook
    assert oridecon.ai.prompt.PromptInputSanitizedHook is PromptInputSanitizedHook
