...
from oridecon.ai.prompt.variables.validators import resolve_variables
...
class StringPromptTemplate(AbstractPromptTemplate):
    ...
    Supports all :class:`~oridecon.ai.prompt.rendering.engine.RenderFormat`
    modes (f-string by default).

    Args:
        name: Unique template name.
        template: Raw template string, e.g. ``"Hello, {name}!"``.
        variables: Declared :class:`~oridecon.ai.prompt.variables.types.PromptVariable`
                   objects.  Any undeclared variable is passed through as-is.
        format: Rendering format.  Defaults to
                :attr:`~oridecon.ai.prompt.rendering.engine.RenderFormat.F_STRING`.
