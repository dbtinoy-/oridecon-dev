...
class TestGetSubsystemConfig:
    """Tests for oridecon.ai.config.get_subsystem_config."""

    def test_returns_known_field_when_set(self) -> None:
        from oridecon.ai.config import AIConfig, get_subsystem_config

        try:
            from oridecon.ai.config import ClientConfig as LLMConfig
        except ImportError:
            pytest.skip("oridecon-ai-llm not installed")
...
class TestAIConfig:
    ...
    def test_production_security_validator_blocks_insecure_key(self, monkeypatch) -> None:
        from oridecon.ai.config import AIConfig
        try:
            from oridecon.ai.llm.config import ClientConfig as LLMConfig
            from pydantic import SecretStr
        except ImportError:
            pytest.skip("oridecon-ai-llm or pydantic not installed")
