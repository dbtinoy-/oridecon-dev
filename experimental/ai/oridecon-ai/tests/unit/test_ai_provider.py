"""Tests for oridecon.ai.provider module (sub-provider composition)."""

from unittest.mock import AsyncMock, MagicMock, patch

import pytest

from oridecon.ai.config import (
    AIConfig,
    GovernanceConfig,
    VectorConfig,
)
...
class TestAIProviderRegister:
    ...
    async def test_register_delegates_to_llm_provider(self, mock_container):
        ...
        with patch("importlib.metadata.entry_points", return_value=[]), \
             patch("oridecon.ai.llm.di.provider.LLMProvider") as MockLLM:
    ...
    async def test_register_delegates_to_vector_provider(self, mock_container):
        ...
        with patch("importlib.metadata.entry_points", return_value=[]), \
             patch("oridecon.vector.di.provider.VectorProvider") as MockVector:
    ...
    async def test_register_delegates_to_rag_provider(self, mock_container):
        ...
        with patch("importlib.metadata.entry_points", return_value=[]), \
             patch("oridecon.ai.rag.di.provider.RAGProvider") as MockRAG:
...
class TestFactoriesDirect:
    """Tests for factory functions in oridecon.ai.di.factories."""
    ...
    async def test_create_vector_store_without_infra_store_returns_mock(self):
        ...
        from oridecon.vector.testing.mocks import MockVectorStore
