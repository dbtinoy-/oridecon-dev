...
def build_gateway_app(config: Any, host: str = "127.0.0.1") -> Any:
    ...
    ``oridecon.ai.modules`` and ``oridecon.modules`` entry-point groups;
    ...
    relay_module = _load_module("oridecon.ai.modules", "relay")
    gateway_module = _load_module("oridecon.ai.modules", "relay-gateway")
    http_module = _load_module("oridecon.modules", "http")
    web_module = _load_module("oridecon.modules", "web")

    app = Application(name="oridecon-relay-gateway")
    app.add_modules(
        [
...
def serve_gateway(
    ...
    The server runner is loaded from the ``oridecon.servers`` entry-point
    group (name ``"web"``, provided by ``oridecon-web``) — never imported
    ...
    Raises:
        ModuleNotFoundError: When no ``oridecon.servers`` runner named
            ``"web"`` is installed.
    """
    run_server = _load_module("oridecon.servers", "web")
