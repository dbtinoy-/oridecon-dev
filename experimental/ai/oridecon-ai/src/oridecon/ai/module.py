...
class AIModule(Module):
    ...
    :class:`~oridecon.ai.llm.LLMModule` and
    :class:`~oridecon.ai.rag.RAGModule` may be imported independently
    ...
    def configure(cls, config: Any | None = None, **kwargs: Any) -> DynamicModule:
        ...
        Args:
            config: :class:`~oridecon.ai.config.AIConfig` or ``None``
                for framework defaults.
            **kwargs: Additional keyword arguments forwarded to
                :class:`~oridecon.ai.di.provider.AIProvider`.
    ...
    def stub(cls, config: Any = None) -> DynamicModule:
        ...
        for ep in entry_points(group="oridecon.ai.modules"):
            try:
                module_cls = ep.load()
