---
title: oridecon-ai Troubleshooting
description: Common errors with `oridecon-ai`, their causes, and fixes.
---

## Missing sub-package

```
ImportError: No module named 'oridecon.ai.llm'
```

**Cause:** `oridecon-ai-llm` is not installed.
...
**Cause:** The `api_key` field contains a placeholder value (`sk-...`, `sk-test`, `change-me`) and `ORI_ENV=production`.

**Fix:** Set a valid API key via `ORI_AI_LLM__API_KEY` environment variable.
...
**Fix:** Check the debug log message `"Registered AI subsystem via entry-point"` to confirm discovery. Verify the entry-point group is `oridecon.ai.subsystems` in the package's `pyproject.toml`.
...
**Fix:** Ensure the client class satisfies `LLMClientProtocol` from `oridecon.contracts.ai.llm`.
...
**Symptom:** A `oridecon-ai-*` package is installed but its services are not registered. No `Registered AI subsystem via entry-point` log message appears.

**Cause:** The package's `pyproject.toml` declares the entry point under `[project.entry-points."oridecon.ai.subsystems"]` but the package is not importable, or `importlib.metadata` raised an `ImportError`.

**Fix:** Verify the entry point is correctly declared:

```toml
[project.entry-points."oridecon.ai.subsystems"]
fine_tuning = "oridecon_ai_fine_tuning.di.provider:FineTuningProvider"
```

Confirm the provider module is importable:

```bash
uv run python -c "from oridecon_ai_fine_tuning.di.provider import FineTuningProvider; print('ok')"
```
