"""
╔══════════════════════════════════════════════════════════╗
║  Oridecon — The Python framework that gives your        ║
║  AI a pattern to follow.                                ║
╚══════════════════════════════════════════════════════════╝

This demo shows the progression from raw API calls to
the full power of Oridecon's contract-based, DI-driven
AI framework.
"""
...
# Suppress framework logging BEFORE any Oridecon imports
import structlog
...
os.environ.setdefault("ORI_LOGGING__LEVEL", "ERROR")
os.environ.setdefault("ORI_QUIET", "1")
...
async def stage3_structured():
    from oridecon import Application
    from oridecon.ai.llm import ClientConfig, LLMModule
    from oridecon.ai.llm.structured import build_json_schema, extract_json_block
    from oridecon.contracts.ai import ChatMessage, LLMClientProtocol, Role
...
async def stage4_streaming():
    from oridecon import Application
    from oridecon.ai.llm import ClientConfig, LLMModule
    from oridecon.ai.llm.thinking import normalize_thinking_text
    from oridecon.contracts.ai import ChatMessage, LLMClientProtocol, Role
...
async def search_knowledge(topic: str) -> str:
    """Search the Oridecon knowledge base for framework information."""
...
# via the Oridecon framework. No mocks, no simulated output.
# ═══════════════════════════════════════════════════════════
...
async def main():
    ...
    print("  ── Example 2: After ── same call through Oridecon DI")
    print("  ────  contracts, Result types, provider-agnostic")
    await stage2_oridecon_llm()
    print()
    print("  ----------------")
    print()
    ...
    print("  ────  streaming with thinking normalization via Oridecon")
