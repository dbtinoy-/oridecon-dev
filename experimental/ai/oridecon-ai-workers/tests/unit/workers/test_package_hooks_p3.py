"""P3 hook surface import verification for oridecon-ai-workers."""
...
def test_workers_hooks_root_module_exists() -> None:
    ...
    assert oridecon.ai.workers.WorkerJobStartedHook is WorkerJobStartedHook
    assert oridecon.ai.workers.WorkerJobCompletedHook is WorkerJobCompletedHook
    assert (
        oridecon.ai.workers.WorkerMaintenanceRunHook is WorkerMaintenanceRunHook
    )
