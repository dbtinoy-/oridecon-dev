...
Internal design of the `oridecon-ai-workers` package.

---

## Role in the System

`oridecon-ai-workers` provides a concurrent worker pool for AI task execution — batch embedding generation, document ingestion, scheduled maintenance, and dead-letter-queue recovery. It sits in the AI subsystem, depending only on `oridecon` and `oridecon-contracts`.
...
flowchart LR
    ...
    subgraph Workers[oridecon-ai-workers]
        WP[WorkersProvider]
...
The package consumes protocols from `oridecon-contracts` and defines one local protocol:
...
| `AIError` | `contracts/ai/errors.py` | Exception hierarchy base (`WorkerError < AIError < OrideconError`) |
...
|-----------|---------|------|
| `WorkerError` | `AIError` | `ORI_ERR_AIWORK_001` |
| `DLQError` | `WorkerError` | `ORI_ERR_AIWORK_002` |
| `MaintenanceError` | `WorkerError` | `ORI_ERR_AIWORK_003` |
...
| Custom queue backend | Implement `TaskQueueProtocol` from `oridecon-contracts` |
