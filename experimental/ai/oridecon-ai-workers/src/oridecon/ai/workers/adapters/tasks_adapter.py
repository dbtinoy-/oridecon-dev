"""Adapter for oridecon-tasks integration (OPTIONAL).

Bridges ``oridecon-ai`` workers with any task queue backend that satisfies
``oridecon.contracts.tasks.TaskQueueProtocol`` / ``oridecon.contracts.tasks.JobProtocol``.

Both ``JobAdapter`` and ``OrideconTasksAdapter`` work with *any* conforming
implementation — including ``oridecon.tasks`` — without importing from that
package directly.  This avoids a cross-extension import while still providing
the adapters for backwards compatibility.

Example::

    q = OrideconTasksAdapter(container.resolve(TaskQueueProtocol))
...
from oridecon.contracts.infra.tasks import JobProtocol, JobStatus, TaskQueueError

if TYPE_CHECKING:
    from oridecon.contracts import TaskQueueProtocol
    from oridecon.result import Result

HAS_ORI_TASKS = True  # Contracts are always available.


class JobAdapter:
    """Adapts any ``oridecon.contracts.tasks.JobProtocol``-conforming object to the protocol.
    ...
    async def update_status(
        ...
        directly without a ``OrideconJobStatus`` import.
...
class OrideconTasksAdapter:
    """Implements ``TaskQueueProtocol`` by delegating to any conforming queue backend.

    This adapter bridges ``oridecon-ai`` workers with any
    ``oridecon.contracts.tasks.TaskQueueProtocol`` implementation (including
    ``oridecon.tasks.TaskQueueProtocol``) without importing from that package.

    Example::

        >>> from oridecon.ai.workers.adapters import OrideconTasksAdapter
        >>> queue = OrideconTasksAdapter(container.resolve(TaskQueueProtocol))
