...
Internal design of the `oridecon-ai-session` package.
...
`oridecon-ai-session` depends only on `oridecon` and `oridecon-contracts`. All runtime communication with other AI packages flows through protocols resolved via the DI container — never direct imports.
...
### From `oridecon.contracts.ai.session`
...
### From `oridecon.contracts.ai.memory`
...
|-------------|--------|--------------|
| `oridecon.ai.subsystems` | `SessionProvider` | `oridecon-ai` orchestrator |
| `oridecon.ai.modules` | `SessionModule` | Application DI setup |
