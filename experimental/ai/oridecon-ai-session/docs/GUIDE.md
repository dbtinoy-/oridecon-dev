---
title: oridecon-ai-session Guide
...
| `oridecon-cache` | Optional | Persistent session store |
| `oridecon-sql` | Optional | Database session store |

## Problem

AI conversations are stateful — each turn depends on previous turns. A session might need to be paused and resumed, forked into alternative branches, restored from a checkpoint, or shared across multiple agents. **oridecon-ai-session** provides the infrastructure for managing conversation state throughout its lifecycle — creation, turns, checkpointing, branching, context pruning, and multi-agent coordination.
...
The primary entrypoint is `SessionManagerProtocol` from `oridecon.contracts.ai.session`. The default implementation is `SessionManagerImpl`.

```python
from oridecon.ai.session import SessionManagerImpl
from oridecon.ai.session import SessionConfig, InMemorySessionStore
...
# Add turn
from oridecon.contracts.ai.session import SessionTurn
...
from oridecon.ai.session import CacheSessionStore, DatabaseSessionStore
...
```python
from oridecon.ai.session import BranchManager
...
```python
from oridecon.ai.session import AppendMerge, SelectiveMerge
...
```python
from oridecon.ai.session import GroupSession, RoundRobinTurnManager
...
```python
from oridecon.ai.session import SessionContext
...
```python
from oridecon.ai.session import RelevanceContextPruner
...
```python
from oridecon.ai.session import SessionMiddleware
```
...
```python
from oridecon.ai.session import SessionAnalytics
...
- [Architecture](/packages/oridecon-ai-session/architecture/) — provider lifecycle, module mapping, extension points
- [Configuration](/packages/oridecon-ai-session/configuration/) — full config reference
- [How-tos](/packages/oridecon-ai-session/howtos/) — task-oriented recipes
- [Troubleshooting](/packages/oridecon-ai-session/troubleshooting/) — common errors and fixes
