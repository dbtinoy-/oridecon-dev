...
from oridecon.serialization.backends.json import dumps_str, loads
...
class DatabaseSessionStore:
    ...
    Uses ``DatabaseProviderProtocol`` from ``oridecon-contracts`` so the
    ...
    async def save_checkpoint(self, checkpoint: SessionCheckpoint) -> None:
        ...
        """
        from oridecon.ai.session.stores.cache import _state_to_dict  # local helper
    ...
    def _row_to_checkpoint(row: Any) -> SessionCheckpoint:
        ...
        from oridecon.ai.session.stores.cache import _dict_to_state  # local helper
