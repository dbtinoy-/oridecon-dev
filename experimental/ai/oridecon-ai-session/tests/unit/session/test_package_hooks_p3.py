"""P3 hook surface import verification for oridecon-ai-session."""
...
def test_session_hooks_root_module_exists() -> None:
    ...
    assert oridecon.ai.session.SessionStartedHook is SessionStartedHook
    assert (
        oridecon.ai.session.SessionCheckpointCreatedHook
        is SessionCheckpointCreatedHook
    )
    assert oridecon.ai.session.SessionClosedHook is SessionClosedHook
