...
class TestCachedFeedbackStoreAdvanced:
    ...
    async def test_save_delegates_and_invalidates(
        self,
        cached_store: CachedFeedbackStore,
        ...
        mock_cache.delete.assert_any_call("feedback:session:owner-1:session-1")
        mock_cache.delete.assert_any_call("feedback:type:owner-1:rating")
    ...
    async def test_save_without_session(
        ...
        mock_cache.delete.assert_any_call("feedback:type:owner-1:text")
    ...
    async def test_find_by_session_cache_hit(
        ...
        cached = [
            FeedbackItem(
                feedback_type=FeedbackType.RATING, value=1.0, owner_id="owner-1"
            )
        ]
        mock_cache.get = AsyncMock(return_value=cached)

        store = CachedFeedbackStore(mock_store, mock_cache)
        items = await store.find_by_session("session-1", owner_id="owner-1")
        assert len(items) == 1
    ...
    async def test_find_by_session_cache_miss(
        ...
        items = await cached_store.find_by_session("session-1", owner_id="owner-1")
        mock_store.find_by_session.assert_called_once_with(
            "session-1", owner_id="owner-1"
        )
    ...
    async def test_find_by_type_cache_hit(
        ...
        store = CachedFeedbackStore(mock_store, mock_cache)
        items = await store.find_by_type(
            FeedbackType.RATING, owner_id="owner-1", limit=10
        )
        assert len(items) == 1
    ...
    async def test_find_by_type_cache_miss(
        ...
        items = await cached_store.find_by_type(
            FeedbackType.RATING, owner_id="owner-1", limit=10
        )
        mock_store.find_by_type.assert_called_once_with(
            FeedbackType.RATING, owner_id="owner-1", limit=10
        )
        mock_cache.set.assert_called_once()
    ...
    async def test_aggregate_delegates_to_store(
        ...
        await cached_store.aggregate(owner_id="owner-1", window_hours=48)
        mock_store.aggregate.assert_called_once_with(
            owner_id="owner-1", window_hours=48
        )
