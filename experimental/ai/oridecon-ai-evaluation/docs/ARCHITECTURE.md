...
Internal design of the `oridecon-ai-evaluation` package.
...
flowchart BT
    subgraph Presentation
        UI[oridecon-ui]
        Admin[oridecon-admin]
    end
    subgraph AI Subsystem
        LLM[oridecon-ai-llm]
        RAG[oridecon-ai-rag]
        Agents[oridecon-ai-agents]
        Evaluation[oridecon-ai-evaluation]
    end
    subgraph Infra
...
depends on `oridecon-contracts` for protocols and types, optionally
on `oridecon-ai-llm` for embedding-based evaluation, and on
`oridecon-vector` for vector store-backed comparisons.
...
All contracts live in `oridecon.contracts.ai.evaluation`:
...
| `EmbeddingClientProtocol` | Protocol (from `oridecon.contracts.ai.llm`) | Used optionally by `EmbeddingDistanceEvaluator` |
...
|---|---|---|
| `enabled` | `True` | `ORI_AI_EVALUATION__ENABLED` |
| `default_threshold` | `0.8` | `ORI_AI_EVALUATION__DEFAULT_THRESHOLD` |
| `embedding_model` | `"text-embedding-3-small"` | `ORI_AI_EVALUATION__EMBEDDING_MODEL` |
| `include_metadata` | `True` | `ORI_AI_EVALUATION__INCLUDE_METADATA` |
| `max_samples` | `None` | `ORI_AI_EVALUATION__MAX_SAMPLES` |
| `max_retries` | `3` | `ORI_AI_EVALUATION__MAX_RETRIES` |
| `timeout_seconds` | `30` | `ORI_AI_EVALUATION__TIMEOUT_SECONDS` |
