# Host integration guide — `oridecon-ai-relay-gateway`
...
```python
import asyncio
from oridecon.ai.relay.gateway import RelayGatewayConfig, RelayGatewayProvider
from oridecon.contracts.ai.relay import RelayChannel, RelayFormat
...
from oridecon.ai.relay.gateway import (
...
wrapped = CredentialInjectingHTTPClient(
    wrapped=my_http_client,          # e.g. oridecon-http's HTTPClient
...
library (e.g. `oridecon-http`'s `HTTPClient`, which forwards kwargs to
