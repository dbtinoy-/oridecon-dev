...
from oridecon.ai.relay.gateway.admin.contributor import RelayGatewayAdminContributor
...
class TestAdminBoot:
    ...
    async def test_on_admin_boot_raises_if_an_action_handler_cannot_be_resolved(
        ...
        from oridecon.ai.relay.gateway.admin import contributor as contributor_module

        bad_actions = (
            replace(
                contributor_module._ACTIONS[0],
                handler="oridecon.ai.relay.gateway.admin.actions:does_not_exist",
            ),
        )
