...
This module holds the built-in provider registration *data* consumed by
:class:`~lexigram.ai.llm.registry.core.ProviderRegistry`. Each client
module is imported lazily inside :func:`register_builtin_providers`, which
runs once at registry construction time, keeping all SDK imports deferred
away from module top-levels while still populating the registry catalogue.
"""
...
def register_builtin_providers(registry: ProviderRegistry) -> None:
    """Register all built-in providers on the given registry.

    Args:
        registry: Provider registry to populate (mutated in place).
    """
    # OpenAI
