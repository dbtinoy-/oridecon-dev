---
title: oridecon-ai-mcp Guide
description: Mental model, core concepts, and typical usage of Oridecon's MCP integration
---
...
| `oridecon-ai-agents` | Optional | Agent system bridging |
| `oridecon-ai-skills` | Optional | Skills-to-MCP bridging |
...
The package wraps the [Model Context Protocol](https://modelcontextprotocol.io) as a first-class Oridecon citizen: handlers, transports, and connectors are wired through the DI container with the same provider/module lifecycle as any other extension.
...
```python
from oridecon.ai.mcp import MCPController, tool, MCPModule
...
```python
from oridecon.ai.mcp.client import MCPClientModule, MCPConnection
...
### Bridge to oridecon-ai-skills

When `oridecon-ai-skills` is installed and its provider is registered, `SkillToolAdapter` is automatically created — all skills become MCP tools.
