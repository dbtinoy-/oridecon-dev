...
```python
from oridecon.ai.mcp import MCPController, tool
...
```python
from oridecon.ai.mcp import MCPController, resource
...
```python
from oridecon.ai.mcp import MCPController, prompt
...
```python
from oridecon.ai.mcp.client import MCPClient, StdioClientTransport
...
```python
from oridecon.ai.mcp.client import MCPClient, SSEClientTransport
...
```python
from oridecon.ai.mcp.client import MCPClientModule, MCPConnection, MCPClientRegistry
...
```python
from oridecon.contracts.mcp.protocols import MCPToolProviderProtocol
...
Install `oridecon-ai-skills` alongside `oridecon-ai-mcp`. The `MCPProvider` automatically detects `SkillRegistryProtocol` and `SkillExecutorProtocol` in the container and registers `SkillToolAdapter` — no extra wiring needed.
...
```python
from oridecon.ai.mcp import MCPModule, MCPConfig
...
Connect an existing oridecon-agents `ToolRegistryProtocol` to MCP:

```python
from oridecon.ai.mcp import ToolRegistryAdapter
from oridecon.contracts.ai.agents import ToolRegistryProtocol
