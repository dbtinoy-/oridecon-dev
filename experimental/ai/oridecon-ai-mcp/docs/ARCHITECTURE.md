...
Internal design of the `oridecon-ai-mcp` package — the Model Context Protocol (MCP) server/client bridge for AI ↔ external tool integration.
...
flowchart BT
    subgraph AI[AI Subsystem]
        ...
        Skills[oridecon-ai-skills]
        LLM[oridecon-ai-llm]
    end
    subgraph MCP[oridecon-ai-mcp]
...
**Import rule:** `oridecon-ai-mcp` depends on `oridecon` and `oridecon-contracts` only. Bridges to agents, skills, and LLM are resolved via DI container protocols — never direct imports.
...
ToolHandler → _CombinedToolProvider
  ...
  └── SkillToolAdapter          ← Optional oridecon-ai-skills bridge
```
...
```mermaid
flowchart LR
    subgraph Contracts[oridecon-contracts /mcp/exceptions]
...
| Skill bridge | Install `oridecon-ai-skills` — `SkillToolAdapter` auto-wires |
