...
class TestFilesystemConnectorPaths:
    ...
    def test_is_within_sandbox_true(self) -> None:
        ...
        with patch.object(Path, "exists", return_value=True):
            ...
            with patch("oridecon.ai.mcp.connectors.filesystem.Path.resolve", return_value=path):
...
class TestConnectorExports:
    ...
    def test_filesystem_exported(self) -> None:
        from oridecon.ai.mcp.connectors import filesystem

        assert filesystem is not None

    def test_github_exported(self) -> None:
        from oridecon.ai.mcp.connectors import github

        assert github is not None

    def test_sql_exported(self) -> None:
        from oridecon.ai.mcp.connectors import sql

        assert sql is not None

    def test_web_fetch_exported(self) -> None:
        from oridecon.ai.mcp.connectors import web_fetch

        assert web_fetch is not None

    def test_web_search_exported(self) -> None:
        from oridecon.ai.mcp.connectors import web_search

        assert web_search is not None

    def test_slack_exported(self) -> None:
        from oridecon.ai.mcp.connectors import slack

        assert slack is not None

    def test_google_drive_exported(self) -> None:
        from oridecon.ai.mcp.connectors import google_drive
