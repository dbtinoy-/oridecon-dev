"""MCP provider for Oridecon dependency injection."""
...
from oridecon.ai.mcp.di.provider_connectors import (
    _ConnectorResourceBundle,
    _ConnectorToolBundle,
    boot_connectors,
)
from oridecon.ai.mcp.exceptions import MCPInitializationError
...
from oridecon.ai.mcp.transport import SSETransport, StdioTransport
...
from oridecon.contracts.mcp.protocols import MCPAuthorizerProtocol
...
class MCPProvider(Provider):
    ...
    set, an :class:`~oridecon.ai.mcp.client.MCPClient` is also registered.
    ...
    async def _boot_service_providers(self, container: BootContainerProtocol) -> None:
        ...
        from oridecon.ai.mcp.controllers import ServiceToolProvider
    ...
    async def _boot_skill_bridge(self, container: BootContainerProtocol) -> None:
        """Register SkillToolAdapter if oridecon-ai-skills is available.

        When ``SkillRegistryProtocol`` and ``SkillExecutorProtocol`` are
        registered in the container (by ``oridecon-ai-skills``), a
        :class:`~oridecon.ai.mcp.adapters.skill_adapter.SkillToolAdapter` is
    ...
    async def _boot_handlers(self, container: BootContainerProtocol) -> None:
        ...
        if tool_provider is not None and service_tool_provider is not None:
            from oridecon.ai.mcp.controllers import _CombinedToolProvider
        ...
        if connector_resource_bundle is not None:
            from oridecon.ai.mcp.controllers import _CombinedResourceProvider
        ...
        try:
            from oridecon.ai.mcp.server.observability import (
                ObservableResourceHandler,
                ObservableToolHandler,
            )
            from oridecon.contracts.observability.ai import AIMetricsProtocol
    ...
    async def health_check(self, timeout: float = 5.0) -> HealthCheckResult:
        ...
        Returns:
            A :class:`~oridecon.contracts.core.health.HealthCheckResult`
