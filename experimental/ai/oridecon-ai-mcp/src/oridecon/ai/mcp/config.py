"""MCP server configuration for the Oridecon framework."""
...
from oridecon.ai.mcp.constants import ENV_NESTED_DELIMITER, ENV_PREFIX
...
class WebFetchConnectorConfig:
    ...
    user_agent: str = "oridecon-mcp/1.0"
...
class MCPConfig(BaseConfig):
    ...
    server_name: str = Field(default="oridecon-mcp")
    ...
    When set, :class:`~oridecon.ai.mcp.client.MCPClient` is registered in the
    container using :class:`~oridecon.ai.mcp.client.SSEClientTransport`.
    For stdio-based clients, construct :class:`~oridecon.ai.mcp.client.MCPClient`
    ...
    When non-empty, :class:`~oridecon.ai.mcp.client.MCPClient` is registered
    using :class:`~oridecon.ai.mcp.client.StdioClientTransport`.
