"""MCP server management commands for the Oridecon CLI."""
...
# ---------------------------------------------------------------------------
# Entry-point discovery helpers (inlined from oridecon-cli to avoid
# cross-extension imports — oridecon-ai-mcp must not import oridecon-cli)
# ---------------------------------------------------------------------------
...
def _load_app(entry_point: str) -> object:
    ...
    Returns:
        The Oridecon Application instance.
    """
...
def _build_script_server(module: object) -> object:
    ...
    no full Oridecon application required.
...
def serve(
    ...
    profile: Annotated[
        str | None,
        typer.Option("--profile", help="Config profile (sets ORI_PROFILE)."),
    ] = None,
) -> None:
    """Start an MCP server backed by your Oridecon application or a script file.

    Three invocation modes:

    **App mode** — loads a full Oridecon application and serves its MCP module::

        oridecon mcp serve                        # auto-detect
        oridecon mcp serve my_app.app:create_app  # explicit entry point

    **Script mode** — loads a .py file or module with @tool-decorated functions,
    no application required::

        oridecon mcp serve tools.py               # from a file
        oridecon mcp serve my_app.mcp_tools       # from a dotted module

    **Transport options**::

        oridecon mcp serve                        # stdio (Claude Desktop)
        oridecon mcp serve --transport sse        # HTTP port 8001
    """
    import asyncio
    import os
    ...
    try:
        ...
        if profile:
            os.environ["ORI_PROFILE"] = profile
        ...
        async def _run() -> None:
            from oridecon.app.runner import start_application, stop_application

            oridecon_app = _load_app(entry_point)
            await start_application(oridecon_app)  # type: ignore[arg-type]
            if transport == "stdio":
                _route_logs_to_stderr()
            try:
                mcp_server = oridecon_app.container.resolve(  # type: ignore[attr-defined]
                    "oridecon.ai.mcp.server.core.MCPServer"
                )
                ...
                await stop_application(oridecon_app)  # type: ignore[arg-type]
