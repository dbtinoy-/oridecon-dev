...
def aiobotocore_stub() -> Iterator[MagicMock]:
    """Stub aiobotocore.session for SQS tests and restore sys.modules after."""
    ...
    yield mock_aiobotocore
    sys.modules.pop("aiobotocore", None)
    sys.modules.pop("aiobotocore.session", None)


@pytest.mark.asyncio
async def test_sqs_queue_connect_disconnect(aiobotocore_stub: MagicMock):
    """Test connect and close."""
    mock_session = aiobotocore_stub.session.get_session.return_value
...
async def test_sqs_queue_subscribe():
    ...
    async def receive_side_effect(*args, **kwargs):
        ...
        return {
            "Messages": [
                {"Body": '{"payload": "msg1", "topic": "test"}', "ReceiptHandle": "rh1"}
            ]
        }
