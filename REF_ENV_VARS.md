...
- **Missing descriptions**: 818 of 1584 (51%)
- **Complex defaults**: 30 — default values that could not be statically resolved
...
| `LEX_ADMIN__AUDIT__REDACTION_FIELD_DENYLIST` | tuple[str, ...] | — | Field names whose values are always redacted in audit payloads. | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.audit.redaction_field_denylist` |
| `LEX_ADMIN__AUDIT__REDACTION_PATTERNS` | tuple[str, ...] | — | Pattern-based redaction strategies to apply. | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.audit.redaction_patterns` |
...
| `LEX_ADMIN__AUTH__SESSION_SECRET` | str | 'change-me-in-production' | Session secret for signing | `lexigram-admin/src/lexigram/admin/config.py:AdminConfig.auth.session_secret` |
...
| Env Var | Type | Default | Description | Source |
|---------|------|---------|-------------|--------|
| `LEX_AI_PROMPT__DEFAULT_FORMAT` | str | 'f_string' | — | `lexigram-ai-prompt/src/lexigram/ai/prompt/config.py:PromptConfig.default_format` |
