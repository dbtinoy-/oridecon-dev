...
def cors() -> None:
    from oridecon.web.cors import allow
...
def _write_violations(root: Path) -> None:
    (root / "pyproject.toml").write_text(
        '[project]\nname = "workspace"\n\n[tool.uv.workspace]\nmembers = ["oridecon-web"]\n',
        encoding="utf-8",
    )
    package_root = root / "oridecon-web"
    package_root.mkdir()
    (package_root / "pyproject.toml").write_text(
        '[project]\nname = "oridecon-web"\n', encoding="utf-8"
    )
    src = package_root / "src" / "oridecon" / "web"
...
def test_security_rules_fire_on_violations(tmp_path: Path) -> None:
    ...
    findings = run_rules(tmp_path, packages=("oridecon-web",)).findings
...
def test_security_rules_ignore_safe_code(tmp_path: Path) -> None:
    package_root = tmp_path / "oridecon"
    package_root.mkdir()
    (package_root / "pyproject.toml").write_text(
        '[project]\nname = "oridecon"\n', encoding="utf-8"
    )
    src = package_root / "src" / "oridecon"
    src.mkdir(parents=True)
    (src / "__init__.py").write_text("", encoding="utf-8")
    ...
    findings = run_rules(tmp_path, packages=("oridecon",)).findings
