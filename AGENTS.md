...
> This document is the **authoritative ruleset for writing and changing framework/package code**.
> Repository-wide engineering workflow (build, lint, type, test, CI, versioning, git rules) lives in [DEVELOPMENT.md](./DEVELOPMENT.md).
...
| **Dead feature-flag / configuration constants** | A flag or constant with no code path reading it is dead configuration; either wire it to behavior or remove it |
...
- `with_defaults()` is inherited from `lexigram.primitives.registry.Registry`
  ...
  `default_strategies()`.
- Standalone registries (their own storage/register API, not extending the
  primitive) declare built-ins with the same `_default_entries()` hook and
  consume it from a thin `with_defaults()` — never a second declaration.
- Registries with no in-package built-in set (plugin/contributor-driven)
  leave `_default_entries()` unimplemented: `with_defaults()` raises (or
  simply does not exist on standalone ones), and they are populated
  explicitly via `register()` / `register_many()`.
...
5. **Tests** — cover `generate()` into `tmp_path` (file created, no-collision
   ...
   control is needed.
6. **Version bump** — per [DEVELOPMENT.md](./DEVELOPMENT.md#36-versioning) §3.6,
   build-segment increment of *that* package.
...
- [ ] Are all direct dependencies declared by this package actually imported
      by its source (or hard runtime requirements of its integrations)?
- [ ] Does every public constant / feature flag actually have a read path?
...
- [DEVELOPMENT.md](./DEVELOPMENT.md) — build, lint, type, test, CI, versioning, git rules
- [CONTRIBUTING.md](./CONTRIBUTING.md) — human-facing contribution workflow
- [.importlinter](./.importlinter) — enforced package boundary contracts
