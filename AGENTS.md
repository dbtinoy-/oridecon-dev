...
## 4. Git Integration Rules (MANDATORY)

### 4.1 Never Merge — Rebase Only

**NEVER use `git merge` to bring arena/feature branches into the working branch.**
Merge commits pollute history with noise like `Merge branch 'arena/...' into ...`.

**Always rebase:**
```bash
# Fetch the branch
git fetch <remote> <arena-branch>

# Rebase your working branch onto it (fast-forwards, no merge commit)
...
If a rebase has conflicts, resolve them in place — never fall back to merge.

### 4.2 Co-authored-by Cleanup

When pulling commits from arena or other automated agents, strip
`Co-authored-by:` lines from commit messages before committing:

```bash
# During cherry-pick
git cherry-pick --no-commit <hash>
git commit -m "♻️ refactor(admin): description here"

# Or amend an existing message
git commit --amend -m "♻️ refactor(admin): cleaned message"
```

### 4.3 Commit Isolation

- **Commit early, commit small.** Each feature/fix gets its own commit.
- **No merge commits** in the main working tree.
- **No Co-authored-by** lines from arena agents.

### 4.4 Repo Visibility & PR Flow

| Repo | Visibility | Purpose |
|------|-----------|---------|
| `dbtinoy-/oridecon` | **PUBLIC** | Production framework, PyPI source |
| `dbtinoy-/oridecon-dev` | **PRIVATE** | Arena staging, never public |

**No merge PRs in `dbtinoy-/oridecon-dev`** — arena pushes land via rebase/cherry-pick only.

**PR flow (oridecon-dev → oridecon):**

1. Arena agents push commits to `oridecon-dev` (dev branch).
2. Pull locally, cherry-pick each commit with `--no-commit`, rewrite message
   (strip `Co-authored-by:`, clean scope/emoji), commit individually.
3. Push cleaned commits to `oridecon-dev` main+dev.
4. File PR from `oridecon-dev` main → `oridecon` main.

This keeps SHAs stable — cherry-pick preserves original commit content, only the
message changes. No duplicate commits, no drift.

**Why private?** `oridecon-dev` is an arena sandbox. It contains raw agent output,
intermediate states, and commit noise that should never be public. Only the clean,
reviewed PR to `oridecon` goes public.

### 4.5 Never Reference oridecon-dev Publicly

Do not mention `oridecon-dev` in commit messages, PR descriptions, docs, or
READMEs. It is an internal staging repo. When referring to the flow, say
"arena staging" or just "dev branch".
