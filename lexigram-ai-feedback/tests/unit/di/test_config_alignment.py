"""Config-alignment tests for FeedbackProvider."""

from __future__ import annotations

import pytest

from lexigram.ai.feedback.config import FeedbackConfig
from lexigram.ai.feedback.di.provider import FeedbackProvider


class _FakeRegistrar:
    def __init__(self) -> None:
        self.singletons: dict[type, object] = {}

    def singleton(
        self, cls: type, instance: object = None, factory: object = None
    ) -> None:
        self.singletons[cls] = factory if factory is not None else instance

    def transient(self, *args: object, **kwargs: object) -> None:
        pass


class TestConfigAlignment:
    def test_provider_declares_config_key_and_model(self) -> None:
        provider = FeedbackProvider()
        assert provider.config_key == FeedbackConfig.config_section
        assert provider.config_model is FeedbackConfig

    @pytest.mark.asyncio
    async def test_injected_config_used_when_no_explicit(self) -> None:
        provider = FeedbackProvider()
        provider.config = FeedbackConfig(enabled=False)
        container = _FakeRegistrar()

        await provider.register(container)

        assert container.singletons[FeedbackConfig] is provider.config

    @pytest.mark.asyncio
    async def test_explicit_config_wins_over_injected(self) -> None:
        explicit = FeedbackConfig(enabled=True)
        provider = FeedbackProvider(config=explicit)
        provider.config = FeedbackConfig(enabled=False)
        container = _FakeRegistrar()

        await provider.register(container)

        assert container.singletons[FeedbackConfig] is explicit

    @pytest.mark.asyncio
    async def test_default_config_binds_when_nothing_supplied(self) -> None:
        provider = FeedbackProvider()
        container = _FakeRegistrar()

        await provider.register(container)

        assert isinstance(container.singletons[FeedbackConfig], FeedbackConfig)
