...
class InlineEditCell(Component):
    ...
    def _input_el(self) -> Any:
        ...
        base_cls = "w-full px-2 py-1 text-sm rounded border border-primary-400 focus:outline-none focus:ring-2 focus:ring-primary-500 bg-muted dark:text-foreground"
    ...
    def render(self) -> object:
        ...
        if not self.editable:
            return el(
                "span",
                self.value,
                class_=f"text-sm text-foreground {self.css_class}".strip(),
            )
