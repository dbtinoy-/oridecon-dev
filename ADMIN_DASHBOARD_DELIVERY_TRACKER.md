...
| Settings forms | Field widgets, effective-value context, validation recovery, readonly state, HTMX/native parity, navigation safety | Settings forms now resolve duplicate checkbox submissions correctly, reject invalid values without persisting defaults, preserve submitted values with inline accessible errors, expose bounds/scope/source and explicit-versus-default value metadata, use textarea/URL/numeric controls where appropriate, show active/restart-required/dormant runtime status, provide sticky reset/save actions with duplicate-submit protection, warn before leaving dirty forms, and support separate read-versus-edit permission metadata with view-only rendering. |
...
- [x] Add separate read-versus-edit permission metadata for settings, enforce
      both gates server-side, and render read-only views without save actions.
- [x] Add explicit runtime applicability states for settings (`active`,
      `restart_required`, and `dormant`) with accessible status messaging.
- [x] Add effective-value source metadata that distinguishes explicit store
      values from application defaults and leaves opaque stores as unknown.
- [x] Add non-secret before/after diffs to successful settings audit events;
      secret fields remain excluded from audit metadata.
- [ ] Add safe settings rollback and optimistic concurrency where the underlying
      store and audit contracts support it.
