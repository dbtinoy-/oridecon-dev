...
class RedisGovernancePersistence:
    ...
    async def incr_requests(self, key: str, window: float) -> int:
        ...
        try:
            raw_val_result = await self._cache.get(counter_key)
            if hasattr(raw_val_result, "is_ok"):
                raw_val = (
                    raw_val_result.unwrap_or(None) if raw_val_result.is_ok() else None
                )
            else:
                raw_val = raw_val_result
    ...
    async def add_spend(self, key: str, amount: float, ttl: int) -> float:
        ...
        try:
            ...
            # If persistence fails, return the requested amount (fail-open)
            return amount
