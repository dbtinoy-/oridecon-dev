...
``AdminRole`` dataclass, and its create form must render schema fields with no
errors.
"""
...
class TestRolesResource:
    def test_model_is_rbac_admin_role(self) -> None:
        assert RolesResource.model is AdminRole

    def test_schema_generation_covers_role_fields(self) -> None:
        schema = FormSchemaGenerator().from_pydantic(AdminRole)
