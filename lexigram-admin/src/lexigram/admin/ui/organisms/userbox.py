    ...
    def render(self) -> Any:
        ...
        if not self.avatar_url:
            avatar = el(
                ...
                class_="w-8 h-8 rounded-full bg-primary-100 text-primary-600 flex items-center justify-center font-bold text-sm dark:bg-primary-900/30 dark:text-primary-400 border border-gray-200 dark:border-gray-700",
            )
        else:
            avatar = el(
                "img",
                src=self.avatar_url,
                class_="w-8 h-8 rounded-full border border-gray-200 dark:border-gray-700",
            )

        name = el(
            "div",
            self.username,
            class_="font-medium text-sm text-gray-700 dark:text-gray-300 truncate flex-1 text-left",
            x_show="!sidebarMini",
        )
        ...
        trigger = el(
            ...
            el(
                ...
                class_="ml-auto h-4 w-4 text-gray-400",
                fill="none",
                viewBox="0 0 24 24",
                stroke="currentColor",
            ),
            class_="flex items-center space-x-3 p-2 hover:bg-white dark:hover:bg-gray-800 transition-colors cursor-pointer w-full border border-transparent hover:border-gray-200 dark:hover:border-gray-700",
        ...
        for menu_item in self.user_menu_items:
            ...
            if menu_item.get("icon"):
                ...
                icon_node = Icon(
                    menu_item["icon"],
                    class_name="w-4 h-4 mr-2 text-gray-400 group-hover:text-gray-500 dark:text-gray-400",
                )
            ...
            items.append(
                el(
                    ...
                    menu_item.get("label", ""),
                    href=href,
                    class_="group flex items-center px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-md",
                    hx_get=href,
        ...
        if items:
            items.append(el("div", class_="h-px my-1 bg-gray-200 dark:bg-gray-700"))
        ...
        items.append(
            el(
                ...
                "Sign out",
                href="/admin/logout",
                class_="block px-4 py-2 text-sm text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-md",
            ),
        )
