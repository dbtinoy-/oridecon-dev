    ...
    def render(self) -> Any:
        ...
        for value, label in self.options:
            attrs = {
                ...
                "class": "block px-3 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-700 flex items-center gap-2",
                "hx_on": f"click:console.log('view:{value}')",
                # Simple serialized JS to update the trigger icon on click
                "onclick": "let svg = this.querySelector('svg').cloneNode(true); svg.setAttribute('class', 'h-4 w-4 text-gray-600 dark:text-gray-300'); this.closest('details').querySelector('summary span').innerHTML = ''; this.closest('details').querySelector('summary span').appendChild(svg); this.closest('details').open = false;",
            }
            ...
            icon_el = get_icon(
                icon_name,
                size="h-4 w-4 text-gray-400 group-hover:text-gray-500",
            )
            ...
            if value == self.current:
                ...
                attrs["class"] = (
                    "block px-3 py-2 text-sm font-medium bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-gray-100 cursor-default pointer-events-none flex items-center gap-2"
                )
        ...
        trigger_icon = get_icon(
            current_icon_name,
            size="h-4 w-4 text-gray-600 dark:text-gray-300",
        )

        trigger_el = el(
            "span",
            trigger_icon,
            class_="inline-flex items-center justify-center p-1 rounded-md hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors h-8 w-8",
            role="button",
            tabindex="0",
            ...
            el(
                ...
                el(
                    ...
                    *items,
                    ...
                    class_="absolute left-0 mt-2 w-40 bg-white dark:bg-gray-800 rounded-md shadow-lg ring-1 ring-black ring-opacity-5 dark:ring-white dark:ring-opacity-10 z-[100] py-1 focus:outline-none origin-top-left",
                ),
