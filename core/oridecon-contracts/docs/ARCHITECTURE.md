...
Internal design of the `oridecon-contracts` package.

---

## Role in the System

`oridecon-contracts` is the **zero-dependency foundation layer** of the Oridecon ecosystem. Every protocol, shared value type, base exception, and cross-package enum lives here.

```mermaid
flowchart BT
    Ext[Extension packages<br/>oridecon-web · oridecon-sql · oridecon-ai-llm<br/>oridecon-ai-rag · oridecon-admin · oridecon-cache]
    Core[oridecon<br/>Core framework · DI · Config · IoC]
    Contracts[oridecon-contracts<br/>Protocols · Types · Exceptions · Enums]
...
> **If two or more packages need to reference the same type, protocol, or exception, it lives in `oridecon-contracts`. No exceptions.**

```mermaid
flowchart TD
    Q{Shared across<br/>2+ packages?}
    Q -->|Yes| Contracts[oridecon-contracts]
...
| `exceptions/` | `OrideconError`, `DomainError`, `InfrastructureError`, `ContainerError`, `SecurityError`, `ResilienceError`, `ProviderError` |
...
| `PlanStep` | `oridecon-ai-agents` | Plan-and-execute internal |
| `Chunk`, `PipelineContext` | `oridecon-ai-rag` | RAG implementation internals |
| `CacheEntry`, `CacheStats` | `oridecon-cache` | Cache internals |
...
from oridecon.contracts.ai import AIError
...
from oridecon_ai_llm.exceptions import LLMRateLimitError
```
...
If a name exists in `oridecon-contracts`, no extension package may redefine it. Re-export aliases (`ai/vector.py` → `data/vector/protocols.py`) are permitted for ergonomic imports — simple re-exports only, no added logic.
