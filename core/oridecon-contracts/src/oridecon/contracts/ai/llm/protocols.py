...
from oridecon.contracts.ai.exceptions import ExtractionError, LLMError
from oridecon.contracts.ai.llm.types import ChatMessage, StreamChunk
...
class ChatMessageProtocol(Protocol):
    ...
    protocol.  Implemented concretely by ``oridecon.ai.llm.types.ChatMessage``.
    """
...
class CompletionProtocol(Protocol):
    ...
    protocol.  Implemented concretely by ``oridecon.ai.llm.types.Completion``.
    """
...
class PromptRendererProtocol(Protocol):
    ...
    Consumed by: oridecon-ai-platform prompt submodule, oridecon-ai-rag.
    """
...
class PromptOptimizerProtocol(Protocol):
    ...
    Consumed by: oridecon-ai-platform prompt submodule.
    """
