...
class DocumentProtocol(Protocol):
    ...
    fields — used across oridecon-vector and oridecon-ai-rag.
    """
...
class DocumentVectorStoreProtocol(Protocol):
    ...
    :class:`~oridecon.contracts.data.vector.protocols.VectorStoreProtocol`
    (infrastructure layer).

    All vector database drivers (Qdrant, ChromaDB, PGVector, etc.) are adapted
    to this shape via ``oridecon-vector`` so callers can swap backends.
    """
...
class Document:
    ...
    in packages that must not depend on ``oridecon-vector``.
    """
