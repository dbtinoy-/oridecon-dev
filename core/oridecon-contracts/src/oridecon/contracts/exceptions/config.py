...
if TYPE_CHECKING:
    from oridecon.contracts.core.config import ConfigIssue
...
class ConfigurationError(OrideconError):
    ...
    def from_validation_error(cls, error: Exception) -> ConfigurationError:
        ...
        Args:
            error: A ``ValidationError`` instance from either
                ``oridecon.contracts.validation`` or ``pydantic``.
