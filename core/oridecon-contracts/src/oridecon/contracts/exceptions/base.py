"""Base exception classes for Oridecon Framework.

This module contains the canonical exception hierarchy for the entire Oridecon
ecosystem. All framework exceptions should inherit from OrideconError.
"""
...
class OrideconError(Exception):
    """Root exception for the entire Oridecon ecosystem.

    All Oridecon exceptions inherit from this class. This ensures that
    isinstance checks work identically across core, events, web, and
    all subpackages.

    Attributes:
        code: Machine-readable error code (e.g., 'ORI_CONTAINER_001')
    ...
    def docs_url(self) -> str:
        ...
        return f"https://docs.oridecon.dev/reference/errors/{self.code}"

    def __str__(self) -> str:
        base = f"[{self.code}] {self.message}"
        if self.hint:
            base += f"\n  → Fix: {self.hint}"
        if self.code != "ORI_ERR_CORE_001":
            base += f"\n  → See: {self.docs_url}"
        return base
