...
(``oridecon-sql``, ``oridecon-events``, ``oridecon-tasks``, ``oridecon-ai``,
…) can import directly from here without depending on each other.

Typical usage::

    from oridecon.contracts.data.identifiers import Table, Column, table, column
...
from oridecon.contracts.data.sql.sql import InvalidIdentifierError
