...
Defines the contract for database migration execution so that ``oridecon-cli``
and other consumers can resolve a runner from the container without importing
``oridecon-sql`` directly.

Concrete implementations (Alembic adapter, ``SimpleMigrationManager``, etc.)
live in ``oridecon-sql`` and are registered by ``DatabaseProvider`` at boot time.
