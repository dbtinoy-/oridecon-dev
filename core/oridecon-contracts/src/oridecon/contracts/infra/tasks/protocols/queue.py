"""Task queue and worker protocols."""
...
if TYPE_CHECKING:
    ...
    from oridecon.contracts.infra.tasks.enums import JobStatus
...
class TaskManagerProtocol(Protocol):
    ...
    Implemented by ``oridecon.tasks.background_task_manager.BackgroundTaskManager``
    (LEX-006).  Consumers resolve this protocol from the container instead of
    importing the implementation package (e.g. ``oridecon.monitor``).
    """
...
class TaskProviderProtocol(Protocol):
    ...
    def build_idempotency_manager(
        ...
        Cross-package consumers (e.g. ``oridecon-multimedia``) build the
        idempotency machinery through this factory instead of importing
        ``oridecon-tasks`` internals directly.
    ...
    def build_idempotent_task_manager(
        ...
        Cross-package consumers (e.g. ``oridecon-multimedia``) obtain their
        duplicate-protected task submitter through this factory instead of
        importing ``oridecon-tasks`` internals directly.
