...
class ResiliencePipelineFactoryProtocol(Protocol):
    ...
    ``oridecon-resilience`` registers a concrete implementation.  Other
    extension packages (e.g. ``oridecon-sql``) request an optional
    ``ResiliencePipelineFactory | None`` via DI injection so they can build
    pre-configured pipelines without importing from ``oridecon-resilience``
    directly.
...
class RateLimiterProtocol(Protocol):
    ...
    ``oridecon-resilience`` is the canonical owner of all rate-limiter
