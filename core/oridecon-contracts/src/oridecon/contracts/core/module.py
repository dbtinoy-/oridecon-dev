"""Module protocol definitions for the Oridecon framework.

These protocols define the structural contracts for modules at the
contract layer.  External packages (``oridecon-web``, ``oridecon-sql``,
...
``oridecon.di.module`` within the core package.
"""
...
class ModuleMetadataProtocol(Protocol):
    ...
    Satisfied by :class:`oridecon.di.module.ModuleMetadata`.
    """
...
class ModuleProtocol(Protocol):
    ...
    A class satisfies this protocol if it has a ``__oridecon_module__``
    ...
    Usage in type hints::
        ...
        def process_module(mod: ModuleProtocol) -> None:
            meta = mod.__oridecon_module__
            print(f"Module {meta.name} has {len(meta.providers)} providers")
    """

    @property
    def __oridecon_module__(self) -> ModuleMetadataProtocol: ...
...
class DynamicModuleProtocol(Protocol):
    ...
    Satisfied by :class:`oridecon.di.module.DynamicModule`.
...
class CompiledModuleGraphProtocol(Protocol):
    ...
    Satisfied by :class:`oridecon.di.module.CompiledModuleGraph`.
    """
...
class ModuleCompilerProtocol(Protocol):
    ...
    Satisfied by :class:`oridecon.di.module.ModuleCompiler`.
    """
...
class ModuleRegistryProtocol(Protocol):
    ...
    Satisfied by :class:`oridecon.di.module.ModuleRegistry`.
    """
