...
from oridecon.contracts.audit.types import AuditEntry
...
class AuditVerifierSchedulerProtocol(Protocol):
    ...
    Implementations are provided by the ``oridecon-sql`` package and registered
    in the DI container. Extension packages (e.g. ``oridecon-admin``) resolve
