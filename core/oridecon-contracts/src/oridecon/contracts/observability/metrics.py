...
class AlertDispatcherProtocol(Protocol):
    ...
    PagerDuty, Slack, etc.).  ``oridecon-monitor`` ships a built-in
    :class:`~oridecon.monitor.alerts.LoggingAlertDispatcher` that writes
...
class MetricsFactoryProtocol(Protocol):
    """Create metric instruments. Extended interface for oridecon-monitor."""
...
class MetricsCollectorProtocol(
    ...
    """Full metrics capability. Implemented by oridecon-monitor.
...
class HealthCheckRegistryProtocol(Protocol):
    ...
    :class:`~oridecon.contracts.core.health.HealthCheckCategory` so that
    ...
    def add(
        ...
        *,
        ...
        Args:
            ...
            category: :class:`~oridecon.contracts.core.health.HealthCheckCategory`
