...
if TYPE_CHECKING:
    from oridecon.contracts.domain import DomainModelProtocol
...
# ``DomainEvent`` in ``oridecon.contracts.domain.events`` now serves as the
...
class Command(Message, Generic[TResult]):
    ...
    (which lives in ``oridecon``, not ``oridecon-contracts``) to avoid a
