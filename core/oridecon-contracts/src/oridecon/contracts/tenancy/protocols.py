...
if TYPE_CHECKING:
    ...
    from oridecon.contracts.tenancy.commands import (
        CreateTenantCommand,
        UpdateTenantCommand,
    )
    from oridecon.contracts.tenancy.errors import TenantError
    from oridecon.contracts.tenancy.types import TenantInfo, TenantResolutionContext
...
class TenantResolverProtocol(Protocol):
    ...
    :class:`~oridecon.tenancy.resolution.chain.CompositeResolver`.
...
class TenantProviderProtocol(Protocol):
    ...
    :class:`~oridecon.tenancy.stores.memory.InMemoryTenantProvider` (for
    testing/dev) and ``SQLTenantProvider`` (when
    ``oridecon-tenancy[sql]`` is installed).
    """
    ...
    async def get_tenant(self, tenant_id: str) -> TenantInfo | None:
        ...
        Returns:
            The :class:`~oridecon.contracts.tenancy.types.TenantInfo` record,
    ...
    async def get_tenant_by_slug(self, slug: str) -> TenantInfo | None:
        ...
        Returns:
            The matching :class:`~oridecon.contracts.tenancy.types.TenantInfo`,
    ...
    async def list_tenants(self, *, active_only: bool = True) -> list[TenantInfo]:
        ...
        Returns:
            List of matching :class:`~oridecon.contracts.tenancy.types.TenantInfo`
            records.
        """
        ...
    ...
    async def create_tenant(
        ...
        Args:
            command: :class:`~oridecon.contracts.tenancy.commands.CreateTenantCommand`
    ...
    async def update_tenant(
        ...
        Args:
            ...
            command: :class:`~oridecon.contracts.tenancy.commands.UpdateTenantCommand`
...
class TenantConfigProviderProtocol(Protocol):
    ...
    :class:`~oridecon.tenancy.config_overrides.service.TenantConfigService`
