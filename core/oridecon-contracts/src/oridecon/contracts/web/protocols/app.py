...
class HTTPApplicationProtocol(Protocol):
    ...
    ``oridecon-graphql`` mounting a ``/graphql`` endpoint) must depend on
    this protocol — NOT on ``oridecon-web`` — to avoid cross-extension imports.
...
class WebContributorProtocol(Protocol):
    ...
    entry-point discovery. This allows packages like ``oridecon-graphql``
    and ``oridecon-admin`` to expose web routes without requiring the
