...
so that packages outside of ``oridecon-web`` can produce SSE responses without
...
class ServerSentEvent:
    ...
    implementation (e.g. ``oridecon-web``).
...
class SseResponseFactoryProtocol(Protocol):
    ...
    Implementations are provided by ``oridecon-web`` and registered in the
...
class ReactiveSseBridgeProtocol(Protocol):
    ...
    Implementations are provided by ``oridecon-web``
    (``oridecon.web.transport.reactive.sse_from_stream``) and registered in
    the DI container during ``WebProvider.register()``.  Consumers such as
    ``oridecon-admin`` resolve this protocol instead of importing the
    bridge from ``oridecon-web`` directly.
    ...
    def __call__(
        ...
        Args:
            stream: A reactive event stream (``oridecon.reactive``).
