...
``discover_providers`` is the function ``oridecon/src/oridecon/app/factory.py``'s
own docstring already promises: "Provider auto-discovery via entry points is
the responsibility of ``oridecon-plugins``." It is the *single* implementation
of the discover→filter→load→instantiate path — ``PluginEngineProvider``
(``oridecon.plugins.engine``) delegates its discovery to this primitive.
"""
...
from oridecon.contracts.core.constants import EP_PLUGINS, EP_PROVIDERS
    ...
    *not* plugins and are left out; they are composed explicitly by the
    ...
    Args:
        disabled: Entry-point names to skip (e.g. from
            :func:`oridecon.plugins.state.load_disabled`).
