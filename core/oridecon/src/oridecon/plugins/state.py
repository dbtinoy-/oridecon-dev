...
from oridecon.serialization import JSONDecodeError, dumps_str, loads_str
...
_ENV_STATE_PATH = "ORIDECON_PLUGINS_STATE_PATH"
_DEFAULT_STATE_PATH = Path(".oridecon/plugins.json")
...
def load_disabled(path: str | Path | None = None) -> set[str]:
    ...
    Args:
        path: Explicit state-file path. Falls back to the
            ``ORIDECON_PLUGINS_STATE_PATH`` env var, then
            ``.oridecon/plugins.json`` relative to the working directory.
