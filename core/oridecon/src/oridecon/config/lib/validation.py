"""Config validation orchestration for Oridecon Framework.
...
def validate_all_configs(
    ...
    Args:
        ...
        env: Target environment override.  When ``None`` (the default)
            ...
            ``ORI_ENV``/``APP_ENV``).  Passing an explicit value forces
            every config to validate against that environment.

    Returns:
        A flat list of all :class:`~oridecon.contracts.core.config.ConfigIssue`
...
def is_insecure_secret(field: str, value: str) -> bool:
    ...
    """
    from oridecon.config.constants import INSECURE_SECRET_VALUES, SECRET_FIELD_PATTERNS
