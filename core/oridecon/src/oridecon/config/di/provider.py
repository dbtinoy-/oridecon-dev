...
:class:`~oridecon.config.main.OrideconConfig` as a singleton in the
DI container.
...
.. note::
    ...
    ``boot()`` may call ``container.resolve(OrideconConfig)``.
...
class ConfigProvider(Provider):
    ...
    def __init__(
        self,
        config_class: type[Any] = OrideconConfig,
    ...
    async def register(self, container: ContainerRegistrarProtocol) -> None:
        ...
        if self._config_instance is None:
            ...
            prefix = self.env_prefix or "ORI_"
        ...
        # Also register as OrideconConfig if it's a subclass of OrideconConfig
        # and not already registered as the main config_class.
        if self.config_class is not OrideconConfig and isinstance(
            self._config_instance, OrideconConfig
        ):
            container.singleton(OrideconConfig, instance=self._config_instance)
