"""Base configuration class for Oridecon Framework."""
...
class BaseConfig(DomainModel):
    """Base configuration class for all Oridecon configs.

    Features:
    - Environment variable override (ORI_ prefix)
    ...
    #: after the ``ORI_`` prefix in env vars (``ORI_CACHE__`` → ``"cache"``).
    ...
    def from_yaml(
        ...
        *,
        ...
        This is THE canonical way to load Oridecon configuration.  The loading
        order is deterministic and explicit:

        1. YAML file values (base layer)
        2. Profile overlay (if ORI_PROFILE is set or provided)
        3. ``ORI_*`` environment variables override YAML values
        ...
        Args:
            ...
            profile: Profile name (overrides ORI_PROFILE env var).
        ...
        """
        from oridecon.config.lib import (
            ConfigLoader,
        ...
        if yaml_path.exists():
            ...
            logger.info(
                ...
                path=str(yaml_path),
                hint="create application.yaml or set ORI_* env vars",
            )

        # 2. Profile overlay
        import os

        resolved_profile = profile or os.environ.get("ORI_PROFILE")
        if resolved_profile:
        ...
        # 3. Env vars
        if env_override:
            loader.add_source(EnvironmentConfigSource("ORI_"))
    ...
    def environment(self) -> Environment:
        ...
        Reads from the ``env`` field first; falls back to ``ORI_ENV``.
        """
    ...
    def to_safe_dict(self) -> dict[str, Any]:
        ...
        Fields whose names match :data:`~oridecon.config.constants.SECRET_FIELD_PATTERNS`
        are replaced with ``"***"``.
    ...
    def from_env_profile(
        ...
        *,
        ...
        overlays ``application.{profile}.yaml`` if ``ORI_PROFILE`` is set or
        *profile* is given.  ``ORI_*`` environment variables are applied last.

        This is the recommended method for **production, staging, Docker,
        Kubernetes, and CI/CD** deployments.  Set ``ORI_PROFILE=production``
        ...
        If *profile* is omitted the ``ORI_PROFILE`` environment variable is
        consulted.
