...
:class:`~oridecon.security.guards.GuardChainImpl`,
:class:`~oridecon.security.sanitization.InputSanitizer`, and
:class:`~oridecon.security.config.InputSanitizerConfig`.

HTTP security middleware (CORS, CSRF, CSP, response headers) is registered
by ``WebProvider`` in ``oridecon-web`` — it does not belong here.
"""
...
from oridecon.security.config import HashingConfig, InputSanitizerConfig, SecurityConfig
from oridecon.security.guards.chain import GuardChainImpl
from oridecon.security.hashing import PBKDF2KDF, Blake2bHasher, Sha256Hasher
...
class SecurityProvider(Provider):
    ...
    Registers:
    - :class:`~oridecon.security.config.SecurityConfig`
    - :class:`~oridecon.security.config.InputSanitizerConfig`
    - :class:`~oridecon.security.config.HashingConfig`
    - :class:`~oridecon.security.sanitization.InputSanitizer` (as both
      concrete and :class:`~oridecon.contracts.security.InputSanitizerProtocol`)
    - :class:`~oridecon.security.hashing.Sha256Hasher` as the default
      :class:`~oridecon.contracts.security.HasherProtocol`
    - :class:`~oridecon.security.hashing.PBKDF2KDF` as the default
      :class:`~oridecon.contracts.security.KeyDerivationProtocol`
    - :class:`~oridecon.security.guards.GuardChainImpl` (empty chain; consumers
    ...
    async def boot(self, container: BootContainerProtocol) -> None:
        ...
        from oridecon.security.hashing import ambient as hashing_ambient
    ...
    async def health_check(self, timeout: float = 5.0) -> HealthCheckResult:
        ...
        :attr:`~oridecon.contracts.core.health.HealthStatus.HEALTHY`.
