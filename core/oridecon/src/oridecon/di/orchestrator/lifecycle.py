...
from oridecon.di.context import clear_module_context, set_module_context
from oridecon.di.provider import ProviderState
...
class LifecycleManager:
        ...
        for provider in ordered:
            for dep in getattr(provider, "dependencies", ()):
                if dep not in available:
                    from oridecon.contracts.exceptions.provider import ProviderError
    ...
    async def _inject_provider_config(
        ...
        OrideconConfig from the container and calls get_section() to populate
        ...
        Raises:
            ConfigurationError: If config_key is set but config_model is not,
                or if OrideconConfig is not registered in the container.
        """
    ...
    def _validate_modules(self) -> list[str]:
        ...
        for provider in self._registry._providers.values():
            module_meta = getattr(provider.__class__, "__oridecon_module__", None)
            if not module_meta:
                continue
