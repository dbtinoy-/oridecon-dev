...
class ModuleMetadata:
    ...
    ``__oridecon_module__`` attribute.  Contains **structural data** plus
    ...
    .. note::
        ...
        This type will migrate to ``oridecon.contracts.core.module``
    ...
    __oridecon_owner__: type | None = field(default=None, init=False, repr=False)
    ...
    def configure_builder(
        ...
        :class:`~oridecon.contracts.exceptions.provider.ModuleCycleError`
        ...
        """
        from oridecon.contracts.exceptions.provider import ModuleCycleError
        from oridecon.di.module.dynamic import DynamicModule

        if _visited is None:
            _visited = set()

        owner = self.__oridecon_owner__
        ...
        for imp in self.imports:
            if isinstance(imp, DynamicModule):
                ...
                imp_meta = getattr(imp, "__oridecon_module__", None)
