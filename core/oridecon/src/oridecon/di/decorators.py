"""Dependency injection decorators for Oridecon Framework.
...
Example::
    ...
    from oridecon.di import injectable, singleton, inject
...
INJECTABLE_ATTR: str = "__oridecon_injectable__"
INJECT_ATTR: str = "__oridecon_inject__"
SCOPE_ATTR: str = "__oridecon_scope__"
PROVIDER_ATTR: str = "__oridecon_provider__"
...
class Injectable:
    ...
    def __call__(self, cls: type[T]) -> type[T]:
        ...
        """
        cast("Any", cls).__oridecon_injectable__ = {
            "scope": self.scope,
            "name": self.name,
        }
        return cls
...
def inject(func_or_cls: Any) -> Any:
    ...
    if inspect.isclass(func_or_cls):
        cast("Any", func_or_cls).__oridecon_inject__ = True
        return func_or_cls
...
def provider(
    ...
    """Class decorator that configures a :class:`~oridecon.di.provider.Provider`.
