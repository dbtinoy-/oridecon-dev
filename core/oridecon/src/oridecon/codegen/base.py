...
from oridecon.contracts.exceptions.infra import CollidingFileError, InfrastructureError
...
class GeneratorBase:
    ...
    (``uv add 'oridecon[codegen]'``); construction, name normalization,
    ...
    def _build_environment(self) -> jinja2.Environment:
        try:
            ...
            raise InfrastructureError(
                ...
                "uv add 'oridecon[codegen]' (or pip install 'oridecon[codegen]')"
            ) from exc
    ...
    def _stub_override_root(
        ...
        Only ``oridecon.*`` modules get an override layer; the package path
        stops before the ``cli`` segment (``oridecon.web.cli.generators.x``
        → ``stubs/oridecon/web``). Returns None when there is no anchor or
        the module lives outside the framework namespace.
        """
        if project_anchor is None:
            return None
        parts = module_name.split(".")
        if not parts or parts[0] != "oridecon":
            return None
