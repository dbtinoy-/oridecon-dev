"""Registry introspection utilities for Oridecon Framework.

Provides runtime discovery and metadata retrieval for all
:class:`~oridecon.primitives.registry.Registry` instances across the
...
def entry_point_catalog() -> dict[str, list[dict[str, str]]]:
    """Discover all Oridecon entry-point groups and their entries.

    Scans ``importlib.metadata`` for every group whose name starts
    with ``oridecon.`` and returns a mapping of group name to a list
    of entry-point metadata dicts.

    Returns:
        ``{"oridecon.cache.backends": [{"name": "redis", "value": "..."}]}``
    """
    catalog: dict[str, list[dict[str, str]]] = {}

    try:
        from oridecon.contracts.core.constants import ALL_ENTRY_POINT_GROUPS
    except ImportError:
...
class RegistryIntrospector:
    ...
    async def from_container(
        ...
        Args:
            container: A :class:`~oridecon.contracts.core.di.ContainerResolverProtocol`.
