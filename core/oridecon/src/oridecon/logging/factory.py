"""Logger creation and retrieval for the Oridecon Framework.
...
def get_logger(name: str | None = None) -> Any:
    ...
    Args:
        name: Logger name.  If ``None``, returns the root ``"oridecon"``
            logger. If the name does not start with ``"oridecon."``, that
    ...
    """
    if name is None:
        logger_name = "oridecon"
    elif name.startswith("oridecon."):
        logger_name = name
    else:
        logger_name = f"oridecon.{name}"
