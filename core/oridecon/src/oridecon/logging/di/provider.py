...
class LoggingProvider(Provider):
    ...
    async def register(self, container: ContainerRegistrarProtocol) -> None:
        ...
        from oridecon.logging.redaction import DelegatingRedactor

        logger_factory = LoggerFactoryImpl()
        container.singleton(LoggerFactoryProtocol, logger_factory)
        container.singleton(Logger, get_logger("oridecon"))
        container.singleton(RedactorProtocol, DelegatingRedactor())

    async def boot(self, container: ContainerResolverProtocol) -> None:
        """Apply logging configuration from OrideconConfig."""
        from oridecon.logging.configurator import apply_config

        config = await container.resolve(OrideconConfig)
