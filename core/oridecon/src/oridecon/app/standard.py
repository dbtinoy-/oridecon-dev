"""StandardModule — batteries-included provider bundle for production Oridecon applications.

``StandardModule`` is the **superset** of :class:`~oridecon.app.module.CoreModule`.
It adds :class:`~oridecon.serialization.di.provider.SerializationProvider` and
:class:`~oridecon.concurrency.di.provider.ConcurrencyProvider` on top of the
...
production applications.  Use :class:`~oridecon.app.module.CoreModule` only
...
Usage::
    ...
    from oridecon.app import Application, StandardModule
...
def _load_standard_providers() -> tuple[type[Provider], ...]:
    ...
    ``ConcurrencyProvider`` lives in ``oridecon-concurrency`` (a separate package)
    to avoid a circular package dependency.  It is always available at runtime
    because the workspace/install always includes ``oridecon-concurrency``.
    """
    from oridecon.concurrency.di.provider import ConcurrencyProvider  # noqa: PLC0415
...
class StandardModule(Module):
    """Batteries-included provider bundle for production Oridecon applications.

    Extends :class:`~oridecon.app.module.CoreModule` with serialization and
