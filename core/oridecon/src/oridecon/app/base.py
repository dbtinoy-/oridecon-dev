"""Application class — the composition root for Oridecon."""
...
from oridecon.app.constants import DEFAULT_SHUTDOWN_TIMEOUT
from oridecon.app.exceptions import AppShutdownError
from oridecon.app.health_probes import HealthProbeMixin
from oridecon.app.invoker import Invoker
from oridecon.app.lifecycle import ApplicationLifecycle
from oridecon.app.pipeline import MiddlewarePipeline
from oridecon.app.secrets import SecretsMixin
from oridecon.config import OrideconConfig
from oridecon.contracts.core import MiddlewarePipelineProtocol, MiddlewareProtocol
from oridecon.contracts.core.config import ConfigProtocol
from oridecon.contracts.core.health import AggregateHealthResult, HealthCheckCategory
from oridecon.di.container import Container
from oridecon.di.orchestrator import ProviderOrchestrator
from oridecon.logging import get_logger
from oridecon.logging.configurator import apply_config as _apply_logging_config
...
if TYPE_CHECKING:
    ...
    from oridecon.logging import LoggerProtocol
...
class Application(SecretsMixin, HealthProbeMixin):
    ...
    Or for long-running processes::
        ...
        from oridecon.app import run_application
    ...
    def __init__(
        ...
        self._config = config or OrideconConfig.from_env_profile()
        self._container.singleton(OrideconConfig, self._config)
    ...
    @property
    def config(self) -> OrideconConfig:
    ...
    def discover_providers(self, *packages: str) -> None:
        ...
        Scans each package recursively for :class:`~oridecon.di.provider.Provider`
        ...
        """
        from oridecon.app.discovery import discover_injectables, discover_providers
        ...
        if injectables:
            from oridecon.app.injectable_provider import InjectableAutoProvider
    ...
    def discover_modules(
        self,
        *,
        entry_point_group: str = "oridecon.modules",
        ...
        Args:
            entry_point_group: Entry-point group to scan. Default "oridecon.modules".
    ...
    def _validate_config(self) -> None:
        ...
        Delegates to :func:`oridecon.app.config_validation.validate_app_config`;
        error-severity issues (e.g. ``debug=True`` in production) abort the
        boot with :class:`ConfigurationError`.
        """
        from oridecon.app.config_validation import validate_app_config
    ...
    async def _handle_lifespan(self, receive: Any, send: Any) -> None:
        ...
        from oridecon.app.asgi_lifespan import handle_lifespan
