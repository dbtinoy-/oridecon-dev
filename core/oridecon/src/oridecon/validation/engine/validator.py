...
Aggregates :class:`~oridecon.validation.rules.Rule` instances per field name
and runs them all at once, collecting every error into a single
:class:`~oridecon.contracts.validation.ValidationError`.
...
from oridecon.contracts.exceptions.domain import FieldError, ValidationError
...
from oridecon.validation.rules import AbstractAsyncRule, AbstractRule
...
class AsyncValidator(Generic[T]):
    ...
    Mirrors :class:`ValidatorImpl` but supports :class:`~oridecon.validation.rules.AsyncRule`
    instances alongside regular :class:`~oridecon.validation.rules.Rule` instances.
