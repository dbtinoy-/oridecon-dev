...
Registers :class:`~oridecon.middleware.chain.MiddlewareChain` and
:class:`~oridecon.middleware.exception_filters.ExceptionFilterChain`
as container singletons and discovers third-party middleware providers
registered under the ``oridecon.middleware`` entry-point group.
"""
...
class MiddlewareProvider(Provider):
    ...
    On ``register()``:
      ...
      - Scans the ``oridecon.middleware`` entry-point group and delegates
    ...
    async def register(self, container: ContainerRegistrarProtocol) -> None:
        ...
        from oridecon.contracts.web.protocols import ExceptionFilterChainProtocol
    ...
    async def _discover_providers(self, container: ContainerRegistrarProtocol) -> None:
        """Scan the ``oridecon.middleware`` entry-point group.

        Each entry point that resolves to a :class:`~oridecon.di.provider.Provider`
        subclass is instantiated and its :meth:`~oridecon.di.provider.Provider.register`
