...
from oridecon.di.module import ModuleError, create_module
...
class TestImportValidation:
    ...
    def test_valid_module_import_succeeds(self) -> None:
        ...
        assert hasattr(MainMod, "__oridecon_module__")
...
class TestCircularDependencyDetection:
    ...
    def test_self_referencing_module_raises(self) -> None:
        ...
        # which doesn't check this since __oridecon_module__ isn't set yet)
        SelfRef.__oridecon_module__.imports = [SelfRef]

        builder = MockBuilder()
        with pytest.raises(ModuleError, match="Circular module dependency"):
            SelfRef.__oridecon_module__.configure_builder(builder)
    ...
    def test_two_module_cycle_raises(self) -> None:
        ...
        A.__oridecon_module__.imports = [B]
        B.__oridecon_module__.imports = [A]

        builder = MockBuilder()
        with pytest.raises(ModuleError, match="Circular module dependency"):
            A.__oridecon_module__.configure_builder(builder)
    ...
    def test_no_cycle_works(self) -> None:
        ...
        builder = MockBuilder()
        RootMod.__oridecon_module__.configure_builder(builder)
...
class TestModuleError:
    ...
    def test_module_error_in_all(self) -> None:
        ...
        import importlib
        mod_module = importlib.import_module("oridecon.di.module")
...
class TestModuleDecorator:
    ...
    def test_decorator_attaches_metadata(self) -> None:
        """@module sets __oridecon_module__ on the class."""

        @create_module(name="test_mod")
        class TestMod:
            pass

        assert hasattr(TestMod, "__oridecon_module__")
        assert TestMod.__oridecon_module__.name == "test_mod"
    ...
    def test_default_name_is_class_name(self) -> None:
        ...
        assert MyModule.__oridecon_module__.name == "MyModule"

    def test_owner_cls_set(self) -> None:
        """__oridecon_owner__ is set for cycle detection."""

        @create_module(name="owned")
        class OwnedMod:
            pass

        assert OwnedMod.__oridecon_module__.__oridecon_owner__ is OwnedMod
