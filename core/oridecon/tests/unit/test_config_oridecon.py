...
class TestOrideconConfigEnvSection:
    """Test folding of the ``ORI_ORIDECON__*`` env namespace into the root."""

    def test_folds_oridecon_env_section_into_root(self, tmp_path, monkeypatch):
        """ORI_ORIDECON__* vars populate root fields of OrideconConfig."""
        monkeypatch.setenv("ORI_ORIDECON__DEBUG", "true")
        monkeypatch.setenv("ORI_ORIDECON__LOGGING__JSON_FORMAT", "true")
    ...
    def test_extension_sections_stay_in_model_extra(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ORI_MONITOR__BACKEND_TYPE", "memory")
...
class TestOrideconLoggingJsonFormat:
    """End-to-end: ORI_ORIDECON__LOGGING__JSON_FORMAT produces JSON logs."""
    ...
    def test_json_format_env_var_emits_json_on_stdout(
        ...
        from oridecon.logging.configurator import apply_config, reset_logging
        ...
        try:
            ...
            get_logger("oridecon.test").info("hello_json", key="value")
        finally:
            reset_logging()
...
class TestOrideconLoggingLevel:
    """End-to-end: ORI_ORIDECON__LOGGING__LEVEL controls the global level."""
    ...
    def test_level_env_var_drops_events_below_threshold(
        ...
        monkeypatch.setenv("ORI_ORIDECON__LOGGING__LEVEL", "ERROR")
        monkeypatch.setenv("ORI_ORIDECON__LOGGING__JSON_FORMAT", "true")
        ...
        try:
            apply_config(config.logging)
            get_logger("oridecon.config_level_test").info("should_be_dropped")
            get_logger("oridecon.config_level_test").error("should_be_kept")
        finally:
            reset_logging()
