...
class TestConfigProtocol:
    ...
    def test_oridecon_config_satisfies_protocol(self) -> None:
        """OrideconConfig instances pass isinstance check against ConfigProtocol."""
    ...
    def test_base_config_satisfies_protocol(self) -> None:
        ...
        config = MyConfig()
        # BaseConfig has get() — but get_section() is on OrideconConfig.
        ...
        # BaseConfig provides get(). get_section() is only on OrideconConfig.
        assert has_get
...
class TestConfigProviderProtocolRegistration:
    ...
    async def test_registers_config_protocol(self) -> None:
        ...
        provider = ConfigProvider(config_class=OrideconConfig)
        ...
        assert OrideconConfig in call_args_list
...
class TestEnvVarPrecedence:
    ...
    def test_env_override_yaml(self) -> None:
        ...
        try:
            os.environ["ORI_APP_NAME"] = "env_value"
            ...
            os.environ.pop("ORI_APP_NAME", None)
...
class TestAppConfigRemoval:
    ...
    def test_app_config_not_in_config_package(self) -> None:
        """AppConfig is no longer exported from oridecon.config."""
        import oridecon.config as config_pkg

        assert not hasattr(config_pkg, "AppConfig")

    def test_oridecon_config_has_app_name(self) -> None:
        """OrideconConfig still provides app_name as a flat field."""
        config = OrideconConfig()
        assert config.app_name == "oridecon-app"
...
class TestBaseConfigIsDomainModel:
    ...
    def test_oridecon_config_is_domain_model(self) -> None:
        """OrideconConfig extends DomainModel through BaseConfig."""
        from oridecon.domain import DomainModel

        assert issubclass(OrideconConfig, DomainModel)
