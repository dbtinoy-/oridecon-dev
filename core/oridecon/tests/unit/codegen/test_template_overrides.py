...
from oridecon.codegen.base import GeneratorBase
from oridecon.contracts.cli.generators import GenerationResult
...
class TestStubOverrideRoot:
    def test_derives_dotted_package_path_from_cli_module(self, tmp_path: Path) -> None:
        root = GeneratorBase._stub_override_root(
            "oridecon.web.cli.generators.controller", tmp_path
        )
        assert root == tmp_path / "stubs" / "oridecon" / "web"
    ...
    def test_isolates_packages(self, tmp_path: Path) -> None:
        ...
        sql = GeneratorBase._stub_override_root(
            "oridecon.sql.cli.generators.model", tmp_path
        )
        assert web != sql

    def test_none_without_anchor(self) -> None:
        assert (
            GeneratorBase._stub_override_root("oridecon.web.cli.generators.x", None)
            is None
        )

    def test_none_for_non_oridecon_module(self, tmp_path: Path) -> None:
...
class TestSearchRootOrdering:
    def test_explicit_template_root_wins_over_everything(self, tmp_path: Path) -> None:
        ...
        result = roots(
            ...
            module_name="oridecon.web.cli.generators.controller",
    ...
    def test_stub_layer_sits_between_explicit_and_package(self, tmp_path: Path) -> None:
        ...
        assert result == [
            tmp_path / "stubs" / "oridecon" / "web",
            tmp_path / "pkg_templates",
        ]

    def test_no_override_layer_for_non_oridecon_module(self, tmp_path: Path) -> None:
        result = roots(
            tmp_path,
...
class TestResolveTemplateRootBehavior:
    ...
    def test_existing_stubs_dir_wins_for_oridecon_module(
        self, tmp_path: Path, monkeypatch
    ) -> None:
        import sys
        import types

        stubs = tmp_path / "stubs" / "oridecon" / "web"
        stubs.mkdir(parents=True)

        def fake_anchor(start):  # noqa: ARG001
            return tmp_path

        monkeypatch.setattr("oridecon.codegen.base.find_project_anchor", fake_anchor)

        holder = tmp_path / "controller.py"
        holder.write_text("", encoding="utf-8")
        fake_module = types.ModuleType("oridecon.web.cli.generators.controller")
        fake_module.__file__ = str(holder)
        monkeypatch.setitem(
            sys.modules, "oridecon.web.cli.generators.controller", fake_module
        )

        FakeWebGenerator = type("FakeWebGenerator", (GeneratorBase,), {})
        FakeWebGenerator.__module__ = "oridecon.web.cli.generators.controller"  # type: ignore[misc]
