...
class TestMiddlewareConfigConstants:
    def test_env_prefix_double_underscore(self) -> None:
        from oridecon.middleware.constants import ENV_PREFIX

        assert ENV_PREFIX == "ORI_MIDDLEWARE__"

    def test_env_nested_delimiter_exists(self) -> None:
        from oridecon.middleware.constants import ENV_NESTED_DELIMITER

        assert ENV_NESTED_DELIMITER == "__"

    def test_env_prefix_in_all(self) -> None:
        import oridecon.middleware.constants as c
    ...
    def test_config_has_no_local_env_prefix(self) -> None:
        ...
        import oridecon.middleware.config as cfg_mod

        src = inspect.getsource(cfg_mod)
        assert 'ENV_PREFIX: str = "ORI_MIDDLEWARE__"' not in src
