import pytest
from oridecon.di.module import Module, ModuleMetadata, module
...
def test_module_decorator_no_args():
    ...
    assert hasattr(MyModule, "__oridecon_module__")
    meta = MyModule.__oridecon_module__
    assert isinstance(meta, ModuleMetadata)
    assert meta.name == "MyModule"
    assert meta.providers == []
    assert meta.imports == []
    assert meta.exports == []
    assert meta.__oridecon_owner__ is MyModule
...
def test_module_decorator_with_args():
    ...
    assert MyModule.__oridecon_module__.name == "CustomName"
    assert MyModule.__oridecon_module__.providers == [MockProvider]
    assert MyModule.__oridecon_module__.exports == [str]
    assert MyModule.__oridecon_module__.__oridecon_owner__ is MyModule
...
def test_module_decorator_with_imports():
    ...
    assert MainModule.__oridecon_module__.imports == [DependencyModule]
