...
Verifies that when ``ORI_PROFILE`` is set, values from
...
class TestProfileOverlay:
    def test_no_profile_loads_base_yaml_only(
        ...
        """With no ORI_PROFILE set, only the base YAML is loaded."""
        monkeypatch.delenv("ORI_PROFILE", raising=False)
    ...
    def test_profile_overlay_merges_values_over_base(
        ...
        """With ORI_PROFILE=test, ``application.test.yaml`` overlays the base."""
        profile_yaml = tmp_path / "application.test.yaml"
        profile_yaml.write_text(
            "app_name: test_app\ndebug: true\n",
            encoding="utf-8",
        )
        monkeypatch.setenv("ORI_PROFILE", "test")
    ...
    def test_profile_overlay_missing_file_falls_back_to_base(
        ...
        monkeypatch.setenv("ORI_PROFILE", "nonexistent_profile_xyz")
    ...
    def test_profile_overlay_partial_keys_preserve_base_values(
        ...
        monkeypatch.setenv("ORI_PROFILE", "staging")
    ...
    def test_no_base_yaml_with_profile_still_loads_profile(
        ...
        monkeypatch.setenv("ORI_PROFILE", "ci")
