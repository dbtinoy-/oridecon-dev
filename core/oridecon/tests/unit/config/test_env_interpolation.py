...
from oridecon.config.lib.merge import interpolate_env_vars, interpolate_string
...
class TestInterpolateString:
    ...
    def test_top_level_var_resolved(self) -> None:
        ...
        os.environ["ORI_TEST_ISTR_VAR"] = "hello"
        try:
            assert interpolate_string("${ORI_TEST_ISTR_VAR}") == "hello"
        finally:
            del os.environ["ORI_TEST_ISTR_VAR"]

    def test_default_used_when_var_unset(self) -> None:
        """${VAR:default} returns default when VAR is not set."""
        os.environ.pop("ORI_TEST_ISTR_MISSING", None)
        assert interpolate_string("${ORI_TEST_ISTR_MISSING:fallback}") == "fallback"

    def test_empty_default_produces_empty_string(self) -> None:
        """${VAR:} with explicit empty default produces empty string, not error."""
        os.environ.pop("ORI_TEST_ISTR_EMPTY", None)
        assert interpolate_string("${ORI_TEST_ISTR_EMPTY:}") == ""

    def test_missing_var_no_default_raises(self) -> None:
        """${VAR} with no default and VAR unset raises ConfigurationError."""
        os.environ.pop("ORI_TEST_ISTR_ABSENT", None)
        with pytest.raises(ConfigurationError, match="ORI_TEST_ISTR_ABSENT"):
            interpolate_string("${ORI_TEST_ISTR_ABSENT}")

    def test_error_message_names_missing_var(self) -> None:
        """The ConfigurationError message includes the missing variable name."""
        os.environ.pop("ORI_TEST_ISTR_NAMED", None)
        with pytest.raises(ConfigurationError) as exc_info:
            interpolate_string("${ORI_TEST_ISTR_NAMED}")
        assert "ORI_TEST_ISTR_NAMED" in str(exc_info.value)
    ...
    def test_var_set_overrides_default(self) -> None:
        ...
        os.environ["ORI_TEST_ISTR_PRESENT"] = "env_val"
        try:
            assert interpolate_string("${ORI_TEST_ISTR_PRESENT:default_val}") == "env_val"
        finally:
            del os.environ["ORI_TEST_ISTR_PRESENT"]

    def test_partial_string_interpolation(self) -> None:
        """Interpolation works when ${VAR} is embedded inside a longer string."""
        os.environ["ORI_TEST_ISTR_HOST"] = "localhost"
        os.environ["ORI_TEST_ISTR_PORT"] = "5432"
        try:
            result = interpolate_string(
                "postgresql://${ORI_TEST_ISTR_HOST}:${ORI_TEST_ISTR_PORT}/db"
            )
            assert result == "postgresql://localhost:5432/db"
        finally:
            del os.environ["ORI_TEST_ISTR_HOST"]
            del os.environ["ORI_TEST_ISTR_PORT"]
...
class TestInterpolateEnvVars:
    ...
    def test_top_level_scalar_resolved(self) -> None:
        ...
        os.environ["ORI_TEST_IEV_SCALAR"] = "top_value"
        try:
            config: dict = {"name": "${ORI_TEST_IEV_SCALAR}"}
            interpolate_env_vars(config)
            assert config["name"] == "top_value"
        finally:
            del os.environ["ORI_TEST_IEV_SCALAR"]

    def test_nested_dict_field_resolved(self) -> None:
        """${VAR} inside a nested dict value is substituted."""
        os.environ["ORI_TEST_IEV_NESTED"] = "nested_value"
        try:
            config: dict = {"outer": {"inner": "${ORI_TEST_IEV_NESTED}"}}
            interpolate_env_vars(config)
            assert config["outer"]["inner"] == "nested_value"
        finally:
            del os.environ["ORI_TEST_IEV_NESTED"]

    def test_list_of_strings_resolved(self) -> None:
        """${VAR} inside list string items is substituted."""
        os.environ["ORI_TEST_IEV_ITEM"] = "list_item"
        try:
            config: dict = {"items": ["${ORI_TEST_IEV_ITEM}", "plain"]}
            interpolate_env_vars(config)
            assert config["items"][0] == "list_item"
            assert config["items"][1] == "plain"
        finally:
            del os.environ["ORI_TEST_IEV_ITEM"]

    def test_list_of_dicts_resolved(self) -> None:
        """${VAR} inside dicts nested inside lists is substituted."""
        os.environ["ORI_TEST_IEV_DICT_ITEM"] = "dict_item_value"
        try:
            config: dict = {"items": [{"key": "${ORI_TEST_IEV_DICT_ITEM}"}]}
            interpolate_env_vars(config)
            assert config["items"][0]["key"] == "dict_item_value"
        finally:
            del os.environ["ORI_TEST_IEV_DICT_ITEM"]
    ...
    def test_missing_var_raises_at_dict_level(self) -> None:
        ...
        os.environ.pop("ORI_TEST_IEV_MISSING", None)
        config: dict = {"key": "${ORI_TEST_IEV_MISSING}"}
        with pytest.raises(ConfigurationError, match="ORI_TEST_IEV_MISSING"):
            interpolate_env_vars(config)

    def test_default_used_at_nested_level(self) -> None:
        """${VAR:default} in nested dict uses default when VAR is unset."""
        os.environ.pop("ORI_TEST_IEV_NESTED_DEF", None)
        config: dict = {"outer": {"inner": "${ORI_TEST_IEV_NESTED_DEF:fallback}"}}
        interpolate_env_vars(config)
...
class TestEndToEndInterpolation:
    ...
    def test_top_level_string_var_resolved(self) -> None:
        ...
        yaml = "app_name: ${ORI_TEST_E2E_APP_NAME}\n"
        os.environ["ORI_TEST_E2E_APP_NAME"] = "my-app"
        try:
            ...
            del os.environ["ORI_TEST_E2E_APP_NAME"]
            Path(path).unlink(missing_ok=True)

    def test_top_level_default_used_when_var_unset(self) -> None:
        """${VAR:default} in top-level YAML uses the inline default."""
        yaml = "app_name: ${ORI_TEST_E2E_MISSING:default-app}\n"
        os.environ.pop("ORI_TEST_E2E_MISSING", None)
        with tempfile.NamedTemporaryFile(
            mode="w", suffix=".yaml", delete=False
        ) as f:
            f.write(yaml)
    ...
    def test_nested_dataclass_field_resolved(self) -> None:
        ...
        yaml = "quota:\n  limit: ${ORI_TEST_E2E_LIMIT}\n  window_seconds: 3600\n"
        os.environ["ORI_TEST_E2E_LIMIT"] = "50"
        try:
            with tempfile.NamedTemporaryFile(
                mode="w", suffix=".yaml", delete=False
            ) as f:
            ...
            from oridecon.config.lib.sources import ConfigLoader, FileConfigSource
            ...
            del os.environ["ORI_TEST_E2E_LIMIT"]
            Path(path).unlink(missing_ok=True)
    ...
    def test_list_of_objects_in_yaml_resolved(self) -> None:
        ...
        yaml = (
            "items:\n"
            "  - key: ${ORI_TEST_E2E_LIST_ITEM_A}\n"
            "  - key: static\n"
        )
        os.environ["ORI_TEST_E2E_LIST_ITEM_A"] = "dynamic_a"
        try:
            with tempfile.NamedTemporaryFile(
                mode="w", suffix=".yaml", delete=False
            ) as f:
            ...
            del os.environ["ORI_TEST_E2E_LIST_ITEM_A"]
            Path(path).unlink(missing_ok=True)
    ...
    def test_missing_var_no_default_raises_at_load_time(self) -> None:
        ...
        yaml = "app_name: ${ORI_TEST_E2E_ABSENT_VAR}\n"
        os.environ.pop("ORI_TEST_E2E_ABSENT_VAR", None)
        with tempfile.NamedTemporaryFile(
            mode="w", suffix=".yaml", delete=False
        ) as f:
            f.write(yaml)
            path = f.name
        try:
            with pytest.raises(ConfigurationError, match="ORI_TEST_E2E_ABSENT_VAR"):
    ...
    def test_empty_default_var_produces_empty_string(self) -> None:
        ...
        yaml = "app_name: ${ORI_TEST_E2E_EMPTY_DEF:}\n"
        os.environ.pop("ORI_TEST_E2E_EMPTY_DEF", None)
        with tempfile.NamedTemporaryFile(
            mode="w", suffix=".yaml", delete=False
        ) as f:
            f.write(yaml)
