"""Unit tests for oridecon core security constants.

Adapted from oridecon-security/tests/unit/test_security_constants.py.
TestVersion is removed — the core constants.py does not expose __version__.
Adds origin-guard assertion proving the module resolves to oridecon core.
"""
...
class TestConstantsModuleIsCore:
    """Verify constants resolves to oridecon core, not oridecon-security."""

    def test_constants_module_is_core_package(self) -> None:
        spec = importlib.util.find_spec("oridecon.security.constants")
        assert spec is not None
        assert spec.origin is not None
        assert "oridecon-security" not in spec.origin, (
            f"Expected constants to resolve to oridecon core, got: {spec.origin!r}"
        )
...
class TestEnvironmentPrefix:
    ...
    def test_env_prefix(self) -> None:
        from oridecon.security import constants as const  # noqa: PLC0415

        assert const.ENV_PREFIX == "ORI_SECURITY__"
...
class TestAllExports:
    ...
    def test_version_token_not_in_core_constants(self) -> None:
        ...
        The version token referenced 'oridecon-security' metadata, which no
        ...
        assert not hasattr(const, "__version__"), (
            ...
            "that was a oridecon-security artifact."
        )
