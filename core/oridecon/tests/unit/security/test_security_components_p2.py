...
Derived from oridecon-security/tests/unit/test_security_components.py.
...
from oridecon.security.encryption.service import EncryptionService
...
class TestCoreComponentsAreInCorePackage:
    """Verify all moved modules resolve to oridecon core, not oridecon-security."""

    def test_encryption_service_module_is_core(self) -> None:
        spec = importlib.util.find_spec("oridecon.security.encryption.service")
        assert spec is not None
        assert spec.origin is not None
        assert "oridecon-security" not in spec.origin, spec.origin

    def test_guards_chain_module_is_core(self) -> None:
        spec = importlib.util.find_spec("oridecon.security.guards.chain")
        assert spec is not None
        assert "oridecon-security" not in (spec.origin or "")

    def test_sanitization_sanitizer_module_is_core(self) -> None:
        spec = importlib.util.find_spec("oridecon.security.sanitization.sanitizer")
        assert spec is not None
        assert "oridecon-security" not in (spec.origin or "")
...
class TestEncryptionService:
    ...
    def test_decrypt_wrong_key_raises(self) -> None:
        ...
        from oridecon.security.exceptions import DecryptionError
...
class TestSecurityProvider:
    ...
    def test_provider_is_importable(self) -> None:
        from oridecon.security.provider import SecurityProvider  # noqa: PLC0415
    ...
    def test_provider_priority_is_infrastructure(self) -> None:
        from oridecon.contracts.core import ProviderPriority  # noqa: PLC0415
        from oridecon.security.provider import SecurityProvider  # noqa: PLC0415
