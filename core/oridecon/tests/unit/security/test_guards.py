...
Adapted from oridecon-security/tests/unit/test_guards.py.
...
class TestGuardsModuleIsCore:
    """Verify guards resolves to oridecon core, not oridecon-security."""

    def test_guards_chain_module_is_core_package(self) -> None:
        spec = importlib.util.find_spec("oridecon.security.guards.chain")
        assert spec is not None, "oridecon.security.guards.chain must be importable"
        assert spec.origin is not None
        assert "oridecon-security" not in spec.origin, (
            f"Expected oridecon.security.guards.chain to resolve to oridecon core, "
            f"but got: {spec.origin!r}"
        )

    def test_guards_decorator_module_is_core_package(self) -> None:
        spec = importlib.util.find_spec("oridecon.security.guards.decorator")
        assert spec is not None
        assert spec.origin is not None
        assert "oridecon-security" not in spec.origin, (
            f"Expected oridecon.security.guards.decorator to resolve to oridecon core, "
            f"but got: {spec.origin!r}"
        )
