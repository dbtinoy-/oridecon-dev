...
Adapted from oridecon-security/tests/unit/test_secrets_rotation.py.
...
from oridecon.security.types import SecretRotationPolicy
...
class TestSecretsRotationModuleIsCore:
    """Verify secrets rotation resolves to oridecon core, not oridecon-security."""

    def test_secrets_rotation_module_is_core_package(self) -> None:
        spec = importlib.util.find_spec("oridecon.security.secrets.rotation")
        assert spec is not None
        assert spec.origin is not None
        assert "oridecon-security" not in spec.origin, (
            f"Expected secrets.rotation to resolve to oridecon core, "
            f"got: {spec.origin!r}"
        )
