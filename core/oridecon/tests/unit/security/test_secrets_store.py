...
Adapted from oridecon-security/tests/unit/test_secrets_store.py.
...
from oridecon.security.exceptions import SecretAccessError
from oridecon.security.secrets import (
    EnvSecretStore,
    FileSecretStore,
    InMemorySecretStore,
    SecretValue,
...
class TestSecretsModuleIsCore:
    """Verify secrets store resolves to oridecon core, not oridecon-security."""

    def test_secrets_store_module_is_core_package(self) -> None:
        spec = importlib.util.find_spec("oridecon.security.secrets.store")
        assert spec is not None
        assert spec.origin is not None
        assert "oridecon-security" not in spec.origin, (
            f"Expected secrets.store to resolve to oridecon core, got: {spec.origin!r}"
        )
...
def test_env_secret_store_missing_raises() -> None:
    ...
    with pytest.raises(SecretNotFoundError):
        store.get_secret("DOES_NOT_EXIST_ORI_TEST")
