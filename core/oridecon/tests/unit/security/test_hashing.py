...
from oridecon.contracts.security import HasherProtocol, KeyDerivationProtocol
from oridecon.di.container import Container
from oridecon.security.config import HashingConfig, SecurityConfig
from oridecon.security.hashing import Blake2bHasher, PBKDF2KDF, Sha256Hasher
from oridecon.security.provider import SecurityProvider
...
class TestDigestHashers:
    ...
    async def test_sha256_hasher_is_deterministic(self) -> None:
        ...
        hasher = Sha256Hasher()
        digest = await hasher.hash("oridecon")

        assert digest == hashlib.sha256(b"oridecon").hexdigest()
        assert await hasher.verify("oridecon", digest) is True
        assert await hasher.verify("oridecon!", digest) is False
    ...
    async def test_blake2b_hasher_is_deterministic(self) -> None:
        ...
        assert digest == hashlib.blake2b(b"oridecon").hexdigest()
        assert await hasher.verify("oridecon", digest) is True
        assert await hasher.verify("oridecon!", digest) is False
...
class TestPBKDF2KDF:
    ...
    async def test_pbkdf2_uses_stable_encoded_format(self) -> None:
        ...
        encoded = await kdf.derive("oridecon", salt=salt)
        parts = encoded.split("$")
        ...
        assert await kdf.verify("oridecon", encoded) is True
        assert await kdf.verify("oridecon!", encoded) is False
