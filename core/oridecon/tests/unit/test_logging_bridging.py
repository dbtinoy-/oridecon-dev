...
from oridecon.logging import apply_config, configure_logging, get_logger, reset_logging
from oridecon.logging.config import LoggingConfig, RedactionConfig, SamplingConfig
...
class TestGetLogger:
    ...
    def test_default_logger_name(self) -> None:
        """get_logger(None) returns a logger with name 'oridecon'."""
        logger = get_logger()
        assert hasattr(logger, "name")
        assert logger.name == "oridecon"

    def test_auto_prefix(self) -> None:
        """get_logger('mymodule') prepends 'oridecon.' prefix."""
        logger = get_logger("mymodule")
        assert logger.name == "oridecon.mymodule"

    def test_no_double_prefix(self) -> None:
        """get_logger('oridecon.di') does not double-prefix."""
        logger = get_logger("oridecon.di")
        assert logger.name == "oridecon.di"
...
class TestPerLoggerLevels:
    ...
    def test_levels_parameter_accepted(self) -> None:
        ...
        configure_logging(
            level="INFO",
            levels={"oridecon.di": "DEBUG", "oridecon.web": "WARNING"},
        )

    def test_level_filter_drops_below_threshold(self) -> None:
        """Events below per-logger level are dropped."""
        from oridecon.logging.processors import _level_filter_processor

        configure_logging(
            level="DEBUG",  # global allows DEBUG
            levels={"oridecon.noisy": "WARNING"},  # but noisy module is WARNING
        )

        event_dict = {"event": "test", "logger": "oridecon.noisy"}
    ...
    def test_level_filter_ignores_unconfigured_loggers(self) -> None:
        ...
        event_dict = {"event": "test", "logger": "oridecon.other"}
    ...
    def test_level_filter_matches_prefix(self) -> None:
        ...
        configure_logging(
            level="DEBUG",
            levels={"oridecon.di": "ERROR"},
        )

        # Sub-logger should inherit the prefix rule
        event_dict = {"event": "test", "logger": "oridecon.di.container"}
        with pytest.raises(structlog.DropEvent):
    ...
    def test_no_levels_means_no_filtering(self) -> None:
        ...
        event_dict = {"event": "test", "logger": "oridecon.anything"}
        result = _level_filter_processor(None, "debug", event_dict)
        assert result is event_dict
