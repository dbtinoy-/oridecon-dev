...
``OrideconConfig.validate_for_environment()`` (e.g. blocking
...
class TestBootTimeConfigValidation:
    ...
    async def test_debug_in_production_refuses_to_start(self):
        app = Application(
            config=OrideconConfig(debug=True, env=Environment.PRODUCTION)
        )
    ...
    async def test_valid_production_config_starts(self):
        app = Application(
            config=OrideconConfig(debug=False, env=Environment.PRODUCTION)
        )
        await app.start()
    ...
    async def test_debug_in_development_is_allowed(self):
        app = Application(
            config=OrideconConfig(debug=True, env=Environment.DEVELOPMENT)
        )
        await app.start()
        assert app.state == AppState.RUNNING
        await app.stop()
