...
class TestContainerExceptionHierarchy:
    ...
    def test_all_inherit_from_oridecon_error(self) -> None:
        """All container exceptions inherit from OrideconError."""
        assert issubclass(ContainerError, OrideconError)
        assert issubclass(DependencyError, OrideconError)
        assert issubclass(CircularDependencyError, OrideconError)
        assert issubclass(UnresolvableDependencyError, OrideconError)
        assert issubclass(RegistrationError, OrideconError)
        assert issubclass(ContainerBuildError, OrideconError)
        assert issubclass(ProtocolValidationError, OrideconError)
        assert issubclass(ContainerValidationError, OrideconError)
        assert issubclass(ScopedResolutionError, OrideconError)
...
class TestContainerErrorCodes:
    ...
    def test_protocol_validation_error_has_code(self) -> None:
        ...
        assert exc._code == "ORI_ERR_DI_007"
