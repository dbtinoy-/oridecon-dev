...
def _fake_entry_points(
    ...
    def _entry_points(*, group: str) -> list[Any]:
        if group == "oridecon.plugins":
            return descriptor_eps
        if group == "oridecon.providers":
            return provider_eps
        return []

    monkeypatch.setattr("oridecon.plugins.discovery._entry_points", _entry_points)
...
async def test_engine_skips_disabled_providers(
    ...
    from oridecon.plugins.state import save_disabled
...
async def test_engine_registers_via_shared_discovery_primitive(
    ...
    from oridecon.plugins import engine as engine_mod
