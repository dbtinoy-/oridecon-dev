...
The LifecycleManager must resolve OrideconConfig from the container and
...
class TestProviderConfigInjection:
    ...
    async def test_config_key_without_model_raises_at_registration(self) -> None:
        ...
        container = Container()
        container.singleton(OrideconConfig, OrideconConfig())
    ...
    async def test_config_key_without_lex_config_in_container_raises(self) -> None:
        """Declaring config_key when OrideconConfig is not registered raises ConfigurationError."""
        ...
        container = Container()
        # OrideconConfig intentionally NOT registered

        orchestrator = ProviderOrchestrator(container)
        orchestrator.add(OrphanProvider())

        with pytest.raises(ConfigurationError, match="OrideconConfig is not registered"):
            await orchestrator.register_all(container)
