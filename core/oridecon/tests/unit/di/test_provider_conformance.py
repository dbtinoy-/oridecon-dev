...
``oridecon-*/src/`` tree defines ``shutdown``, ``register``, or
...
#: ``oridecon/tests/unit/di/`` → ``oridecon/tests/unit/`` → ``oridecon/tests/``
#: → ``oridecon/`` → ``<repo-root>``
...
def _src_paths() -> list[Path]:
    """Return all ``oridecon-*/src`` and ``oridecon/src`` directories.

    Returns:
        Sorted list of existing source-tree root directories that belong to
        Oridecon extension packages plus the core ``oridecon`` package.
    """
    paths: list[Path] = sorted(_REPO_ROOT.glob("oridecon-*/src"))
    core_src = _REPO_ROOT / "oridecon" / "src"
    if core_src.is_dir():
...
def _discover_providers() -> list[type[Provider]]:
    ...
    Uses :func:`pkgutil.walk_packages` so that each ``oridecon-*`` namespace
