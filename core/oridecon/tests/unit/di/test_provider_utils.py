...
from oridecon.di.provider_utils import resolve_credential, resolve_optional
...
async def test_resolve_credential_prefers_secret_store(monkeypatch) -> None:
    store = _SecretStore({"ori_key": "stored-value"})
    monkeypatch.setenv("ORI_KEY", "env-value")

    result = await resolve_credential(store, "ori_key")
...
async def test_resolve_credential_falls_back_to_env_var(monkeypatch) -> None:
    monkeypatch.setenv("ORI_MISSING_FALLBACK", "env-value")

    result = await resolve_credential(None, "ori_missing_fallback")
