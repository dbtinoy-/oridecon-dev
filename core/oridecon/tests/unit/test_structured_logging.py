...
All logging must use structured logging via oridecon.logging.get_logger().
"""
...
def test_no_print_statements_in_core() -> None:
    """Verify that no print() calls exist in core oridecon source files."""
    issues: list[str] = []
    src_dir = Path(__file__).parent.parent.parent / "src" / "oridecon"
