...
class TestUserHandlingIntegration:
    ...
    def user_profile_projection(self):
        ...
        try:
            from oridecon.events.projections.user_profile import UserProfileProjection

            return UserProfileProjection()
        except ImportError:
            pytest.skip(
                "oridecon.events module not available in isolated testing environment",
            )
    ...
    async def test_context_aware_service_with_projection(
        ...
        with patch("oridecon.contracts.core.context.Context", mock_context):
