...
# module if the optional `oridecon.sql` package is not installed so CI can run
# without DB provider present.
import importlib

try:
    importlib.import_module("oridecon.sql.provider")
except ImportError:
    pytest.skip(
        "Integration DB tests require 'oridecon.sql' package",
...
from oridecon.contracts.exceptions import DatabaseError

pytest.importorskip("oridecon.di.modules.database")
from oridecon.config import DatabaseConfig
from oridecon.di.modules.database import DatabaseModule
from oridecon.di.providers.database import DatabaseService

# Optional imports: state store implementations live in specialized packages
try:
    from oridecon.cache.backends.memory_state import MemoryStateStore
except ImportError:
...
try:
    ...
    spec = importlib.util.find_spec("oridecon.sql.backends.postgres")
    if spec is None:
        PostgresStateStore = None
    else:
        try:
            mod = importlib.import_module("oridecon.sql.backends.postgres")
...
# Migration manager comes from `oridecon.sql`
try:
    from oridecon.sql.migrations.api import AlembicManager as MigrationManager
except ImportError:
    MigrationManager = None

# Database health check helper (optional)
try:
    from oridecon.di.providers.database import DatabaseHealthCheck
except ImportError:
