---
title: "oridecon Guide"
description: "Comprehensive guide to the Oridecon core framework — DI container, provider pattern, module system, and configuration."
---
...
## Overview
**Oridecon** is the core framework — an async-first DI/IoC platform for building large Python applications. It provides the plumbing that every extension package (`oridecon-web`, `oridecon-sql`, `oridecon-ai-*`, etc.) builds on top of:
...
Think of Oridecon as the **wiring harness** for your application. You declare what services you need (as typed constructor parameters), and the container delivers them. You never call `new` or instantiate dependencies manually.
...
```python
from oridecon import singleton, injectable, scoped, transient
...
```python
from oridecon import module, Module
...
# Utilities
from oridecon.result import collect, partition, as_result, try_catch
```
...
# From environment profile (reads ORI_ENV to determine environment)
...
import asyncio
from oridecon import Application, ProviderPriority, OrideconConfig
from oridecon.contracts.core.di import (
    BootContainerProtocol,
    ContainerRegistrarProtocol,
)
