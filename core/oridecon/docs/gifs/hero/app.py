...
"""
╔══════════════════════════════════════════════════════════╗
║  Oridecon API — from raw routes to production-ready     ║
╚══════════════════════════════════════════════════════════╝
...
os.environ.setdefault("ORI_LOGGING__LEVEL", "CRITICAL")
...
async def example2_quickstart():
    ...
    from oridecon.web.quickstart.core import (
    ...
    async with AsyncClient(
        ...
        resp = await client.get("/hello")
        print(f"  GET /hello -> {resp.status_code} {resp.json()}")
        resp = await client.get("/hello/Oridecon")
        print(f"  GET /hello/Oridecon -> {resp.status_code} {resp.json()}")
    _reset_quickstart_registry()
    _QUICKSTART_ROUTE_REGISTRY.clear()
...
async def example3_di():
    ...
    from oridecon.web.routing.decorators import get
    ...
    try:
        ...
        async with AsyncClient(
            ...
            resp = await client.get("/greet/Oridecon")
            print(f"  GET /greet/Oridecon -> {resp.status_code} {resp.json()}")
    finally:
        await app.stop()
...
async def example4_validation():
    ...
    from oridecon.web.di.provider import WebProvider
    from oridecon.web.routing.controllers import Controller
    from oridecon.web.routing.decorators import post
...
async def example5_middleware():
    ...
    from oridecon.web.config import CSRFConfig, WebConfig
    from oridecon.web.di.provider import WebProvider
    from oridecon.web.routing.controllers import Controller
    from oridecon.web.routing.decorators import get
...
async def example6_crud():
    ...
    from oridecon.web.di.provider import WebProvider
    from oridecon.web.routing.controllers import Controller
    from oridecon.web.routing.decorators import get, post
    from pydantic import BaseModel
    from starlette.responses import JSONResponse
...
# Oridecon apps (no mocks: real routes, DI, validation, caching)
# ═══════════════════════════════════════════════════════════
