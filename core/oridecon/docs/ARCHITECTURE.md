---
title: "oridecon Architecture"
description: "Architecture overview of the Oridecon core package and its role in the system."
---
# Architecture
...
`oridecon` is the **core package** — the foundation that every extension package builds on. It provides the DI container, application lifecycle, provider pattern, configuration system, and the `Result` type.

```
Extension packages (oridecon-web, oridecon-sql, …)
         ↑ depends on
    ┌────┴────┐
    │ oridecon │  ← DI container, Application, Provider, Module, Config, Result
    └────┬────┘
         ↑ depends on
    ┌────┴────────┐
    │ oridecon-contracts │  ← Protocols, types, exceptions (zero deps)
    └─────────────────┘
```
...
### Result (`oridecon.result`)

Re-exports from `oridecon.contracts.core.result`. The `Result[T, E]` type is a discriminated union:
...
Sources (ordered by priority, later wins):
  ...
  6. CLI options
         │
         ▼
   OrideconConfig (Pydantic/BaseConfig model)
         │
         ▼
   Container singleton (OrideconConfig + ConfigProtocol)
```
...
**Core providers** (included with `oridecon`):

| Provider | Name | Priority | Registers |
|----------|------|----------|-----------|
| `ConfigProvider` | `"config"` | `CRITICAL` | `OrideconConfig`, `ConfigProtocol` |
...
`oridecon` depends on protocols from `oridecon-contracts`:

| Protocol | Import | Used By |
|----------|--------|---------|
| `ContainerRegistrarProtocol` | `oridecon.contracts.core.di` | `Provider.register()` parameter |
| `ContainerResolverProtocol` | `oridecon.contracts.core.di` | Resolution API |
| `BootContainerProtocol` | `oridecon.contracts.core.di` | `Provider.boot()` parameter |
| `ConfigProtocol` | `oridecon.contracts.core.config` | `OrideconConfig` implements this |
| `ProviderPriority` | `oridecon.contracts.core.provider` | `Provider.priority` field |
| `HealthCheckResult` | `oridecon.contracts.core.health` | `Provider.health_check()` return |
| `ClockProtocol` | `oridecon.contracts.core.clock` | Ambient clock capability |
| `IdGeneratorProtocol` | `oridecon.contracts.core.identity` | Identity generation |
| `LoggerProtocol` | (oridecon.logging) | Structured logging |
