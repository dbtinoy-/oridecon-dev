---
title: "oridecon Security"
description: "Security considerations for the Oridecon core package — DI container, configuration, and ambient capabilities."
---
# Security
...
This document covers security considerations for the `oridecon` core package — primarily the DI container, configuration system, and ambient capabilities.
...
| Secrets leaked in config dumps | High | `OrideconConfig.to_dict(redact_secrets=True)` redacts fields matching secret patterns (`api_key`, `password`, `token`) |
...
- Authentication/authorization is handled by `oridecon-auth`
- Web security (CORS, CSRF, rate limiting) is handled by `oridecon-web`
- Encryption and hashing are handled by `oridecon.security.*` (optional `security` extras)
...
All configuration can be overridden via `ORI_` prefixed environment variables:

```bash
export ORI_SQL__BACKEND__URL="postgresql://app:secret@db.example.com/app"
```

### Secrets Validation

Oridecon validates secrets before any provider boots:

```python
from oridecon import Application
from oridecon.config.secrets import SecretsPolicy
import os
...
Oridecon provides three **ambient capabilities** — process-level singletons that are exceptions to the DI rule:

| Capability | Import | Risks | Secure Usage |
|------------|--------|-------|-------------|
| **Clock** | `from oridecon.primitives import clock` | Time-based attacks (prediction, replay) | Use `clock.now()` for timestamps; inject `FixedClock` in tests |
| **Identity** | `from oridecon import identity` | UUID predictability | `identity.new_uuid()` uses `uuid4()` (random) |
| **Hashing** | `from oridecon import hashing` | Weak hash choices | Default is SHA-256; `hashing.hash_hex(data)` |
...
| **Set `ORI_QUIET=1` in production** | Suppresses the startup banner which may leak version info |
...
Found a security vulnerability? Open an issue on [GitHub](https://github.com/dbtinoy-/oridecon/issues) with the label `security`.
