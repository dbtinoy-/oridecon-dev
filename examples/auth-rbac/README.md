...
> Module name: `rbac_console` — run with `PYTHONPATH=examples/auth-rbac/src uv run python -m rbac_console`
...
```bash
uv run pytest examples/auth-rbac/tests -q
```
