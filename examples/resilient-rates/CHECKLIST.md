...
> `examples/<slug>/CHECKLIST.md` (delete any line that genuinely does not apply,
> never leave it half-ticked). Source of truth: Blueprint §6 in
> `specs/2026-08-25-examples-code-alignment.md`.

Demo: `resilient-rates` · Port: `7073` · Reviewed: `2026-08-25` (Wave 0 Task A reference retrofit)
...
- [x] `uv run --group tooling pytest examples/<dir>/tests -q`
- [x] `uv run ruff check examples/<dir> && uv run ruff format --check examples/<dir>`
- [x] `uv run mypy examples/<dir>/src` (via `make type-examples`)
- [x] `python -m compileall -q examples/<dir>`
- [x] smoke entry exercised (`make smoke-examples`)

## Fleet & docs

- [x] Registered in hub `registry.py` (correct slug/port/module triple)
- [x] Makefile `EXAMPLE_TEST_DIRS`, `EXAMPLE_COMPILE_DIRS`, `smoke-examples` updated
- [x] `examples/README.md`: at-a-glance section + running command row
- [x] docs.oridecon.dev/examples table row added
