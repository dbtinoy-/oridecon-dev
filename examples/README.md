...
- **Hub console** — `http://127.0.0.1:7000` lists every demo with live
  status (`PYTHONPATH=examples/demo-hub/src uv run python -m demo_hub`, :7000)

The launchpad for visitors:

- **Single process** — the hub boots each demo's real `Application`
  in-process and mounts it under `/examples/<slug>/`; no other ports needed
...
(cd examples/demo-hub && PYTHONPATH=src uv run python -m demo_hub)                 # fleet console (:7000)
...
make test-examples                                                              # every demo test suite
```
...
- **Tests** — every demo runs its suite in the workspace env
  (`make test-examples`)
- **Compile check** — demo sources are compile-gated (`make verify-examples`)
- **One command** — `make check-examples` runs tests + compile checks and is
