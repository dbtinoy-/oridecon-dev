...
> Module name: `mfa_console` — run with `PYTHONPATH=examples/auth-mfa/src uv run python -m mfa_console`
...
```bash
uv run pytest examples/auth-mfa/tests -q
```
