...
> Module name: `memory_chat` — run with `PYTHONPATH=examples/memory-chat/src uv run python -m memory_chat`
...
```bash
uv run pytest examples/memory-chat/tests -q
```
