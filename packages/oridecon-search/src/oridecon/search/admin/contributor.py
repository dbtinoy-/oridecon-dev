"""Admin contributor for oridecon-search.

Registers the ``oridecon.admin.contributors`` entry point for oridecon-search
and surfaces the global search page (``/admin/search``, served by
oridecon-admin) in the admin navigation.  Depends only on
``oridecon-contracts`` + ``oridecon``.
"""
...
class SearchAdminContributor(BaseAdminContributor):
    """Admin contributor for the oridecon-search package.
    ...
    def get_navigation_items(self) -> Sequence[NavigationContribution]:
        ...
        Returns:
            ...
            page (``/admin/search``, served by oridecon-admin).
        """
    ...
    def get_management_pages(self) -> Sequence[ManagementPageDefinition]:
        ...
        Returns:
            ...
            oridecon-admin core at ``/admin/search`` (wired in via the
