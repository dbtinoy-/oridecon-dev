...
Lives in ``oridecon-search`` (``oridecon.search.backends.postgres``).
The implementation depends only on ``DatabaseProviderProtocol`` from
``oridecon.contracts`` so it carries no dependency on ``oridecon-sql``
...
from oridecon.search.backends.filters import render_postgres
