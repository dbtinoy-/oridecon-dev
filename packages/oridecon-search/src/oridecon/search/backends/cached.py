...
``oridecon.search.lib.caching`` with a clean decorator that delegates
caching to the ``CacheBackendProtocol`` from contracts.

This keeps ``oridecon-search`` from reimplementing caching infrastructure
...
from oridecon import hashing as ambient_hashing
