---
title: oridecon-search Backends
description: Supported search backends in oridecon-search — MeiliSearch, Elasticsearch, OpenSearch, Typesense, SQL-native, and in-memory
---

`oridecon-search` supports multiple full-text search backends through a common `SearchEngine` protocol. Each backend implements indexing, querying, filtering, faceting, and health-checking against the same interface, so you can switch between them with a config change.
