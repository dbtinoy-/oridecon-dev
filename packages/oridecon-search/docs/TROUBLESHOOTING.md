---
title: oridecon-search Troubleshooting
...
**Error:** `BackendError` (code `ORI_ERR_SEARCH_003`)
...
**Error:** `SearchValidationError` (code `ORI_ERR_SEARCH_004`)
...
```python
from oridecon.search import validate_search_query, sanitize_search_query
...
**Error:** `SearchIndexError` (code `ORI_ERR_SEARCH_009`)
...
```python
from oridecon.contracts.search import IndexManagerProtocol
...
**Cause:** A DB-backed backend (POSTGRES, MYSQL) is configured but `oridecon-sql` is not installed or `DatabaseProvider` is not registered.

**Fix:** Install `oridecon-sql` and add its provider:
...
```python
from oridecon.search.types import SearchStrategy
