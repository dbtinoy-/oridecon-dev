...
`oridecon-search` wraps any search backend behind `SearchEngineProtocol` and manages the provider lifecycle, index management, and result mapping.
...
```python
from oridecon.search import SearchResponse, SearchResult, SearchQuery
...
```python
from oridecon.search import SearchProvider
from oridecon.search.config import SearchConfig, BackendType, MeiliSearchConfig
from oridecon.contracts.search import SearchEngineProtocol
...
```python
from oridecon.search import FederatedSearchEngine
...
```python
from oridecon.contracts.search import DocumentTransformerProtocol
...
```python
from oridecon.search import validate_search_query, sanitize_search_filters
