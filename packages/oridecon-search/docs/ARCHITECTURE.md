...
Internal design of the `oridecon-search` package.

---

## Role in the System

`oridecon-search` provides **full-text and faceted search** as a pluggable abstraction layer. It sits between domain services and search backends (Elasticsearch, MeiliSearch, Typesense, SQL-based FTS), translating `SearchEngineProtocol` calls into backend-specific operations.
...
flowchart BT
    ...
    subgraph Search[oridecon-search]
        SE[SearchEngine]
        QB[QueryBuilder]
        DI[DocumentIndexer]
...
The `DocumentTransformer` extends `ReadOnlyMapper[dict, dict]` from `oridecon.data.mapper`. Transformation pipelines consist of ordered `TransformationRule` instances, each with an optional condition and error handling:
...
Beyond simple flat conditions, the builder accepts composable `FilterExpression` trees (`AndExpression`, `OrExpression`, `NotExpression`) from `oridecon.contracts.data` via `filter_expr()`.
...
|----------|----------------------|---------------|
| `SearchEngineProtocol` | `oridecon.contracts.search` | `SearchEngine` ABC, `DefaultSearchEngine`, `FederatedSearchEngine`, each backend, `CachedSearchBackend` |
| `SearchableProtocol` | `oridecon.contracts.search` | `SearchableModel` |
| `IndexManagerProtocol` | `oridecon.contracts.search` | Per-backend index managers |
| `SearchAnalyticsProtocol` | `oridecon.contracts.search` | `SearchAnalyticsRecorder` |
| `DocumentTransformerProtocol` | `oridecon.contracts.search` | `DocumentTransformer`, `DefaultDocumentTransformer` |
| `DatabaseSearchBackendProtocol` | `oridecon.contracts.search` | Postgres/MySQL backends |
| `CacheBackendProtocol` | `oridecon.contracts.infra.cache` | `CachedSearchBackend` (wraps an inner backend) |
| `DatabaseProviderProtocol` | `oridecon.contracts.data` | Resolved at boot for SQL-backed backends |
| `FilterExpression` | `oridecon.contracts.data` | `SearchQueryBuilder.filter_expr()` |
| `HookRegistryProtocol` | `oridecon.contracts.hooks` | `SearchIndexedHook`, `SearchQueryExecutedHook` |
| `EventBusProtocol` | `oridecon.contracts.events` | `IndexingCompletedEvent`, `SearchExecutedEvent` |
...
```python
from oridecon.search.backends.cached import CachedSearchBackend
...
```python
from oridecon.search.query import SuggestionEngine
