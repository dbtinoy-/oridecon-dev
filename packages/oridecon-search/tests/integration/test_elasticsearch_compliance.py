...
from oridecon.testing.compliance import SearchEngineCompliance
from oridecon.testing.integration.fixtures import (  # noqa: F401
...
class TestElasticsearchCompliance(SearchEngineCompliance):
    ...
    provided by ``oridecon.testing.integration.fixtures``.  The suite is
    ...
    async def create_engine(self) -> object:
        ...
        """
        try:
            from oridecon.search.backends.elasticsearch.backend import (
                ElasticsearchBackend,  # noqa: F401
            )
            from oridecon.search.config import ElasticsearchConfig  # noqa: F401
        except ImportError:
