...
from oridecon.search.exceptions import CacheError
from oridecon.search.lib.caching import (
    CacheConfig,
    CacheEntry,
    CacheManager,
    QueryCache,
...
class TestSearchCache:
    ...
    async def test_get_expired(self, cache: SearchCache) -> None:
        ...
        with patch('oridecon.search.lib.caching.ambient_clock.now', return_value=datetime.now(timezone.utc)):
            result = await cache.get(key)
            assert result is None
    ...
    async def test_generate_key(self, cache: SearchCache) -> None:
        ...
        with patch('oridecon.search.lib.caching.ambient_hashing') as mock_hashing:
