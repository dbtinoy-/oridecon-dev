"""P2 hook surface import verification for oridecon-search."""
...
def test_search_hooks_root_module_exists() -> None:
    import oridecon.search
    from oridecon.search.hooks import (
        SearchIndexedHook,
    ...
    assert oridecon.search.SearchIndexedHook is SearchIndexedHook
    assert oridecon.search.SearchQueryExecutedHook is SearchQueryExecutedHook


def test_search_hook_payloads_are_frozen_and_keyword_only() -> None:
    from oridecon.search.hooks import SearchIndexedHook, SearchQueryExecutedHook
