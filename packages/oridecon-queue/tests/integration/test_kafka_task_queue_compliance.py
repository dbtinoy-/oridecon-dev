...
from oridecon.testing.compliance import QueueBackendCompliance
...
class TestKafkaQueueCompliance(QueueBackendCompliance):
    ...
    ``oridecon.testing.integration.fixtures``.  A unique Kafka topic name is
    ...
    async def _setup(self, kafka_producer: object) -> None:
        ...
        self._topic = f"oridecon-test-{uuid.uuid4().hex[:12]}"
    ...
    async def create_backend(self, queue_name: str = "test-queue") -> object:
        ...
        """
        try:
            from oridecon.queue.backends.kafka import KafkaQueue  # noqa: F401
        except ImportError:
