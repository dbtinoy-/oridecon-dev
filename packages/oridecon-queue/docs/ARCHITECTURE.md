...
Internal design of the `oridecon-queue` package.

---

## Role in the System

`oridecon-queue` provides the **message bus and queue** abstraction for Oridecon applications. It sits in the Messaging tier alongside `oridecon-events` (CQRS/ES) and `oridecon-tasks` (background jobs).

- **Depends on:** `oridecon`, `oridecon-contracts`
...
Messages are defined as a frozen dataclass in `oridecon-contracts`:
...
| `redis` | `RedisQueue` | `oridecon-queue[redis]` | `RedisDriverConfig` |
| `rabbitmq` | `RabbitMQQueue` | `oridecon-queue[rabbitmq]` | `RabbitMQDriverConfig` |
| `kafka` | `KafkaQueue` | `oridecon-queue[kafka]` | `KafkaDriverConfig` |
| `sqs` | `SQSQueue` | `oridecon-queue[sqs]` | `SQSDriverConfig` |
| `azure_servicebus` | `AzureServiceBusQueue` | `oridecon-queue[azure]` | `AzureServiceBusDriverConfig` |
| `gcp_pubsub` | `GCPPubSubQueue` | `oridecon-queue[gcp]` | `GCPPubSubDriverConfig` |
...
All queue contracts live in `oridecon-contracts` and are consumed by `oridecon-queue` without redefining anything:
...
| `QueueError` | `queue.errors` | Base domain error (`ORI_ERR_QUEUE_001`) |
...
One leaf exception per backend (`RedisQueueError`, `RabbitMQQueueError`, `KafkaQueueError`, `SQSQueueError`, `AzureServiceBusQueueError`, `GCPPubSubQueueError`), all extending `QueueError` with codes `ORI_ERR_QUEUE_002`–`007`.
...
| Custom serialization | Backend uses `oridecon.serialization` — swap via config |
