---
title: oridecon-queue Configuration
...
All keys can be overridden with environment variables using the prefix `ORI_QUEUE__`:

```bash
ORI_QUEUE__BACKENDS__0__DRIVER=redis \
ORI_QUEUE__BACKENDS__0__NAME=cache \
ORI_QUEUE__BACKENDS__0__PRIMARY=true \
ORI_QUEUE__BACKENDS__0__REDIS__URL=redis://localhost:6379/0
```
...
| `exchange` | `str` | `"oridecon"` | Exchange name |
...
| `client_id` | `str` | `"oridecon"` | Client ID |
| `group_id` | `str` | `"oridecon-consumers"` | Consumer group ID |
...
```bash
ORI_QUEUE__BACKENDS__0__NAME=default \
ORI_QUEUE__BACKENDS__0__DRIVER=redis \
ORI_QUEUE__BACKENDS__0__PRIMARY=true \
ORI_QUEUE__BACKENDS__0__MAX_RETRIES=5 \
ORI_QUEUE__BACKENDS__0__REDIS__URL=redis://localhost:6379/1 \
ORI_QUEUE__BACKENDS__0__REDIS__MAX_CONNECTIONS=20
```
