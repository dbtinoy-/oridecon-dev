---
title: oridecon-queue Backends
description: Compare Redis, RabbitMQ, Kafka, SQS, and in-memory backends for oridecon-queue
sidebar:
  order: 3
---

`oridecon-queue` provides pluggable queue backends behind the `QueueProtocol` contract from `oridecon-contracts`. All backends support the same publish/consume/dead-letter API with per-backend configuration.

## Supported Backends

| Backend | Extra | Driver | Production-ready | Best for |
|---------|-------|--------|-----------------|----------|
| Redis | `oridecon-queue[redis]` | `redis[asyncio]>=5.0` | Yes | Simple queues, task distribution |
| RabbitMQ | `oridecon-queue[rabbitmq]` | `aio-pika>=9.0` | Yes | Reliable messaging, routing, RPC |
| Kafka | `oridecon-queue[kafka]` | `aiokafka>=0.10` | Yes | High-throughput event streaming |
| SQS | `oridecon-queue[sqs]` | `aiobotocore>=2.0` | Yes | AWS-managed queues, serverless |
| Memory | (core) | in-process `queue.Queue` | Development / test | Local dev, unit tests |

Install extras: `pip install oridecon-queue[redis,rabbitmq,kafka,sqs]`.
