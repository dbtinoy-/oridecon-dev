...
``oridecon.contracts.data.outbox`` with the SQL implementation in
``oridecon.sql.outbox`` (``SQLOutboxStore`` + ``OutboxPublisher``).
"""
...
class BatchedPublisher:
    ...
    Note:
        ...
        (``OutboxStoreProtocol`` in ``oridecon.contracts.data.outbox`` with
        ``oridecon.sql.outbox.SQLOutboxStore`` and
        ``oridecon.sql.outbox.OutboxPublisher``) inside the caller's
