"""QueueModule — IoC module for oridecon-queue."""
...
class QueueModule(Module):
    ...
    Registers :class:`~oridecon.contracts.queue.protocols.QueueProtocol` for
    ...
    def configure(cls, config: QueueConfig | Any | None = None) -> DynamicModule:
        ...
        Args:
            config: :class:`~oridecon.queue.config.QueueConfig` or ``None``
                to use defaults (reads from environment variables).

        Returns:
            A :class:`~oridecon.queue.module.DynamicModule` descriptor.
        """
        from oridecon.queue.admin.contributor import QueueAdminContributor
        from oridecon.queue.admin.handlers.consumer_lag import ConsumerLagWidgetHandler
        from oridecon.queue.admin.handlers.failed_messages import (
            FailedMessagesWidgetHandler,
        )
        from oridecon.queue.admin.handlers.queue_depth import QueueDepthWidgetHandler
    ...
    def scope(cls, *consumers: type, **kwargs: Any) -> DynamicModule:
        ...
        **not** create a new queue connection.
        ...
        Returns:
            A :class:`~oridecon.di.module.DynamicModule` scoped to this feature.
        """
        token = type(
            f"_{cls.__name__}Scope",
            (cls,),
            {"__oridecon_scope_of__": cls},
        )
        return DynamicModule(
            module=token,
    ...
    def stub(cls, config: QueueConfig | None = None) -> DynamicModule:
        ...
        """
        from oridecon.queue.config import NamedQueueConfig, QueueConfig
        from oridecon.queue.di.provider import QueueProvider
