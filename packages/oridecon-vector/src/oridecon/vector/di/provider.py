"""DI provider for oridecon-vector."""
...
from oridecon.vector.backends.registry import VectorStoreRegistry
from oridecon.vector.config import VectorConfig
from oridecon.vector.constants import (
    BACKEND_PGVECTOR,
)
...
class VectorProvider(Provider):
    ...
    def _create_store(self, config: VectorConfig) -> Any:
        ...
        :class:`~oridecon.vector.backends.pgvector.PgVectorStore` requires a
    ...
    async def _maybe_wrap_with_tenancy(
        ...
        from oridecon.vector.tenancy.decorator import TenantVectorStoreDecorator
        from oridecon.vector.tenancy.resolver import (
        ...
        if kind == "pinecone_namespace":
            from oridecon.vector.tenancy.pinecone_namespace import (
