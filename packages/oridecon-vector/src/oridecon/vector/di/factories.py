...
Wraps an infra ``VectorStoreProtocol`` (from oridecon-vector) in a
...
async def create_vector_store(
    ...
    *,
    ...
    When ``infra_store`` is provided (registered by oridecon-vector's provider),
    a :class:`~oridecon.vector.adapters.vector_store.VectorStoreAdapter` is returned
    that wraps it. Otherwise, a
    :class:`~oridecon.vector.testing.mocks.MockVectorStore` is returned — suitable
    for testing and environments without an infra store.

    Args:
        config: Vector configuration block.
        infra_store: Infra-level vector store (from oridecon-vector), if present.
    ...
    if config.enable_cache and cache_backend:
        from oridecon.vector.embedding.cache import EmbeddingCache
    ...
    if config.backend != "mock":
        logger.warning(
            ...
            "Add oridecon-vector's VectorModule to your app to use a real backend.",
            config.backend,
        )
