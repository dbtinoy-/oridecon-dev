...
from oridecon.vector.backends.base import BaseVectorCollection
...
class WeaviateCollection(BaseVectorCollection):
    ...
    async def upsert(self, records: list[VectorRecord]) -> UpsertResult:
        ...
        Returns:
            :class:`~oridecon.contracts.data.vector.UpsertResult`.
        """
        try:
    ...
    async def search(self, query: SearchQuery) -> list[SearchResult]:
        ...
        Returns:
            List of :class:`~oridecon.contracts.data.vector.SearchResult`
        ...
        try:
            ...
            if query.filter is not None:
                from oridecon.vector.backends.weaviate.filters import (
    ...
    async def get(self, ids: list[str]) -> list[VectorRecord]:
        ...
        Returns:
            Found :class:`~oridecon.contracts.data.vector.VectorRecord` objects
    ...
    async def delete(self, ids: list[str]) -> DeleteResult:
        ...
        Returns:
            :class:`~oridecon.contracts.data.vector.DeleteResult`.
        """
        deleted = 0
        for obj_id in ids:
            try:
    ...
    async def delete_by_filter(self, filter: Any) -> DeleteResult:  # noqa: A002
        ...
        Args:
            filter: A :class:`~oridecon.contracts.data.vector.filters.MetadataFilter`
                (``MetadataCondition`` or ``MetadataConditionGroup``) describing
                which records to delete.

        Returns:
            :class:`~oridecon.contracts.data.vector.DeleteResult` with
