...
from oridecon.vector.config import NamedVectorConfig, VectorConfig
...
async def test_multi_backend_populates_store_services_per_entry() -> None:
    ...
    with patch(
        "oridecon.vector.backends.memory.MemoryVectorStore",
...
async def test_multi_backend_boot_uses_asyncio_gather() -> None:
    ...
    with patch("oridecon.vector.di.provider.asyncio.gather", side_effect=spy_gather):
        await provider.boot(container)
...
async def test_single_backend_health_check_path_unchanged() -> None:
    ...
    with patch(
        "oridecon.vector.backends.memory.MemoryVectorStore", return_value=mock_store
    ):
        await provider.boot(container)
