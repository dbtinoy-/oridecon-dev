...
Internal design of the `oridecon-vector` package.

---

## Role in the System

`oridecon-vector` is the vector store abstraction layer. It implements `VectorStoreProtocol` and `VectorCollectionProtocol` from `oridecon-contracts`, providing pluggable backends for embedding storage and similarity search. It is an infrastructure dependency for AI workloads (RAG, memory, agents) and is consumed through DI — never imported directly.

```mermaid
flowchart LR
    AI[AI Workloads<br/>RAG · Memory · Agents]
    Adapter[VectorStoreAdapter<br/>DocumentVectorStoreAdapter]
    VS[oridecon-vector<br/>VectorStoreProtocol]
...
**Import direction:** AI packages depend on `oridecon-vector` through contracts and DI — no direct imports. The `VectorStoreProtocol` lives in `oridecon-contracts`; `oridecon-vector` provides implementations.

---

## Backend Abstraction

All backends implement `VectorStoreProtocol` (collection lifecycle) and `VectorCollectionProtocol` (vector CRUD) from `oridecon.contracts.data.vector.protocols`. The abstract base classes `BaseVectorStore` and `BaseVectorCollection` provide shared initialization logic.
...
├── cli/
│   ├── contributor.py       # CLI contributor for oridecon-cli
...
From `oridecon-contracts`:

| Protocol / Type | Module |
|----------------|--------|
| `VectorStoreProtocol` | `oridecon.contracts.data.vector.protocols` |
| `VectorCollectionProtocol` | `oridecon.contracts.data.vector.protocols` |
| `DistanceMetric` | `oridecon.contracts.data.vector.enums` |
| `IndexType` / `IndexState` | `oridecon.contracts.data.vector.enums` |
| `CollectionConfig` / `CollectionInfo` | `oridecon.contracts.data.vector.types` |
| `SearchQuery` / `SearchResult` | `oridecon.contracts.data.vector.types` |
| `VectorRecord` / `UpsertResult` / `DeleteResult` | `oridecon.contracts.data.vector.types` |
| `MetadataCondition` / `MetadataConditionGroup` / `Filter` | `oridecon.contracts.data.vector.filters` |
| `FilterOperator` / `MetadataFilter` | `oridecon.contracts.data.vector.filters` |
| `Document` / `RAGSearchResult` | `oridecon.contracts.ai.vector` |
| `DatabaseProviderProtocol` | `oridecon.contracts.data.sql.database` (pgvector only) |
| `CacheBackendProtocol` | `oridecon.contracts.infra.cache` (optional) |
| `HealthCheckResult` / `HealthStatus` | `oridecon.contracts.core.health` |
| `ProviderPriority` | `oridecon.contracts.core` |
...
```mermaid
flowchart LR
    subgraph Contract[oridecon-contracts]
        IE[InfrastructureError]
    end
    subgraph Vector[oridecon-vector]
        VE[VectorError]
...
```python
# oridecon/vector/di/provider.py
...
| CLI commands | Add entries via `VectorCliContributor` | `oridecon vector status` |
| Testing mocks | Use `MockVectorStore` or `MockVectorStoreWithErrors` | `from oridecon.vector.testing.mocks import MockVectorStore` |
