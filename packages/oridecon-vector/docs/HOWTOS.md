...
```python
from oridecon.vector import VectorConfig
...
results = await retriever.search("What is Oridecon?", k=10)
```
...
reranked = await reranker.rerank(query="What is Oridecon?", documents=results, top_k=5)
