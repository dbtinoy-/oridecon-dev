---
title: oridecon-vector Backends
description: Supported vector store backends in oridecon-vector — pgvector, Pinecone, Qdrant, Chroma, Weaviate, and in-memory
---

`oridecon-vector` provides a unified `VectorStoreProtocol` for vector similarity search across multiple backends. Each backend implements collection management, vector upsert, similarity search, filter compilation, and health checking through the same interface.
...
```python
from oridecon.vector import VectorProvider, VectorConfig
...
`oridecon-vector` provides an `OpenAICompatibleEmbeddingClient` for generating embeddings before upsert, and `EmbeddingCache` to avoid re-computing embeddings for duplicate content.
