# Vector Stores in oridecon-vector

Deep-dive companion to the `oridecon-vector` README: store operations, embedding clients and caches, hybrid retrieval, reranking, adapters, AI/RAG integration, and multi-tenancy.
...
```python
from oridecon.contracts.data.vector.enums import DistanceMetric, IndexType
...
```python
from oridecon.vector.embedding.cache import InMemoryEmbeddingCache
...
```python
from oridecon.vector.search.hybrid import HybridRetriever, HybridSearchConfig
...
While `oridecon-vector` is a general-purpose vector storage layer, it integrates
seamlessly with AI and RAG features:

- **`oridecon-ai-rag`** — Uses `VectorStoreProtocol` for retrieval-augmented generation
- **`oridecon-ai`** — Uses embedding clients and vector stores for semantic search and memory
- **`oridecon-cache`** — Can use `SemanticCacheProtocol` (backed by vector stores) for semantic caching
...
`oridecon-vector` supports per-tenant isolation via resolved collection names.
...
from oridecon.vector.config import VectorTenancyConfig, VectorConfig
...
(`oridecon.primitives.context.TENANT_ID`). Tenants with the same logical
