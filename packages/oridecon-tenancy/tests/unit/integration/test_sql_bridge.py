...
from oridecon.primitives.context import TENANT_ID, DEFAULT_KEYS, Context, ContextVarRegistry
...
async def test_bridge_passes_through_non_http_scope() -> None:
    ...
    try:
        ...
        pytest.skip("oridecon-sql not installed")
...
def test_bridge_importable_with_sql() -> None:
    """TenantSQLContextBridge is importable when oridecon-sql is installed."""
    try:
        from oridecon.tenancy.integration.sql_bridge import TenantSQLContextBridge
...
async def test_bridge_propagates_tenant_to_real_db_context() -> None:
    ...
    try:
        from oridecon.sql.context import create_db_context
        from oridecon.tenancy.integration.sql_bridge import TenantSQLContextBridge
    except ImportError:
        pytest.skip("oridecon-sql not installed")
