...
| `oridecon-sql` | Optional | Schema-per-tenant |
| `oridecon-cache` | Optional | Cached tenant config |
...
Doing this ad-hoc leads to scattered tenant checks, inconsistent isolation, and subtle data leaks. `oridecon-tenancy` solves this with a declarative resolution chain, pluggable validation, and strategy-based isolation.
...
- **oridecon-web** — `TenantContextMiddleware` registers automatically as ASGI middleware when `oridecon-web` is present
- **oridecon-cache** — `TenantCacheKeyDecorator` prefixes cache keys with the active tenant ID (opt-in via `IntegrationConfig`)
- **oridecon-sql** — `TenantSQLContextBridge` syncs the tenant context into SQL scoped sessions (opt-in via `IntegrationConfig`)
