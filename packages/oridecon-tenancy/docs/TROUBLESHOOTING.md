---
title: oridecon-tenancy Troubleshooting
...
**Cause:** `TenantContextMiddleware` was not registered, or `oridecon-web` is not installed.  
**Fix:** Verify `oridecon-web` is installed and the middleware registry is available. Check boot logs for `tenant_context_middleware_registered`.
...
**Cause:** The database user lacks `CREATE SCHEMA` permissions, or `oridecon-sql` is not installed.  
**Fix:** Grant schema creation permissions. Install `oridecon-tenancy[sql]`. Switch to row-level isolation as a temporary workaround.
...
**Cause:** `IntegrationConfig.sql_context_bridge` is `False`, or `oridecon-sql` is not installed.  
**Fix:** Set `sql_context_bridge: true` in tenancy config. Verify `oridecon-sql` is installed.
...
**Cause:** The isolation strategy (`row_level` or `schema`) failed during `auto_provision_isolation`. Common causes: missing database permissions, or `oridecon-sql` not installed for schema-based isolation.
...
```python
from oridecon.tenancy.config_overrides import TenantConfigService
