---
title: oridecon-tenancy Strategies
description: Isolation models, resolution strategies, configuration, migration, and testing
---

## Isolation Models

`oridecon-tenancy` provides three isolation strategies, each with a different tradeoff between isolation strength, operational cost, and complexity.
...
| **Migration effort** | Requires `oridecon-tenancy[sql]` extra |
...
| **Complexity** | `tenant_id` in every query (handled by `oridecon-sql`'s `TenantScope`) |
...
```python
from oridecon.tenancy import TenantContextMiddleware
from oridecon.primitives.context import TENANT_ID, Context
...
```python
from oridecon.tenancy import TenantGuard
...
```python
from oridecon.tenancy import InMemoryTenantProvider, TenancyProvider, TenancyConfig
from oridecon.tenancy.resolution.registry import ResolverRegistry
from oridecon.primitives.context import Context, TENANT_ID
