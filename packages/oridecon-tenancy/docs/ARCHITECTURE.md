...
Internal design of the `oridecon-tenancy` package.
...
```mermaid
flowchart BT
    Web[oridecon-web<br/>ASGI routing · middleware chain]
    Tenancy[oridecon-tenancy<br/>Resolution · Enforcement · Lifecycle<br/>Isolation · Config Overrides]
    Sql[oridecon-sql<br/>DB access · query scoping]
    Cache[oridecon-cache<br/>Key-value cache]
    Events[oridecon-events<br/>Domain event bus]
    Auth[oridecon-auth<br/>JWT · authentication]
...
The sub-provider pattern mirrors `oridecon-auth`'s `AuthBundleProvider` — each sub-provider has a single responsibility and can be reasoned about independently.

---

## Contracts Used

From `oridecon-contracts` (`oridecon.contracts.tenancy.*`):
...
The `TenancyConfig` has four sub-configs loaded from the `tenancy:` key in `application.yaml` with `ORI_TENANCY__*` env var overrides:

```python
# oridecon/tenancy/di/provider.py
...
Registration creates the resolver chain (populated via `ResolverRegistry.from_config()`), the default in-memory tenant store, the isolation strategy registry, the cached config provider, and integration bridges (cache key prefixing and SQL context propagation). Boot wires the ASGI middleware into `oridecon-web`'s middleware registry when available.
...
| Event handling | Domain events (TenantProvisioned, etc.) | Published to event bus | Subscribe via oridecon-events handlers |
| CLI generators | `oridecon tenancy gen resolver` | Resolver scaffold | Extend `cli/generators/resolver.py` |
