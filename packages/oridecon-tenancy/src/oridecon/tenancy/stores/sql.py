...
Available when ``oridecon-tenancy[sql]`` is installed.
Raises ``ImportError`` if ``oridecon-sql`` is not available.
"""
...
from oridecon.contracts.tenancy.errors import TenantError, TenantNotFoundError
from oridecon.contracts.tenancy.types import TenantInfo, TenantStatus
...
class SQLTenantProvider:
    """SQL-backed tenant store using the ``oridecon-sql`` repository pattern.

    Requires ``oridecon-tenancy[sql]`` and a ``tenants`` table with columns:
    ...
    def __init__(self, db: DatabaseProviderProtocol) -> None:
        ...
        Args:
            db: Database provider implementing
                :class:`~oridecon.contracts.data.DatabaseProviderProtocol`.
        """
        self._db = db
    ...
    async def get_tenant(self, tenant_id: str) -> TenantInfo | None:
        ...
        Returns:
            :class:`~oridecon.contracts.tenancy.types.TenantInfo` or ``None``.
        """
    ...
    async def list_tenants(self, *, active_only: bool = True) -> list[TenantInfo]:
        ...
        Returns:
            List of :class:`~oridecon.contracts.tenancy.types.TenantInfo`.
        """
        if active_only:
    ...
    def _row_to_tenant(row: Any) -> TenantInfo:
        ...
        Returns:
            Parsed :class:`~oridecon.contracts.tenancy.types.TenantInfo`.
        """
        if hasattr(row, "get"):
            data = dict(row)
        else:
