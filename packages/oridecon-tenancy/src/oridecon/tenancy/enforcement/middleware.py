...
class TenantContextMiddleware:
    ...
    :class:`~oridecon.primitives.context.Context` and stores the
    :class:`~oridecon.contracts.tenancy.types.TenantInfo` in
    ``scope["state"]["tenant"]`` for downstream use by
    :class:`~oridecon.tenancy.enforcement.guard.TenantGuard`.  The context
    ...
    def __init__(
        ...
        Args:
            ...
            resolver: :class:`~oridecon.tenancy.resolution.chain.CompositeResolver`
                instance.
            validator: :class:`~oridecon.tenancy.enforcement.validator.TenantValidator`
                instance.
            ctx: The shared :class:`~oridecon.primitives.context.Context`.
        """
        self._app = app
    ...
    def _build_resolution_context(scope: Scope) -> TenantResolutionContext:
        """Build a :class:`~oridecon.contracts.tenancy.types.TenantResolutionContext`
        from the ASGI scope.
