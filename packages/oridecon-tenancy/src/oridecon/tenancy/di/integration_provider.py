...
from oridecon.tenancy.config import IntegrationConfig
from oridecon.tenancy.integration.cache_decorator import TenantCacheKeyDecorator
...
class TenantIntegrationProvider(Provider):
    ...
    - Cache key decoration: skipped if ``oridecon-cache`` is not installed.
    - SQL context bridge: skipped if ``oridecon-sql`` is not installed.
    """
    ...
    async def _boot_sql_bridge(self, container: BootContainerProtocol) -> None:
        ...
        Args:
            container: The DI container for boot phase.
        """
        from oridecon.contracts.web.middleware import MiddlewareRegistryProtocol
        from oridecon.primitives.context import Context
        from oridecon.tenancy.integration.sql_bridge import TenantSQLContextBridge
