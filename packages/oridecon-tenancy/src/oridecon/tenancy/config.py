...
from oridecon.tenancy.constants import (
...
class IntegrationConfig:
    ...
    Attributes:
        cache_key_prefix: When ``True``, tenant-prefix cache keys via
            :class:`~oridecon.tenancy.integration.cache_decorator.TenantCacheKeyDecorator`.
        sql_context_bridge: When ``True``, sync the core ``TENANT_ID`` context
            key into oridecon-sql's DB context via
            :class:`~oridecon.tenancy.integration.sql_bridge.TenantSQLContextBridge`.
    """
