...
Internal design of the `oridecon-storage` package.

---

## Role in the System

`oridecon-storage` lives in the **Data & Persistence** tier. It implements `BlobStoreProtocol` and `StorageDriverProtocol` from `oridecon-contracts` and provides pluggable drivers for S3, GCS, Azure Blob, Cloudflare R2, local filesystem, and in-memory storage.
...
| `register()` | Resolves config; validates driver config; creates driver via `DriverRegistry`; binds `BlobStoreProtocol` (singleton). In multi-backend mode, registers named bindings per entry. Discovers `oridecon.storage.backends` entry points |
...
|----------|--------|---------------|
| `BlobStoreProtocol` | `oridecon.contracts.infra.storage.protocols` | Registered by `StorageProvider`; injected into application services |
| `StorageDriverProtocol` | `oridecon.contracts.infra.storage.protocols` | Extended by `AbstractDriver` — adds `copy`, `move`, `copy_batch` |
| `FileInfo` | `oridecon.contracts.infra.storage.models` | Return type for upload, info, download, list |
| `Uploadable` | `oridecon.contracts.infra.storage.models` | Union type: `bytes \| str \| BinaryIO \| AsyncIterator` |
| `UploadOptions` | `oridecon.contracts.infra.storage.models` | Upload metadata: content_type, public, metadata, cache_control |
| `HealthCheckResult` | `oridecon.contracts.core` | Return type for driver health checks |
...
```
packages/oridecon-storage/src/oridecon/storage/
...
flowchart LR
    ...
    subgraph Storage[oridecon-storage]
        SE[StorageError]
...
| Point | Mechanism |
|-------|-----------|
| Custom driver | Implement `BlobStoreProtocol`, register via `DriverRegistry` or `oridecon.storage.backends` entry point |
| Entry point registration | `[project.entry-points."oridecon.storage.backends"]` in `pyproject.toml` → Provider subclass |
