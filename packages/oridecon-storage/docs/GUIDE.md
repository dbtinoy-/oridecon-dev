...
`oridecon-storage` exposes **two storage paradigms**:
...
**Protocol:** `BlobStoreProtocol` from `oridecon-contracts`.
...
**Classes:** `InMemoryKVStorage`, `LocalStorage` in `oridecon.storage.kv`.
...
The `DriverRegistry` maps driver names to implementations and creates driver instances from config. It is extensible — third-party packages can register new drivers via the `oridecon.storage.backends` entry-point group.
...
```python
from oridecon.storage import UploadOptions
...
```python
from oridecon.storage.kv import InMemoryKVStorage
...
```python
from oridecon.storage.config import StorageS3Config, EncryptionConfig
...
- ✅ Set `storage.default_driver` to the production backend and override via `ORI_STORAGE__DEFAULT_DRIVER` in each environment.
- ❌ Don't store secrets (passwords, API keys) in KV — use `oridecon-auth` or a vault.
