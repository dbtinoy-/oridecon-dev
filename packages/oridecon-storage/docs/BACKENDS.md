---
title: oridecon-storage Backends
description: Supported storage backends in oridecon-storage — S3, GCS, Azure Blob, R2, local filesystem, and in-memory
---

`oridecon-storage` provides blob storage through a unified `BlobStoreProtocol` interface. Each driver implements upload, download, delete, list, URL generation, and health-checking. The package also provides a separate key-value storage subsystem (`oridecon.storage.kv`) for small string-keyed records with TTL.
...
```python
from oridecon.storage import StorageProvider, StorageConfig
...
`oridecon-storage` exposes two distinct storage paradigms in one package:
...
**Key-Value (KV) Storage** — `oridecon.storage.kv` for small string-keyed JSON records with optional TTL. Two backends:
