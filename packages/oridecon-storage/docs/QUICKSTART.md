---
title: oridecon-storage Quickstart
description: Get up and running with unified blob storage in minutes.
---

:::tip[Package]
`oridecon-storage` — Unified blob storage abstraction for S3, GCS, Azure Blob, Cloudflare R2, and local filesystem.
:::
...
```bash
uv add "oridecon-storage[aws]"    # aiobotocore for S3, R2
uv add "oridecon-storage[gcp]"    # gcloud-aio-storage for GCS
uv add "oridecon-storage[azure]"  # azure-storage-blob for Azure
```
...
from oridecon.storage import StorageModule, StorageConfig
