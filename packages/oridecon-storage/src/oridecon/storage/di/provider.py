...
from oridecon.contracts.core import HealthCheckCategory
from oridecon.contracts.core.config import ConfigProtocol
from oridecon.di.provider import Provider
from oridecon.logging import get_logger
from oridecon.storage import constants as storage_const
from oridecon.storage.backends.registry import DriverRegistry
...
class StorageProvider(Provider):
    ...
    Registers both the :class:`~oridecon.storage.backends.registry.DriverRegistry`
    (for extensibility) and the configured :class:`~oridecon.contracts.BlobStoreProtocol`
    ...
    async def register(self, container: ContainerRegistrarProtocol) -> None:
        ...
        container via :class:`~oridecon.contracts.core.config.ConfigProtocol`.
        Binds:

        - :class:`~oridecon.storage.backends.registry.DriverRegistry` — the
          extensible driver factory (singleton).
        - :class:`~oridecon.contracts.BlobStoreProtocol` — the configured backend driver
    ...
    async def _register_multi_backend(
        ...
        Args:
            container: DI registrar supplied by the framework.
        """
        from oridecon.storage.config import StorageConfig as _StorageConfig
    ...
    async def _discover_backends(self, container: ContainerRegistrarProtocol) -> None:
        """Scan the ``oridecon.storage.backends`` entry-point group.
        ...
        eps = _meta.entry_points(group="oridecon.storage.backends")
        for ep in eps:
            try:
                candidate = ep.load()
