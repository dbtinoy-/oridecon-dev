"""In-memory KV storage backend for Oridecon.
...
from oridecon.contracts.infra.storage import StorageBackendProtocol, StorageType
...
class InMemoryKVStorage(StorageBackendProtocol):
    """Pure in-memory :class:`~oridecon.contracts.StorageBackendProtocol` implementation.
    ...
    async def list_keys(
        ...
        mirroring the behaviour of :class:`~oridecon.storage.kv.local.LocalStorage`.
    ...
    async def health_check(self, timeout: float = 5.0) -> HealthCheckResult:
        ...
        Returns:
            :class:`~oridecon.contracts.HealthCheckResult` with status
            ``HEALTHY``.
        """
