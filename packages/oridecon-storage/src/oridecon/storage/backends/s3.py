...
if TYPE_CHECKING:
    from oridecon.storage.backends.protocols import _S3ClientProtocol
...
from oridecon.contracts.infra.storage import FileInfo, Uploadable
from oridecon.storage.backends._s3_helpers import (
    coerce_body_bytes,
    parse_last_modified,
)
from oridecon.storage.backends._s3_upload_mixin import _S3UploadMixin
from oridecon.storage.backends.base import AbstractDriver
from oridecon.storage.config import EncryptionConfig
...
class S3Driver(_S3UploadMixin, AbstractDriver):
    ...
    def __init__(
        ...
        if not _AIOBOTOCORE_AVAILABLE:
            raise ImportError(
                "S3 driver requires aiobotocore. Install with: pip install oridecon-storage[aws]",
            )
