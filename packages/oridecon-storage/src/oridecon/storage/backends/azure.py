...
from oridecon.contracts.infra.storage import FileInfo, Uploadable, UploadOptions
from oridecon.storage.backends._azure_helpers import (
    coerce_last_modified,
    is_blob_not_found,
    to_bytes,
)
from oridecon.storage.backends.base import AbstractDriver
from oridecon.storage.exceptions import StorageError, StorageFileNotFoundError
from oridecon.storage.lib.content_type import get_content_type
from oridecon.storage.lib.paths import sanitize_path
...
class AzureDriver(AbstractDriver):
    ...
    Install the optional dependency with::
        ...
        pip install oridecon-storage[azure]
    ...
    def __init__(
        ...
        if not _AZURE_AVAILABLE:
            raise ImportError(
                ...
                "Install with: pip install oridecon-storage[azure]",
            )
    ...
    async def upload(
        ...
        **options: Any,
        ...
        Returns:
            :class:`~oridecon.contracts.infra.storage.FileInfo` for the stored blob.
    ...
    async def info(self, path: str) -> FileInfo:
        ...
        Returns:
            :class:`~oridecon.contracts.infra.storage.FileInfo`.
    ...
    async def list(self, prefix: str = "") -> AsyncGenerator[FileInfo, None]:
        """Yield :class:`~oridecon.contracts.infra.storage.FileInfo` for blobs matching *prefix*.

        Args:
            prefix: Blob name prefix filter (empty string lists all blobs).

        Yields:
            :class:`~oridecon.contracts.infra.storage.FileInfo` entries.
    ...
    async def health_check(self, timeout: float = 5.0) -> HealthCheckResult:
        ...
        Returns:
            :class:`~oridecon.contracts.types.HealthCheckResult`.
        """
        from oridecon.contracts.core import HealthCheckResult
