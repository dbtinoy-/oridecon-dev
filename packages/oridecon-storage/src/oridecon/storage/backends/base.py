...
class AbstractDriver(ABC, BlobStoreProtocol):
    ...
    memory) must implement.  Extends the :class:`~oridecon.contracts.BlobStoreProtocol`
    ...
    async def upload(
        ...
        **options: Any,
        ...
        Returns:
            :class:`~oridecon.contracts.infra.storage.FileInfo` with size, ETag, etc.
        """
    ...
    async def list(
        ...
        """Yield :class:`~oridecon.contracts.infra.storage.FileInfo` for every
    ...
    async def health_check(self, timeout: float = 5.0) -> HealthCheckResult:
        ...
        :class:`~oridecon.storage.providers.StorageProvider` can report
        storage health to the framework's health-check system.

        Returns:
            :class:`~oridecon.contracts.types.HealthCheckResult` with status and
    ...
    async def write_stream(
        ...
        Returns:
            :class:`~oridecon.contracts.infra.storage.FileInfo` for the stored file.
        """
        chunks: list[bytes] = []
    ...
    async def copy(self, src: str, dst: str) -> FileInfo:
        ...
        Returns:
            :class:`~oridecon.contracts.infra.storage.FileInfo` for the new file.
        """
    ...
    async def move(self, src: str, dst: str) -> FileInfo:
        ...
        Returns:
            :class:`~oridecon.contracts.infra.storage.FileInfo` for the file at its
            new location.
        """
