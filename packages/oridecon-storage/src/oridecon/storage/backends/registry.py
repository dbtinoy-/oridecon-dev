...
returns a :class:`~oridecon.contracts.BlobStoreProtocol`.  There are no per-driver
registry classes — third parties extend the registry by calling
:meth:`DriverRegistry.register`.

The canonical path inside a Oridecon *Application* is to resolve
:class:`DriverRegistry` from the DI container (registered by
:class:`~oridecon.storage.providers.StorageProvider`).
"""
...
from oridecon.primitives.registry import BackendRegistry as _CoreBackendRegistry
...
def _create_s3(config: Any) -> BlobStoreProtocol:
    ...
    if not _S3_AVAILABLE:
        raise ImportError(
            ...
            "Install with: pip install oridecon-storage[aws]",
        )
...
def _create_gcs(config: Any) -> BlobStoreProtocol:
    ...
    if not _GCS_AVAILABLE:
        raise ImportError(
            ...
            "Install with: pip install oridecon-storage[gcs]",
        )
...
class DriverRegistry(_CoreBackendRegistry):
    ...
    Extends :class:`oridecon.primitives.registry.BackendRegistry` so that all
    ...
    the ``oridecon.storage.backends`` group.

    Usage::

        # Resolved from DI container (preferred inside Oridecon Application)
    ...
    def get_driver(self, driver_type: str, config: Any) -> BlobStoreProtocol:
        ...
        Args:
            ...
            config: :class:`~oridecon.storage.config.StorageConfig` instance.

        Returns:
            A ready-to-use :class:`~oridecon.contracts.BlobStoreProtocol` instance.
