"""P2 hook surface import verification for oridecon-http."""
...
def test_http_hooks_root_module_exists() -> None:
    import oridecon.http
    from oridecon.http.hooks import (
        HTTPRequestSentHook,
    ...
    assert oridecon.http.HTTPRequestSentHook is HTTPRequestSentHook
    assert oridecon.http.HTTPResponseReceivedHook is HTTPResponseReceivedHook


def test_http_hook_payloads_are_frozen_and_keyword_only() -> None:
    from oridecon.http.hooks import HTTPRequestSentHook, HTTPResponseReceivedHook
