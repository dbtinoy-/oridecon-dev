...
Retry requires `oridecon-resilience`:
...
```python
from oridecon.http import HTTPProvider
...
```python
from oridecon.http import build_url, parse_url_parts
...
```python
from oridecon.http import InterceptorProtocol, RequestContext, ResponseContext
...
```python
from oridecon.http.config import HTTPClientConfig, ConnectionPoolConfig
