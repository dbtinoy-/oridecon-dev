...
```python
from oridecon.http import InterceptorProtocol
...
When `oridecon-resilience` is registered, the HTTP client automatically wires:
...
```python
from oridecon.http import HTTPClientConfig, ConnectionPoolConfig
