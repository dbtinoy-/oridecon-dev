...
Internal design of the `oridecon-http` package.
...
```mermaid
flowchart BT
    Contracts[oridecon-contracts<br/>HTTPClientProtocol · InterceptorProtocol<br/>HttpResponse · HttpStatusError]
    Core[oridecon<br/>DI · Provider · Module · Config · Result]
    Http[oridecon-http<br/>HTTPClient · ConnectionPool · Retry · Interceptors]
    Resilience[oridecon-resilience<br/>RetryPolicyProtocol · CircuitBreakerProtocol]
...
`oridecon-http` depends on `oridecon` and `oridecon-contracts`. Resilience is optional — resolved from the container at boot. Consumer code communicates through `HTTPClientProtocol`.
...
All consumers receive `HttpResponse` (from `oridecon-contracts`) — a dataclass decoupling them from `aiohttp`:
...
`oridecon-http` has no built-in auth handlers. Authentication uses standard HTTP header mechanisms:
...
```
HTTPClientError (ORI_ERR_HTTP_001)
...
|----------------|--------|---------|
| `HTTPClientProtocol` | `oridecon.contracts.web.http_protocols` | Outbound HTTP client interface |
| `InterceptorProtocol` | `oridecon.contracts.web.http_protocols` | Request/response interception |
| `InterceptorChainProtocol` | `oridecon.contracts.web.http_protocols` | Interceptor chain management |
| `HttpResponse` | `oridecon.contracts.web.http_models` | Transport-agnostic response |
| `HttpStatusError` | `oridecon.contracts.web.http_models` | 4xx/5xx error with response |
| `HTTPSessionProtocol` | `oridecon.contracts.web.http_protocols` | HTTP session abstraction |
| `ServerSentEvent` | `oridecon.contracts.web.sse` | SSE event dataclass |
| `RetryPolicyProtocol` | `oridecon.contracts.infra.resilience` | Retry policy (optional DI) |
| `CircuitBreakerProtocol` | `oridecon.contracts.infra.resilience` | Circuit breaker (optional DI) |
| `ResiliencePipelineProtocol` | `oridecon.contracts.infra.resilience` | Composable resilience (optional DI) |
| `MetricsRecorderProtocol` | `oridecon.contracts.observability` | Request metrics (optional DI) |
