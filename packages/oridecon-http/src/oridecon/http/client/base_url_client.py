...
A lightweight wrapper around :class:`~oridecon.http.client.HTTPClient` that
adds base-URL resolution, default-header merging, and a context-manager
lifecycle.  Returns :class:`~oridecon.contracts.web.models.HttpResponse`
directly from verb methods (raising on infrastructure failures rather than
returning a :class:`~oridecon.result.Result`).
...
from oridecon.contracts.web.http_models import HttpResponse, HttpStatusError
from oridecon.http.client.http_client import HTTPClient
from oridecon.http.config import ConnectionPoolConfig, HTTPClientConfig
...
class StreamContext:
    ...
    def raise_for_status(self) -> None:
        """Raise :class:`~oridecon.contracts.web.models.HttpStatusError` for 4xx/5xx.
...
class BaseURLHTTPClient:
    ...
    Wraps :class:`~oridecon.http.client.HTTPClient` and adds:

    * ``base_url`` prepended to every relative request path.
    * Default headers merged into every request.
    * Direct :class:`~oridecon.contracts.web.models.HttpResponse` returns
    ...
    * Lazy initialisation — no explicit ``start()`` call required; the
    ...
    Args:
        ...
        retry: Optional retry policy injected into the underlying
            :class:`~oridecon.http.client.HTTPClient`.
        circuit_breaker: Optional circuit breaker injected into the
            underlying :class:`~oridecon.http.client.HTTPClient`.
    ...
    async def request(self, method: str, url: str, **kwargs: Any) -> HttpResponse:
        """Make an HTTP request, returning :class:`~oridecon.contracts.web.models.HttpResponse` directly.

        Infrastructure failures are raised as exceptions (not wrapped in
        ``Result``).  HTTP 4xx/5xx responses are returned as-is; call
        :meth:`~oridecon.contracts.web.models.HttpResponse.raise_for_status`
        ...
        Returns:
            Framework :class:`~oridecon.contracts.web.models.HttpResponse`.
        """
