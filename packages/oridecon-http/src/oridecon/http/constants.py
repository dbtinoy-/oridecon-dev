"""HTTP client constants for the oridecon.http module.

Shared HTTP values (methods, content-types, header names, encoding) are
imported from ``oridecon.contracts.web.constants`` and re-exported here
...
try:
    __version__: str = importlib.metadata.version("oridecon-http")
except ImportError:
    __version__ = "0.0.0"

ENV_PREFIX: str = "ORI_HTTP__"
"""Environment variable prefix for HTTP configuration."""

ENV_NESTED_DELIMITER: str = "__"
"""Delimiter for nested env var keys (e.g. ORI_HTTP__POOL__TIMEOUT)."""

# Re-export shared constants from contracts so callers that import from
# oridecon.http.constants continue to work unchanged.
