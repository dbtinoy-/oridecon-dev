"""Exceptions for the oridecon.http module.
...
:class:`~oridecon.http.client.HTTPClient` and match on
...
class HTTPStatusError(HTTPClientError):
    ...
    :meth:`~oridecon.http.client.HTTPClient.request` method receive
    a plain :class:`~oridecon.contracts.web.HttpResponse` with the
    non-2xx status code.

    Attributes:
        status: HTTP status code (4xx or 5xx).
        response: The full :class:`~oridecon.contracts.web.HttpResponse`
            that was received.
    """

    _code: str = "ORI_ERR_HTTP_007"
