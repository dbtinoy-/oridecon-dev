---
title: oridecon-testing Troubleshooting
...
**Exception:** `oridecon.testing.TokenBudgetExceededError`
...
**Solution:**
```python
from oridecon.testing import requires_redis
...
**Exception:** `oridecon.testing.SnapshotMismatchError`
...
**Cause:** The marker is registered by `oridecon-testing`'s pytest plugin, but the plugin is not loaded. This happens when `oridecon-testing` is not installed in the test environment's Python path, or when pytest is run without `-p oridecon.testing.plugin`.

**Fix:** Ensure the plugin is loaded:

```bash
# Auto-discovered when oridecon-testing is installed
uv run pytest ...

# Or load explicitly
uv run pytest -p oridecon.testing.plugin ...
```
...
```python
from oridecon.testing import SnapshotAsserter
