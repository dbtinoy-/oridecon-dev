---
title: oridecon-testing Guide
description: Fakes, compliance suites, test environments, and test clients for the Oridecon Framework
---
...
```python
from oridecon.testing import FakeCache, FakeEventBus
...
| Client | Purpose |
|--------|---------|
| `WebTestBed` / `WebTestClient` | HTTP integration tests with the Oridecon web layer |
...
```python
from oridecon.testing import AITestClient
...
These integrate with the `Result` pattern from `oridecon.result`:
...
`oridecon-testing` registers a pytest plugin (`oridecon.testing.plugins.pytest`) that provides:
...
```python
from oridecon.testing import override, testbed
