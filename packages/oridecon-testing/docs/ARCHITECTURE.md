...
Internal design of the `oridecon-testing` package.

---

## Role in the System

`oridecon-testing` is the **cross-package testing toolkit** for the Oridecon Framework. Unlike every other extension package, it **may import from any extension package** — its purpose is providing test doubles, compliance suites, and test clients for the entire framework. All cross-package imports are declared as optional dependencies.
...
flowchart BT
    ...
    subgraph Testing[oridecon-testing]
    ...
    subgraph Framework[Oridecon Ecosystem]
        CT[oridecon-contracts · Protocols]
        LX[oridecon · Core Framework]
        SQL[oridecon-sql]
        WEB[oridecon-web]
        AUTH[oridecon-auth]
...
**Import rule:** `oridecon-testing` is the one package allowed to import directly from any other extension. All consumed extension packages are optional dependencies in `pyproject.toml`.
...
| Principle | Rationale |
|-----------|-----------|
| **Fake at the contract boundary** | Test doubles implement a `Protocol` from `oridecon-contracts`, never mock internals |
...
│   ├── container.py         # OrideconContainerHarness — isolated container for unit tests
...
│   ├── db.py                # Database fixtures (conditional on oridecon-sql)
│   ├── web.py               # Web fixtures (conditional on oridecon-web)
│   ├── tasks.py             # Task fixtures (conditional on oridecon-tasks)
│   ├── ai.py                # AI fixtures (conditional on oridecon-ai-*)
...
|--------|-------------------|---------|
| `WebTestClient` | `oridecon-web` | HTTP integration testing with ASGI |
| `DatabaseTestClient` | `oridecon-sql` | Database query/transaction testing |
| `AITestClient` | `oridecon-ai-*` | LLM/RAG pipeline testing with token budget |
| `TaskTestClient` | `oridecon-tasks` | Background task testing |
| `WebSocketClient` | `oridecon-web` | WebSocket integration testing |
...
|----------|--------|------|-----------|
| `EventBusProtocol` | `oridecon.contracts.events` | `FakeEventBus` | `InMemoryEventBus` |
| `CommandBusProtocol` | `oridecon.contracts.events` | `FakeCommandBus` | `InMemoryCommandBus` |
| `QueryBusProtocol` | `oridecon.contracts.events` | `FakeQueryBus` | `InMemoryQueryBus` |
| `DomainEventPublisherProtocol` | `oridecon.contracts.events` | `FakeEventBus` | `InMemoryEventBus` |
| `CacheBackendProtocol` | `oridecon.contracts.cache` | `FakeCache` | `InMemoryCacheBackend` |
| `AuditLoggerProtocol` | `oridecon.contracts.audit` | `FakeAuditLogger` | `InMemoryAuditLogger` |
| `RepositoryProtocol` | `oridecon.contracts.data` | — | `InMemoryRepository` |
| `UnitOfWorkProtocol` | `oridecon.contracts.data` | `FakeUnitOfWork` | `InMemoryUnitOfWork` |
| `DistributedLockProtocol` | `oridecon.contracts.resilience` | — | `InMemoryDistributedLock` |
| `LoggerProtocol` | `oridecon.contracts.logging` | `FakeLogger` | — |
| `ConfigProtocol` | `oridecon.contracts.core.config` | `FakeConfig` | — |
| `StateStoreProtocol` | `oridecon.contracts.state` | `FakeStateStore` | — |
| `MetricsCollectorProtocol` | `oridecon.contracts.metrics` | `FakeMetricsCollector` | — |
| `TracerProtocol` | `oridecon.contracts.tracing` | `FakeTracer` | — |
| `ClockProtocol` | `oridecon.contracts.core.clock` | `FakeClock`, `FixedClock` | — |
| `BlobStoreProtocol` | `oridecon.contracts.storage` | — | `InMemoryBlobStore` |
| `OutboxProtocol` | `oridecon.contracts.events` | — | `InMemoryOutbox` |
...
flowchart LR
    ...
    subgraph Compliance[oridecon-testing/compliance]
...
flowchart LR
    ...
    subgraph Testing[oridecon-testing]
        TE[TestingError<br/>ORI_ERR_TEST_001]
        TB[TokenBudgetExceededError<br/>ORI_ERR_TEST_002]
    end
    subgraph Memory[oridecon-testing/memory]
...
| Point | Mechanism |
|-------|-----------|
| New fake | Implement any protocol from `oridecon-contracts`, add to `fakes/`, register in `fake()` lookup |
