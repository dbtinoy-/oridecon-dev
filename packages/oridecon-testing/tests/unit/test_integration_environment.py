...
from oridecon.testing import IntegrationEnvironment
...
_DB_PROVIDER_MODULE = "oridecon.sql.di.provider"
_CACHE_PROVIDER_MODULE = "oridecon.cache.di.provider"
...
class TestWithDatabase:
    ...
    def test_raises_import_error_when_package_missing(self) -> None:
        with mock.patch.dict(sys.modules, {_DB_PROVIDER_MODULE: None}):
            with pytest.raises(ImportError, match="oridecon-sql is required"):
...
class TestWithCache:
    ...
    def test_raises_import_error_when_package_missing(self) -> None:
        with mock.patch.dict(sys.modules, {_CACHE_PROVIDER_MODULE: None}):
            with pytest.raises(ImportError, match="oridecon-cache is required"):
