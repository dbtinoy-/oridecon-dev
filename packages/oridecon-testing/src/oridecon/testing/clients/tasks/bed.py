...
from oridecon.testing.clients.tasks.mocks import MockTaskExecutor, MockTaskQueue
...
class TaskTestBed(TestEnvironment):
    ...
    Unlike the generic :class:`~oridecon.testing.fixtures.bed.TestEnvironment`,
    ...
    async def setup(self) -> None:  # type: ignore[override]
        ...
        Creates a fresh :class:`~oridecon.testing.clients.tasks.MockTaskQueue`
        and :class:`~oridecon.testing.clients.tasks.MockTaskExecutor`, then
        enqueues the three sample tasks returned by
        :meth:`~oridecon.testing.clients.tasks.TaskTestData.sample_tasks`.
        Does **not** start an ``Application`` or DI container.
        """
        from oridecon.tasks import JobProtocol, Priority
    ...
    def get_enqueued_tasks(self) -> list[Any]:
        ...
        Returns:
            A list of :class:`~oridecon.tasks.JobProtocol` objects, in enqueue order.
