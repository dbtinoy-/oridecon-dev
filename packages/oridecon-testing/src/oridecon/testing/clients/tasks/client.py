...
from oridecon.testing.clients.tasks.mocks import MockTaskResult, MockTasksProvider
...
class TaskTestClient:
    ...
    async def enqueue_test_tasks(self) -> list[str]:
        ...
        Creates :class:`~oridecon.tasks.JobProtocol` instances from
        :meth:`~oridecon.testing.clients.tasks.TaskTestData.sample_tasks` and
        enqueues them.
    ...
    async def execute_test_task(
        ...
        Args:
            task: A :class:`~oridecon.tasks.JobProtocol` (or any object compatible
