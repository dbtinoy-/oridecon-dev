...
from oridecon.testing.clients.web.client import TestResponse, WebTestClient
...
class WebTestBed(TestEnvironment):
    ...
    Extends :class:`~oridecon.testing.fixtures.bed.TestEnvironment` to add
    ...
    - ``bed.web_provider``: The :class:`~oridecon.web.di.provider.WebProvider` instance.
    - ``bed.container``: The DI container (for overrides and resolution).

    Args:
        provider_or_app: A :class:`~oridecon.web.di.provider.WebProvider` instance
            or a Oridecon :class:`~oridecon.app.base.Application`.
