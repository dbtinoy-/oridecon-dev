...
try:
    from oridecon.sql.abstractions.connection import (  # type: ignore[attr-defined]
        ConnectionPool,
        DatabaseConnection,
    )
except ImportError:
    # Fallback if oridecon-sql is not available
    ConnectionPool: Any = Any  # type: ignore[no-redef]
    DatabaseConnection: Any = Any  # type: ignore[no-redef]
from oridecon.testing.clients.db import (
    DatabaseTestBed,
    DatabaseTestClient,
)
...
async def db_transaction(
    ...
    active :class:`~oridecon.contracts.data.ConnectionProtocol` so that the
