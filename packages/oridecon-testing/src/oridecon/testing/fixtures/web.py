...
"""Pytest fixtures for Oridecon Web testing."""
...
from oridecon.testing.clients.web import WebTestBed, WebTestClient
...
def mock_web_response() -> Any:
    ...
    from oridecon.web import JSONResponse
...
async def app_bed() -> Any:
    """Yield an :class:`~oridecon.testing.testbed.AppTestBed` for the default app.

    Auto-detects ``create_app`` from ``src/*/app.py`` (same convention as
    ``oridecon run``).  Tests that need a specific factory should call
    :meth:`~oridecon.testing.testbed.AppTestBed.from_factory` directly::

        async with AppTestBed.from_factory("my_app.app:create_app") as bed:
            resp = await bed.client.get("/health")

    Yields:
        A booted :class:`~oridecon.testing.testbed.AppTestBed` or ``None``
        when no entry point can be found.
    """
    from oridecon.cli.registry.server import discover_entry_point
