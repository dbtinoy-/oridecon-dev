...
test environments for testing Oridecon providers and applications.
...
# `oridecon.testing.bed` (some tests historically import MockDatabaseProvider from here).
try:
    from oridecon.testing.mocks.base import (
...
try:
    ...
    def get_logger(name: str | None = None) -> _Logger:  # type: ignore[misc]
        ...
        return _logging.getLogger(name or "oridecon.testing.bed")
...
try:
    from oridecon.primitives.context import Context as _Context
...
class TestEnvironment:
    ...
    Oridecon providers and applications. It manages the application lifecycle,
    ...
    def fake(self, contract: type) -> TestEnvironment:
        ...
        from :mod:`oridecon.testing.fakes`.  Raises :exc:`ValueError` if no
    ...
    async def setup(self) -> Application:
        ...
        ``setup()`` call.
        """
        from oridecon.app.base import AppState
    ...
    def resolve_sync(self, service_type: type) -> Any:
        ...
        if self.container:
            from oridecon.concurrency import run_sync
