...
Example ``conftest.py``::
    ...
    from oridecon.testing.testkit.fixtures import test_environment, test_event_bus  # noqa: F401

Or use directly in tests::

    from oridecon.testing.testkit.fixtures import test_container
...
def test_container(test_environment: TestEnvironment) -> Container:
    """Yield the :class:`~oridecon.di.container.Container` from the test environment."""
    return test_environment.container


@pytest.fixture
def test_event_bus(test_environment: TestEnvironment) -> InMemoryEventBus:
    """Yield the :class:`~oridecon.memory.event_bus.InMemoryEventBus` from the test environment."""
