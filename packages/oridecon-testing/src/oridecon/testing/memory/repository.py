...
pattern backed by :class:`~oridecon.data.repository.base.AbstractRepository`.
Intended as a zero-infrastructure drop-in for unit tests and local development.
For production, replace with ``oridecon-sql``'s SQLRepository or a
...
class InMemoryRepository(AbstractRepository[T, Any], Generic[T]):
    ...
    This class lives in ``oridecon-testing`` intentionally — it is not exported
    from ``oridecon`` core.  Production code should depend on the
    ``RepositoryProtocol`` protocol from ``oridecon.contracts.data`` and receive a
    concrete implementation (e.g. ``SQLRepository`` from ``oridecon-sql``)
    ...
    Extends :class:`~oridecon.data.repository.base.AbstractRepository` and
    implements all required primitives.  For production persistence, use
    ``oridecon-sql``'s ``SQLRepository`` or any other storage backend.
