...
:class:`~oridecon.data.uow.base.AbstractUnitOfWork` with a no-op
...
database (e.g. ``oridecon-sql``'s ``SQLUnitOfWork``).
"""
...
class InMemoryUnitOfWork(AbstractUnitOfWork):
    ...
    inherited from :class:`~oridecon.data.uow.base.AbstractUnitOfWork`.
