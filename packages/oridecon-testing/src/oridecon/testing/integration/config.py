...
class IntegrationTestConfig:
    ...
    postgres_dsn_raw: str = field(
        default_factory=lambda: os.environ.get(
            "ORI_TEST_POSTGRES_DSN_RAW",
            "postgresql://oridecon:oridecon@localhost:15432/oridecon_test",
        )
    )
    redis_url: str = field(
        default_factory=lambda: os.environ.get(
    ...
    kafka_bootstrap: str = field(
        default_factory=lambda: os.environ.get(
            "ORI_TEST_KAFKA_BOOTSTRAP",
            "localhost:19092",
        )
    )
    minio_endpoint: str = field(
        default_factory=lambda: os.environ.get(
            "ORI_TEST_MINIO_ENDPOINT",
            "localhost:19000",
        )
    )
    minio_access_key: str = field(
        default_factory=lambda: os.environ.get(
            "ORI_TEST_MINIO_ACCESS_KEY",
            "minioadmin",
        )
    )
    minio_secret_key: str = field(
        default_factory=lambda: os.environ.get(
            "ORI_TEST_MINIO_SECRET_KEY",
            "minioadmin",
        )
    )
    elasticsearch_url: str = field(
        default_factory=lambda: os.environ.get(
            "ORI_TEST_ELASTICSEARCH_URL",
            "http://localhost:19200",
        )
    )
    mongodb_dsn: str = field(
        default_factory=lambda: os.environ.get(
            "ORI_TEST_MONGODB_DSN",
