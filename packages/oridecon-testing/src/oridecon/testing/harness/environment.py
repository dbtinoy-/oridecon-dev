...
Unlike :class:`~oridecon.testing.fixtures.bed.TestEnvironment`, which
...
class IntegrationEnvironment(TestEnvironment):
    """A :class:`~oridecon.testing.fixtures.bed.TestEnvironment` pre-configured
    ...
    def with_database(
        ...
        *,
        ...
        Raises:
            ImportError: If ``oridecon-sql`` is not installed.
        """
        try:
            from oridecon.sql.di.provider import (
                DatabaseService,
            )
        except ImportError as exc:  # pragma: no cover
            raise ImportError(
                "oridecon-sql is required for IntegrationEnvironment.with_database(). "
                "Install it with: uv add oridecon-sql"
            ) from exc
    ...
    def with_cache(
        ...
        *,
        ...
        Raises:
            ImportError: If ``oridecon-cache`` is not installed.
        """
        try:
            ...
            raise ImportError(
                "oridecon-cache is required for IntegrationEnvironment.with_cache(). "
                "Install it with: uv add oridecon-cache"
            ) from exc
    ...
    def with_all(
        ...
        *,
        ...
        try:
            ...
            raise ImportError(
                "oridecon-sql is required for IntegrationEnvironment.with_all(). "
                "Install it with: uv add oridecon-sql"
            ) from exc
        ...
        try:
            ...
            raise ImportError(
                "oridecon-cache is required for IntegrationEnvironment.with_all(). "
                "Install it with: uv add oridecon-cache"
            ) from exc
    ...
    async def setup(self) -> Any:
        ...
        if self._config_overrides:
            from oridecon.testing.fakes import FakeConfig

            fake_cfg = FakeConfig(self._config_overrides)
            from oridecon.contracts.core.config import (
                ConfigProtocol,
            )
