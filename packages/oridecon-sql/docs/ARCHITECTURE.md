...
Internal design of the `oridecon-sql` package.
...
```mermaid
flowchart LR
    subgraph Extensions["oridecon-*"]
        Web["oridecon-web · admin · auth · ..."]
    end
...
`oridecon-sql` is an **infrastructure-layer** extension providing async SQL database access. Other extensions consume `DatabaseProviderProtocol` from contracts through the container — never `oridecon-sql` directly.
...
| `exceptions.py` | 31 leaf exceptions under `DatabaseError` (`ORI_ERR_SQL_001`–`031`) |
...
```mermaid
flowchart LR
    subgraph CLI["oridecon db migrate"]
...
```bash
oridecon db migrate run                  # Apply pending migrations
oridecon db migrate rollback --steps 1   # Roll back one migration
oridecon db migrate history              # Show migration history
oridecon db status                       # Database and migration status
```
...
Database errors extend `DatabaseError` from contracts. All 31 leaf types carry `_code` strings (`ORI_ERR_SQL_001`–`031`). Sub-hierarchy: `DatabaseConnectionError` (refused/timeout/pool), `QueryError` (syntax/binding), `IntegrityError` (duplicate key/fk/null/check), `TransactionError` (serialization/deadlock/rollback), `SchemaError` (table/column not found), `RepositoryError`, `UnitOfWorkError`, `DriverError`, `LockError`, `DatabaseTimeoutError`.
