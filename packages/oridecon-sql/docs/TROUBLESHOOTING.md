---
title: oridecon-sql Troubleshooting
...
**Cause:** The async database driver is not installed. `oridecon-sql` depends only on SQLAlchemy and aiosqlite (SQLite) by default.
...
- Enable SQL echo: `export ORI_SQL__OPERATIONS__ECHO=true` (logs all SQL to the structured logger)
- Enable debug logging: `export ORI_LOGGING__LEVEL=DEBUG` for detailed lifecycle and query logs
