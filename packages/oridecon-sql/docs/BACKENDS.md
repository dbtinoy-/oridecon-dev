---
title: oridecon-sql Backends
description: Compare Postgres, MySQL, and SQLite backends for oridecon-sql
sidebar:
  order: 3
---

`oridecon-sql` ships three database backends. All implement `DatabaseProviderProtocol` from `oridecon-contracts` and expose the same repository, unit-of-work, and migration APIs.

## Supported Backends

| Backend | Extra | Driver | Production-ready | Best for |
|---------|-------|--------|-----------------|----------|
| Postgres | `oridecon-sql[postgres]` | `asyncpg>=0.29.0` | Yes | Primary production database |
| MySQL | `oridecon-sql[mysql]` | `aiomysql>=0.1.0` | Yes | Existing MySQL/MariaDB deployments |
| SQLite | `oridecon-sql[sqlite]` | `aiosqlite>=0.22.1` | Development / test | Local dev, CI, single-user apps |

:::note
SQLite (`aiosqlite`) is a core dependency — no extra needed. Install extras for Postgres and MySQL: `pip install oridecon-sql[postgres,mysql]`.
:::
...
```python
from oridecon.sql import DatabaseModule, DatabaseConfig
...
```python
from oridecon.sql.context import run_in_threadpool_with_context
