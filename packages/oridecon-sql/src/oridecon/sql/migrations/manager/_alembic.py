...
This module provides migration management capabilities for Oridecon DB.
...
Example:
    Using AlembicManager::
        ...
        from oridecon.sql.migrations.manager import AlembicManager
...
from oridecon.sql.exceptions import DatabaseError, QueryError
from oridecon.sql.migrations.engine import MigrationEngine
from oridecon.sql.migrations.introspector import SchemaIntrospector
from oridecon.sql.migrations.orchestrator import MigrationOrchestrator
from oridecon.sql.migrations.types import MigrationInfo, MigrationStatus
...
class AlembicManager:
    """Alembic migration manager facade for Oridecon DB.
    ...
    def _create_alembic_config(self) -> Config:
        ...
        # Package root (packages/oridecon-sql/) — parents[4] would land in
    ...
    async def get_applied_migrations(self) -> list[MigrationRecord]:
        ...
        revision.  Each returned :class:`~oridecon.contracts.MigrationRecord`
        ...
        Returns:
            List of :class:`~oridecon.contracts.MigrationRecord` for every
