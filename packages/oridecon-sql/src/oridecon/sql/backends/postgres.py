...
from oridecon.contracts import ConnectionPoolProtocol, RetryConfig
from oridecon.contracts.core import HealthCheckResult, HealthStatus
from oridecon.contracts.infra.resilience import CircuitBreakerProtocol
from oridecon.logging import get_logger
from oridecon.primitives import clock as ambient_clock
from oridecon.sql.backends._postgres_connection import PostgresConnection
from oridecon.sql.backends._ssl import (
    SSLCertPathConfig,
    SSLConfig,
    SSLConfigRegistry,
    SSLRequireConfig,
...
from oridecon.sql.lib.retry import retry_call
from oridecon.sql.monitoring import DatabaseMonitor
...
class PostgresConnectionPool(ConnectionPoolProtocol):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if not HAS_POSTGRES:
            raise ImportError(
                "asyncpg is required for PostgreSQL support. Install with: pip install oridecon-sql[postgres]",
            )
    ...
    async def _create_pool(self, pool_kwargs: dict[str, Any]) -> None:
        ...
        try:
            self._pool = await asyncpg.create_pool(
                ...
                server_settings={"application_name": "oridecon"},
                **pool_kwargs,
            )
