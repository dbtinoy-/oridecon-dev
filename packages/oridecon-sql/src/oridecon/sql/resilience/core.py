...
When a :class:`~oridecon.contracts.resilience.ResiliencePipelineFactory` is
injected (provided by ``oridecon-resilience``), builds real circuit-breaker /
retry pipelines with database-specific defaults.  When ``None`` is injected
(or ``oridecon-resilience`` is not installed), a lightweight no-op pass-through
is used instead — so ``oridecon-sql`` has **no** hard runtime dependency on the
resilience extension.

Cross-extension communication follows the contracts + DI pattern from
``AGENTS.md``.  ``oridecon-sql`` never imports from ``oridecon-resilience``
directly; it only consumes the :class:`~oridecon.contracts.resilience.ResiliencePipelineFactory`
...
class _NoOpPipeline:
    """Pass-through pipeline used when oridecon-resilience is not installed."""
...
class DatabaseResilienceHandler:
    ...
    Accepts an optional :class:`~oridecon.contracts.resilience.ResiliencePipelineFactory`
    ...
    ``None`` is passed a lightweight no-op pipeline is used, so ``oridecon-sql``
    has no hard runtime dependency on ``oridecon-resilience``.
    """
    ...
    def __init__(
        ...
        Args:
            pipeline_factory: Optional factory provided by ``oridecon-resilience``
    ...
    def get_pipeline(self, name: str) -> ResiliencePipelineProtocol | _NoOpPipeline:
        ...
        Returns:
            A :class:`~oridecon.contracts.resilience.ResiliencePipelineProtocol`
    ...
    def _build_pipeline(self, name: str) -> ResiliencePipelineProtocol | _NoOpPipeline:
        ...
        if self._pipeline_factory is None:
            logger.debug(
                "oridecon_resilience_not_configured",
...
#
# ``oridecon.sql.lib.retry`` re-exports ``retry_call`` from here.
# ---------------------------------------------------------------------------
...
async def retry_call(
    ...
    **kwargs: Any,
    ...
    Uses :class:`~oridecon.contracts.resilience.RetryConfig` from contracts
    ...
    Raises:
        :class:`~oridecon.resilience.errors.RetryExhaustedError`: When all
            attempts are exhausted.
        Any exception raised by *func* that is not in ``config.retry_on``.
    """
    from oridecon.contracts.exceptions.resilience import RetryExhaustedError
