"""Configuration models for Oridecon Database.
...
Example:
    ...
    # From environment
    config = DatabaseConfig.from_env()  # reads ORI_DB__* env vars
...
from oridecon.sql import constants as const
from oridecon.validation import ConfigDict, Field, SecretStr, model_validator
...
class DatabaseConfig(BaseConfig):
    """Hierarchical root configuration for Oridecon Database.
    ...
    Attributes:
        ...
        audit_hmac_key: Optional HMAC key (plain text or base64) for audit
            checksum signing. Set via ``ORI_DB__AUDIT_HMAC_KEY``.
    ...
    def from_env(cls, prefix: str = const.ENV_PREFIX) -> DatabaseConfig:
        ...
        is treated as a nesting delimiter, so ``ORI_SQL__BACKEND__URL``
        maps to ``backend.url``.
        ...
        Example::
            ...
            # With ORI_SQL__BACKEND__URL=postgresql://user:pass@host/db set:
            config = DatabaseConfig.from_env()
            assert config.backend.url.get_secret_value().startswith("postgresql")
        """
        from oridecon.config.lib.sources import ConfigLoader, EnvironmentConfigSource
...
class ComponentPostgresConfig(DomainModel):
    ...
    dsn: str = Field(default="postgresql://localhost:5432/oridecon")
    state_table: str = Field(default="oridecon_state")
    lock_table: str = Field(default="oridecon_locks")
