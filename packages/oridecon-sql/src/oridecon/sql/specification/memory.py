"""SpecificationProtocol pattern implementation for Oridecon Framework.
...
class MemorySpecification(Generic[T]):
    """Abstract base class for in-memory specifications in oridecon-sql.

    Implements the ``SpecificationProtocol`` protocol from ``oridecon.contracts.domain``
