...
This is the **canonical** Unit of Work implementation for ``oridecon-sql``.
...
:class:`IdentityMap`
    ...
    :class:`SimpleUnitOfWork` and :class:`~oridecon.sql.managers.ScopedUnitOfWork`
    to avoid duplicate loads within the same transaction scope.

.. seealso::
    :mod:`oridecon.sql.managers` — scoped connection + UoW integration built
    on top of :class:`DatabaseManager`.
"""

from __future__ import annotations

from oridecon.sql.unit_of_work.decorators import transactional
from oridecon.sql.unit_of_work.identity_map import (
    EntityChange,
    EntitySnapshot,
    IdentityMap,
)
