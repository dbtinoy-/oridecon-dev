...
class SeedManager:
    ...
    async def initialize(self) -> None:
        ...
        async with self.engine.begin() as conn:
            ...
            await conn.execute(
                text(
                    f"""
                    CREATE TABLE IF NOT EXISTS oridecon_seeds (
    ...
    async def get_applied_seeds(self) -> set[str]:
        ...
        async with self.engine.connect() as conn:
            result = await conn.execute(text("SELECT filename from oridecon.seeds"))
        ...
        for seed_file in seed_files:
            ...
            try:
                ...
                async with self.engine.begin() as conn:
                    await conn.execute(
                        text(
                            "INSERT INTO oridecon_seeds (filename) VALUES (:filename)",
                        ),
    ...
    async def reset(self) -> None:
        ...
        async with self.engine.begin() as conn:
            await conn.execute(text("TRUNCATE TABLE oridecon_seeds"))
