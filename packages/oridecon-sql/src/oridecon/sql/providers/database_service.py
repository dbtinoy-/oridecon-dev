"""Database provider for Oridecon Framework integration (Consolidated).
...
from oridecon.sql.config import DatabaseConfig
from oridecon.sql.logging import ConsoleQueryLogger
from oridecon.sql.managers import DatabaseManager
from oridecon.sql.providers._connection_mixin import _ConnectionMixin
from oridecon.sql.providers._health_mixin import _HealthMixin
from oridecon.sql.providers._query_mixin import _QueryMixin
...
def _load_postgres(url: str, **kwargs: Any) -> DatabaseProviderProtocol:
    from oridecon.sql.providers.postgres_provider import (
        PostgresProvider,
    )
...
def _load_mysql(url: str, **kwargs: Any) -> DatabaseProviderProtocol:
    from oridecon.sql.providers.mysql_provider import MySQLProvider
    ...
    def _infer_provider_type(self, url: str) -> str:
        ...
        if url.startswith(("mongodb://", "mongodb+srv://")):
            ...
            raise ConfigurationError(
                "MongoDB URLs are not supported by oridecon-sql. "
                "Use oridecon-nosql instead: from oridecon.nosql import NoSQLProvider"
            )

        from oridecon.sql.lib import infer_provider_type_from_url
