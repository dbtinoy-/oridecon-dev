"""CLI integration utilities for oridecon-sql.

Provides a single, stable factory entry-point for the Oridecon CLI so that
CLI tools do not need to couple to internal class names inside ``oridecon-sql``.
"""

from __future__ import annotations

from oridecon.sql.migrations.manager import SimpleMigrationManager
from oridecon.sql.providers import DatabaseService
...
def create_cli_migration_manager(
    ...
    Constructs a :class:`~oridecon.sql.providers.DatabaseService` and a
    :class:`~oridecon.sql.migrations.manager.SimpleMigrationManager` from
    ...
    names from ``oridecon.sql`` directly.
    ...
    Returns:
        A :class:`~oridecon.sql.migrations.manager.SimpleMigrationManager`
