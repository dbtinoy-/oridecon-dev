...
_SPECS: tuple[tuple[str, str, str, str], ...] = (
    (
        ...
        "oridecon.sql.cli.generators.database_repository:DatabaseRepositoryGenerator",
        "src/repositories",
    ),
    (
        "filter",
        "Generate a query filter for database models",
        "oridecon.sql.cli.generators.filter:FilterGenerator",
        "src/filters",
    ),
    (
        "seeder",
        "Generate a database seeder for test/dev data",
        "oridecon.sql.cli.generators.seeder:SeederGenerator",
        "seeds",
    ),
    (
        "health",
        "Generate a database health check",
        "oridecon.sql.cli.generators.health_check:HealthCheckGenerator",
        "src/health",
    ),
    (
        "model",
        "Generate a Pydantic entity model with Create/Update DTOs",
        "oridecon.sql.cli.generators.entity_model:EntityModelGenerator",
        "src/models",
    ),
    (
        "migration",
        "Generate a chained alembic migration for an entity",
        "oridecon.sql.cli.generators.entity_migration:EntityMigrationGenerator",
        "migrations/versions",
    ),
    (
        "service",
        "Generate a service with unit of work",
        "oridecon.sql.cli.generators.service:ServiceGenerator",
        "src/services",
    ),
)
...
class SqlCliContributor:
    """CLI contributor for the oridecon-sql package."""
    ...
    def get_commands(self) -> list[CommandContribution]:
        ...
        return [
            CommandContribution(
                ...
                app_factory_path="oridecon.sql.cli.commands:create_db_app",
                contributor="sql",
                category="database",
    ...
    def get_health_checks(self) -> list[HealthCheckContribution]:
        ...
        return [
            HealthCheckContribution(
                ...
                check_path="oridecon.sql.cli.checks:check_database_connection",
                contributor="sql",
                category="database",
                timeout=15.0,
                critical=True,
    ...
    def get_doctor_checks(self) -> list[DoctorCheckContribution]:
        ...
        return [
            DoctorCheckContribution(
                ...
                check_path="oridecon.sql.cli.doctor:check_database_url",
                contributor="sql",
                category="database",
            ),
            DoctorCheckContribution(
                ...
                check_path="oridecon.sql.cli.doctor:check_migrations_dir",
                contributor="sql",
                category="database",
                can_fix=True,
            ),
    ...
    def get_shell_context(self) -> list[ShellContextContribution]:
        ...
        return [
            ShellContextContribution(
                ...
                factory_path="oridecon.sql.cli.shell:provide_db",
                contributor="sql",
            ),
            ShellContextContribution(
                name="migration",
                description="Migration runner for interactive use",
                factory_path="oridecon.sql.cli.shell:provide_migration",
                contributor="sql",
            ),
        ]
