...
Note: The ``db`` command name is registered as a built-in by ``oridecon-cli``
...
``oridecon contrib inspect sql`` and any tooling that introspects contributed
command surfaces.
"""
...
def create_db_app() -> typer.Typer:
    """Create the ``oridecon db`` command group.
