...
filter algebra and pagination specifications from ``oridecon.contracts.data``.

Example::

    from oridecon.sql.query.builder import QueryBuilder
    from oridecon.contracts.data import FieldEq, SortSpecification
...
class SearchQueryBuilder(QueryBuilder):
    ...
    Search-extension packages (``oridecon-search``, ``oridecon-sql``) accept
    ...
    Example::
        ...
        from oridecon.contracts.data import FieldEq
        from oridecon.sql.query import SearchQueryBuilder
    ...
    def filter_expr(self, expression: FilterExpression) -> SearchQueryBuilder:
        ...
        Args:
            expression: A ``FilterExpression`` (e.g. ``FieldEq``, ``AndExpr``)
                from ``oridecon.contracts.data.protocols``.
