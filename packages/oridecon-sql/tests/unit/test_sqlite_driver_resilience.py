...
from oridecon.contracts import HealthStatus
...
class TestSQLiteDriver:
    ...
    async def test_sqlite_pool_with_monitoring(self, mock_monitor):
        ...
        with patch("oridecon.sql.backends.sqlite._pool.aiosqlite") as mock_aiosqlite:
            mock_conn = AsyncMock()
    ...
    async def test_sqlite_pool_retry_logic(self):
        ...
        with patch("oridecon.sql.backends.sqlite._pool.aiosqlite"):
            # Mock retry_call to track calls
            with patch(
                "oridecon.sql.backends.sqlite._pool.retry_call", new_callable=AsyncMock
            ) as mock_retry:
                mock_conn = AsyncMock()
    ...
    def test_sqlite_import_error_when_unavailable(self):
        ...
        with patch("oridecon.sql.backends.sqlite._pool.HAS_SQLITE", False):
            with pytest.raises(
                ImportError,
