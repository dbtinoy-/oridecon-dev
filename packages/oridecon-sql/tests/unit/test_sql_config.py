"""Unit tests for oridecon.sql.config module."""
...
from oridecon.sql import config as sql_config
...
class TestDatabaseConfig:
    ...
    @patch.dict(os.environ, {"ORI_ENV": "development"})
    ...
    @patch.dict(os.environ, {"ORI_ENV": "production"})
...
class TestComponentPostgresConfig:
    ...
    def test_defaults(self):
        ...
        assert config.dsn == "postgresql://localhost:5432/oridecon"
        assert config.state_table == "oridecon_state"
        assert config.lock_table == "oridecon_locks"
