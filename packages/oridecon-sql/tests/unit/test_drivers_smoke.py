...
from oridecon.sql.backends.sqlite import (
    SQLiteConnection,
    SQLiteConnectionPool,
    create_sqlite_pool,
)
from oridecon.sql.monitoring import DatabaseMonitor, InMemoryDbMetricsCollector

# Import availability flags
try:
    from oridecon.sql.backends.postgres import HAS_POSTGRES
except ImportError:
    HAS_POSTGRES = False

try:
    from oridecon.sql.backends.mysql import HAS_MYSQL
except ImportError:
    HAS_MYSQL = False

try:
    from oridecon.sql.backends.sqlite import HAS_SQLITE
except ImportError:
    HAS_SQLITE = False
...
class TestPostgresDriver:
    ...
    @pytest.mark.skipif(not HAS_POSTGRES, reason="asyncpg not available")
    @pytest.mark.asyncio
    @patch("oridecon.sql.backends.postgres.asyncpg.create_pool", new_callable=AsyncMock)
    ...
    @pytest.mark.skipif(not HAS_POSTGRES, reason="asyncpg not available")
    @pytest.mark.asyncio
    @patch("oridecon.sql.backends.postgres.asyncpg.create_pool")
...
class TestMySQLDriver:
    ...
    @pytest.mark.skipif(not HAS_MYSQL, reason="aiomysql not available")
    @pytest.mark.asyncio
    @patch("oridecon.sql.backends.mysql._pool.aiomysql.create_pool", new_callable=AsyncMock)
...
class TestSQLiteDriver:
    ...
    @pytest.mark.skipif(not HAS_SQLITE, reason="aiosqlite not available")
    @pytest.mark.asyncio
    @patch("oridecon.sql.backends.sqlite._pool.aiosqlite")
...
class TestDriverImports:
    ...
    def test_postgres_import(self):
        ...
        if importlib.util.find_spec("oridecon.sql.backends.postgres") is None:
            pytest.skip("asyncpg not available")

    def test_mysql_import(self):
        """Test MySQL driver can be imported"""
        if importlib.util.find_spec("oridecon.sql.backends.mysql") is None:
            pytest.skip("aiomysql not available")

    def test_sqlite_import(self):
        """Test SQLite driver can be imported"""
        if importlib.util.find_spec("oridecon.sql.backends.sqlite") is None:
