...
from oridecon.sql.config import (
...
class TestEnvironmentConfig:
    ...
    @patch.dict(
        os.environ,
        {
            "ORI_SQL__BACKEND__URL": "postgresql://user:pass@localhost:5432/db",
            "ORI_SQL__POOL__MAX_SIZE": "20",
            "ORI_SQL__POOL__MAX_OVERFLOW": "10",
            "ORI_SQL__POOL__TIMEOUT": "45",
            "ORI_SQL__OPERATIONS__ECHO": "true",
        },
    )
    def test_from_env_casting(self):
        """Test that DatabaseConfig.from_env() auto-loads and casts ORI_SQL__* env vars."""
        # DatabaseConfig.from_env() uses EnvironmentConfigSource("ORI_SQL__")
    ...
    @patch.dict(
        os.environ,
        {
            "ORI_CUSTOM_DB__BACKEND__URL": "sqlite:///test.db",
            "ORI_CUSTOM_DB__POOL__MAX_SIZE": "15",
        },
    )
