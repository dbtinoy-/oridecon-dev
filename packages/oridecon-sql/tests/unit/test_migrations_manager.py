...
def mock_alembic_components(monkeypatch):
    ...
    monkeypatch.setattr("oridecon.sql.migrations.manager._alembic.ALEMBIC_AVAILABLE", True)

    with patch("oridecon.sql.migrations.manager._alembic.Config") as MockConfig, \
         patch("oridecon.sql.migrations.manager._alembic.MigrationEngine") as MockEngine, \
         patch("oridecon.sql.migrations.manager._alembic.SchemaIntrospector") as MockIntrospector, \
         patch("oridecon.sql.migrations.manager._alembic.MigrationOrchestrator") as MockOrchestrator:
