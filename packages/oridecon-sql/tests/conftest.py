...
# this mirrors the similar logic in other oridecon packages and is
# intentionally simple so that pytest can find `oridecon.sql`,
# `oridecon.contracts` and `oridecon.resilience` even though they live
# outside the tests directory during development.
PROJECT_ROOT = os.path.abspath(os.path.join(BASE_DIR, ".."))
sys.path.insert(0, os.path.join(PROJECT_ROOT, "src"))
sys.path.insert(0, os.path.join(PROJECT_ROOT, "..", "oridecon-contracts", "src"))
...
# may pull in optional packages like oridecon.admin that are not available
# in all test environments.

# The fixtures the test suite expects (clean_database, seeded_database, etc.)
# are provided by oridecon.testing.fixtures.db; import them lazily below in
...
try:
    ...
    from oridecon.testing.fixtures.db import (
        clean_database,
        db_test_bed,
        mock_connection,
    ...
    # Best-effort: some developer environments don't have oridecon-testing avail.
    pass
...
async def test_bed():
    ...
    dependencies like `oridecon.admin`). If `oridecon.testing` isn't
    available in this environment, skip the dependent tests gracefully.
    """
    try:
        import importlib

        testing = importlib.import_module("oridecon.testing.fixtures.bed")
        TestEnvironment = getattr(testing, "TestEnvironment")
    except (ImportError, AttributeError):
        pytest.skip(
            "oridecon.testing.TestEnvironment not available in this environment",
        )
...
async def postgres_provider(integration_config):
    ...
    """
    from oridecon.sql.providers.postgres_provider import PostgresProvider
    from oridecon.sql.providers.sqlite_provider import SQLiteProvider
...
def _shorten_pool_poll_interval():
    ...
    try:
        from oridecon.sql.pool.connection import AbstractConnectionPool
    except ImportError:
        yield
        return
