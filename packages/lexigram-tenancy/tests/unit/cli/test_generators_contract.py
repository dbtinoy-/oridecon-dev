"""Generator contract test for the tenancy CLI contributor."""

from __future__ import annotations

from pathlib import Path

from lexigram.tenancy.cli.contributor import TenancyCliContributor
...
def test_all_generators_render(tmp_path: Path) -> None:
    ...
    count = assert_contributor_generators_render(
        TenancyCliContributor(), tmp_path=tmp_path
    )
    assert count >= 1
