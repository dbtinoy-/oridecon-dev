---
title: oridecon-web Middleware
description: Middleware architecture, built-in middleware, custom middleware, and testing
---

## Architecture

oridecon-web's middleware follows the ASGI model — each middleware wraps the next in a chain:
...
|-----------|--------|---------|
| `SecurityHeadersMiddleware` | `oridecon.web.middleware.security` | HSTS, CSP, X-Frame-Options, cross-origin isolation |
| `CORSMiddleware` | `oridecon.web.middleware.cors` | Cross-origin resource sharing |
| `CSRFProtectionMiddleware` | `oridecon.web.security.csrf.middleware` | CSRF token validation |
| `RateLimitMiddleware` | `oridecon.web.middleware.rate_limit` | Sliding-window rate limiting |
| `InputSanitizationMiddleware` | `oridecon.web.middleware.sanitization` | Strips null bytes and script injection from query params |
| `RequestBodySizeLimitMiddleware` | `oridecon.web.middleware.body_limit` | Enforces `max_body_size` (default 10 MiB) |
| `AccessLogMiddleware` | `oridecon.web.middleware.access_log` | Structured request logging |
| `RequestIDMiddleware` | `oridecon.web.middleware.request_id` | Generate and propagate `X-Request-Id` headers |
| `TimingMiddleware` | `oridecon.web.middleware.timing` | `X-Response-Time` header |
| `CompressionMiddleware` | `oridecon.web.middleware.compression` | Gzip/brotli response compression |
| `StaticFilesMiddleware` | `oridecon.web.middleware.static` | Serve static files |
...
For middleware that needs DI-resolved services, implement `MiddlewareProtocol` from `oridecon.web.middleware.base`:

```python
from oridecon.web.middleware import MiddlewareChain
...
Packages can register middleware via the `oridecon.web.contributors` entry point. The `WebContributorRegistry` merges them at boot.
...
Use `TestClient` from `oridecon.testing`:

```python
from oridecon.testing import WebTestClient
from oridecon.web import WebProvider
