#!/usr/bin/env python3
"""Integration test for oridecon-web with mocked oridecon"""
...
def test_web_provider_integration():
    """Test basic WebProvider integration with oridecon"""
    logger.info("Testing oridecon-web integration with oridecon...")

    # Create a mock context for this test only
    mock_modules = {
        "oridecon": Mock(),
        "oridecon.di": Mock(),
        "oridecon.providers": Mock(),
        "oridecon.application": Mock(),
    }
    ...
    mock_modules["oridecon.di"].Container = Mock(return_value=mock_container)
    mock_modules["oridecon.providers"].Provider = FakeProvider
    mock_modules["oridecon.application"].Application = mock_application
    ...
    try:
        ...
        from oridecon.web.server.provider import WebProvider

        # Create a test controller
        class TestController(Controller):
            @get("/test")
            async def test_endpoint(self):
                return {"message": "Hello from oridecon-web!"}
...
def test_full_integration():
    ...
    try:
        ...
        from oridecon.web.routing.controllers import Controller
        from oridecon.web import get, post
        from oridecon.web.server.provider import WebProvider
...
async def main():
    ...
    logger.info("Oridecon-Web Integration Testing with Oridecon-Core")
    logger.info("=" * 60)
    ...
    if passed == total:
        ...
        logger.info("   WebProvider registers with oridecon Application")
        ...
        logger.info("Oridecon-web is fully integrated with oridecon!")
