...
- Both decorators are importable from ``oridecon.web``

Each test creates a fresh :class:`~oridecon.web.quickstart._QuickstartApp`
...
from oridecon.web.quickstart import _PendingRoute, _QuickstartApp
...
# ---------------------------------------------------------------------------
# Test 1 — importable from oridecon.web
# ---------------------------------------------------------------------------


def test_singleton_and_injectable_importable_from_oridecon_web() -> None:
    """Both decorators can be imported directly from ``oridecon.web``."""
    from oridecon.web import injectable, singleton  # noqa: F401
...
async def test_singleton_resolved_in_route_handler() -> None:
    ...
    Uses :func:`~oridecon.web.quickstart.singleton` to mark a class, then
    verifies that the HTTP response reflects the service's output.
    """
    from oridecon.web.quickstart import singleton
...
async def test_injectable_new_instance_per_request() -> None:
    ...
    """
    from oridecon.web.quickstart import injectable
...
def test_quickstart_registry_populated_by_decorators() -> None:
    ...
    """
    from oridecon.web.quickstart import (
