...
from oridecon.web.config import (
    APIDocsConfig,
    RateLimitConfig,
    RateLimitRuleConfig,
    ServerConfig,
...
class TestWebProviderConfig:
    ...
    def test_web_provider_config_custom_values(self):
        ...
        config = WebProviderConfig(
            ...
            middleware=["oridecon.web.middleware.cors.CORSMiddleware"],
            exception_filters=["oridecon.web.filters.DefaultExceptionFilter"],
        )

        assert config.openapi_title == "My API"
        assert config.openapi_version == "2.0.0"
        assert config.middleware == ["oridecon.web.middleware.cors.CORSMiddleware"]
        assert config.exception_filters == [
            "oridecon.web.filters.DefaultExceptionFilter",
        ]
