...
from oridecon.web.routing.health_checks import ComponentHealth, HealthCheckResponse
...
async def test_health_returns_200_when_healthy(test_bed) -> None:
    ...
    with patch(
        "oridecon.web.routing.health.WebHealthChecker.check_health",
...
async def test_probe_routes_use_application_health(
    ...
    with patch(
        f"oridecon.app.base.Application.{method_name}",
        new_callable=AsyncMock,
