...
async def test_quickstart_di_injection() -> None:
    ...
    Verifies that :func:`~oridecon.web.routing.route_handlers._wrap_script_handler`
...
async def test_quickstart_singleton_service_injected() -> None:
    ...
    The ``@singleton`` decorator marks the class with ``__oridecon_injectable__``.
    ...
    async def greet_handler(svc: GreetingService) -> dict:
        return {"message": svc.greet("Oridecon")}
    ...
    assert response.status_code == 200
    assert response.json() == {"message": "Hello, Oridecon!"}
