"""Unit tests for oridecon.app.discovery — provider, controller, and injectable scanning."""
...
from oridecon.di.decorators import INJECTABLE_ATTR
...
class TestDiscoverProviders:
    def test_nonexistent_package_returns_empty_list(self) -> None:
        result = discover_providers(["oridecon_test_nonexistent_pkg_xyz"])
        assert result == []
...
class TestDiscoverControllers:
    def test_nonexistent_package_returns_empty_list(self) -> None:
        result = discover_controllers(["oridecon_test_nonexistent_ctrl_pkg_xyz"])
        assert result == []
...
class TestDiscoverInjectables:
    def test_finds_singleton_decorated_class(self) -> None:
        from oridecon.di.decorators import singleton
    ...
    def test_nonexistent_package_returns_empty_list(self) -> None:
        result = discover_injectables(["oridecon_test_nonexistent_inj_pkg_xyz"])
        assert result == []

    def test_multiple_decorated_classes_all_returned(self) -> None:
        from oridecon.di.decorators import injectable, singleton
