...
from oridecon.web.middleware.adapter import _OrideconMiddlewareAdapter
...
def _build(middleware: Any) -> Starlette:
    """Wrap a route with the Oridecon adapter for the given middleware."""
    return Starlette(
        middleware=[
            StarletteMiddleware(_OrideconMiddlewareAdapter, oridecon_mw=middleware)
        ],
...
def test_registry_falls_back_to_the_oridecon_adapter() -> None:
    ...
    assert adapted.cls is _OrideconMiddlewareAdapter
    assert adapted.kwargs["oridecon_mw"] is _NativeASGIMiddleware
