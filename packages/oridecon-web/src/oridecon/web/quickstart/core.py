"""Quick-start module for single-file Oridecon web applications.
...
by the :class:`~oridecon.web.routing.route_handlers.CoreRouteHandler` when the
application boots.
...
def singleton(cls: type[T]) -> type[T]:
    ...
    Combines the ``oridecon.di.singleton`` DI marker (which sets
    ``__oridecon_injectable__``) with a direct entry in the quickstart
    ...
    from oridecon.di.decorators import Injectable
...
def injectable(cls: type[T]) -> type[T]:
    ...
    Example::
        ...
        from oridecon.web import app, get, injectable
        from oridecon.logging import get_logger
...
class _QuickstartApp:
    ...
    On first ASGI call the real :class:`~oridecon.app.base.Application` and
    :class:`~oridecon.web.di.provider.WebProvider` are created and booted.
    ...
    def __init__(self) -> None:
        ...
        env = os.environ.get("ORI_ENV", os.environ.get("APP_ENV", "development"))
        if env == "production":
            import warnings

            warnings.warn(
                "oridecon.web quickstart module is loaded in a production environment "
                "(ORI_ENV=production). Quickstart is for development only. "
    ...
    def _collect_script_routes(self) -> list[_PendingRoute]:
        ...
        for mod in list(sys.modules.values()):
            ...
            if module_name.startswith("oridecon"):
                continue
    ...
    async def _ensure_booted(self) -> None:
        ...
        self._application = Application(name="oridecon-quickstart")
...
#: Import as ``from oridecon.web import app``.
