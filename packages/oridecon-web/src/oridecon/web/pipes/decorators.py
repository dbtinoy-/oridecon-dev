"""Pipes decorators for Oridecon Framework.
...
from oridecon.web.protocols import PipeProtocol
...
def use_pipes(*pipe_instances: PipeProtocol) -> Callable[[T], T]:
    ...
    def decorator(target: T) -> T:
            ...
            for name, method in inspect.getmembers(
                ...
                if not name.startswith("_"):  # Skip private methods
                    existing: list[PipeProtocol] = (
                        getattr(method, "_oridecon_pipes", None) or []
                    )
                    # Prepend class-level pipes so they run before method-level ones
                    cast("Any", method)._oridecon_pipes = (
        ...
        existing_pipes: list[PipeProtocol] = (
            getattr(target, "_oridecon_pipes", None) or []
        )
        cast("Any", target)._oridecon_pipes = existing_pipes + list(pipe_instances)
        return target
