"""Default middleware stack for Oridecon Web.
...
every Oridecon web application.
...
Example::
    ...
    from oridecon.web.middleware import DefaultMiddlewareStack
...
class DefaultMiddlewareStack:
    """Builds the default Oridecon middleware stack as an explicit, composable list.

    Encapsulates the minimum set of middlewares that every Oridecon web
    application applies so users can see and extend it without magic.

    :class:`~oridecon.web.di.provider.WebProvider` delegates to this class
    internally, ensuring that all code paths produce the same default stack.

    Args:
        container: The resolved DI container.  Required for
            :class:`~oridecon.web.middleware.di_scope.DIScopeMiddleware` which
            provides request-scoped dependency resolution.
        extra_middlewares: Additional user-supplied Oridecon middlewares to
            include in the stack.  They are adapted to Starlette
            ``Middleware`` wrappers via
            :class:`~oridecon.web.middleware.registry.MiddlewareAdapterRegistry`
    ...
    def __init__(
        ...
        Args:
            ...
            extra_middlewares: Optional extra Oridecon middlewares to include.
        """
