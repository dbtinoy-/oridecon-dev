...
from oridecon.web.protocols import ExecutionContextProtocol
...
class ParameterBinder:
    ...
    async def bind(self, context: WebExecutionContext) -> dict[str, Any]:
        ...
        """
        from oridecon.web.routing.validation import validate_and_merge_request
        ...
        for param_name, param in self.sig.parameters.items():
            ...
            if value is None:
                if param_name in path_params:
                    ...
                    if hasattr(param.default, "_oridecon_param_info"):
                        value = param.default._oridecon_param_info.get("default")
                    else:
                        value = param.default
    ...
    async def _run_pipes(
        ...
        from oridecon.web.protocols import ParamMetadata

        # Collect pipes
        pipes = []

        # 1. Parameter-level pipes
        info = getattr(param.default, "_oridecon_param_info", {})
        if info and "pipes" in info and info["pipes"]:
            pipes.extend(info["pipes"])

        # 2. Handler-level pipes (@use_pipes)
        handler_pipes = getattr(context.handler, "_oridecon_pipes", [])
