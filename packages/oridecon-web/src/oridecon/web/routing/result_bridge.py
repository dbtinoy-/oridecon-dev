...
def _exception_type_urn(exc: Exception) -> str:
    """Derive a urn:oridecon:{slug} type URI from an exception class name."""
...
class ResultResponseMapper:
    ...
    def to_response(self, result: Any, success_status: int = 200) -> Response:
        ...
        Returns:
            A :class:`~oridecon.web.transport.responses.JSONResponse` with
    ...
    def error_to_response(cls, error: Any) -> Response:
        ...
        Returns:
            A :class:`~oridecon.web.transport.responses.JSONResponse` with the
