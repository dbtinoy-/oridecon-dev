...
from oridecon.web.routing.param_coercion import (
    _cached_get_type_hints_for_handler,
    _create_validation_model,
    _get_cached_combined_model,
    _get_cached_query_model,
    _is_simple_type,
    _set_cached_combined_model,
    _set_cached_query_model,
)
...
def get_or_build_combined_model(handler: Any) -> type[DomainModel] | None:
    ...
    for i, (name, param) in enumerate(sig.parameters.items()):
        ...
        meta = getattr(param.default, "_oridecon_param_info", None)
        if meta is None:
        ...
        if _is_simple_type(annotation) or is_explicit_source:
            # If default is a Oridecon decorator, use its internal default instead
            if hasattr(default, "_oridecon_param_info"):
                decorator_info = default._oridecon_param_info
...
async def validate_and_merge_request(
    ...
    for i, (p_name, param) in enumerate(sig.parameters.items()):
        ...
        info = getattr(param.default, "_oridecon_param_info", None)
        if info:
    ...
    try:
        ...
        from oridecon.contracts.exceptions.domain import (
            ValidationError as OrideconValidationError,
        )

        if isinstance(e, OrideconValidationError):
            raise
        ...
        if validator and callable(validator):
            ...
            if val is not None:
                ...
                try:
                    res = validator(val)
                    if res is False:
                        # Create a mock Oridecon error for consistency
                        from oridecon.contracts.exceptions.domain import FieldError
