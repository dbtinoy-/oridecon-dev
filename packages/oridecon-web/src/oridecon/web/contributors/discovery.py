...
``oridecon.web.contributors`` entry-point group. Failed contributors
...
from oridecon.contracts.web import WebContributorProtocol
from oridecon.logging import get_logger

logger = get_logger(__name__)

ENTRY_POINT_GROUP = "oridecon.web.contributors"


def load_web_contributors() -> list[WebContributorProtocol]:
    """Load web contributors from entry-points.

    Scans the ``oridecon.web.contributors`` entry-point group and loads
    ...
    Example:
        A package registers a contributor via pyproject.toml::
            ...
            [project.entry-points."oridecon.web.contributors"]
            graphql = "oridecon.graphql.web.contributor:GraphQLWebContributor"
