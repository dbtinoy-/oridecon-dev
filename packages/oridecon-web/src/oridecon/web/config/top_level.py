...
from oridecon.validation import (
    ConfigDict,
    Field,
    SecretStr,
    model_validator,
...
from oridecon.web.config.api_docs import APIDocsConfig, StaticFileConfig
from oridecon.web.config.rate_limit import RateLimitConfig, RoleGuardConfig
from oridecon.web.config.server import ServerConfig
...
class WebConfig(BaseConfig):
    """Hierarchical root configuration for Oridecon Web.
    ...
    def validate_production_security(self) -> WebConfig:
        ...
        if (
            ...
            raise ValueError(
                ...
                "set specific origins via ORI_WEB__CORS__ALLOWED_ORIGINS.",
            )
        if self.environment == Environment.PRODUCTION:
            if self.cors.allowed_origins and "*" in self.cors.allowed_origins:
                raise ValueError(
                    ...
                    "You MUST set specific origins via ORI_WEB__CORS__ALLOWED_ORIGINS.",
                )
            if not self.security.csrf.enabled:
                raise ValueError(
                    "CRITICAL SECURITY ERROR: CSRF protection is disabled in PRODUCTION.\n"
                    "You MUST enable it via ORI_WEB__SECURITY__CSRF__ENABLED.",
                )
            csrf_key: str | None = (
            ...
            if not csrf_key or csrf_key.strip() == "":
                raise ValueError(
                    ...
                    "You MUST set one via ORI_WEB__SECURITY__CSRF__SECRET_KEY.",
                )
            if not self.security.allowed_hosts:
                raise ValueError(
                    "CRITICAL SECURITY ERROR: no allowed_hosts configured for PRODUCTION.\n"
                    "You MUST configure ORI_WEB__SECURITY__ALLOWED_HOSTS "
