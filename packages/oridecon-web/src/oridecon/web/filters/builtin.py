"""Built-in exception filters for Oridecon Web."""
...
from oridecon.web.errors.html_error_renderer import DebugHtmlErrorRenderer
from oridecon.web.exceptions import (
    DependencyResolutionError,
    HTTPError,
)
from oridecon.web.transport.responses import JSONResponse


class ValidationErrorFilter:
    """Filter for handling Pydantic and Oridecon validation errors."""
    ...
    async def handle(self, exc: Any, _request: Any) -> JSONResponse:
        ...
        from oridecon.web.errors.problem_detail import ProblemDetail

        pd = ProblemDetail(
            type="urn:oridecon:validation-error",
...
class DefaultExceptionFilter:
    ...
    def __init__(self, debug: bool = False) -> None:
        ...
        Args:
            debug: When ``True``, HTML error pages are rendered for browser
                ...
                the ``ORI_DEBUG`` environment variable.
        """
        self._debug = debug
        self._html_renderer = DebugHtmlErrorRenderer()

    def can_handle(self, exc: Exception) -> bool:
        """Handle HTTPErrors and OrideconErrors specifically."""
        from oridecon.contracts.exceptions import OrideconError

        return (
            isinstance(exc, (HTTPError, OrideconError))
    ...
    async def handle(self, exc: Exception, request: Any) -> Any:
        ...
        if isinstance(exc, HTTPError):
            ...
            pd = ProblemDetail(
                type=(
                    f"urn:oridecon:{code.lower().replace('_', '-')}"
                    if code
                    else "about:blank"
                ),
                title=exc.detail,
        ...
        status_code = 500
        type_uri = "urn:oridecon:internal-error"
        title = "Internal Server Error"

        if isinstance(exc, NotFoundError):
            status_code, type_uri, title = 404, "urn:oridecon:not-found", "Not Found"
        elif isinstance(exc, (PermissionDeniedError, AuthorizationError)):
            status_code, type_uri, title = 403, "urn:oridecon:forbidden", "Forbidden"
        elif isinstance(exc, AuthenticationError):
            status_code, type_uri, title = (
                401,
                "urn:oridecon:unauthorized",
                "Unauthorized",
            )
        elif isinstance(exc, ConflictError):
            status_code, type_uri, title = 409, "urn:oridecon:conflict", "Conflict"
        elif isinstance(exc, DomainError):
            status_code, type_uri, title = (
                400,
                "urn:oridecon:bad-request",
                "Bad Request",
            )
