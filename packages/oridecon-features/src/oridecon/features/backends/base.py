...
:class:`~oridecon.contracts.feature_flags.FlagProvider` protocol with a
richer ``evaluate`` coroutine and a complete suite of per-type evaluation
strategy helpers.

Concrete providers (:class:`~oridecon.feature_flags.backends.local.LocalProvider`,
:class:`~oridecon.feature_flags.backends.env.EnvProvider`, etc.) inherit from
...
from oridecon.features.constants import DEFAULT_ENABLED
...
class AbstractFlagProvider(ABC):
    ...
    This class is *not* a :class:`~oridecon.contracts.feature_flags.FlagProvider`
    protocol implementation — ``get_flag_definition`` returns a
    :class:`~oridecon.feature_flags.types.Flag` model, not a bare boolean.
    Use :class:`~oridecon.features.backends.local.LocalProvider`
