...
* ``true`` / ``false`` — :attr:`~oridecon.feature_flags.types.FlagType.BOOLEAN`
* ``percentage:N`` — :attr:`~oridecon.feature_flags.types.FlagType.PERCENTAGE` rollout
* ``users:user1,user2,...`` — :attr:`~oridecon.feature_flags.types.FlagType.USER_LIST`
"""
...
from oridecon.features.constants import ENV_PREFIX
from oridecon.features.types import Flag, FlagContext, FlagEvaluation, FlagType
