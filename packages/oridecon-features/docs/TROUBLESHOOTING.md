---
title: oridecon-features Troubleshooting
...
**Fix:** Install and register `oridecon-events`. The feature-flag module works without it — events are a silent no-op when absent.
...
**Symptom:** Flags set via environment variables (e.g. `ORI_FLAG_MY_FLAG=true`) are not picked up by `EnvProvider`.

**Cause:** `EnvProvider` uses `flag_env_prefix` (default: `ORI_FLAG_`) to build environment variable keys. If your variables use a different prefix like `ORI_FEATURES__`, the provider won't find them.
