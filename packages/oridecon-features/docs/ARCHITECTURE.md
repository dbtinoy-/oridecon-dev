...
Internal design of the `oridecon-features` package.
...
flowchart LR
    ...
    subgraph Features[oridecon-features]
        FM[FlagManager]
        LP[LocalProvider]
        EP[EnvProvider]
...
`oridecon-features` provides feature-flag evaluation for the Oridecon Framework. It sits in the infrastructure layer, available to every package and service via `FlagProviderProtocol`. The `FlagManager` is the primary entry point — it wraps any `AbstractFlagProvider`, adds TTL caching, runtime overrides, change listeners, audit logging, and optional event-bus integration.
...
From `oridecon-contracts` (`oridecon.contracts.feature_flags`):
...
The package's `protocols.py` re-exports all three protocols so consumers may import from `oridecon.features` rather than `oridecon.contracts.feature_flags`.
