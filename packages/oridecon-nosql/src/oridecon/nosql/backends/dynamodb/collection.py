...
from oridecon.contracts.data.nosql.nosql import BulkWriteResult, DocumentResult
from oridecon.logging import get_logger
from oridecon.nosql.exceptions import DuplicateKeyError, NoSQLError
...
class DynamoDBCollection:
    ...
    async def insert_one(self, document: dict[str, Any]) -> DocumentResult:
        ...
        Returns:
            :class:`~oridecon.contracts.data.nosql.nosql.DocumentResult`
            with ``document_id`` set to the generated or provided ``_id``.

        Raises:
            :class:`~oridecon.nosql.exceptions.DuplicateKeyError`: When
                DynamoDB raises ``ConditionalCheckFailedException`` because
                an item with the same primary key already exists.
            :class:`~oridecon.nosql.exceptions.NoSQLError`: On other
    ...
    async def insert_many(self, documents: list[dict[str, Any]]) -> BulkWriteResult:
        ...
        Returns:
            :class:`~oridecon.contracts.data.nosql.nosql.BulkWriteResult`.

        Raises:
            :class:`~oridecon.nosql.exceptions.NoSQLError`: On DynamoDB failure.
        """
        if not documents:
