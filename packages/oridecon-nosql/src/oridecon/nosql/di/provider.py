"""DI provider for oridecon-nosql."""
...
# Module-level name so tests can patch "oridecon.nosql.di.provider.MongoDBDocumentStore".
# Falls back to None when the optional motor/pymongo dependency is not installed;
# production code that needs MongoDB must install oridecon-nosql[mongodb].
try:
...
class NoSQLProvider(Provider):
    ...
    Reads :class:`~oridecon.nosql.config.NoSQLConfig`, creates the
