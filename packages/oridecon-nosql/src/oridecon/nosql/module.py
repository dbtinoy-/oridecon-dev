...
if TYPE_CHECKING:
    from oridecon.nosql.config import NoSQLConfig
...
class NoSQLModule(Module):
    ...
    Registers :class:`~oridecon.contracts.data.nosql.nosql.DocumentStoreProtocol`
    ...
    def configure(
        ...
        Args:
            config: :class:`~oridecon.nosql.config.NoSQLConfig` or ``None``
    ...
    def stub(cls, config: NoSQLConfig | None = None) -> DynamicModule:
        ...
        Args:
            config: Optional :class:`~oridecon.nosql.config.NoSQLConfig` override.
