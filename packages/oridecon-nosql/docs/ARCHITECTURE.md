...
Internal design of the `oridecon-nosql` package.

---

## Role in the System

`oridecon-nosql` lives in the **Data & Persistence** tier alongside `oridecon-sql`. It implements `DocumentStoreProtocol` and `CollectionProtocol` from `oridecon-contracts` and provides pluggable backends for document-oriented storage.

```mermaid
flowchart BT
    App[Application]
    NoSQL[oridecon-nosql]
    SQL[oridecon-sql]
    Graph[oridecon-graph]
    Contracts[oridecon-contracts<br/>· DocumentStoreProtocol<br/>· CollectionProtocol]
    Core[oridecon<br/>DI · Container · Provider · Module]
...
**Dependency rule:** `oridecon-nosql` imports from `oridecon` and `oridecon-contracts` only. It never imports from `oridecon-sql`, `oridecon-graph`, or any other extension package.
...
`DocumentRepository[TEntity, TKey]` provides the same developer experience as `SQLRepository` from `oridecon-sql`, but operates on documents instead of SQL rows.
...
|----------|--------|---------------|
| `DocumentStoreProtocol` | `oridecon.contracts.data.nosql.nosql` | Registered by `NoSQLProvider`; injected into services |
| `CollectionProtocol` | `oridecon.contracts.data.nosql.nosql` | Returned by `store.collection()`; implemented by `MongoDBCollection` |
| `DocumentResult`, `BulkWriteResult` | `oridecon.contracts.data.nosql.nosql` | Return types for CRUD operations |
| `FilterExpression` (subclasses) | `oridecon.contracts.data.protocols` | Input to `to_filter()` for spec-to-query conversion |
| `HealthCheckResult` | `oridecon.contracts.core.health` | Returned by `health_check()` |
| `DomainEvent` | `oridecon.contracts.domain.events` | Base for lifecycle events |

### Exceptions

```
NoSQLError(OrideconError)
