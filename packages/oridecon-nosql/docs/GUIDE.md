...
| `oridecon-cache` | Optional | Connection caching |
...
`oridecon-nosql` provides a **document-oriented** data layer parallel to `oridecon-sql`'s relational layer. The key abstractions are:
...
`DocumentStoreProtocol` (from `oridecon-contracts`) is the top-level handle. It connects, disconnects, hands out `CollectionProtocol` handles, and supports multi-document transactions.
...
```python
from oridecon.nosql.repository.base import DocumentRepository
...
```python
from oridecon.nosql import AggregationPipeline
...
Use filter expressions from `oridecon.contracts.data.protocols` for reusable queries:

```python
from oridecon.contracts.data.protocols import FieldEq, AndExpr
...
```python
from oridecon.nosql import MigrationManager, CreateIndex, AddField
