...
from oridecon.nosql.config import NamedNoSQLConfig, NoSQLConfig
...
class TestNoSQLProviderMultiBackend:
    ...
    async def test_register_multi_backend_named_bindings(self) -> None:
        ...
        with patch("oridecon.nosql.di.provider.MongoDBDocumentStore"):
    ...
    async def test_boot_connects_all_stores_in_parallel(self) -> None:
        ...
        with patch("oridecon.nosql.di.provider.MongoDBDocumentStore", side_effect=[mock_store_1, mock_store_2]):
            await provider.register(container)
