...
from oridecon.nosql.exceptions import DuplicateKeyError, NoSQLConnectionError, NoSQLError
...
class TestDynamoDBBackendConnect:
    ...
    async def test_connect_uses_explicit_credentials(self) -> None:
        ...
        with patch.dict(sys.modules, {"aioboto3": aioboto3_mock}):
            with patch(
                "oridecon.nosql.backends.dynamodb.backend.DynamoDBBackend._probe_connectivity",
...
class TestExports:
    ...
    def test_dynamodb_backend_importable_from_backends(self) -> None:
        from oridecon.nosql.backends import DynamoDBBackend as _DDB

        assert _DDB is DynamoDBBackend

    def test_dynamodb_config_importable_from_nosql(self) -> None:
        from oridecon.nosql import DynamoDBConfig as _Cfg  # type: ignore[attr-defined]
