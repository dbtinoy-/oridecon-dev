...
```python
from oridecon.auth import PersistentTokenRevocationStore
...
```python
from oridecon.auth import PasswordHasher
...
:::tip
Oridecon also supports the `KeyDerivationProtocol` (Argon2id) from `oridecon.contracts.security` for environments that prefer Argon2 over bcrypt.
:::
...
The `AuthMiddlewareConfig.login_rate_limit` field sets a default rate limit of `5/minute` on login routes. Use the `RateLimitMiddleware` from `oridecon-web` in front of auth endpoints for additional protection:
...
```bash
export ORI_AUTH__TOKEN__SECRET_KEY="$(openssl rand -hex 32)"
export ORI_AUTH__SECRET_KEY="$(openssl rand -hex 32)"
```
...
:::caution
**Common misconfiguration is no longer possible**: JWT signature verification is **always** enabled and cannot be disabled. If `DEVELOPMENT` boots without `ORI_AUTH__TOKEN__SECRET_KEY`, an ephemeral secret is generated so verification still runs (tokens invalidated on restart). `PRODUCTION` / `STAGING` raise at boot without a strong, non-default secret.
:::
