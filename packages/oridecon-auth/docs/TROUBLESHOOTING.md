...
**Exception:** `AccountLockedError` (from `oridecon.auth.exceptions`)
...
```python
from oridecon.contracts.auth import LoginAttemptTrackerProtocol
...
**Exception:** `TokenExpiredError` (from `oridecon.auth.exceptions`)
...
You MUST set a secure secret key via ORI_AUTH__TOKEN__SECRET_KEY.
```

**Solution:** Set a strong secret key:
```bash
export ORI_AUTH__TOKEN__SECRET_KEY=$(python3 -c "import secrets; print(secrets.token_hex(32))")
export ORI_AUTH__SECRET_KEY=$(python3 -c "import secrets; print(secrets.token_hex(32))")
```
...
**Exception:** `TokenAudienceError` (from `oridecon.auth.exceptions`)
...
**Exception:** `TokenBlacklistedError` (from `oridecon.auth.exceptions`)
...
**Exception:** `PasswordPolicyError` (from `oridecon.auth.exceptions`)
...
- **Verify config loading:** `AuthConfig()` reads `ORI_AUTH__*` env vars — check they're set correctly
