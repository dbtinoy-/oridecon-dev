...
Internal design of the `oridecon-auth` package.

---

## Role in the System

`oridecon-auth` is the **security** layer, depending only on `oridecon` and `oridecon-contracts`. Other packages consume auth capabilities through protocols resolved from the DI container — never direct imports.

```mermaid
flowchart BT
    App[Application code]
    Auth[auth — AuthBundleProvider<br/>Authentication · Tokens · Sessions<br/>Authorization · RBAC · ABAC]
    Core[oridecon — DI · Config · Events · Hooks]
    Contracts[oridecon-contracts — AuthenticatorProtocol · AuthorizerProtocol<br/>TokenManagerProtocol · PasswordHasherProtocol<br/>UserStoreProtocol · SessionRepositoryProtocol]
...
**Rules:** `AuthError` (contracts) is the catch-all. `TokenError`/`VerificationError` are expected, recoverable — use `Result`. Leaf exceptions (23 total, all with `ORI_ERR_AUTH_*` codes) live in `oridecon.auth.exceptions`. Infrastructure failures propagate as exceptions, never wrapped in `Result`.
