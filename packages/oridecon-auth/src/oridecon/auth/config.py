"""Configuration models for Oridecon Auth.

This module provides Pydantic models for configuring authentication
and authorization in Oridecon applications.
...
Example:
    ...
    # From environment
    config = AuthConfig()  # reads ORI_AUTH__* env vars
"""
...
class JWTConfig(BaseConfig):
    ...
    - ``DEVELOPMENT``: A missing secret falls back to a generated ephemeral
      ...
      invalidated on restart. Set ``ORI_AUTH__TOKEN__SECRET_KEY`` for a
        ...
        if env in _STRICT_ENVS:
            ...
            if self.required_audience is None:
                raise ValueError(
                    ...
                    "ORI_AUTH__TOKEN__REQUIRED_AUDIENCE or token.required_audience in config.",
                )
...
class AuthConfig(BaseConfig):
    """Hierarchical root configuration for Oridecon Auth.
