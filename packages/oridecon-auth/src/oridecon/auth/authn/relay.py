...
Application-level adapter binding oridecon-auth's ``APIKeyManager``
...
importing oridecon-auth.  Credential parsing follows new-api's
...
class RelayApiKeyVerifier:
    """Verify inbound relay keys through oridecon-auth's API key manager.
    ...
    :class:`~oridecon.auth.authn.apikeys.APIKeyManager`.  It never logs
...
def _normalize(raw_key: str) -> str:
    ...
    Harmless for oridecon's ``sk_live_...`` keys (no leading dash) and
