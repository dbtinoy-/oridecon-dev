...
Collaborators of :mod:`oridecon.auth.authn.oauth2`: they normalise requests
...
class OrideconConnectSession:
    """Session adapter for authlib that delegates to ``HTTPClientProtocol``.

    Bridges authlib's internal session interface to Oridecon's
    ...
    async def request(
        ...
        url: str,
        **kwargs: Any,
    ) -> OrideconConnectResponse:
        """Make a request using the injected ``HTTPClientProtocol``."""
        if self._http_client is None:
            raise RuntimeError("HTTP client not configured for OrideconConnectSession")
        ...
        return OrideconConnectResponse(response)

    async def get(self, url: str, **kwargs: Any) -> OrideconConnectResponse:
        if self._http_client is None:
            raise RuntimeError("HTTP client not configured for OrideconConnectSession")
        response = await self._http_client.get(url, **kwargs)
        return OrideconConnectResponse(response)

    async def post(self, url: str, **kwargs: Any) -> OrideconConnectResponse:
        if self._http_client is None:
            raise RuntimeError("HTTP client not configured for OrideconConnectSession")
        response = await self._http_client.post(url, **kwargs)
        return OrideconConnectResponse(response)

    async def put(self, url: str, **kwargs: Any) -> OrideconConnectResponse:
        if self._http_client is None:
            raise RuntimeError("HTTP client not configured for OrideconConnectSession")
        response = await self._http_client.put(url, **kwargs)
        return OrideconConnectResponse(response)

    async def delete(self, url: str, **kwargs: Any) -> OrideconConnectResponse:
        if self._http_client is None:
            raise RuntimeError("HTTP client not configured for OrideconConnectSession")
        response = await self._http_client.delete(url, **kwargs)
        return OrideconConnectResponse(response)

    async def patch(self, url: str, **kwargs: Any) -> OrideconConnectResponse:
        if self._http_client is None:
            raise RuntimeError("HTTP client not configured for OrideconConnectSession")
        response = await self._http_client.patch(url, **kwargs)
        return OrideconConnectResponse(response)

    async def head(self, url: str, **kwargs: Any) -> OrideconConnectResponse:
        if self._http_client is None:
            raise RuntimeError("HTTP client not configured for OrideconConnectSession")
