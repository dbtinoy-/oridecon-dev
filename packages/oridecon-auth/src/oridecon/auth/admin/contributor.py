"""Admin contributor for oridecon-auth — surfaces session, token, and login
...
_ACTIONS: tuple[AdminActionDefinition, ...] = (
    AdminActionDefinition(
        ...
        contributor="auth",
        handler="oridecon.auth.admin.actions:revoke_all_sessions",
        icon="shield-off",
...
class AuthAdminContributor(BaseAdminContributor):
    """Admin contributor for the oridecon-auth package.
    ...
    async def on_admin_boot(self, container: ContainerResolverProtocol) -> None:
        ...
        from oridecon.auth.admin.handlers.failed_logins import (
    ...
    def get_management_pages(self) -> Sequence[ManagementPageDefinition]:
        ...
        return [
            ManagementPageDefinition(
                ...
                route_path="/auth",
                handler="oridecon.auth.admin.pages.overview:AuthOverviewPage",
                icon="lock-closed",
            ...
            ManagementPageDefinition(
                ...
                handler="oridecon.auth.admin.pages.users:AuthUsersPage",
                icon="users",
                category=PageCategory.SECURITY,
            ),
            ManagementPageDefinition(
                ...
                handler="oridecon.auth.admin.pages.sessions:AuthSessionsPage",
                icon="activity",
                category=PageCategory.SECURITY,
            ),
            ManagementPageDefinition(
                ...
                handler="oridecon.auth.admin.pages.tokens:AuthTokensPage",
                icon="key",
                category=PageCategory.SECURITY,
            ),
        ]
