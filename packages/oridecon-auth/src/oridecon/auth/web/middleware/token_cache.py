...
class TokenCache:
    ...
    * ``oridecon.cache.backends.memory.MemoryCacheBackend`` (from
      *oridecon-cache*) for a process-level shared cache with richer
      configuration.
    * A Redis-backed ``CacheBackendProtocol`` implementation for cross-process token
      invalidation.

    Note: ``oridecon-auth`` does not import from ``oridecon-cache`` directly
