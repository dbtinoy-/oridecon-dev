"""GuardProtocol services for authorization in Oridecon Framework"""
...
from oridecon.auth.types import GuardContext
from oridecon.contracts.web import ResponseFactoryProtocol
from oridecon.contracts.web.guard import GuardProtocol
from oridecon.logging import get_logger
from oridecon.primitives.context import Context, get_request_context

if TYPE_CHECKING:
    from collections.abc import Callable

    from oridecon.contracts.web import ResponseProtocol
...
def _get_request_resolver(request: Any) -> Any | None:
    ...
    if isinstance(scope, dict):
        return scope.get("oridecon_resolver")
...
def use_guards(
    *guards: GuardProtocol,
    ...
    .. note::
        ...
        internal use within ``oridecon-auth``.  For general-purpose use outside
        the auth subsystem, prefer ``oridecon.security.guards.use_guards`` which
