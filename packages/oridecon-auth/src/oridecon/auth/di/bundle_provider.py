...
from oridecon.auth.di.sub_providers.admin_provider import AuthAdminProvider
...
class AuthBundleProvider(Provider):
    """Composite provider that wires the full Oridecon auth stack.

    Composes :class:`~oridecon.auth.di.AuthenticationProvider`,
    :class:`~oridecon.auth.di.sub_providers.token_provider.TokenProvider`,
    :class:`~oridecon.auth.di.sub_providers.session_provider.SessionProvider`,
    and :class:`~oridecon.auth.di.AuthorizationProvider` so that callers only
    ...
    Args:
        config: Shared :class:`~oridecon.auth.config.AuthConfig` forwarded to
            every sub-provider.  When ``None``, each sub-provider uses its own
            defaults.
        initial_roles: Optional initial RBAC roles forwarded to
            :class:`~oridecon.auth.di.AuthorizationProvider`.
        enable_passkeys: When ``True``, append
            :class:`~oridecon.auth.di.sub_providers.passkey_provider.PasskeyProvider`
            to the sub-provider list (requires the WebAuthn extra to be
            installed).
        kwargs: Extra keyword arguments forwarded to
            :class:`~oridecon.auth.di.AuthenticationProvider`.
    """
    ...
    def _compose_sub_providers(self) -> None:
        ...
        if self._enable_passkeys:
            try:
                from oridecon.auth.di.sub_providers.passkey_provider import (
                    PasskeyProvider,
                )
    ...
    async def health_check(self, timeout: float = 5.0) -> HealthCheckResult:
        ...
        Returns :attr:`~oridecon.contracts.core.HealthStatus.DEGRADED` if any
        ...
        Returns:
            An aggregated :class:`~oridecon.contracts.core.HealthCheckResult`.
        """
