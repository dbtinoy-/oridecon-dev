...
:attr:`~oridecon.auth.config.AuthConfig.oauth2_providers` mapping before
...
def detect_oauth_providers_from_config(
    ...
    ``application.yaml`` (or via the ``ORI_AUTH__OAUTH2_PROVIDERS__*``
    ...
    Args:
        oauth2_providers: Raw provider mapping from
            :attr:`~oridecon.auth.config.AuthConfig.oauth2_providers`.
