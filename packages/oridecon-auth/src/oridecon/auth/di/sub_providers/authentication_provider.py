...
from oridecon.auth.authn.password_hasher import (
...
from oridecon.auth.storage.token_store import InMemoryUserStore, UserStoreProtocol
from oridecon.contracts import HealthCheckResult, HealthStatus, ProviderPriority
from oridecon.contracts.auth import PasswordHasherProtocol, PasswordPolicyProtocol
...
from oridecon.di.markers import Inject
...
if TYPE_CHECKING:
    ...
    from oridecon.contracts.auth.exceptions import TokenError
    from oridecon.contracts.auth.token import VerifiedToken
...
def _build_token_manager(
    ...
    Returns:
        A configured :class:`~oridecon.auth.authn.jwt.JWTTokenManager`.
    """
    ...
    if not secret_key:
        if env in _STRICT_ENVS:
            raise ConfigurationError(
                ...
                "Set ORI_AUTH__TOKEN__SECRET_KEY (or token.secret_key in config)."
            )
...
class AuthenticationProvider(Provider):
    ...
    Args:
        config: The resolved :class:`~oridecon.auth.config.AuthConfig`.
            JWT credentials (``config.token.secret_key``, ``config.token.algorithm``)
            are used to build a :class:`~oridecon.auth.authn.jwt.JWTTokenManager`
    ...
    async def get_user(self, user_id: str) -> Any | None:
        ...
        Satisfies :class:`~oridecon.contracts.auth.AuthProviderProtocol`.
    ...
    async def verify_token(self, token: str) -> Result[VerifiedToken, TokenError]:
        ...
        """
        from oridecon.auth.exceptions import TokenInvalidError
