...
class SessionProvider(Provider):
    """Registers :class:`~oridecon.auth.session.manager.SessionManagerImpl` in the container.
    ...
    def __init__(self, config: Any = None, **kwargs: Any) -> None:
        ...
        Args:
            config: Optional :class:`~oridecon.auth.config.AuthConfig`.  Used
    ...
    async def health_check(self, timeout: float = 5.0) -> HealthCheckResult:
        ...
        Returns:
            A healthy :class:`~oridecon.contracts.core.HealthCheckResult`.
        """
