...
``RelayApiKeyVerifier`` binds oridecon-auth's ``APIKeyManager`` behind
the framework's ``RelayAuthVerifierProtocol`` so the relay gateway can
enforce API-key auth without importing oridecon-auth.  The tests use a
...
from oridecon.contracts.ai.relay import RelayAuthError, RelayAuthIdentity
