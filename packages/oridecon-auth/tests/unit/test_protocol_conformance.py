"""Protocol conformance tests for oridecon-auth implementations.

Verifies at test-time that the concrete classes in oridecon-auth satisfy
the ``@runtime_checkable`` protocols defined in ``oridecon-contracts``.
...
from oridecon.auth.authn.security import PasswordHasher
...
class TestCachedUserStorePubSub:
    ...
    async def test_update_user_publishes_invalidation(self) -> None:
        ...
        from oridecon.auth.storage.cached_user_store import (
    ...
    async def test_handle_invalidation_evicts_l1_cache(self) -> None:
        ...
        from oridecon.auth.storage.cached_user_store import CachedUserStore
