"""OAuth2Manager flows plus user-info and auth-provider tests."""
...
from oridecon.auth.authn.oauth2 import (
...
class TestOAuth2Manager:
    ...
    async def test_get_authorization_url(self, mocker, providers):
        ...
        mocker.patch("oridecon.auth.authn.oauth2.HAS_AUTHLIB", True)
        mock_client_class = mocker.MagicMock()
        mocker.patch("oridecon.auth.authn.oauth2.AsyncOAuth2Client", mock_client_class)
    ...
    async def test_get_authorization_url_unknown_provider(self, providers):
        ...
        with patch("oridecon.auth.authn.oauth2.HAS_AUTHLIB", True):
            manager = OAuth2Manager(providers)
            with pytest.raises(
    ...
    async def test_exchange_code_for_token_pkce_enforcement(self, providers):
        ...
        from oridecon.auth.exceptions import OAuth2Error
    ...
    async def test_exchange_code_for_token_error(self, mocker, providers):
        ...
        from oridecon.auth.authn.oauth2 import OAuthError
    ...
    def test_manager_without_authlib(self):
        ...
        with patch("oridecon.auth.authn.oauth2.HAS_AUTHLIB", False):
    ...
    @patch("oridecon.auth.authn.oauth2.HAS_AUTHLIB", True)
    @pytest.mark.asyncio
