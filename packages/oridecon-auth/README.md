...
Authentication and authorization for the Oridecon Framework — JWT, OAuth2, SAML, RBAC, and multi-tenancy.

---

## Overview

Complete authentication and authorization stack for Oridecon — JWT, OAuth2, RBAC,
...
seamless integration with `oridecon-web` middleware.
...
# Optional extras
uv add "oridecon-auth[oauth2,saml]"
```
...
from oridecon.auth import AuthModule, AuthConfig, JWTConfig
...
```bash
export ORI_AUTH__SECRET_KEY=your-secret-key
export ORI_AUTH__TOKEN__SECRET_KEY=your-jwt-secret
export ORI_AUTH__TOKEN__ALGORITHM=HS256
export ORI_AUTH__RBAC__DEFAULT_ROLE=viewer
```
...
```python
from oridecon.auth import AuthModule, AuthConfig, JWTConfig
from oridecon.contracts.core import Duration
...
|-------|---------|---------|-------------|
| `secret_key` | — | `ORI_AUTH__SECRET_KEY` | Top-level signing secret (**required**) |
| `token.secret_key` | — | `ORI_AUTH__TOKEN__SECRET_KEY` | JWT signing secret (**required**) |
| `token.algorithm` | `HS256` | `ORI_AUTH__TOKEN__ALGORITHM` | JWT algorithm: HS256, RS256, ES256 |
| `token.access_token_expire` | `30m` | `ORI_AUTH__TOKEN__ACCESS_TOKEN_EXPIRE` | Access token lifetime (duration string, e.g. `30m`, `1h30m`) |
| `rbac.enabled` | `True` | `ORI_AUTH__RBAC__ENABLED` | Enable RBAC |
| `rbac.default_role` | `viewer` | `ORI_AUTH__RBAC__DEFAULT_ROLE` | Default role for new users |
...
|------|-----------------|
| `src/oridecon/auth/module.py` | AuthModule definition |
| `src/oridecon/auth/config.py` | AuthConfig, JWTConfig, RBACConfig |
| `src/oridecon/auth/di/bundle_provider.py` | AuthBundleProvider wiring |
| `src/oridecon/auth/di/sub_providers/token_provider.py` | TokenProvider (boots policy) |
| `src/oridecon/auth/authn/jwt.py` | JWTTokenManager implementation |
| `src/oridecon/auth/authn/_jwt_lifecycle.py` | verify_token (enforces policy) |
| `src/oridecon/auth/authz/service.py` | AuthorizationService |

## JWT verification policy

`oridecon-auth` enforces **verified-only** JWT decoding — signature verification
cannot be disabled.
...
```bash
export ORI_AUTH__TOKEN__SECRET_KEY="a-stable-dev-secret-at-least-32-chars"
```
