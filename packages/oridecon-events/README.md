...
CQRS, Event Sourcing, and messaging for Oridecon — command bus, event bus, event
...
|-------|---------|---------|-------------|
| `event_store_backend` | `memory` | `ORI_EVENTS__EVENT_STORE_BACKEND` | Store backend: `postgres`, `sqlite`, `mongodb`, `memory` |
| `event_bus.max_concurrent_handlers` | `10` | `ORI_EVENTS__EVENT_BUS__MAX_CONCURRENT_HANDLERS` | Max concurrent handler tasks |
| `event_bus.enable_dead_letter` | `True` | `ORI_EVENTS__EVENT_BUS__ENABLE_DEAD_LETTER` | Send failed events to dead-letter queue |
...
|------|-----------------|
| `src/oridecon/events/module.py` | EventsModule definition |
| `src/oridecon/events/config.py` | EventsConfig and all config sub-models |
| `src/oridecon/events/di/provider.py` | EventsProvider wiring |
