"""Admin contributor for oridecon-events — surfaces event throughput and
dead-letter widgets into the Oridecon admin dashboard.
"""
...
_ACTIONS: tuple[AdminActionDefinition, ...] = (
    AdminActionDefinition(
        ...
        contributor="events",
        handler="oridecon.events.admin.actions:replay_dead_letter",
        icon="refresh-cw",
        category="operations",
    ),
)
...
class EventsAdminContributor(BaseAdminContributor):
    """Admin contributor for the oridecon-events package.
    ...
    async def on_admin_boot(self, container: ContainerResolverProtocol | None) -> None:
        ...
        if container is None:
            return
        from oridecon.events.admin.handlers.dead_letter_count import (
            DeadLetterCountWidgetHandler,
        )
        from oridecon.events.admin.handlers.events_throughput import (
            EventsThroughputWidgetHandler,
        )
        from oridecon.events.admin.handlers.live_events import (
    ...
    def get_management_pages(self) -> Sequence[ManagementPageDefinition]:
        return [
            ManagementPageDefinition(
                ...
                contributor="events",
                route_path="/events",
                handler="oridecon.events.admin.pages.overview:EventsOverviewPage",
            ...
            ManagementPageDefinition(
                ...
                handler="oridecon.events.admin.pages.history:EventsHistoryPage",
                category=PageCategory.INFRASTRUCTURE,
                icon="clock",
            ...
            ManagementPageDefinition(
                ...
                handler="oridecon.events.admin.pages.dead_letter:EventsDeadLetterPage",
                category=PageCategory.INFRASTRUCTURE,
                icon="alert-triangle",
