...
Uses oridecon-contracts StateStoreProtocol protocol for storage — no direct Redis dependency.
Key format:
  oridecon:events:{stream_id}:events  -> JSON list of serialized events
  oridecon:events:{stream_id}:version -> integer version
  oridecon:events:_streams            -> JSON list of known stream IDs
"""
...
class RedisEventStore(AbstractEventStore):
    ...
    Uses the :class:`~oridecon.contracts.state.protocols.StateStoreProtocol` protocol
    ...
    def __init__(
        ...
        namespace: str = "oridecon:events",
    ) -> None:
        """Initialise the Redis-backed event store.

        Args:
            store: A :class:`~oridecon.contracts.state.protocols.StateStoreProtocol`
    ...
    def _deserialize_event(self, data: dict[str, Any]) -> Event:
        """Reconstruct an :class:`~oridecon.events.messages.event.Event` from a dict.
        ...
        required.
        """
        from oridecon.events.messages.event import Event as _Event
    ...
    async def append(
        ...
        Args:
            ...
            expected_version: If provided, the current stream version must
                equal this value or :class:`~oridecon.events.exceptions.ConcurrencyError`
                is raised.
    ...
    async def read(
        ...
        Returns:
            Ordered list of :class:`~oridecon.events.messages.event.Event` objects.
        """
        raw = await self._store.get(self._events_key(stream_id))
        if raw is None:
            return []
