...
from a :class:`~oridecon.contracts.security.secrets.SecretStoreProtocol`
so that keys are never stored in application code.

Only events decorated with :func:`~oridecon.events.decorators.encryption.encrypted_event`
...
class EncryptedEventSerializer:
    ...
    *json_serializer*.  Encrypted events have their payload field replaced by
    ...
    Key material is retrieved lazily via :class:`~oridecon.contracts.security.secrets.SecretStoreProtocol`
