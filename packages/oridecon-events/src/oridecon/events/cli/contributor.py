...
_SPECS: tuple[tuple[str, str, str, str], ...] = (
    (
        ...
        "oridecon.events.cli.generators.event_handler:EventHandlerGenerator",
        "src/handlers",
    ),
    (
        "saga",
        "Generate a saga orchestrator with compensating actions",
        "oridecon.events.cli.generators.saga:SagaGenerator",
        "src/sagas",
    ),
    (
        "event",
        "Generate a domain event class",
        "oridecon.events.cli.generators.event_generator:EventGenerator",
        "src/events",
    ),
    (
        "command",
        "Generate a CQRS command handler",
        "oridecon.events.cli.generators.command_handler:CommandHandlerGenerator",
        "src/commands",
    ),
    (
        "query",
        "Generate a CQRS query handler",
        "oridecon.events.cli.generators.query_handler:QueryHandlerGenerator",
        "src/queries",
    ),
    (
        "projection",
        "Generate an event projection for read models",
        "oridecon.events.cli.generators.projection:ProjectionGenerator",
        "src/projections",
    ),
)
...
class EventsCliContributor:
    """CLI contributor for the oridecon-events package."""
    ...
    def get_commands(self) -> list[CommandContribution]:
        ...
        return [
            CommandContribution(
                ...
                app_factory_path="oridecon.events.cli.commands:create_events_app",
                contributor="events",
                category="events",
    ...
    def get_health_checks(self) -> list[HealthCheckContribution]:
        ...
        return [
            HealthCheckContribution(
                ...
                check_path="oridecon.events.cli.checks:check_event_bus",
                contributor="events",
                category="events",
                timeout=5.0,
            ),
    ...
    def get_doctor_checks(self) -> list[DoctorCheckContribution]:
        ...
        return [
            DoctorCheckContribution(
                ...
                check_path="oridecon.events.cli.doctor:check_event_store",
                contributor="events",
                category="events",
            ),
        ]
    ...
    def get_shell_context(self) -> list[ShellContextContribution]:
        ...
        return [
            ShellContextContribution(
                ...
                factory_path="oridecon.events.cli.shell:provide_event_bus",
                contributor="events",
            ),
        ]
    ...
    def get_schema_setup(self) -> list[SchemaSetupContribution]:
        ...
        return [
            SchemaSetupContribution(
                ...
                setup_fn_path="oridecon.events.cli.schema_setup:ensure_saga_records",
