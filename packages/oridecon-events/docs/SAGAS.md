...
oridecon-events provides two saga implementations:

| Approach | Module | Use case |
|----------|--------|----------|
| `SagaManager` | `oridecon.events.sagas.manager` | Sequential steps, simple compensation chain |
| `SagaOrchestrator` | `oridecon.events.sagas.orchestrator` | Parallel steps, conditional transitions, state-machine flow |
...
oridecon-events uses **orchestration** — you define the step sequence and compensation logic in one place.
...
```python
from oridecon.events.sagas.base import saga_step
...
```python
from oridecon.events.sagas.base import SagaBase, saga_step
...
```python
from oridecon.events.sagas.store import InMemorySagaStore
