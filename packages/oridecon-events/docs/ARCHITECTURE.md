...
Internal design of the `oridecon-events` package — CQRS, event sourcing, projections, and saga orchestration for Oridecon applications.

---

## Role in the System

`oridecon-events` provides the CQRS and event sourcing layer. It occupies the **messaging and workflow** tier alongside `oridecon-queue` and `oridecon-tasks`.

```mermaid
flowchart BT
    E[oridecon-events] & Q[oridecon-queue] & T[oridecon-tasks] --> C[oridecon · contracts]
    E -.-> R[oridecon-resilience]
    T -.-> R
```

**Depends on:** `oridecon`, `oridecon-contracts` | **May import:** `oridecon-resilience`
...
```
packages/oridecon-events/src/oridecon/events/
