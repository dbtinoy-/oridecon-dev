...
Domain events are immutable facts. They extend `DomainEvent` from `oridecon.contracts.domain`:
...
Commands represent an intent to mutate state. They extend the `Command` base class from `oridecon.contracts.events`:
...
```python
from oridecon.events import command_handler
...
from oridecon.contracts.events import EventHandlerProtocol
...
The event store persists events to a durable backend. `oridecon-events` supports:

| Backend | Config | Extra |
|---------|--------|-------|
| In-Memory | `InMemoryEventStoreConfig` | (built-in) |
| PostgreSQL | `PostgresEventStoreConfig` | `oridecon-events[postgres]` |
| MongoDB | `MongoDBEventStoreConfig` | `oridecon-events[mongo]` |
| SQLite | `SqliteConfig` | `oridecon-events[sqlite]` |
...
from oridecon.events import AggregateStatus
...
```python
from oridecon.events import EventSourcingRepository
...
```python
from oridecon.contracts.events import ProjectionProtocol
...
```python
from oridecon.contracts.workflow import SagaProtocol
