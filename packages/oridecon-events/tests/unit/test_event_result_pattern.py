...
from oridecon.events.services import EventBusWithResultPattern
...
class TestEventBusResultPattern:
    ...
    async def test_error_hierarchy_correct(self) -> None:
        ...
        # All event errors inherit from OrideconError
        ...
        assert isinstance(event_err, OrideconError)

        handler_not_found = HandlerNotFoundError("not found")
        assert isinstance(handler_not_found, EventError)
        assert isinstance(handler_not_found, OrideconError)

    @pytest.mark.asyncio
    async def test_error_codes_set_correctly(self) -> None:
        """Verify event errors have correct error codes."""
        assert EventError._code == "ORI_ERR_EVT_001"
        assert HandlerNotFoundError._code == "ORI_ERR_EVT_002"
        assert DuplicateHandlerError._code == "ORI_ERR_EVT_003"
