"""Generator contract test for the notification CLI contributor."""

from __future__ import annotations

from pathlib import Path

from lexigram.notification.cli.contributor import NotificationCliContributor
...
def test_all_generators_render(tmp_path: Path) -> None:
    ...
    count = assert_contributor_generators_render(
        NotificationCliContributor(), tmp_path=tmp_path
    )
    assert count >= 1
