...
Uses ``CacheBackendProtocol`` from ``oridecon-contracts`` as the storage layer.
This module imports ONLY from ``oridecon-contracts`` and ``oridecon`` core —
never directly from ``oridecon-cache``.  The ``CacheBackendProtocol`` instance is
...
from oridecon.contracts.exceptions.idempotency import IdempotencyStoreError
