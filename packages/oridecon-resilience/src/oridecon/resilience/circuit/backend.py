...
from oridecon.resilience.circuit.breaker import CircuitState
...
class DistributedCircuitBreakerBackend:
    """Distributed circuit breaker backend backed by a :class:`oridecon.contracts.StateStoreProtocol`.
    ...
    Example::
        ...
        from oridecon.contracts import StateStoreProtocol
        from oridecon.resilience.circuit.backend import DistributedCircuitBreakerBackend
    ...
    def __init__(self, store: StateStoreProtocol, ttl: int = 3600) -> None:
        ...
        Args:
            store: A :class:`~oridecon.contracts.StateStoreProtocol` implementation
                (e.g. the Redis-backed state store from ``oridecon-sql``).
