"""Unit tests for oridecon-resilience exceptions.

These tests verify the exception hierarchy and behavior in oridecon.resilience.exceptions.
"""
...
class TestResilienceExceptionHierarchy:
    ...
    def test_resilience_error_inherits_from_oridecon_error(self) -> None:
        from oridecon.contracts.exceptions import OrideconError

        assert issubclass(ResilienceError, OrideconError)
...
class TestRetryExhaustedError:
    ...
    def test_retry_exhausted_error_code(self) -> None:
        error = RetryExhaustedError()
        assert error._code == "ORI_ERR_RES_008"
...
class TestCircuitOpenError:
    ...
    def test_circuit_open_error_code(self) -> None:
        error = CircuitOpenError()
        assert error._code == "ORI_ERR_RES_009"
...
class TestBulkheadRejectedError:
    ...
    def test_bulkhead_rejected_error_code(self) -> None:
        error = BulkheadRejectedError()
        assert error._code == "ORI_ERR_RES_010"
...
class TestResilienceTimeoutError:
    ...
    def test_resilience_timeout_error_code(self) -> None:
        error = ResilienceTimeoutError()
        assert error._code == "ORI_ERR_RES_011"
...
class TestExceptionAllExports:
    ...
    def test_all_contains_all_exceptions(self) -> None:
        from oridecon.resilience import exceptions as exc_module
