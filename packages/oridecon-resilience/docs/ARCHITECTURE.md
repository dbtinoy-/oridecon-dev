...
Internal design of the `oridecon-resilience` package.
...
**Import direction rule:** A pipeline may import circuit breakers, retry policies, and rate limiters. A circuit breaker may not import from the pipeline. All implementations import from `oridecon.contracts.infra.resilience` for protocols, configs, and enums.
...
All protocols in `oridecon.contracts.infra.resilience`:
...
Config models and enums also live in contracts so any extension package can reference them without depending on `oridecon-resilience`:
...
| Custom idempotency store | Implement `IdempotencyStoreProtocol` from `oridecon.contracts.core.idempotency` |
