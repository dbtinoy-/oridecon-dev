---
title: oridecon-monitor Metrics
...
|------|-------|----------|----------|
| **Counter** | `from oridecon.monitor.metrics import Counter` | Monotonically increasing | Request count, error count |
| **Gauge** | `from oridecon.monitor.metrics import Gauge` | Up/down value | Active connections, queue depth |
| **Histogram** | `from oridecon.monitor.metrics import Histogram` | Value distribution with buckets | Request latency, payload size |
| **Summary** | `from oridecon.monitor.metrics import Summary` | Quantile-based distribution | SLA tracking (p50, p95, p99) |
...
|-------------|------|--------|-------------|
| `oridecon_requests_total` | Counter | `method`, `status` | Total HTTP requests |
| `oridecon_request_duration_seconds` | Histogram | `method` | Request latency distribution |
| `oridecon_active_connections` | Gauge | — | Current active connections |
| `oridecon_hook_events_total` | Counter | `hook`, `package` | Hook-triggered events |

These are automatically created and wired when `MonitorProvider.boot()` runs. If you register a `HookRegistryProtocol`, the monitor subscribes to framework hooks and emits the `oridecon_hook_events_total` counter with per-hook labels.
...
```python
from oridecon.monitor.config import MonitorConfig, BackendType
...
```python
from oridecon.monitor.metrics import Counter
...
```python
from oridecon.monitor.health import HealthCheckerRegistry, health_checker
...
```python
from oridecon.monitor.tracing import SpanKind, SpanStatus
...
```python
from oridecon.monitor.instrumentation.decorators import traced
