...
| `oridecon-notification` | Optional | Alerting integration |
...
`oridecon-monitor` provides a unified observability layer behind protocols. Swap backends (Memory → Prometheus → OpenTelemetry) without changing application code.
...
Application Code
        ...
        ▼
 ┌────────────────────────────────┐
 │ oridecon-monitor (MonitorProvider) │
 │  ┌──────────┐ ┌───────┐ ┌────┐ │
...
```python
from oridecon.monitor import Counter, Gauge, Histogram, Summary
...
```python
from oridecon.monitor import Tracer, SpanKind, SpanStatus
...
```python
from oridecon.monitor.health import HealthCheckRegistry, HealthCheck
...
```python
from oridecon.monitor import traced, metered
...
```python
from oridecon.monitor import MonitorProvider
from oridecon.monitor.backends.prometheus import PrometheusBackend
from oridecon.monitor.middleware import PrometheusMiddleware
...
```python
from oridecon.monitor.middleware import HealthCheckProvider
...
```python
from oridecon.monitor import PerformanceMonitor
...
- Configure core logging redaction via `ORI_ORIDECON__LOGGING__REDACTION__*` (not `MonitorConfig`) to prevent secrets from appearing in logs
