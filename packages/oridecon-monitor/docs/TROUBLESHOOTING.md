---
title: oridecon-monitor Troubleshooting
description: Common errors, causes, and fixes.
---

## Backend not available

**Error:** `BackendNotAvailableError` (code `ORI_ERR_MONITOR_002`)
...
```bash
# For Prometheus
uv add "oridecon-monitor[prometheus]"
# For OpenTelemetry
uv add "oridecon-monitor[otel]"
# For system metrics
uv add "oridecon-monitor[system]"
```

## Metric not found

**Error:** `MetricNotFoundError` (code `ORI_ERR_MONITOR_003`)
...
```python
from oridecon.monitor import Counter
...
**Error:** `InvalidMetricError` (code `ORI_ERR_MONITOR_004`)
...
**Error:** `SpanNotFoundError` (code `ORI_ERR_MONITOR_006`)
...
export ORI_MONITOR__DEBUG=true
```
