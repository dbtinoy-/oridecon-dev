...
Internal design of the `oridecon-monitor` package.
...
flowchart LR
    ...
    Contracts[oridecon-contracts<br/>observability protocols]
    Prom[Prometheus]
    OTel[OpenTelemetry]
    Console[Console / Log]
    Mem[In-Memory]
...
`oridecon-monitor` replaces the framework's no-op observability stubs with real metric collection, tracing, and health-check infrastructure. It is activated by adding `MonitorModule` to the application's imports.

---

## Metrics Model

Four metric instrument types, each implementing `MetricProtocol` from `oridecon-contracts`:
...
The provider runs at `ProviderPriority.INFRASTRUCTURE` so its singleton bindings override the core framework's no-op `ObservabilityProvider`. Third-party backend packages declare `oridecon.monitoring.backends` entry points discovered during `register()`.

---

## Contracts Used

| Contract (from `oridecon-contracts`) | Bound Implementation | Purpose |
|--------------------------------------|---------------------|---------|
...
| Entry-point backend | Third-party packages declare `oridecon.monitoring.backends` entry points |
