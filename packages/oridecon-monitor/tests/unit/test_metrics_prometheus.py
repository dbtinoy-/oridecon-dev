...
from oridecon.monitor.metrics.prometheus import PrometheusExporter, get_prometheus_exporter


def test_prometheus_exporter_creation():
    """Test creating PrometheusExporter."""
    with patch("oridecon.monitor.metrics.prometheus.CollectorRegistry") as mock_registry_class:
...
def test_prometheus_exporter_get_or_create_metrics():
    ...
    with patch("oridecon.monitor.metrics.prometheus.Counter") as mock_counter_class, \
         patch("oridecon.monitor.metrics.prometheus.Gauge") as mock_gauge_class, \
         patch("oridecon.monitor.metrics.prometheus.Histogram") as mock_hist_class, \
         patch("oridecon.monitor.metrics.prometheus.Summary") as mock_summary_class:
...
def test_prometheus_exporter_export():
    ...
    with patch("oridecon.monitor.metrics.prometheus.generate_latest", return_value=b"metrics_data"):
        exporter = PrometheusExporter(registry=MagicMock())
...
def test_prometheus_exporter_no_prometheus():
    ...
    with patch("oridecon.monitor.metrics.prometheus.Counter", None), \
         patch("oridecon.monitor.metrics.prometheus.Gauge", None):
        exporter = PrometheusExporter()
        assert exporter.get_or_create_counter("c") is None
        assert exporter.get_or_create_gauge("g") is None
        
        # Test export fallback
        with patch("oridecon.monitor.metrics.prometheus.generate_latest", None):
