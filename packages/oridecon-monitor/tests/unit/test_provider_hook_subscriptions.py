...
async def test_request_hook_records_metrics_and_traces() -> None:
    ...
    metric = provider.metrics_collector.get_metric("oridecon_hook_events_total")
    ...
    assert span.attributes["oridecon.hook.name"] == "request.received"
    assert span.attributes["oridecon.hook.package"] == "web"
