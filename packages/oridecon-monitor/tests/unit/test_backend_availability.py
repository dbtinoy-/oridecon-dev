...
def _load_exceptions(monkeypatch):
    ...
    sys.modules.setdefault("oridecon.monitor.exceptions", mod)
...
def test_opentelemetry_backend_raises_when_missing(monkeypatch):
    ...
    sys.modules.setdefault("oridecon", types.ModuleType("oridecon"))
    sys.modules.setdefault("oridecon.monitor", types.ModuleType("oridecon.monitor"))

    # Minimal placeholder for monitor.protocols
    protocols_mod = sys.modules.setdefault(
        "oridecon.monitor.protocols", types.ModuleType("oridecon.monitor.protocols"),
    )
    ...
    types_mod = sys.modules.setdefault(
        "oridecon.monitor.types", types.ModuleType("oridecon.monitor.types"),
    )
    ...
    mod = _load_module_from_path(path, "oridecon.monitor.backends._test_opentelemetry")
    ...
    assert "Install with: pip install oridecon-monitor[otel]" in str(exc.value)
...
def test_prometheus_backend_raises_when_missing(monkeypatch):
    ...
    mod = _load_module_from_path(path, "oridecon.monitor.backends._test_prometheus")
    ...
    assert "Install with: pip install oridecon-monitor[prometheus]" in str(exc.value)
