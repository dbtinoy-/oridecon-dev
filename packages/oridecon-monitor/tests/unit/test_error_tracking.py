"""Tests for optional error tracking integration in Oridecon Monitor."""
...
from oridecon.monitor.config import ErrorTrackingConfig
...
class TestSetupErrorTracking:
    ...
    def test_config_env_loading(self, tmp_path, monkeypatch):
        """ORI_MONITOR__ERROR_TRACKING__DSN populates MonitorConfig."""
        from oridecon.monitor.config import MonitorConfig

        monkeypatch.setenv("ORI_MONITOR__ERROR_TRACKING__DSN", "https://a@b/1")
        monkeypatch.setenv("ORI_MONITOR__ERROR_TRACKING__ENVIRONMENT", "staging")
