...
async def test_otel_backend_initialization():
    ...
    with patch("oridecon.monitor.backends.opentelemetry.HAS_OPENTELEMETRY", True):
        # Mocking the actual opentelemetry SDK calls in the module where they are used
        with patch("oridecon.monitor.backends.opentelemetry.MeterProvider") as mock_meter_provider, \
             patch("oridecon.monitor.backends.opentelemetry.TracerProvider") as mock_tracer_provider, \
             patch("oridecon.monitor.backends.opentelemetry.metrics") as mock_metrics, \
             patch("oridecon.monitor.backends.opentelemetry.trace") as mock_trace:
...
async def test_otel_backend_no_otel():
    ...
    with patch("oridecon.monitor.backends.opentelemetry.HAS_OPENTELEMETRY", False):
