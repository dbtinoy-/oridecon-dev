...
from oridecon.monitor.exceptions import BackendNotAvailableError


@pytest.mark.asyncio
async def test_prometheus_backend_initialization():
    """Test Prometheus backend initialization."""
    with patch("oridecon.monitor.backends.prometheus.HAS_PROMETHEUS", True):
        with patch("oridecon.monitor.backends.prometheus.start_http_server") as mock_start_server:
...
async def test_prometheus_backend_initialization_error():
    ...
    with patch("oridecon.monitor.backends.prometheus.HAS_PROMETHEUS", True):
        with patch("oridecon.monitor.backends.prometheus.start_http_server", side_effect=OSError("already in use")):
...
async def test_prometheus_backend_no_prometheus():
    ...
    with patch("oridecon.monitor.backends.prometheus.HAS_PROMETHEUS", False):
...
def test_prometheus_backend_record_metric():
    ...
    with patch("oridecon.monitor.backends.prometheus.HAS_PROMETHEUS", True):
        with patch("oridecon.monitor.backends.prometheus.PromCounter") as mock_counter, \
             patch("oridecon.monitor.backends.prometheus.PromGauge") as mock_gauge, \
             patch("oridecon.monitor.backends.prometheus.PromHistogram") as mock_hist:
