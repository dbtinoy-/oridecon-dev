...
the ``oridecon-monitor`` extension. Falls back to NoOp implementations
when the full monitoring stack is not available.

Example::

    from oridecon.observability.di import ObservabilityService
...
class ObservabilityService:
    ...
    Automatically detects if the ``oridecon-monitor`` extension is
