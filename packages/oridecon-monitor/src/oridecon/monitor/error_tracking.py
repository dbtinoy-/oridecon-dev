...
``ORI_MONITOR__ERROR_TRACKING__DSN``).  When no DSN is configured a
...
Example:
    ...
    from oridecon.monitor.error_tracking import setup_error_tracking
...
def setup_error_tracking(config: ErrorTrackingConfig) -> ErrorTrackerProtocol:
    ...
    try:
        ...
        logger.warning(
            ...
            detail="Install sentry-sdk or oridecon-monitor extras to enable error tracking",
        )
        return NullErrorTracker()
