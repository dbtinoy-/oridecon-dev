...
Example:
    >>> from oridecon.monitor import instrument_database
        >>> from oridecon.sql import DatabaseService
    >>>
...
_OTEL_INSTALL_HINT = "pip install oridecon-monitor[otel]"
...
def instrument_database(provider: Any) -> None:
    """Instrument a Oridecon DatabaseService with OpenTelemetry tracing.
    ...
    tracer = trace.get_tracer("oridecon.sql")
    meter = metrics.get_meter("oridecon.sql")
