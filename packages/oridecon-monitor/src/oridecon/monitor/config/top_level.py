...
from oridecon.monitor import constants as monitor_const
from oridecon.monitor.config.enums import BackendType
from oridecon.monitor.config.exporters import (
    OpenTelemetryConfig,
    PrometheusConfig,
    SLOConfig,
)
from oridecon.monitor.config.health import HealthCheckConfig
from oridecon.monitor.config.metrics import MetricsConfig
from oridecon.monitor.config.tracing import TracingConfig
from oridecon.validation import ConfigDict, Field, SecretStr, field_validator
...
class ErrorTrackingConfig(BaseConfig):
    ...
    enabled (env: ``ORI_MONITOR__ERROR_TRACKING__DSN``).
...
class MonitorConfig(BaseConfig):
    """Hierarchical root configuration for Oridecon Monitor.
    ...
    def validate_for_environment(
        ...
        Returns:
            List of :class:`~oridecon.contracts.core.config.ConfigIssue` instances.
        """
        ...
        if resolved == Environment.PRODUCTION and not self.tracing.enabled:
            issues.append(
                ConfigIssue(
                    ...
                    severity="warning",
                    suggestion=(
                        "Set ORI_MONITOR__TRACING__ENABLED=true and configure "
