...
from oridecon.monitor.profiling import ProfilingRegistry
from oridecon.monitor.tracing import Tracer
...
class _MonitorRegistrationMixin(_MonitorAttrsMixin):
    ...
    async def register(self, container: ContainerRegistrarProtocol) -> None:
        ...
        if self.metrics_exporter is not None:
            ...
            # oridecon-monitor directly.
            from oridecon.monitor.backends.exporters.prometheus import (
                HAS_PROMETHEUS,
    ...
    async def _discover_backends(self, container: ContainerRegistrarProtocol) -> None:
        """Scan the ``oridecon.monitoring.backends`` entry-point group.
        ...
        eps = _meta.entry_points(group="oridecon.monitoring.backends")
        for ep in eps:
            try:
                candidate = ep.load()
