...
from oridecon.monitor.health.sanitize import safe_error_message
...
class _MonitorMetricsMixin(_MonitorAttrsMixin):
    ...
    def record_request(
        ...
        # Record metrics
        counter = self.metrics_collector.get_metric("oridecon_requests_total")
        if counter:
    ...
    def _ensure_request_metrics(self) -> None:
        ...
        if not self.metrics_collector.get_metric("oridecon_requests_total"):
        ...
        if not self.metrics_collector.get_metric("oridecon_request_duration_seconds"):
    ...
    def record_connection_change(self, delta: int) -> None:
        ...
        gauge = self.metrics_collector.get_metric("oridecon_active_connections")
        if gauge:
