"""Full-featured monitoring and observability provider for ``oridecon-monitor``.

This module defines :class:`MonitorProvider`, which replaces the core
framework's no-op :class:`~oridecon.monitor.di.sub_providers.observability.ObservabilityProvider`
with real metric collection, distributed tracing, and health-check
infrastructure when ``oridecon-monitor`` is installed.

Relationship with ``ObservabilityProvider``
--------------------------------------------
The Oridecon core always registers
:class:`~oridecon.monitor.di.sub_providers.observability.ObservabilityProvider`
...
* **``MonitorProvider``** overrides those defaults with real implementations
  (active when ``oridecon-monitor`` is installed and added to the app).
...
from oridecon.monitor.constants import DEFAULT_MAX_SPANS
from oridecon.monitor.error_tracking import ErrorTrackerProtocol
from oridecon.monitor.health import HealthCheckerRegistry
from oridecon.monitor.metrics.collector import (
    MetricsCollectorProtocol as _ConcreteMetricsCollector,
)
from oridecon.monitor.tracing import InMemoryTraceProvider
...
_HOOK_EVENT_COUNTER_NAME = "oridecon_hook_events_total"
...
from oridecon.monitor.di._lifecycle import _MonitorLifecycleMixin
from oridecon.monitor.di._metrics import _MonitorMetricsMixin
from oridecon.monitor.di._registration import _MonitorRegistrationMixin
...
class MonitorProvider(
    ...
    """Monitoring and observability provider for Oridecon Framework"""
    ...
    async def _record_hook_event(self, hook_name: str, **kwargs: Any) -> None:
        ...
        span = self.tracer.start_span(
            f"hook.{hook_name}",
            attributes={
                "oridecon.hook.name": hook_name,
                "oridecon.hook.package": package,
            },
        )
        try:
