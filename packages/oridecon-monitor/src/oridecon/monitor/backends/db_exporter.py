...
This exporter depends on a DatabaseService (`oridecon.sql.providers.database_provider.DatabaseService`)
...
class DBMetricsExporter:
    ...
    *table*.  The table must have the schema::
    ...
    :class:`~oridecon.monitor.config.MonitoringConfig` ``store_in_db`` flag and
    wired automatically by
    :class:`~oridecon.monitor.di.provider.MonitorProvider` when a
    :class:`~oridecon.contracts.data.DatabaseProviderProtocol` binding is
    present in the container.

    Args:
        db_provider: A database provider that exposes an
            ``execute_query(sql, params)`` coroutine.  Typically a
            :class:`~oridecon.sql.providers.database_provider.DatabaseService`
