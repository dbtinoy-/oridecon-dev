"""Prometheus backend — the single public Prometheus integration for Oridecon Monitor.

:class:`PrometheusBackend` is the canonical way to wire Prometheus into a
Oridecon application.  It implements the :class:`~oridecon.contracts.observability.MetricsBackendProtocol`
...
:class:`~oridecon.monitor.backends.exporters.PrometheusMetricsExporter` instead.

**Internal detail**: metric object creation and registry management are handled
by :class:`~oridecon.monitor.metrics.prometheus.PrometheusExporter`, which is an
...
from oridecon.contracts.observability.metrics import MetricsBackendProtocol
from oridecon.logging import get_logger
from oridecon.monitor.exceptions import BackendNotAvailableError
from oridecon.monitor.tracing import Span, SpanContext
    ...
    def __init__(self, port: int = 8000):
        ...
        if not HAS_PROMETHEUS:
            raise BackendNotAvailableError(
                ...
                "Install with: pip install oridecon-monitor[prometheus]",
            )
