...
from oridecon.primitives.context import TENANT_ID
...
class TestGraphPerTenantIsolation:
    ...
    async def test_tenants_get_separate_graph_instances(self) -> None:
        ...
        from oridecon.contracts.data.graph.tenancy import GraphTenancyStrategy
        from oridecon.graph.backends.memory.backend import InMemoryGraphStore
        from oridecon.graph.tenancy.decorator import TenantGraphStoreDecorator
        from oridecon.graph.tenancy.resolver import (
...
class TestNodePropertyIsolation:
    ...
    async def test_find_nodes_receives_tenant_filter(self) -> None:
        ...
        from oridecon.graph.tenancy.decorator import TenantPropertyFilterGraph
        from oridecon.contracts.data.graph.filters import PropertyCondition, PropertyOperator
        from oridecon.contracts.data.types import LogicalOperator
        from oridecon.contracts.data.graph.protocols import GraphProtocol
