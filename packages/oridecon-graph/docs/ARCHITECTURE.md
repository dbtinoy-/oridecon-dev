...
Internal design of the `oridecon-graph` package.

---

## Role in the System

`oridecon-graph` lives in the **Data & Persistence** tier. It implements `GraphStoreProtocol` and `GraphProtocol` from `oridecon-contracts` and provides pluggable backends for Neo4j and in-memory graph stores.

```mermaid
flowchart BT
    App[Application Code]
    Contracts[oridecon-contracts<br/>GraphStoreProtocol · GraphProtocol<br/>GraphNode · GraphEdge · GraphPath<br/>TraversalQuery · PropertyFilter]
    Graph[oridecon-graph<br/>GraphModule · GraphProvider<br/>Backends · CypherCompiler]
...
**Import direction:** Application code depends on `GraphStoreProtocol` from `oridecon-contracts`, never on `oridecon-graph` directly. The container resolves the protocol to the backend chosen at configuration time.
...
**In-Memory** (`oridecon.graph.backends.memory`): Pure-Python `dict` store. No external dependencies. BFS traversal. Default for development/testing.

**Neo4j** (`oridecon.graph.backends.neo4j`): Uses the `neo4j` async driver. Translates all graph operations to Cypher. Production backend.

---

## Graph Model

The canonical graph data types are defined in `oridecon-contracts` as frozen dataclasses:
...
```python
from oridecon.contracts.data.graph import Prop
...
```python
from oridecon.contracts.data.graph import TraversalQuery, StartSpec, TraversalStep
from oridecon.contracts.data.graph.enums import EdgeDirection, ReturnType
...
The `CypherCompiler` in `oridecon.graph.backends.neo4j.cypher` translates `TraversalQuery` to Cypher:
...
All contracts come from `oridecon.contracts.data.graph`:

| Contract | Location | How It's Used |
|----------|----------|---------------|
| `GraphStoreProtocol` | `oridecon.contracts.data.graph.protocols` | Top-level interface registered by `GraphProvider` |
| `GraphProtocol` | `oridecon.contracts.data.graph.protocols` | Returned by `get_graph()`; all node/edge/traversal operations |
| `GraphNode` | `oridecon.contracts.data.graph.types` | Return type for node CRUD |
| `GraphEdge` | `oridecon.contracts.data.graph.types` | Return type for edge CRUD |
| `GraphPath` | `oridecon.contracts.data.graph.types` | Return type for traversal and shortest-path |
| `TraversalQuery` | `oridecon.contracts.data.graph.types` | Input spec for traversal operations |
| `TraversalStep` | `oridecon.contracts.data.graph.types` | Single hop in a traversal |
| `StartSpec` | `oridecon.contracts.data.graph.types` | How to locate traversal start nodes |
| `NodeSpec` / `EdgeSpec` | `oridecon.contracts.data.graph.types` | Input specs for bulk operations |
| `NodeResult` / `EdgeResult` | `oridecon.contracts.data.graph.types` | Creation results |
| `BulkNodeResult` / `BulkEdgeResult` | `oridecon.contracts.data.graph.types` | Bulk operation results |
| `GraphInfo` | `oridecon.contracts.data.graph.types` | Graph database metadata |
| `IndexSpec` / `ConstraintSpec` | `oridecon.contracts.data.graph.types` | Schema operation specs |
| `EdgeDirection` | `oridecon.contracts.data.graph.enums` | Direction for edge queries |
| `ReturnType` | `oridecon.contracts.data.graph.enums` | What traversal returns |
| `IndexKind` | `oridecon.contracts.data.graph.enums` | Index type |
| `ConstraintKind` | `oridecon.contracts.data.graph.enums` | Constraint type |
| `PropertyFilter` | `oridecon.contracts.data.graph.filters` | Filter expression for nodes/edges |
| `Prop` | `oridecon.contracts.data.graph.filters` | Static factory for building filters |
...
```
packages/oridecon-graph/src/oridecon/graph/
...
├── protocols.py             # Re-exports from oridecon-contracts
...
12 exceptions total, all under `GraphError(InfrastructureError)` with unique `ORI_ERR_GRAPH_0xx` codes. Exception hierarchy: connection failures (`GraphConnectionError`), not-found errors for nodes/edges/graphs, schema errors, transaction errors, traversal errors, and Cypher compilation errors.
...
```python
from oridecon.graph import GraphNodeCreatedEvent
...
| Environment config | `ORI_GRAPH__BACKEND=neo4j ORI_GRAPH__NEO4J__URI=bolt://...` |
