---
title: oridecon-graph Backends
description: Compare Neo4j and in-memory backends for oridecon-graph
sidebar:
  order: 3
---

`oridecon-graph` provides graph store backends behind the `GraphStoreProtocol` contract from `oridecon-contracts`. Both backends support the same graph, node, edge, and traversal APIs, including Cypher query compilation for Neo4j.

## Supported Backends

| Backend | Extra | Driver | Production-ready | Best for |
|---------|-------|--------|-----------------|----------|
| Neo4j | `oridecon-graph[neo4j]` | `neo4j>=5.20.0` | Yes | Production graph workloads |
| Memory | (core) | in-process `dict`/`set` | Development / test | Local dev, unit tests, examples |

Install the Neo4j extra: `pip install oridecon-graph[neo4j]`.
...
- **When to choose:** Unit tests, CI pipelines, local development, examples, or when the graph is small and ephemeral.
...
The `CypherCompiler` in `oridecon.graph.backends.neo4j.cypher` compiles `TraversalQuery` objects to parameterized Cypher strings. It supports:
...
```python
from oridecon.graph import GraphConfig, GraphProvider
