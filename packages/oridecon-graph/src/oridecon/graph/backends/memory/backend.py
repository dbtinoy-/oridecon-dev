...
from oridecon.graph.backends.base import BaseGraphStore
...
class InMemoryGraphStore(BaseGraphStore):
    ...
    async def health_check(self, timeout: float = 5.0) -> HealthCheckResult:  # noqa: ASYNC109 — implements HealthCheckProtocol signature
        ...
        Returns:
            Always returns :attr:`~oridecon.contracts.core.health.HealthStatus.HEALTHY`.
    ...
    async def list_graphs(self) -> list[GraphInfo]:
        ...
        Returns:
            A list of :class:`~oridecon.contracts.data.graph.GraphInfo` instances.
