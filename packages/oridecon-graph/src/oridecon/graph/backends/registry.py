...
from oridecon.graph.constants import BACKEND_MEMORY, BACKEND_NEO4J
...
class GraphStoreRegistry:
    ...
    graph store from a :class:`~oridecon.graph.config.GraphConfig`.
    ...
    def with_defaults(cls) -> GraphStoreRegistry:
        ...
        def _neo4j(config: Any) -> Any:
            from oridecon.graph.backends.neo4j import Neo4jGraphStore

            return Neo4jGraphStore(config=config.neo4j)

        def _memory(_config: Any) -> Any:
            from oridecon.graph.backends.memory import InMemoryGraphStore
