...
from oridecon.graph.backends.base import BaseGraph
...
if TYPE_CHECKING:
    ...
    from oridecon.contracts.data.graph.filters import PropertyFilter
...
class Neo4jGraph(BaseGraph):
    ...
    async def create_node(
        ...
        Returns:
            :class:`~oridecon.contracts.data.graph.NodeResult` with the node ID.
    ...
    async def get_node(self, node_id: str) -> GraphNode | None:
        ...
        Returns:
            The :class:`~oridecon.contracts.data.graph.GraphNode` or ``None``.
    ...
    async def find_nodes(
        ...
        Returns:
            Matching :class:`~oridecon.contracts.data.graph.GraphNode` instances.
    ...
    async def create_edge(
        ...
        Returns:
            :class:`~oridecon.contracts.data.graph.EdgeResult` with the
    ...
    async def traverse(self, query: TraversalQuery) -> list[GraphPath]:
        ...
        Returns:
            All :class:`~oridecon.contracts.data.graph.GraphPath` instances
            found by the traversal.

        """
        from oridecon.graph.backends.neo4j.cypher import (
