...
from oridecon.cache.service.health import get_health_status as _get_health_status
from oridecon.cache.service.health import get_metrics as _get_metrics
...
if TYPE_CHECKING:
    from oridecon.contracts import ContainerResolverProtocol

logger = get_logger(__name__)

T = TypeVar("T")

from oridecon.cache.di._registration import _CacheRegistrationMixin
from oridecon.cache.di._services import _CacheServicesMixin
from oridecon.di.provider import Provider
...
class CacheProvider(
    ...
    """
    Oridecon Cache provider for oridecon integration.

    Integrates cache services with oridecon's provider system,
    ...
    async def boot(self, container: ContainerResolverProtocol) -> None:
        ...
        """
        logger.info("Starting Oridecon Cache provider")

        # NOTE: RedisStateStore registration is intentionally delegated to
        # oridecon-sql's provider to avoid cross-extension imports.  If you need
        # a StateStoreProtocol alongside the cache, ensure oridecon-sql is installed and
        ...
        logger.info("Oridecon Cache provider started successfully")
    ...
    async def shutdown(self) -> None:
        ...
        logger.info("Shutting down Oridecon Cache provider")
        ...
        logger.info("Oridecon Cache provider shutdown complete")
    ...
    async def get_health_status(self) -> HealthCheckResult:
        ...
        Returns:
            Structured :class:`~oridecon.contracts.types.HealthCheckResult`.
        """
