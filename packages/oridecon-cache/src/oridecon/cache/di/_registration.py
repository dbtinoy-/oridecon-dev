...
from oridecon.cache.backends.registry import BackendRegistry
...
if TYPE_CHECKING:
    ...
    from oridecon.contracts import ContainerRegistrarProtocol
...
class _CacheRegistrationMixin:
    ...
    async def register(self, container: ContainerRegistrarProtocol) -> None:
        ...
        """
        logger.info("Registering Oridecon Cache services with container")

        # Register config so downstream providers (e.g. RatesProvider) can
        # resolve CacheConfig from the container.
        from oridecon.cache.config import CacheConfig  # noqa: PLC0415
        ...
        from oridecon.cache.service.registry import (
            CacheStatusRegistry,
        ...
        reset_cache_status_registry()
        import oridecon.cache.service.registry as _reg_mod
        ...
        from oridecon.cache.di.provider import (
        ...
        from oridecon.cache.service.core import CacheService  # noqa: PLC0415
        from oridecon.contracts.infra.cache.protocols import CacheBackendProtocol
        ...
        from oridecon.cache.service.warmer import CacheWarmer
        ...
        if self.config:
            from oridecon.cache.stores.redis_state import (
                RedisStateStore,
            )
        ...
        logger.info("Oridecon Cache services registered successfully")

    async def _discover_backends(self, container: ContainerRegistrarProtocol) -> None:
        """Scan the ``oridecon.cache.backends`` entry-point group.

        Any entry point that resolves to a
        :class:`~oridecon.di.provider.Provider` subclass is instantiated
        and its :meth:`~oridecon.di.provider.Provider.register` method is
        ...
        eps = _meta.entry_points(group="oridecon.cache.backends")
        for ep in eps:
            try:
                candidate = ep.load()
    ...
    def _register_semantic_cache(self, container: ContainerRegistrarProtocol) -> None:
        ...
        try:
            from oridecon.cache.semantic.cost_decision import CostAwareCacheDecision
            from oridecon.cache.semantic.protocols import VectorIndexProtocol
            from oridecon.cache.semantic.store import SemanticCacheStore
            from oridecon.cache.semantic.vector_index import FaissVectorIndex
