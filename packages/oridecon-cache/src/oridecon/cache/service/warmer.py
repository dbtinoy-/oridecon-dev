...
Complements :class:`~oridecon.cache.service.stampede.StampedeProtectedCache`
(reactive cache fills) with proactive preloading so that the first real
request to a key never incurs a cache miss.

Wire to ``CacheProvider.boot()`` for startup warming or to ``oridecon-tasks``
...
class CacheWarmer:
    ...
    Args:
        cache: The :class:`~oridecon.contracts.cache.protocols.CacheBackendProtocol`
            to populate.
