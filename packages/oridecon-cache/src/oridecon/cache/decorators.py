...
from oridecon.cache.backends import _compute_hash
...
def _is_domain_model(obj: Any) -> bool:
    ...
    both :class:`oridecon.domain.DomainModel` subclasses **and** Pydantic
...
def _deserialize(value: Any) -> Any:
    ...
    if isinstance(value, dict) and _KEY_MODULE in value and _KEY_CLASS in value:
        from oridecon.cache.serialization.type_registry import DEFAULT_REGISTRY
...
def cacheable(
    ...
    - Reconstruction is **deny-by-default**: type tags are only resolved
      ...
      (:data:`~oridecon.cache.serialization.type_registry.DEFAULT_REGISTRY`);
