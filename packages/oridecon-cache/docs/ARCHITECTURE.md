...
Internal design of the `oridecon-cache` package.

---

## Role in the System

`oridecon-cache` is the **caching abstraction layer** in the Oridecon framework. It depends only on `oridecon` and `oridecon-contracts`. Other packages consume caching through `CacheBackendProtocol` resolved via the DI container — never through direct imports.

```mermaid
flowchart BT
    App[Application Layer]
    Ext[oridecon-* Extensions]
    LC[oridecon-cache<br/>CacheService · Backends · Stampede Protection]
    Core[oridecon<br/>DI · Config · Logging]
    Contracts[oridecon-contracts<br/>CacheBackendProtocol · CacheKeyBuilderProtocol]
...
All cache backends implement `CacheBackendProtocol` from `oridecon-contracts`. Operations return `Result[T, CacheError]` — backend errors are signalled via `Err` rather than exceptions, letting callers decide how to handle failures.
...
flowchart LR
    ...
    subgraph Backends[oridecon-cache · backends]
    ...
    subgraph ThirdParty[Third Party]
        EP[entry-point<br/>oridecon.cache.backends]
    end
...
class CacheProvider(Provider):
    ...
    config_key = "cache"                         # ORI_CACHE__* env vars
```
...
|----------|--------|---------|
| `CacheBackendProtocol` | `oridecon.contracts.infra.cache` | Primary backend interface: `get`, `set`, `delete`, `clear`, `exists`, `get_many`, `set_many`, `delete_many`, `delete_pattern`, `health_check` |
| `CacheKeyBuilderProtocol` | `oridecon.contracts.infra.cache` | Key construction and namespace management |
| `CacheProtectionStrategyProtocol` | `oridecon.contracts.infra.cache` | Stampede protection contract |
| `CacheProviderProtocol` | `oridecon.contracts.infra.cache` | Provider self-reference for DI resolution |
| `CacheHealthCheckerProtocol` | `oridecon.contracts.infra.cache` | Backend health checks |
| `StateStoreProtocol` | `oridecon.contracts.infra` | Low-level key-value store (Redis backends) |
| `AsyncStringSerializerProtocol` | `oridecon.contracts.core.serialization` | Serialization interface (JSON, Pickle, Msgpack) |
| `SemanticCacheProtocol` | `oridecon.contracts.ai.llm` | Semantic caching (optional, requires faiss) |
| `EmbeddingClientProtocol` | `oridecon.contracts.ai.llm` | Embedding generation for semantic caching |
| `HealthCheckResult` | `oridecon.contracts.core` | Structured health check type |
| `HookRegistryProtocol` | `oridecon.contracts.core` | Lifecycle hook emission |
...
| Point | Mechanism |
|-------|-----------|
| Custom backend | Implement `CacheBackendProtocol`, register via `oridecon.cache.backends` entry point |
...
| Operator CLI | Extend the `oridecon cache` CLI commands |
...
Third-party backends self-register using the `oridecon.cache.backends` entry point group. During `register()` the provider scans this group and calls each provider's `register()` method:

```python
# pyproject.toml
[project.entry-points."oridecon.cache.backends"]
