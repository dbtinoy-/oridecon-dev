...
from oridecon.cache.serialization.base import CacheSerializationError, AsyncStringSerializerProtocol
...
class TestSerializationBase:
    ...
    def test_serialization_error_can_be_instantiated(self):
        ...
        assert str(error).startswith("[ORI_ERR_CACHE_015] Test error message")
    ...
    def test_serialization_error_with_args(self):
        ...
        assert isinstance(error, Exception)
        # Note: OrideconError stores only message in args[0], other args in details
        assert "[ORI_ERR_CACHE_015] Error" in str(error)
