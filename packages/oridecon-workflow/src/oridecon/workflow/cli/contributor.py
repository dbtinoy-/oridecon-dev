...
_SPECS: tuple[tuple[str, str, str, str], ...] = (
    (
        ...
        "oridecon.workflow.cli.generators.workflow_def:WorkflowDefinitionGenerator",
        "src/workflows",
    ),
    (
        "pipeline",
        "Generate a pipeline with sequential processing stages",
        "oridecon.workflow.cli.generators.pipeline:PipelineGenerator",
        "src/pipelines",
    ),
    (
        "saga_step",
        "Generate a saga step with compensating transaction",
        "oridecon.workflow.cli.generators.saga_step:SagaStepGenerator",
        "src/sagas",
    ),
)
...
class WorkflowCliContributor:
    """CLI contributor for the oridecon-workflow package."""
    ...
    def get_commands(self) -> list[CommandContribution]:
        ...
        return [
            CommandContribution(
                ...
                app_factory_path="oridecon.workflow.cli.commands:create_workflow_app",
                contributor="workflow",
                category="workflow",
    ...
    def get_health_checks(self) -> list[HealthCheckContribution]:
        ...
        return [
            HealthCheckContribution(
                ...
                check_path="oridecon.workflow.cli.checks:check_workflow_engine",
                contributor="workflow",
                category="workflow",
                timeout=5.0,
            ),
    ...
    def get_shell_context(self) -> list[ShellContextContribution]:
        ...
        return [
            ShellContextContribution(
                ...
                factory_path="oridecon.workflow.cli.shell:provide_workflow_engine",
                contributor="workflow",
            ),
        ]
