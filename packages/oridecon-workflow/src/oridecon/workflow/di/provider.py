"""WorkflowProvider — DI integration for the oridecon-workflow package.

Registers the workflow subsystem (pipelines, bulk operations, sagas)
into the Oridecon DI container with sensible defaults and optional
user-supplied overrides.

Example::

    from oridecon.app import Application
    from oridecon.workflow.di import WorkflowProvider
...
from oridecon.workflow.pipeline import Pipeline
...
class WorkflowProvider(Provider):
    ...
    def __init__(
        ...
        Args:
            ...
            state_machine: Optional pre-configured :class:`~oridecon.workflow.state.StateMachine`
                instance to register under :class:`~oridecon.contracts.workflow.StateMachineProtocol`.
