...
| `oridecon-queue` | Optional | Async workflow execution |
...
`oridecon-workflow` provides four orchestration patterns: **Pipelines** (sequential step chains), **Graph workflows** (DAG-based with branching), **Sagas** (compensating transaction coordination), and **Bulk operations** (batched parallel processing).

## Mental model

```
Workflow orchestration patterns in oridecon-workflow:
...
```python
from oridecon.workflow import Pipeline, step, conditional, parallel
...
|---|---|---|
| `FunctionStep` (via `@step`) | `from oridecon.workflow import step` | Simple async function |
| `ConditionalStep` (via `@conditional`) | `from oridecon.workflow import conditional` | Branch based on predicate |
| `ParallelStep` (via `@parallel`) | `from oridecon.workflow import parallel` | Run substeps concurrently |
...
from oridecon.workflow.graph.node import NodeType
...
```python
from oridecon.result import Ok, Err, Result
...
from oridecon.workflow.config import BulkOperationConfig
from oridecon.contracts.workflow import SagaStoreProtocol
