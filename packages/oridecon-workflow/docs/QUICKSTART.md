...
from oridecon.workflow import Pipeline, step, WorkflowModule
...
async def main() -> None:
    ...
    async with Application.boot(name="wf-demo") as app:
        ...
        result = await pipeline.execute({"name": "Oridecon"})
        print(result)  # "HELLO, ORIDECON!"
...
from oridecon.workflow.graph.node import AbstractWorkflowNode
...
result = await engine.execute("greet", initial_state={"name": "Oridecon"})
if result.is_ok():
