...
# Optional extras
uv add "oridecon-tasks[redis]"     # Redis backend
uv add "oridecon-tasks[rabbitmq]"  # RabbitMQ backend
```
...
from oridecon.tasks import TasksModule
...
```bash
export ORI_TASKS__ENABLED=true
export ORI_TASKS__BACKEND__TYPE=memory
```

> `ORI_TASKS__*` variables feed `TaskConfig` via the container's `OrideconConfig`;
...
|-------|---------|---------|-------------|
| `backend.type` | `memory` | `ORI_TASKS__BACKEND__TYPE` | Queue backend (`redis`, `rabbitmq`, `postgres`, `memory`) |
| `backend.redis_url` | `redis://localhost:6379/0` | `ORI_TASKS__BACKEND__REDIS_URL` | Redis connection URL |
| `backend.amqp_url` | `amqp://localhost` | `ORI_TASKS__BACKEND__AMQP_URL` | AMQP connection URL |
| `worker.worker_count` | `1` | `ORI_TASKS__WORKER__WORKER_COUNT` | Number of worker processes |
| `worker.max_concurrent_tasks` | `10` | `ORI_TASKS__WORKER__MAX_CONCURRENT_TASKS` | Max tasks executed in parallel per worker |
| `worker.default_timeout` | `300` | `ORI_TASKS__WORKER__DEFAULT_TIMEOUT` | Task execution timeout in seconds |
| `scheduler.enabled` | `true` | `ORI_TASKS__SCHEDULER__ENABLED` | Enable cron-based task scheduling |
| `scheduler.timezone` | `UTC` | `ORI_TASKS__SCHEDULER__TIMEZONE` | Timezone for cron schedule evaluation |
...
- **Health checks** — `/health/tasks` endpoint via `oridecon-monitor`
...
|------|----------------|
| `src/oridecon/tasks/module.py` | `TasksModule` class with factory methods |
| `src/oridecon/tasks/di/provider.py` | `TasksProvider` — wires task protocols into DI container |
| `src/oridecon/tasks/config.py` | `TaskConfig` and sub-config classes |
| `src/oridecon/tasks/backends/` | Backend implementations (memory, redis, rabbitmq, postgres) |
| `src/oridecon/tasks/scheduling/` | Cron scheduler and scheduled task decorators |
| `src/oridecon/tasks/background_task_manager.py` | `BackgroundTaskManager` — lifecycle-aware task tracking (LEX-006) |
| `src/oridecon/tasks/scheduled_worker.py` | `ScheduledWorker` + `OnErrorPolicy` — periodic worker base class (LEX-005) |
...
```python
from oridecon.tasks import BackgroundTaskManager
...
```python
from oridecon.tasks import BackgroundTaskManager, OnErrorPolicy, ScheduledWorker
