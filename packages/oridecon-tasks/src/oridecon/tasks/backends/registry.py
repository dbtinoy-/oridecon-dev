...
To register a custom backend::
    ...
    # Inside a Oridecon Application, resolve from the DI container instead:
...
from oridecon.tasks import constants as tasks_const
...
class TaskBackendRegistry(_CoreBackendRegistry):
    ...
    and a factory that accepts a :class:`~oridecon.tasks.config.TaskConfig`
    ...
    def create_backend(self, config: TaskConfig) -> Any:
        ...
        Returns:
            ...
            :class:`~oridecon.contracts.infra.tasks.TaskQueueProtocol`.
...
def _create_redis(config: TaskConfig) -> Any:
    ...
    Requires ``pip install oridecon-tasks[redis]``.
    """
...
def _create_rabbitmq(config: TaskConfig) -> Any:
    ...
    Requires ``pip install oridecon-tasks[rabbitmq]``.
    """
    from oridecon.tasks.backends.rabbitmq import RabbitMQTaskQueue
...
def _create_postgres(config: TaskConfig) -> Any:
    ...
    Requires ``pip install oridecon-tasks[postgres]`` and
    ``config.backend.postgres_dsn`` to be set.
    """
    from oridecon.tasks.backends.postgres import PostgresTaskConfig, PostgresTaskQueue
