...
Example:
    ...
    from oridecon.tasks.progress import CacheBackendProgressStore
...
from oridecon.tasks.progress.core import ProgressInfo, ProgressStore
...
_KEY_PREFIX = "oridecon:task:progress:"
_ACTIVE_SET_KEY = "oridecon:task:progress:__active__"


class CacheBackendProgressStore(ProgressStore):
    """Distributed progress store backed by the platform ``CacheBackendProtocol``.

    Stores serialised :class:`~oridecon.tasks.progress.core.ProgressInfo`
    ...
    async def list_active(self) -> list[ProgressInfo]:
        ...
        Returns:
            List of incomplete :class:`~oridecon.tasks.progress.core.ProgressInfo`.
        """
