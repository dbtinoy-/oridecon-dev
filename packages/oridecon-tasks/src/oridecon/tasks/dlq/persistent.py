...
synchronous :class:`~oridecon.tasks.dlq.core.DeadLetterQueue`.  It
delegates all storage to a :class:`~oridecon.tasks.dlq.backend.DLQBackend`
so that records survive process restarts when combined with a persistent
backend such as :class:`~oridecon.tasks.dlq.backend.StateStoreDLQBackend`.

Example::

    from oridecon.tasks.dlq.backend import StateStoreDLQBackend
...
from oridecon.tasks.dlq.backend import DLQBackend, InMemoryDLQBackend
from oridecon.tasks.dlq.core import FailureRecord
...
class PersistentDeadLetterQueue:
    ...
    Unlike the synchronous :class:`~oridecon.tasks.dlq.core.DeadLetterQueue`,
    ...
    The default backend is :class:`~oridecon.tasks.dlq.backend.InMemoryDLQBackend`,
    which gives the same in-memory behaviour as the original queue.  Swap it for
    :class:`~oridecon.tasks.dlq.backend.StateStoreDLQBackend` to gain persistence.
    """
    ...
    async def get(self, job_id: str) -> FailureRecord | None:
        ...
        Returns:
            :class:`~oridecon.tasks.dlq.core.FailureRecord` if found,
            ``None`` otherwise.
        """
    ...
    async def list_failed(
        ...
        *,
        ...
        Returns:
            List of serialised :class:`~oridecon.tasks.dlq.core.FailureRecord`
            dicts.
        """
    ...
    async def retry(self, job_id: str) -> JobProtocol | None:
        ...
        Returns:
            The :class:`~oridecon.tasks.models.job.JobProtocol` with status reset to
