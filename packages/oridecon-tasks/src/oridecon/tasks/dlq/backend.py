...
- :class:`StateStoreDLQBackend` — persistent, uses any :class:`~oridecon.contracts.StateStoreProtocol`
...
class StateStoreDLQBackend:
    """Persistent DLQ backend backed by a :class:`~oridecon.contracts.StateStoreProtocol`.

    Serialises each :class:`~oridecon.tasks.dlq.core.FailureRecord` as JSON and
    ...
    Args:
        state_store: Any :class:`~oridecon.contracts.StateStoreProtocol` implementation
            (e.g. Redis-backed).
