"""Durable scheduler state persistence for :class:`~oridecon.tasks.scheduling.scheduler.TaskScheduler`.
...
* :class:`DatabaseSchedulerStore` — persists job definitions and next-run times to a
  relational database via :class:`~oridecon.contracts.data.DatabaseProviderProtocol`.
...
(e.g. from ``oridecon-resilience``) to elect the single instance that drives
the scheduler loop.
"""
...
if TYPE_CHECKING:
    ...
    from oridecon.tasks.scheduling.scheduler import ScheduledJob
...
class SchedulerStore(Protocol):
    """Protocol for persisting :class:`~oridecon.tasks.scheduling.scheduler.ScheduledJob` definitions.
...
class DatabaseSchedulerStore:
    ...
    Note:
        ...
        ``oridecon-resilience``) when multiple instances must elect a single
        leader::
