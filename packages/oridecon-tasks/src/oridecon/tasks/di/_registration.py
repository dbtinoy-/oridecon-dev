...
from oridecon.contracts.admin.protocols import AdminContributorProtocol
...
from oridecon.tasks.di._attrs import _TaskAttrsMixin
...
class _TaskRegistrationMixin(_TaskAttrsMixin):
    ...
    async def register(self, container: ContainerRegistrarProtocol) -> None:
        ...
        """
        from oridecon.contracts.infra.tasks import TaskProviderProtocol
        from oridecon.tasks.di.provider import (
    ...
    async def _discover_backends(self, container: ContainerRegistrarProtocol) -> None:
        """Scan the ``oridecon.tasks.backends`` entry-point group.
        ...
        eps = _meta.entry_points(group="oridecon.tasks.backends")
        for ep in eps:
            try:
                candidate = ep.load()
