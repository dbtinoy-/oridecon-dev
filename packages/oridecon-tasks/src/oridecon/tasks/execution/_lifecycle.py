...
Extracted from ``oridecon.tasks.execution.worker`` to keep that module
...
if TYPE_CHECKING:
    from oridecon.contracts.core import HookRegistryProtocol, TaskManagerProtocol
    from oridecon.contracts.infra.resilience import RetryPolicyProtocol
    from oridecon.tasks.dlq.core import DeadLetterQueue
    from oridecon.tasks.observability.core import TaskDashboard
    from oridecon.tasks.progress.core import ProgressStore
...
class TaskWorkerServices:
    ...
    Attributes:
        ...
        retry_policy: Retry policy implementation (from ``oridecon-resilience``).
