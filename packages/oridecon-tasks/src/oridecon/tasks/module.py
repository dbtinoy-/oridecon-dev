...
class TasksModule(Module):
    ...
    def configure(
        ...
        Args:
            queue: :class:`~oridecon.contracts.infra.tasks.TaskQueueProtocol` implementation.
                Defaults to :class:`~oridecon.tasks.backends.memory.MemoryTaskQueue`.
            worker_count: Number of concurrent task workers.
            enable_scheduler: Whether to enable the job scheduler.
            config: :class:`~oridecon.tasks.config.TaskConfig` or ``None`` to
        ...
        from oridecon.tasks.admin.handlers.tasks_summary import (
            TasksSummaryWidgetHandler,
        )
        from oridecon.tasks.config import TaskConfig as _TaskConfig
    ...
    def stub(cls, config: Any = None) -> DynamicModule:
        ...
        Uses a :class:`~oridecon.tasks.backends.memory.MemoryTaskQueue`
