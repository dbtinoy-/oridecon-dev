...
Internal design of the `oridecon-tasks` package.

---

## Role in the System

`oridecon-tasks` provides background asynchronous job processing within a Oridecon application. It is an extension package that depends on `oridecon` + `oridecon-contracts` and imports from `oridecon-resilience` for fault-tolerance primitives (the documented exception for cross-extension imports).
...
flowchart LR
    ...
    Core[oridecon-core]
    Contracts[oridecon-contracts]
    Resilience[oridecon-resilience]
...
`oridecon-tasks` supports four built-in queue backends via a registry pattern (`backends/registry.py`):
...
| **Redis** | `_create_redis` | `oridecon-tasks[redis]` | Fast, persistent, pub/sub patterns | Requires Redis server |
| **RabbitMQ** | `_create_rabbitmq` | `oridecon-tasks[rabbitmq]` | Durable, routing, ACK semantics | Heavier infra |
| **Postgres** | `_create_postgres` | `oridecon-tasks[postgres]` | No extra service, transactional | Poll-based, higher latency |
...
```python
# Retry policy from oridecon-resilience (injected via DI)
...
`TaskProvider` integrates with the Oridecon DI lifecycle:
...
All protocols consumed by `oridecon-tasks` live in `oridecon.contracts.infra.tasks`:

| Protocol | Import Path | Usage |
|----------|-------------|-------|
| `TaskQueueProtocol` | `oridecon.contracts.infra.tasks` | Core queue contract — enqueue, dequeue, ack, nack, close |
| `TaskExecutorProtocol` | `oridecon.contracts.infra.tasks` | Handler dispatch — execute, register_handler, get_handler |
| `TaskProviderProtocol` | `oridecon.contracts.infra.tasks` | Provider lifecycle — register, boot, shutdown, health_check |
| `JobProtocol` | `oridecon.contracts.infra.tasks` | Job data model — id, name, args, kwargs, priority, status |
| `JobTemplateProtocol` | `oridecon.contracts.infra.tasks` | Recurring job template — name, args, kwargs, depends_on |
| `TaskWorkerProtocol` | `oridecon.contracts.infra.tasks` | Worker contract — start, stop |
| `DLQProtocol` | `oridecon.contracts.infra.tasks` | Dead-letter queue — add, get, retry, purge |
| `ProgressTrackerProtocol` | `oridecon.contracts.infra.tasks.progress` | Progress reporting for long-running jobs |
| `CacheBackendProtocol` | `oridecon.contracts.infra.cache` | Optional upgrade for distributed result storage |
| `RetryConfig` | `oridecon.contracts.infra.resilience.models` | Retry policy configuration |
| `HookRegistryProtocol` | `oridecon.contracts.core` | Lifecycle hooks for task events |
| `HealthCheckResult` | `oridecon.contracts.core.health` | Health check data structure |
...
| Point | Mechanism |
|-------|-----------|
| **Custom backend** | Implement `TaskQueueProtocol` + register via `oridecon.tasks.backends` entry point, or use `TaskBackendRegistry.register()` |
...
| **Custom error handler** | Use `RetryPolicyProtocol` from `oridecon-resilience` for custom retry/backoff |
