...
``oridecon.contracts.observability.audit`` so that extension packages
(e.g. ``oridecon-admin``) can schedule audit verification without
...
class AuditScheduler:
    ...
    ``oridecon.contracts.observability.audit`` without requiring callers
    to import anything from ``oridecon-sql``.
