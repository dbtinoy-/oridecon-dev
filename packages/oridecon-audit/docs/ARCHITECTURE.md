...
Internal design of the `oridecon-audit` package.
...
flowchart BT
    ...
    Audit[oridecon-audit<br/>AuditLogger · AuditVerifier · AuditPurger<br/>PolicyBasedRetention · AuditScheduler]
    Contracts[oridecon-contracts<br/>AuditLoggerProtocol · AuditStoreProtocol<br/>AuditVerifierProtocol · RetentionPolicyProtocol<br/>AuditEntry · AuditQuery · AuditEventSeverity]
    Core[oridecon Core<br/>DI · Config · Events · Hooks · Scheduling]
...
`oridecon.contracts.audit.types`:
...
All protocols and shared types live in `oridecon.contracts.audit`:
...
└── cli/
    ...
    ├── commands.py             # `oridecon audit` CLI command group
...
```python
# oridecon/audit/di/bundle_provider.py
