---
title: oridecon-audit Guide
description: Concepts, mental model, and common usage patterns for oridecon-audit
sidebar:
  order: 2
---
...
| `oridecon-sql` | Optional | Audit trail storage |
| `oridecon-queue` | Optional | Async audit events |
...
**oridecon-audit** provides an append-only, HMAC-verified, retention-managed audit trail that integrates with the Oridecon DI container.
...
```python
from oridecon.contracts.audit import AuditLoggerProtocol, AuditQuery
...
- **SQL** (`SqlAuditStore`) — production. Requires `oridecon-audit[sql]`.
...
```python
from oridecon.audit import compute_audit_checksum, verify_audit_checksum
