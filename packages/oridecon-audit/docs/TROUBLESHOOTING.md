---
title: oridecon-audit Troubleshooting
description: Common errors, causes, and fixes for oridecon-audit
sidebar:
  order: 6
---
...
```
ImportError: No module named 'oridecon.audit.store.sql'
```

**Cause:** The SQL backend requires `oridecon-audit[sql]` which adds `oridecon-sql` as a dependency. Without it, `AuditCoreProvider` falls back to `InMemoryAuditStore`.

**Fix:** Install the optional dependency:

```bash
uv add "oridecon-audit[sql]"
```
