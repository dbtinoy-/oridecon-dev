...
from oridecon.webhook import constants as const
...
class WebhookConfig(BaseConfig):
    ...
    Attributes:
        store_backend: Persistence backend. "sql" requires oridecon-webhook[sql].
    ...
    store_backend: str = Field(
        ...
        description="Persistence backend ('sql' requires oridecon-webhook[sql])",
    )
