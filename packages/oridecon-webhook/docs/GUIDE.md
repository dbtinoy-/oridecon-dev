...
| `oridecon-web` | Yes | HTTP server |
| `oridecon-queue` | Optional | Async dispatch |
...
`oridecon-webhook` manages the full lifecycle: subscription CRUD, fan-out delivery with exponential backoff, HMAC payload signing, and a dead-letter queue for events that can't be delivered.
