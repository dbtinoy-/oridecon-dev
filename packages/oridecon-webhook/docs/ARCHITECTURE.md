...
Internal design of the `oridecon-webhook` package.
...
```mermaid
flowchart BT
    Contracts[oridecon-contracts<br/>WebhookSubscription · WebhookEvent · DeliveryAttempt<br/>WebhookSubscriptionStoreProtocol · WebhookDeliveryStoreProtocol]
    Webhook[oridecon-webhook<br/>Delivery pipeline · Subscription management<br/>HMAC security · Dead-letter queue]
    Events[oridecon-events<br/>Domain event bus]
    Admin[oridecon-admin<br/>Admin dashboard]
...
`oridecon-webhook` is the outbound webhook delivery layer. It provides subscription CRUD, event fan-out with exponential-backoff retry, HMAC signature signing and verification, storage backends (in-memory, SQL), and dead-letter queue management.
...
```
packages/oridecon-webhook/src/oridecon/webhook/
...
├── protocols.py             # Re-exports from oridecon.contracts.webhook
...
`DeliveryAttempt` is a frozen dataclass in `oridecon.contracts.webhook.types`:
...
| SQL | `"sql"` | SQL database | Production (requires `oridecon-webhook[sql]`) |
...
The `WebhookCoreProvider` selects the backend based on `config.store_backend`. The SQL backend uses `DatabaseProviderProtocol` from `oridecon-contracts` and creates two tables (`webhook_subscriptions`, `webhook_delivery_attempts`) with appropriate indexes.
...
The package implements protocols from `oridecon.contracts.webhook`:
...
Types (`WebhookSubscription`, `WebhookEvent`, `DeliveryAttempt`, `DeliveryStatus`) are defined in `oridecon.contracts.webhook.types`. The base exception `WebhookError` lives in `oridecon.contracts.webhook.exceptions`.
...
flowchart LR
    ...
    subgraph Package[oridecon-webhook]
...
`WebhookError` lives in `oridecon.contracts.webhook.exceptions` as the base. Leaf exceptions in `oridecon.webhook.exceptions` extend it. Domain operations return `Result[T, WebhookError]` — callers handle at the service boundary.
