...
NOTIF_ROOT = PACKAGE_ROOT / "src/oridecon/notification"
...
def test_mailer_types_live_in_mailer_submodule() -> None:
    assert MailerModule.__module__ == "oridecon.notification.mailer.module"
    assert MailerProvider.__module__ == "oridecon.notification.di.mailer_provider"
    assert SMTPMailer.__module__ == "oridecon.notification.mailer.smtp_mailer"
    assert SendGridMailer.__module__ == "oridecon.notification.mailer.sendgrid_mailer"
