...
class PushBackendRegistry:
    ...
    def with_defaults(cls) -> PushBackendRegistry:
        ...
        def _fcm(entry: Any) -> Any:
            ...
            from oridecon.notification.config import FCMDriverConfig
        ...
        def _apns(entry: Any) -> Any:
            ...
            from oridecon.notification.config import APNsDriverConfig

            if APNsPush is None:
                raise ImportError(
                    "APNsPush unavailable — install oridecon-notification[apns]"
                )
        ...
        def _web_push(entry: Any) -> Any:
            ...
            from oridecon.notification.config import WebPushDriverConfig

            if WebPushChannel is None:
                raise ImportError(
                    "WebPushChannel unavailable — install oridecon-notification[web-push]"
                )
