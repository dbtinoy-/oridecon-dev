...
- **Meta** — calls the Meta (Facebook) WhatsApp Business API directly
  via ``httpx``.  Requires the ``oridecon-notification[whatsapp]`` extra.
"""
...
from oridecon.contracts.mailer import MessageDeliveryReceipt
from oridecon.contracts.notification.types import SMSMessage
from oridecon.logging import get_logger
from oridecon.notification.constants import DEFAULT_TWILIO_TIMEOUT, TWILIO_API_BASE
from oridecon.notification.exceptions import TwilioNotificationError
...
class WhatsAppBackend:
    ...
    - ``"twilio"`` — Twilio Messaging API with ``whatsapp:`` prefix.
      ...
      credentials as :class:`~oridecon.notification.backends.sms.twilio.TwilioSMS`.
    ...
    async def send(
        ...
        Args:
            message: The :class:`~oridecon.contracts.notification.types.SMSMessage`
    ...
    async def _send_meta(
        ...
        try:
            ...
            raise ImportError(
                ...
                "Install with: pip install oridecon-notification[whatsapp]"
            ) from exc
