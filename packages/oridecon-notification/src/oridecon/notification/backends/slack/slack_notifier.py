...
- **Bot API** — uses the ``slack-sdk`` ``AsyncWebClient`` to send rich
  ...
  Requires the ``oridecon-notification[slack]`` extra.
"""
...
class SlackNotifier:
    ...
    - **Bot API** (``mode="bot"``): Uses the ``slack-sdk``
      ...
      ``oridecon-notification[slack]`` extra.
    ...
    async def _send_webhook(
        ...
        try:
            ...
            raise ImportError(
                ...
                "Install with: pip install oridecon-notification[slack]"
            ) from exc
