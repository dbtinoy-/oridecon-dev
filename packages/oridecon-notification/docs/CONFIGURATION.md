---
title: oridecon-notification Configuration
...
Section key: **`notification`** — loaded from `notification:` in `application.yaml` or via `ORI_NOTIFICATION__*` env vars.
...
| Key | Type | Default | Env Var | Description |
|-----|------|---------|---------|-------------|
| `account_sid` | `str \| None` | `None` | `ORI_NOTIFICATION__SMS_BACKENDS__0__TWILIO__ACCOUNT_SID` | Twilio Account SID |
| `auth_token` | `str \| None` | `None` | `ORI_NOTIFICATION__SMS_BACKENDS__0__TWILIO__AUTH_TOKEN` | Twilio Auth Token |
...
Section key: **`mailer`** — loaded from `mailer:` in `application.yaml` or via `ORI_NOTIFICATION__MAILER__*` env vars.
...
Section key: **`inbox`** — loaded from `inbox:` in `application.yaml` or via `ORI_NOTIFICATION__INBOX__*` env vars.
...
| Key | Type | Default | Env Var | Description |
|-----|------|---------|---------|-------------|
| `store_backend` | `str` | `"database"` | `ORI_NOTIFICATION__INBOX__STORE_BACKEND` | Storage: `database` or `memory` |
| `max_page_size` | `int` | `50` | `ORI_NOTIFICATION__INBOX__MAX_PAGE_SIZE` | Max messages per page |
| `retention_days` | `int` | `30` | `ORI_NOTIFICATION__INBOX__RETENTION_DAYS` | Days before message pruning |
| `mark_read_on_fetch` | `bool` | `False` | `ORI_NOTIFICATION__INBOX__MARK_READ_ON_FETCH` | Auto-mark read on fetch |
...
export ORI_NOTIFICATION__SMS_BACKENDS__0__TWILIO__ACCOUNT_SID="AC..."

# Set SendGrid API key for the mailer
export ORI_NOTIFICATION__MAILER__BACKENDS__0__SENDGRID__API_KEY="SG..."

# Set inbox retention
export ORI_NOTIFICATION__INBOX__RETENTION_DAYS=90
```
