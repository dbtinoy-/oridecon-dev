---
title: oridecon-notification Guide
...
`oridecon-notification` solves this by defining **channel protocols** (`SMSChannelProtocol`, `PushChannelProtocol`, `MailerProtocol`) in `oridecon-contracts`, providing **driver implementations** for each major provider, and registering them in the DI container under **Named bindings** for multi-backend support.
...
Every notification channel is abstracted behind a protocol from `oridecon-contracts`:

| Protocol | Import | `send()` Signature |
|----------|--------|--------------------|
| `SMSChannelProtocol` | `oridecon.contracts.notification.protocols` | `send(SMSMessage) -> Result[MessageDeliveryReceipt, NotificationError]` |
| `PushChannelProtocol` | `oridecon.contracts.notification.protocols` | `send(PushMessage) -> Result[...]` + `send_batch(list[PushMessage])` |
| `MailerProtocol` | `oridecon.contracts.mailer.protocols` | `send(EmailMessage) -> Result[MessageDeliveryReceipt, MailerError]` |
...
The `MailerProvider` is auto-discovered via the `oridecon.providers` entry point. Configure it under `mailer:` in YAML:
...
from oridecon.contracts.notification.inbox import InboxMessage
...
- ❌ Don't store credentials in YAML — use `SecretStr` env-var overrides (`ORI_NOTIFICATION__*`)
