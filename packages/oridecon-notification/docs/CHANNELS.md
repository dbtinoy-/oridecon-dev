---
title: oridecon-notification Channels
description: Supported notification channels in oridecon-notification — email, SMS, push, and user inbox
---

`oridecon-notification` delivers messages across multiple channels through a common set of protocols — `MailerProtocol` (email), `SMSChannelProtocol` (SMS), `PushChannelProtocol` (push), and `InboxStoreProtocol` (in-app inbox). Each channel supports multiple providers and named multi-backend DI binding.
...
| **Inbox** | Database | *(core via oridecon-sql)* | Yes | In-app notification bell |
...
```python
from oridecon.notification.mailer import SMTPMailer
...
- **`DatabaseInboxStore`** — production store backed by `oridecon-sql` (default)
...
```python
from oridecon.notification import InMemoryInboxStore
from oridecon.notification.inbox import InboxService
