...
Internal design of the `oridecon-notification` package.

---

## Role in the System

`oridecon-notification` provides multi-channel notification delivery (SMS, push, email, in-app inbox) for Oridecon applications. It is an extension package depending only on `oridecon` and `oridecon-contracts`.
...
flowchart BT
    ...
    subgraph Notification[oridecon-notification]
        Inbox[InboxService]
...
Import constraint: `oridecon-notification` imports only from `oridecon` and `oridecon-contracts`. No cross-extension imports.
...
Channels without templates (SMS, push) construct `SMSMessage` / `PushMessage` value objects directly — no templating layer beyond simple string formatting. The `Mailable` base class ships in `oridecon.notification.mailer.mailable`.
...
Discovered automatically via the `oridecon.providers` entry point (`mailer = oridecon.notification.di.mailer_provider:MailerProvider`).
...
| Protocol | Location (oridecon-contracts) | Implemented By |
|----------|-------------------------------|----------------|
| `SMSChannelProtocol` | `oridecon.contracts.notification.protocols` | `TwilioSMS` |
| `PushChannelProtocol` | `oridecon.contracts.notification.protocols` | `FCMPush`, `APNsPush`, `WebPushChannel` |
| `MailerProtocol` | `oridecon.contracts.mailer.protocols` | `SMTPMailer`, `SendGridMailer`, `RetryingMailer` |
| `InboxStoreProtocol` | `oridecon.contracts.notification.inbox` | `InMemoryInboxStore`, `DatabaseInboxStore` |
| `DeliveryStoreProtocol` | `oridecon.contracts.notification.delivery` | Application-provided (persistent tracking) |

**Value types shared across packages:**

| Type | Location | Used By |
|------|----------|---------|
| `SMSMessage` | `oridecon.contracts.notification.types` | SMS backends |
| `PushMessage` | `oridecon.contracts.notification.types` | Push backends |
| `EmailMessage` | `oridecon.contracts.mailer.types` | Mailer backends |
| `MessageDeliveryReceipt` | `oridecon.contracts.mailer.types` | All channels |
| `InboxMessage` | `oridecon.contracts.notification.inbox` | Inbox service and stores |

**Exception hierarchy:**

```
NotificationError (oridecon-contracts)
...
MailerError (oridecon-contracts)
├── SMTPMailerError
└── SendGridMailerError
```
