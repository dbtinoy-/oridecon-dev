"""Configuration models for Oridecon GraphQL.
...
Example:
    ...
    # From environment
    config = GraphQLConfig()  # reads ORI_GRAPHQL__* env vars
"""
...
from oridecon.graphql.security.rate_limit import RateLimitConfig
from oridecon.graphql.types import CacheScope, SubscriptionProtocol
...
class IntrospectionConfig(BaseConfig):
    ...
    Attributes:
        enabled: Whether introspection is enabled. Introspection is served
            ...
            registers the
            :class:`~oridecon.graphql.core.introspection.IntrospectionGuardExtension`,
        ...
        allowed_environments: Set of environment names (matched case-
            insensitively against ``GraphQLConfig.env`` or ``ORI_ENV``)
...
class PlaygroundConfig(BaseConfig):
    ...
    title: str = "Oridecon GraphQL Playground"
...
class TracingConfig(BaseConfig):
    ...
    enabled: bool = False
    service_name: str = "oridecon-graphql"
...
class MetricsConfig(BaseConfig):
    ...
    enabled: bool = False
    namespace: str = "oridecon_graphql"
...
class GraphQLConfig(BaseConfig):
    """Hierarchical root configuration for Oridecon GraphQL.
    ...
    # Set via GRAPHQL__ENV or ORI_ENV env var
