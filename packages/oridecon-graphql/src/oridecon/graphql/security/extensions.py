...
if TYPE_CHECKING:
    ...
    from oridecon.graphql.security.rate_limit import RateLimiter
...
class RateLimitExtension(SchemaExtension):
    ...
    :class:`~oridecon.graphql.security.rate_limit.RateLimiter` (resolved
    from the DI container by :class:`~oridecon.graphql.providers.GraphQLProvider`)
    and raises :class:`~oridecon.graphql.exceptions.RateLimitError` when the
    limit is exceeded.
    ...
    async def on_operation(self) -> AsyncGenerator[None, None]:
        ...
        if self._rate_limiter is not None:
            ...
            from oridecon.graphql.exceptions import RateLimitError
