...
from oridecon.graphql.types import GraphQLErrorCode


class GraphQLError(OrideconError):
    """Base exception for all GraphQL errors."""

    _code: str = "ORI_ERR_GQL_001"
...
class AuthenticationError(GraphQLError, _ContractsAuthenticationError):
    ...
    Also satisfies ``oridecon.contracts.exceptions.AuthenticationError`` so
    middleware that catches the contracts type will catch this as well.
    """

    _code: str = "ORI_ERR_GQL_009"
...
class AuthorizationError(GraphQLError, _ContractsAuthorizationError):
    ...
    Also satisfies ``oridecon.contracts.exceptions.AuthorizationError`` so
    middleware that catches the contracts type will catch this as well.
    """

    _code: str = "ORI_ERR_GQL_010"
