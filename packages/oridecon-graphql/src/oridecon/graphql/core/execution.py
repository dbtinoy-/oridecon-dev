...
from oridecon.graphql.config import ErrorConfig
from oridecon.graphql.core.context import (
    GraphQLContext,
    GraphQLRequest,
    GraphQLResponse,
)
from oridecon.graphql.core.error_formatter import ErrorFormatter
...
from oridecon.graphql.types import CacheControl, OperationType, QueryMetrics
...
class GraphQLExecutorProtocol:
    ...
    Example:
        ...
        from oridecon.graphql.core import GraphQLExecutorProtocol
    ...
    def __init__(
        ...
        Args:
            ...
            event_bus: Optional EventBusProtocol for lifecycle event publishing.
                Subscribers receive :class:`~oridecon.graphql.events.BeforeExecuteEvent`,
                :class:`~oridecon.graphql.events.AfterExecuteEvent`, and
                :class:`~oridecon.graphql.events.OnErrorEvent`.
        """
        self._schema = schema
    ...
    async def execute(
        ...
        try:
            # Execute with timeout using oridecon timeout utilities (e.g., timeout context)
                ...
                for error in result.errors:
                    ...
                    if original is not None:
                        if isinstance(original, GraphQLError):
                            ...
                            # aren't Oridecon GraphQLErrors but have safe user-facing messages.
                            # Wrap them in a transient GraphQLError so format_error sees them.
                            from oridecon.graphql.exceptions import (
                                GraphQLError as OrideconGraphQLError,
                            )

                            effective_error = OrideconGraphQLError(str(original))
