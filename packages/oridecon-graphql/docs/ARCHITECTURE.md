...
Internal design of the `oridecon-graphql` package.
...
**Import direction:** `oridecon-graphql` depends on `oridecon` and `oridecon-contracts`. It integrates with `oridecon-web` through the `GraphQLController` (registered via `GraphQLWebContributor`). Cross-extension communication goes through contracts and the DI container — never direct imports.
...
```python
from oridecon.graphql import resolver, field
...
Every `DomainError` from `oridecon-contracts` can be mapped to a `GraphQLError` with a specific error code. The `ErrorFormatter` applies masking rules based on `ErrorConfig`:
...
From `oridecon-contracts` (`oridecon.contracts.graphql.*`) and other contract modules:
