...
|-----|------|---------|---------|-------------|
| `name` | `str` | `"graphql"` | `ORI_GRAPHQL__NAME` | Configuration name |
| `enabled` | `bool` | `True` | `ORI_GRAPHQL__ENABLED` | Enable GraphQL module |
| `path` | `str` | `"/graphql"` | `ORI_GRAPHQL__PATH` | GraphQL endpoint path |
| `debug` | `bool` | `False` | `ORI_GRAPHQL__DEBUG` | Enable debug mode; propagates to errors |
| `env` | `str \| None` | `None` | `ORI_GRAPHQL__ENV` / `ORI_ENV` | Environment name |
| `enable_identity_resolution` | `bool` | `False` | `ORI_GRAPHQL__ENABLE_IDENTITY_RESOLUTION` | Resolve OAuth IDs to internal UUIDs |
| `schema_baseline_path` | `str \| None` | `None` | `ORI_GRAPHQL__SCHEMA_BASELINE_PATH` | SDL file path for schema diffing |
