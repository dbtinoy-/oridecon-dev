...
Covers:
    - G4: ``GraphQLConfig`` auto-disables Playground when ``ORI_ENV=production``
...
from oridecon.graphql.config import GraphQLConfig, CacheConfig, PlaygroundConfig
from oridecon.graphql.core.context import GraphQLResponse
...
class TestPlaygroundAutoDisableInProduction:
    ...
    def test_playground_enabled_by_default(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.delenv("ORI_ENV", raising=False)
        config = GraphQLConfig()
        assert config.playground.enabled is True

    def test_playground_disabled_when_oridecon_env_production(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """Playground is forcibly disabled when ORI_ENV=production."""
        monkeypatch.setenv("ORI_ENV", "production")
        config = GraphQLConfig()
    ...
    def test_playground_disabled_case_insensitive(
        ...
        """ORI_ENV check is case-insensitive (PRODUCTION, Production, etc.)."""
        monkeypatch.setenv("ORI_ENV", "PRODUCTION")
        config = GraphQLConfig()
        assert config.playground.enabled is False
    ...
    def test_playground_not_disabled_for_staging(
        ...
        monkeypatch.setenv("ORI_ENV", "staging")
        config = GraphQLConfig()
        assert config.playground.enabled is True
    ...
    def test_playground_not_disabled_for_development(
        ...
        monkeypatch.setenv("ORI_ENV", "development")
        config = GraphQLConfig()
        assert config.playground.enabled is True
...
class TestIntrospectionAutoDisableInProduction:
    ...
    def test_introspection_disabled_when_oridecon_env_production(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """Introspection is forcibly disabled when ORI_ENV=production."""
        monkeypatch.setenv("ORI_ENV", "production")
        cfg = GraphQLConfig()
...
class TestExecutorCacheControlHeader:
    ...
    async def test_cache_control_header_set_when_cache_enabled(
        ...
        from oridecon.graphql.config import GraphQLConfig, CacheConfig
        from oridecon.graphql.core.context import GraphQLContext
        from oridecon.graphql.types import CacheScope
