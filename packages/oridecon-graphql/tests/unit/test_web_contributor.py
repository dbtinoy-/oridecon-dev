...
def test_graphql_web_contributor_matches_protocol() -> None:
    ...
    from oridecon.graphql.web.contributor import GraphQLWebContributor
...
def test_graphql_web_contributor_returns_graphql_controller() -> None:
    ...
    from oridecon.graphql.controllers.graphql import GraphQLController
    from oridecon.graphql.web.contributor import GraphQLWebContributor
...
def test_pyproject_has_web_contributor_entry_point() -> None:
    ...
    web_contributors = entry_points.get("oridecon.web.contributors", {})

    assert "graphql" in web_contributors
    assert (
        web_contributors["graphql"]
        == "oridecon.graphql.web.contributor:GraphQLWebContributor"
    )
