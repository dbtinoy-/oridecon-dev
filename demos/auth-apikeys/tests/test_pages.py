"""Page controller tests for the auth-apikeys demo."""
...
async def test_keys_renders(client: httpx.AsyncClient) -> None:
...
async def test_keys_has_logo(client: httpx.AsyncClient) -> None:
    r = await client.get("/keys", follow_redirects=False)
    assert "/static/logo.png" in r.text
