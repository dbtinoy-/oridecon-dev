...
$("upload-form").addEventListener("submit", upload); $("refresh").addEventListener("click", load); $("artifacts").addEventListener("click", (event) => { const b = event.target.closest("button"); if (!b) return; if (b.dataset.preview) preview(b.dataset.preview); if (b.dataset.access) access(b.dataset.access); if (b.dataset.delete) remove(b.dataset.delete); }); load(); setInterval(load, 8000);
