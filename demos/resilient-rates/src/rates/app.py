...
def build_modules() -> list[object]:
    ...
    return [
        ...
        # It provides retry, circuit breaker, timeout, and bulkhead
        # infrastructure as container-managed singletons.
        ResilienceModule.configure(),
        # CacheModule reads the ``cache:`` section from yaml.
        # It provides an in-memory cache backend with stampede protection
        # (single-flight) and configurable TTL.
        CacheModule.configure(),
        # WebModule is the only module that needs your controllers list —
        # this is the explicit wiring style.  Omit RatesPageController if
