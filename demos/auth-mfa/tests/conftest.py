"""Pytest bootstrap for the auth-mfa demo.
...
2. Boot the real composition root for tests via fixtures.
    ...
    uv run pytest demos/auth-mfa/tests -q        # from repo root works too
"""
...
os.chdir(_DEMO_ROOT)
# Add src/ to sys.path so ``from mfa_console...`` resolves in tests.
sys.path.insert(0, str(_DEMO_ROOT / "src"))

from lexigram.web import WebProvider  # noqa: E402
from mfa_console.app import create_app  # noqa: E402 — after sys.path setup
