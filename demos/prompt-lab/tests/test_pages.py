"""Smoke tests for the lab page routes."""
...
async def test_root_serves_lab(client: httpx.AsyncClient) -> None:
    response = await client.get("/")

    assert response.status_code == 200
    assert "text/html" in response.headers["content-type"]
    assert "Prompt Lab" in response.text
    assert 'data-variant="v1"' in response.text
    assert 'data-variant="v2"' in response.text
