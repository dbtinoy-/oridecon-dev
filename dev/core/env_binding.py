"""Empirical env-var binding verification.

Loads each config family through its real ``from_yaml()`` path with a unique
marker value set for a candidate variable, then checks whether the marker
reaches any declared field of the resulting config tree. This mirrors the
runtime binding rule exactly: a variable binds iff it matches
``LEX_<config_section>__<field_path>`` (or ``LEX_<field>`` for section-less
roots) AND something actually loads the class via ``from_yaml``.
"""
