#!/usr/bin/env python3
"""Generate the CSP-clean admin shell client assets.

The admin shell previously shipped its behaviour as inline ``<script>`` /
``<style>`` blocks (``base.html``, ``ui/templates/shell_scripts.py``, the
command-palette and notification-bell controllers). Under the strict CSP v2
candidate (``STRICT_CSP``: ``script-src 'self'``, ``style-src 'self'``) every
such block is reported by the browser as ``script-src-elem`` /
``style-src-elem`` — for the users page alone that was 10 script blocks, up
to 5 style blocks and 5 inline event handlers.

This generator is the single source of truth for the externalised assets:

- ``static/css/admin-shell.css`` — the static rules that used to live in
  ``<style>`` blocks (base chrome, search overlay, form UX).
- ``static/js/admin-head.js`` — the synchronous ``<head>`` bootstrap
  (dark-mode before first paint, theme override vars, ``toggleTheme``).
- ``static/js/admin-shell.js`` — the deferred shell bundle (Alpine patches,
  htmx glue, loading bar, form UX, navigator, search overlay, delegated
  actions, command-palette and notification-bell controllers).

Per-page dynamic data is delivered through CSP-exempt ``<script
type="application/json">`` data islands or ``data-*`` attributes, never
through inline executable script.

Usage:
    uv run python dev/generators/admin_shell_assets.py
    uv run python dev/generators/admin_shell_assets.py --check
"""
...
SRC = ROOT / "experimental/apps/oridecon-admin/src/oridecon/admin"
CSS_DIR = SRC / "static/css"
JS_DIR = SRC / "static/js"

ADMIN_SHELL_CSS = CSS_DIR / "admin-shell.css"
ADMIN_HEAD_JS = JS_DIR / "admin-head.js"
ADMIN_SHELL_JS = JS_DIR / "admin-shell.js"

ADMIN_SHELL_CSS_TEMPLATE = """\
/**
 * Oridecon Admin — shell chrome styles.
 *
 * Generated by dev/generators/admin_shell_assets.py — do not edit by hand.
 * Replaces the inline <style> blocks that used to ship in base.html and
 * shell_scripts.py, so the strict CSP v2 candidate (style-src 'self')
 * no longer reports style-src-elem violations.
 */

/* ========== Base chrome (was base.html <style>) ========== */

[x-cloak] {
    display: none !important;
}

/* Native <details> disclosure markers are not suppressed reliably by
   runtime-scanned utility classes (the prebuilt Tailwind bundle is static),
   so hide them statically. */
summary {
    list-style: none;
}
...
/* Modern scrollbar styling (Webkit/Chromium) */
::-webkit-scrollbar {
    width: 6px;
    height: 6px;
}
...
/* Smooth animations for drawer and overlays */
...
/* ========== Search overlay (was shell_scripts.search_overlay_markup) ========== */
...
/* ========== Admin form UX (was shell_scripts.admin_form_ux_script) ========== */
...
def _delegated_actions_js() -> str:
    """Delegated replacements for the inline event-handler attributes.

    Each handler attribute is replaced by a ``data-*`` descriptor plus a
    document-level listener, so no ``onclick=``/``onsubmit=`` markup remains
    and the page works under ``script-src-attr 'none'``-style policies.
    """  # noqa: D401
    return r'''
...
def _bulk_download_js() -> str:
    """Bulk-export blob download (B28) for shell pages.

    The export buttons carry ``data-action=\"bulk-download\"`` plus
    ``data-bulk-download-url``/``data-bulk-action``. This bundle defines
    ``window.LexigramDownloadBulk`` because shell pages do not load the
    legacy ``admin.js``; legacy layouts keep their own copy there.
    """
    return r'''
...
def _alpine_controllers_js() -> str:
    """Command-palette and notification-bell Alpine controllers.

    Both were inline ``<script>`` blocks generated from Python f-strings.
    The controllers now read their per-instance configuration from a
    ``<script type="application/json">`` data island rendered next to the
    component, so the shell bundle is fully static and CSP-clean.
    """  # noqa: D401
    return r'''
...
def _shell_behaviour_js() -> str:
    """The behaviour previously shipped inline by shell_scripts.py and base.html."""  # noqa: D401
    return r'''
...
def render_admin_shell_css() -> str:
    """Render the shell stylesheet."""
    return ADMIN_SHELL_CSS_TEMPLATE


def render_admin_head_js() -> str:
    """Render the synchronous head bootstrap."""
    return ADMIN_HEAD_JS_TEMPLATE


def render_admin_shell_js() -> str:
    """Render the deferred shell bundle."""
    parts = [
        "/**",
        ...
        "  'use strict';",
        _shell_behaviour_js(),
        _delegated_actions_js(),
        _bulk_download_js(),
        _alpine_controllers_js(),
        "})();",
        "",
    ]
    return "\n".join(parts)
...
def _replace_if_changed(path: Path, content: str) -> bool:
    """Write ``content`` to ``path`` unless byte-identical (returns changed)."""
...
def main() -> int:
    """Run the generator (or verify output with --check)."""
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument(
        "--check",
        action="store_true",
        help="Fail if any generated asset is stale instead of rewriting it",
    )
    args = parser.parse_args()

    outputs = {
        ADMIN_SHELL_CSS: render_admin_shell_css(),
        ADMIN_HEAD_JS: render_admin_head_js(),
        ADMIN_SHELL_JS: render_admin_shell_js(),
    }
    ...
    if args.check:
        ...
        if stale:
            print("Stale generated admin shell assets:")
            for name in stale:
                print(f"  {name}")
            print("Run `uv run python dev/generators/admin_shell_assets.py`.")
            return 1
        print("admin shell assets: up to date")
        return 0

    changed = [
        str(path.relative_to(ROOT))
        for path, content in outputs.items()
        if _replace_if_changed(path, content)
    ]
    if changed:
        print("Generated:", ", ".join(changed))
    else:
        print("admin shell assets: unchanged")
    return 0
