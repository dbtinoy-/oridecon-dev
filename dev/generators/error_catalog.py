...
Scans all `src/` trees for `_code = "ORI_ERR_*"` assignments and
...
REPO_ROOT = Path.cwd()
SRC_GLOB = "*/src/**/*.py"
ORI_ERR_RE = re.compile(r'ORI_ERR_([A-Z0-9_]+)_(\d{3})')
...
class CodeVisitor(ast.NodeVisitor):
    ...
    def _record_code(self, code_value: ast.AST) -> None:
        """Record a _code assignment if it matches ORI_ERR_* pattern."""
...
class NoCodeVisitor(ast.NodeVisitor):
    ...
    def __init__(self, filepath: str, package: str, known_coded_classes: set[str]) -> None:
        self.filepath = filepath
        self.package = package
        self.known_coded_classes = known_coded_classes  # Class names that already have ORI_ERR_ codes
...
def package_sort_key(pkg: str) -> tuple:
    """Sort packages: contracts first, then oridecon, then extensions alphabetically."""
...
def main() -> None:
    ...
    # --- Generate markdown ---
    lines: list[str] = []
    lines.append("# REF_ERROR_CODES.md — Oridecon Framework Error Code Registry")
    lines.append("")
    ...
    lines.append("")
    lines.append("> This is the **authoritative registry** of all `ORI_ERR_*` error codes in the")
    lines.append("> Oridecon Framework monorepo. Every exception class carrying a `_code` attribute")
    lines.append("> is listed here. Generated by scanning all `*.py` source files (excluding tests")
    lines.append("> and examples) for `_code` assignments matching `ORI_ERR_<DOMAIN>_<NNN>`.")
    lines.append("")
    lines.append("---")
    lines.append("")
    ...
    if not dupes:
        lines.append("✅ **No duplicate codes detected.** Every `ORI_ERR_*` code is unique.")
    else:
    ...
    if pkg_without_codes:
        ...
        lines.append("")
        lines.append("These packages have source files but **no `ORI_ERR_*` codes** registered.")
        lines.append("")
